 ...
 */
	...
	ret = register_abox_log_extra_copy(abox_log_extra_copy);
	if (ret) {
		pr_err("%s : fail to register_abox_log_extra_copy\n", __func__);
		goto err_aboxlog_data;
	}

	ret = register_set_modem_event(set_modem_event);
	if (ret) {
		pr_err("%s : fail to register_set_modem_event\n", __func__);
		goto err_aboxlog_data;
	}

	ret = register_abox_debug_string_update(abox_debug_string_update);
	if (ret) {
		pr_err("%s : fail to register_abox_debug_string_update\n", __func__);
		goto err_aboxlog_data;
	}
