...
/*
 *  Sound Card Driver for Exynos8855
 *
...
static int uaif_init(struct snd_soc_pcm_runtime *rtd)
{
...
static int exynos_late_probe(struct snd_soc_card *card)
{
	...
	if (of_property_read_bool(np, "samsung,routing")) {
		...
		if (ret)
			dev_err(dev, "parsing samsung,routing failed: %d\n", ret);

		ret = snd_soc_dapm_add_routes(dapm, card->of_dapm_routes,
						card->num_of_dapm_routes);
		if (ret)
			dev_err(dev, "some routes failed to register: %d\n", ret);
	}
...
static struct snd_soc_dai_link exynos_dai[100] = {
	...
	{
		.name = "WDMA12 BE",
		.stream_name = "WDMA12 BE",
		.id = ABOX_BE_DAI_ID(1, 12),
		.no_pcm = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = abox_hw_params_fixup_helper,
	...
	{
		.name = "WDMA13 BE",
		.stream_name = "WDMA13 BE",
		.id = ABOX_BE_DAI_ID(1, 13),
		.no_pcm = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = abox_hw_params_fixup_helper,
	...
	{
		.name = "WDMA14 BE",
		.stream_name = "WDMA14 BE",
		.id = ABOX_BE_DAI_ID(1, 14),
		.no_pcm = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = abox_hw_params_fixup_helper,
	...
	{
		.name = "WDMA15 BE",
		.stream_name = "WDMA15 BE",
		.id = ABOX_BE_DAI_ID(1, 15),
		.no_pcm = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = abox_hw_params_fixup_helper,
		...
		struct snd_soc_dai_link *dai_link)
{
	...
	for_each_link_codecs(dai_link, index, component) {
		...
		ret = snd_soc_of_get_dlc(np, NULL, component, index);
		...
		struct snd_soc_dai_link *dai_link)
{
	...
	ret = snd_soc_of_get_dai_name(np, &cpu->dai_name, 0);
out:
	if (ret < 0) {
		if (cpu)
			devm_kfree(dev, cpu);
...
MODULE_DESCRIPTION("ALSA SoC Exynos8855 Sound Driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:exynos8855-audio");
