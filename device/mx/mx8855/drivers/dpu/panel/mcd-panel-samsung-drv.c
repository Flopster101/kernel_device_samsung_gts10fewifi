...
#define MCD_PANEL_MODESET_POST_DELAY_MSEC (10)
...
static struct brightness_queue dimq;
...
#if 1
static void
mcd_exynos_panel_connector_update_brightness(struct exynos_panel *ctx, uint64_t brightness)
{
	...
	panel_info(ctx, "br: %d\n", (u32)brightness);
	call_mcd_panel_func(ctx->mcd_panel_dev, set_brightness, (u32)brightness);
}
#endif
...
{
#if 1
	// @mcd : brightness : adapter -> panel-drv
	u32 new_br = (u32)brightness;

	mcd_exynos_panel_connector_update_brightness(ctx, brightness);
	ctx->cur_br = new_br;
#else
...
static bool exynos_panel_bridge_lastclose(struct exynos_drm_bridge *exynos_bridge)
{
	...
	pr_info("%s\n", __func__);
	mutex_lock(&ctx->mode_lock);
	// call_mcd_panel_func(ctx->mcd_panel_dev, display_off);
	mutex_unlock(&ctx->mode_lock);
				  ...
				  const struct drm_display_mode *adjusted_mode)
{
	...
	if (funcs) {
		struct drm_crtc_state *new_crtc_state;
		struct exynos_drm_crtc_state *new_exynos_crtc_state;
		const bool is_lp_mode = mcd_drm_panel_is_panel_lp_mode(ctx, pmode);
...
static void exynos_panel_set_dqe_xml(struct device *dev, struct exynos_panel *ctx)
{
	...
	if (ctx->desc->dqe_xml_suffix == NULL)
		return;
	strncpy(dqe->xml_suffix, ctx->desc->dqe_xml_suffix, DQE_XML_SUFFIX_SIZE - 1);

	dev_info(ctx->dev, "DQE XML Suffix (%s)\n", dqe->xml_suffix);
}

/*
static void exynos_panel_set_aiqe_xml(struct device *dev, struct exynos_panel *ctx)
{
	struct drm_device *drm_dev = NULL;
	struct drm_crtc *crtc;
	struct exynos_drm_crtc *exynos_crtc = NULL;
	struct exynos_aiqe *aiqe;
	...
	aiqe = exynos_crtc->aiqe;
	if (!aiqe) {
		dev_info(ctx->dev, "%s: aiqe has null\n", __func__);
		return;
	}
	if (ctx->desc->aiqe_xml_suffix == NULL)
		return;
	strncpy(aiqe->xml_suffix, ctx->desc->aiqe_xml_suffix, AIQE_XML_SUFFIX_SIZE - 1);

	dev_info(ctx->dev, "AIQE XML Suffix (%s)\n", aiqe->xml_suffix);
}
*/
		...
		const struct exynos_panel_mode *pmode)
{
	...
	queue_delayed_work(ctx->wqs[MCD_DRM_DRV_WQ_MODESET_POST].wq,
			&ctx->wqs[MCD_DRM_DRV_WQ_MODESET_POST].dwork,
			msecs_to_jiffies(MCD_PANEL_MODESET_POST_DELAY_MSEC));
...
struct dsim_fcmd *create_dsim_fast_cmd(struct dsim_device *dsim, const u8 *payload, u32 size, u32 align)
{
	...
	payload_xfer_unit = rounddown(min((u32)(dsim->g_cmd_sram - 1), size_aligned), align);
...
void wq_modeset_post_handler(struct work_struct *data)
{
	struct mcd_drm_drv_wq *w = container_of(to_delayed_work(data), struct mcd_drm_drv_wq, dwork);
	struct exynos_panel *ctx = container_of(w, struct exynos_panel, wqs[MCD_DRM_DRV_WQ_MODESET_POST]);
	struct panel_display_mode *pdm;
	int ret;

	pdm = mcd_drm_panel_epm_to_pdm(ctx, ctx->current_mode);
	if (pdm == NULL)
		dev_warn(ctx->dev, "%s: failed to find pmode", __func__);

	ret = call_mcd_panel_func(ctx->mcd_panel_dev, set_display_mode_post, pdm);
	if (ret < 0 && ret != -ENOENT)
		dev_err(ctx->dev, "%s failed to execute set_display_mode_post (ret:%d)\n",
				__func__, ret);
}
...
work_func_t mcd_drm_drv_wq_fns[MAX_MCD_DRM_DRV_WQ] = {
	...
	[MCD_DRM_DRV_WQ_MODESET_POST] = wq_modeset_post_handler,
};
...
char *mcd_drm_drv_wq_names[MAX_MCD_DRM_DRV_WQ] = {
	...
	[MCD_DRM_DRV_WQ_MODESET_POST] = "wq_modeset_post",
};
...
u32 mcd_drm_panel_get_data_lane_cnt(struct exynos_panel *ctx)
{
	struct device_node *np;
	int ret;
	u32 cnt = 0;
	...
	ret = of_property_read_u32(np, "SLSI,data_lane_cnt", &cnt);
	if (ret)
		goto err;

	if (cnt > MAX_DSIM_DATALANE_CNT) {
		ret = -ERANGE;
		goto err;
	}

	panel_info(ctx, "data_lane_cnt: %d\n", cnt);

	return cnt;
err:
	/* default value : 4(4-lane) */
	panel_info(ctx, "data_lane_cnt: ret %d %d use default value(4)\n", ret, cnt);
	cnt = 4;
...
mcd_drm_panel_get_exynos_panel_desc(struct exynos_panel *ctx)
{
	...
	desc->data_lane_cnt = mcd_drm_panel_get_data_lane_cnt(ctx);
...
int exynos_panel_probe(struct mipi_dsi_device *dsi)
{
...
#if 0
	/* 16 times dimming setting */
	#define DIMMING_STEP_NUM (16)
	ctx->enable_dimming_brightness = true;
	__bq_reset(&dimq, DIMMING_STEP_NUM);
#else
	/* no dimming setting */
	#define DIMMING_STEP_NUM (1)
	ctx->enable_dimming_brightness = false;
	__bq_reset(&dimq, DIMMING_STEP_NUM);
#endif
	...
	// exynos_panel_set_aiqe_xml(dev, ctx);
