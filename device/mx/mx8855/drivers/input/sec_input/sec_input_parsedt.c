...
int sec_input_parse_dt(struct device *dev)
{
	...
	pdata->gpio_reset = of_get_named_gpio(np, "sec,gpio_reset", 0);
	if (gpio_is_valid(pdata->gpio_reset)) {
		ret = devm_gpio_request(dev, pdata->gpio_reset, "tsp,gpio_reset");
		input_info(true, dev, "%s: gpio_reset: %d, ret: %d\n", __func__, pdata->gpio_reset, ret);
	}
	...
	if (count <= 0) {
		...
		if ((lcd_id_num != count) || (lcd_id_num <= 0)) {
			...
			of_property_read_string_index(np, "sec,firmware_name_mp", 0, &pdata->firmware_name_mp);
			if (pdata->firmware_name_mp == NULL || strlen(pdata->firmware_name_mp) == 0)
				pdata->firmware_name_mp = NULL;
			else
				input_info(true, dev, "%s: fw name_mp(%s)\n", __func__, pdata->firmware_name_mp);
		} else {
			u32 *lcd_id_t;
			u32 lcd_id_mask;
			...
			for (i = 0; i < lcd_id_num; i++) {
				if (((lcd_id_t[i] & lcd_id_mask) == (lcd_type & lcd_id_mask)) || (i == (lcd_id_num - 1))) {
					...
					of_property_read_string_index(np, "sec,firmware_name_mp", i, &pdata->firmware_name_mp);
					if (pdata->firmware_name_mp == NULL || strlen(pdata->firmware_name_mp) == 0)
						pdata->firmware_name_mp = NULL;
					else
						input_info(true, dev, "%s: fw name_mp(%s)\n", __func__, pdata->firmware_name_mp);
	...
	if (pdata->work_queue_probe_enabled) {
		...
		if (!IS_ERR_OR_NULL(pdata->probe_workqueue)) {
			SEC_INPUT_INIT_WORK(dev, &pdata->probe_work, sec_input_probe_work);
...
void sec_input_support_feature_parse_dt(struct device *dev)
{
	...
	pdata->support_touchscreen_lid_switch = of_property_read_bool(np, "sec,support_touchscreen_lid_switch");
	...
	if (of_property_read_u32(np, "sec,dev_extra_event_buffer", &pdata->dev_extra_event_buffer) < 0)
		pdata->dev_extra_event_buffer = 0;
	...
	input_info(true, dev, "support_vbus_notifier:%d support_always_on:%d extra_buffer:%d\n",
			pdata->support_vbus_notifier, pdata->support_always_on, pdata->dev_extra_event_buffer);
}
...
int sec_input_get_lcd_id(struct device *dev)
{
	...
	if (dev_id == MULTI_DEV_SUB)
		connected = get_lcd_info("connected1");
	else
