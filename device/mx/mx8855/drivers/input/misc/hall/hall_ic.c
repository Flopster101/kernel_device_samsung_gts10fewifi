...
#define SW_FOLDER_1		0x00  /* set = folder_1 open, close*/
#define SW_FOLDER_2		0x02  /* set = folder_2 open, close*/
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
#define SW_COVER_FLIP			0x10  /* set = flip cover open, close*/
...
#else
#define SW_COVER_FLIP			0x15  /* set = flip cover open, close*/
...
static ssize_t flip1_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct hall_ic_data *hall;
	int state;
...
static ssize_t flip2_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct hall_ic_data *hall;
	int state;
...
static ssize_t flip1_status_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct hall_ic_data *hall;
	int state;
...
static ssize_t flip2_status_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct hall_ic_data *hall;
	int state;
...
static DEVICE_ATTR_RO(flip1_status);
static DEVICE_ATTR_RO(flip2_status);
#else
static DEVICE_ATTR_RW(flip_status);
static DEVICE_ATTR_RW(flip1_status);
static DEVICE_ATTR_RW(flip2_status);
#endif
...
static struct device_attribute *hall_ic_attrs[] = {
	...
	&dev_attr_flip1_status,
	&dev_attr_flip2_status,
...
static struct hall_ic_pdata *hall_ic_parsing_dt(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		hall->input_name = of_get_property(pp, "input-name", NULL);
		...
		pr_info("%s %s: event: %d\n", __func__, hall->name, hall->event);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
		if (hall->event == 0x15) { /* SW_COVER_FLIP */
			hall->event = 0x10;
		} else if (hall->event == 0x1b) {	/* SW_CERTIFYHALL */
			hall->event = 0x0b;
		} else if (hall->event == 0x1e) {	/* SW_WACOM_HALL */
			hall->event = 0x0c;
		} else if (hall->event == 0x00 || hall->event == 0x02) {	/* SW_FOLDER, SW_FOLDER_1, SW_FOLDER_2 */
//			continue;
