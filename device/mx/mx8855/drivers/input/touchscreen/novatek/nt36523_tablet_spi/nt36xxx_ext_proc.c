...
{
    *m_x_num = ts->plat_data->x_node_num;
    *m_y_num = ts->plat_data->y_node_num;
    memcpy(buf, xdata, ((ts->plat_data->x_node_num * ts->plat_data->y_node_num) * sizeof(int32_t)));
}
...
{
	seq_printf(m, "fw_ver=0x%02X, x_num=%d, y_num=%d\n",
...
static int32_t c_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < ts->plat_data->y_node_num; i++) {
		for (j = 0; j < ts->plat_data->x_node_num; j++) {
			seq_printf(m, "%5d, ", xdata[i * ts->plat_data->x_node_num + j]);
		}
		seq_puts(m, "\n");
	}
