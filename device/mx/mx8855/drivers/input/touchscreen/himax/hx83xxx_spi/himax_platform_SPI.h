...
#define HIMAX_TS_REGULATOR_MAX 10
...
struct himax_i2c_platform_data {
	...
	const char *regulator_name[HIMAX_TS_REGULATOR_MAX];
	...
	int (*early_suspend)(struct device *dev);
	int (*late_suspend)(struct device *dev);
	int (*early_resume)(struct device *dev);
	int (*late_resume)(struct device *dev);
};
