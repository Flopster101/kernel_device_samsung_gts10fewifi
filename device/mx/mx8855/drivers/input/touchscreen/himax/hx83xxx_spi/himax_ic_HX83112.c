...
static void hx83112f_proximity_sleep_in(void)
{
	...
	while (read_db0h[0] != off_db0h[0] && retry > 0 && !atomic_read(&private_ts->plat_data->shutdown_called)) {
		g_core_fp._dd_en_read(1);
		usleep_range(1000, 1001);
	...
	memset(read_db0h, 0x78, sizeof(read_db0h));
	retry = 30;
	while (read_db0h[1] != off_bist[1] && retry > 0 && !atomic_read(&private_ts->plat_data->shutdown_called)) {
		g_core_fp._dd_en_read(1);
		usleep_range(1000, 1001);
	...
	if ((*kp_private_ts)->plat_data->lowpower_mode > 0 && (*kp_private_ts)->proxy_1b_en == 0)
