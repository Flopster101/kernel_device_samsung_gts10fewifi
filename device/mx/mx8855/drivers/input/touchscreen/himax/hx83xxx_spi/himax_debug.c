...
static ssize_t himax_attn_read(char *buf, size_t len)
{
	...
	ret += snprintf(buf_tmp + ret, sizeof(buf_tmp) - ret, "attn = %x\n",
		himax_int_gpio_read(ts_data->plat_data->irq_gpio));
...
static ssize_t himax_int_en_read(char *buf, size_t len)
{
	...
	desc = irq_to_desc(ts->plat_data->irq);
	if (!desc) {
		E("%s: invalid irq number: %d\n", __func__, ts->plat_data->irq);
		goto out;
	}
...
static ssize_t himax_int_en_write(char *buf, size_t len)
{
	...
	if (buf[0] == '0') {
		...
		himax_int_enable(INT_DISABLE_NOSYNC);
		devm_free_irq(ts->dev, ts->plat_data->irq, ts);
	} else if (buf[0] == '3') {
		himax_int_en_set();
	} else
		return -EINVAL;
...
static ssize_t himax_layout_read(char *buf, size_t len)
{
	...
	ret += snprintf(buf_tmp + ret, sizeof(buf_tmp) - ret, "%d ", 0);
	ret += snprintf(buf_tmp + ret, sizeof(buf_tmp) - ret, "%d ",
		ts->plat_data->max_x);
	ret += snprintf(buf_tmp + ret, sizeof(buf_tmp) - ret, "%d ", 0);
	ret += snprintf(buf_tmp + ret, sizeof(buf_tmp) - ret, "%d ",
...
static ssize_t himax_layout_write(char *buf, size_t len)
{
	...
	if (k == 4) {
		ts->plat_data->max_x = (layout[1] - 1);
		ts->plat_data->max_y = (layout[3] - 1);
		I("%d, %d, %d, %d\n",
		0, ts->plat_data->max_x,
		0, ts->plat_data->max_y);
...
static ssize_t himax_debug_level_write(char *buf, size_t len)
{
	...
	if (ts->debug_log_level & BIT(3)) {
		if (ts->pdata->screenWidth > 0 && ts->pdata->screenHeight > 0 &&
			(ts->plat_data->max_x) > 0 &&
			(ts->plat_data->max_y) > 0) {
...
void himax_log_touch_data(int start)
{
	...
	else if (private_ts->plat_data->lowpower_mode > 0 && private_ts->suspended) {
	...
	else if (private_ts->plat_data->ed_enable == 1 && private_ts->suspended) {
		print_size = hx_touch_data->event_size;
			 ...
			 size_t len, loff_t *pos)
{
	...
	if (buf[0] == 'h') { /* handshaking */
		...
		if (buf[1] == 'd' && buf[2] == 'o') {
			input_info(true, private_ts->dev,
						"%s DISPLAY ON!\n",
						HIMAX_LOG_TAG);
			g_core_fp._proximity_display_on();
		} else if (buf[1] == 's' && buf[2] == 'o') {
			input_info(true, private_ts->dev,
						"%s SLEEP OUT!\n",
						HIMAX_LOG_TAG);
			...
			input_info(true, private_ts->dev,
						"%s DISPLAY OFF!\n",
						HIMAX_LOG_TAG);
			g_core_fp._proximity_display_off();
		} else if (buf[1] == 's' && buf[2] == 'f') {
			input_info(true, private_ts->dev,
						"%s SLEEP IN!\n",
						HIMAX_LOG_TAG);
