	...
	if (cfg_flag == 0) {
		...
		for (i = 0; i < (total_read_times); i++) {
			...
			address = (write_addr[3] << 24)
				...
				+ (write_addr[0])
				+ ((i+1) * max_bus_size);
			tmp_addr[3] = (address >> 24) & 0xFF;
			tmp_addr[2] = (address >> 16) & 0xFF;
			tmp_addr[1] = (address >> 8) & 0xFF;
			tmp_addr[0] = address & 0xFF;
			//I("Next address=0x%08X\n", address);
...
static int himax_write_read_reg(uint8_t *tmp_addr, uint8_t *tmp_data, uint8_t hb, uint8_t lb)
{
	...
	do {
		if (atomic_read(&private_ts->plat_data->shutdown_called))
			break;
		...
		if (tmp_data[1] == hb || tmp_data[0] == lb)
			break;
	} while (cnt++ < HIMAX_REG_WRITE_READ_RETRY_TIMES);

	if (cnt >= HIMAX_REG_WRITE_READ_RETRY_TIMES) {
		E("%s: tmp_data 0x%02X 0x%02X after %d retries\n",
			__func__, tmp_data[0], tmp_data[1], cnt);
	...
	if (hx_touch_data->hx_rawdata_buf[0] == pfw_op->data_rawdata_ready_lb[0]
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) { /*mutual*/
				if (ts->diag_cmd == HX_DIAG_EVENTSTACK_DIFF ||
					ts->diag_cmd == HX_DIAG_SRAM_DIFF)
					mutual_data[index + i] = ((int8_t)hx_touch_data->hx_rawdata_buf[i * 2 + 4 + 1]) * 256 + hx_touch_data->hx_rawdata_buf[i * 2 + 4];
				else
					mutual_data[index + i] = (hx_touch_data->hx_rawdata_buf[i * 2 + 4 + 1]) * 256 + hx_touch_data->hx_rawdata_buf[i * 2 + 4];
			} else { /*self*/
				temp1 = i + index;
...
static void himax_mcu_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
	...
	do {
		...
		if (memcmp(tmp_data, back_data, DATA_LEN_4) == 0)
			break;
		retry_cnt++;
...
#define TEMP_LOG "%s: R%02X%02X%02X%02X, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x\n"
...
static void hx_debug(void)
{
	...
	I(TEMP_LOG, __func__,
		pfw_op->addr_chk_fw_status[3], pfw_op->addr_chk_fw_status[2],
		pfw_op->addr_chk_fw_status[1], pfw_op->addr_chk_fw_status[0],
		tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

	g_core_fp.fp_register_read(pfw_op->addr_flag_reset_event, 4, tmp_data, 0);
	I(TEMP_LOG, __func__,
		pfw_op->addr_flag_reset_event[3], pfw_op->addr_flag_reset_event[2],
		pfw_op->addr_flag_reset_event[1], pfw_op->addr_flag_reset_event[0],
		tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
	...
	I(TEMP_LOG, __func__,
		pfw_op->addr_fw_dbg_msg_addr[3], pfw_op->addr_fw_dbg_msg_addr[2],
		pfw_op->addr_fw_dbg_msg_addr[1], pfw_op->addr_fw_dbg_msg_addr[0],
		tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
}
...
static void himax_mcu_read_FW_ver(void)
{
	...
	snprintf(private_ts->plat_data->ic_vendor_name, sizeof(private_ts->plat_data->ic_vendor_name), "HX");
	...
	while (reload_status == 0) {
		...
		if (data_2[3] == 0x00 && data_2[2] == 0x00
			...
			memset(private_ts->plat_data->img_version_of_ic, 0x00,
					sizeof(private_ts->plat_data->img_version_of_ic));
			g_core_fp.fp_sense_off(true);
			goto END;
		} else {
			retry--;
	...
	if (pfw_op->addr_ver_ic_name[3] != 0x78) {
		g_core_fp.fp_register_read(pfw_op->addr_ver_ic_name, 4, data, 0);
		ic_data->ver_ic_name = data[0];
		I("IC name = 0x%02X\n", ic_data->ver_ic_name);
		private_ts->plat_data->img_version_of_ic[SEC_INPUT_FW_IC_VER] = ic_data->ver_ic_name;
	}
	private_ts->plat_data->img_version_of_ic[SEC_INPUT_FW_VER_PROJECT_ID] = ic_data->vendor_cid_maj_ver;
	private_ts->plat_data->img_version_of_ic[SEC_INPUT_FW_MODULE_VER] = ic_data->vendor_panel_ver;
	private_ts->plat_data->img_version_of_ic[SEC_INPUT_FW_VER] = ic_data->vendor_cid_min_ver;
...
static void himax_mcu_return_event_stack(void)
{
	...
	do {
		...
		usleep_range(10000, 11000);
			...
			tmp_data[0] != psram_op->addr_rawdata_end[0]) && retry > 0);
...
static int himax_mcu_switch_mode(int mode)
{
	...
	while (retry != 0 && !atomic_read(&private_ts->plat_data->shutdown_called)) {
		I("[%d] %s Read\n", retry, __func__);
...
int hx_mcu_get_rport_thrsh(void)
{
	...
	if (strcmp(HX_83123A_SERIES_PWON, private_ts->chip_name) == 0 ||
...
static void himax_flash_speed_set(HX_FLASH_SPEED_ENUM speed)
{
	...
	g_core_fp.fp_register_write(pflash_op->addr_spi200_flash_speed, 4, tmp_data, 0);
...
static bool hx_bin_desc_data_get(uint32_t addr, uint8_t *flash_buf)
{
	...
	for (i = 0; i < FW_PAGE_SEZE; i = i + data_sz) {
		...
		if (!chk_end) { /*1. Check all zero*/
			...
			case TP_CONFIG_TABLE:
				...
				break;
#if IS_ENABLED(CONFIG_TOUCHSCREEN_HIMAX_IC_HX83123)
...
bool hx_mcu_bin_desc_get(u8 *data, uint32_t max_sz)
{
	uint32_t addr_t = 0;
	unsigned char *fw_buf = NULL;
	bool keep_on_flag = false;
	bool g_bin_desc_flag = false;
	unsigned char flag_0e = 0x87;
...
static bool himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	output_addr = psram_op->addr_rawdata_addr[3] << 24
...
#endif
	I("%s: Now reset the Touch chip END.\n", __func__);
}
...
#else
	I("%s:already set from dts himax,fix_touch_info\n", __func__);
#endif
...
EXPORT_SYMBOL(g_guest_info_item);
...
static void himax_mcu_resend_cmd_func(bool suspended)
{
	...
	himax_set_gesture(ts->gesture_state);
	g_core_fp.fp_set_SMWP_enable(ts->plat_data->lowpower_mode, suspended);
#endif
#ifdef HX_HIGH_SENSE
	...
	if (ts->plat_data->charger_flag)
		himax_cable_detect_func(ts->dev, true);
#endif
	if (ts->glove_enabled)
		himax_set_ap_change_mode(HIGH_SENSITIVITY_MODE, ts->glove_enabled);

	if (!suspended) {
		if (ts->grip_restore_data[0] == 1) {
			I("%s restore set grip portrait_mode, %d, %d, %d, %d\n",
					__func__, ts->grip_restore_data[1], ts->grip_restore_data[2],
					ts->grip_restore_data[3], ts->grip_restore_data[4]);
			hx_set_grip_portrait_mode(ts->grip_restore_data);
		} else if (ts->grip_restore_data[0] == 2) {
			I("%s restore set grip landscape_mode, %d, %d, %d, %d\n",
					__func__, ts->grip_restore_data[1], ts->grip_restore_data[2],
					ts->grip_restore_data[3], ts->grip_restore_data[4]);
			hx_set_grip_landscape_mode(ts->grip_restore_data);
		}

		if ((ts->edgehandler_restore_data[0] == 0) && (ts->edgehandler_restore_data[1] != 0) && (ts->edgehandler_restore_data[1] < 3)) {
			I("%s restore set grip exception zone, %d, %d, %d\n",
					__func__, ts->edgehandler_restore_data[1], ts->edgehandler_restore_data[2],
					ts->edgehandler_restore_data[3]);
			hx_set_grip_exception_zone(ts->edgehandler_restore_data);
		}
	}
}
#endif
...
#ifdef HX_ZERO_FLASH
#if IS_ENABLED(CONFIG_TOUCHSCREEN_HIMAX_IC_HX83123)
void hx_mcu_setup_opt_hw_crc(u8 *data)
{
	I("%s: Entering\n", __func__);
	if (g_zf_opt_crc.fw_addr && g_zf_opt_crc.en_opt_hw_crc >= OPT_CRC_RANGE) {
		if (data[g_zf_opt_crc.fw_addr + 0x0C] == OPT_CRC_START_ADDR)
			g_zf_opt_crc.en_start_addr = true;

		g_zf_opt_crc.start_addr =
			data[g_zf_opt_crc.fw_addr + 3] << 24 |
			data[g_zf_opt_crc.fw_addr + 2] << 16 |
			data[g_zf_opt_crc.fw_addr + 1] << 8 |
			data[g_zf_opt_crc.fw_addr];

		g_zf_opt_crc.start_data[3] = data[g_zf_opt_crc.fw_addr + 7];
		g_zf_opt_crc.start_data[2] = data[g_zf_opt_crc.fw_addr + 6];
		g_zf_opt_crc.start_data[1] = data[g_zf_opt_crc.fw_addr + 5];
		g_zf_opt_crc.start_data[0] = data[g_zf_opt_crc.fw_addr + 4];

		if (data[g_zf_opt_crc.fw_addr + 0x10 + 0x0C] == OPT_CRC_END_ADDR)
			g_zf_opt_crc.en_end_addr = true;

		g_zf_opt_crc.end_addr =
			data[g_zf_opt_crc.fw_addr + 0x10 + 3] << 24 |
			data[g_zf_opt_crc.fw_addr + 0x10 + 2] << 16 |
			data[g_zf_opt_crc.fw_addr + 0x10 + 1] << 8 |
			data[g_zf_opt_crc.fw_addr + 0x10];

		g_zf_opt_crc.end_data[3] = data[g_zf_opt_crc.fw_addr + 0x10 + 7];
		g_zf_opt_crc.end_data[2] = data[g_zf_opt_crc.fw_addr + 0x10 + 6];
		g_zf_opt_crc.end_data[1] = data[g_zf_opt_crc.fw_addr + 0x10 + 5];
		g_zf_opt_crc.end_data[0] = data[g_zf_opt_crc.fw_addr + 0x10 + 4];
		...
		if (data[g_zf_opt_crc.fw_addr + 0x20 + 0x0C] == OPT_CRC_SWITCH_ADDR)
			g_zf_opt_crc.en_switch_addr = true;

		g_zf_opt_crc.switch_addr =
		data[g_zf_opt_crc.fw_addr + 0x20 + 3] << 24 |
		data[g_zf_opt_crc.fw_addr + 0x20 + 2] << 16 |
		data[g_zf_opt_crc.fw_addr + 0x20 + 1] << 8 |
		data[g_zf_opt_crc.fw_addr + 0x20];

		g_zf_opt_crc.switch_data[3] = data[g_zf_opt_crc.fw_addr + 0x20 + 7];
		g_zf_opt_crc.switch_data[2] = data[g_zf_opt_crc.fw_addr + 0x20 + 6];
		g_zf_opt_crc.switch_data[1] = data[g_zf_opt_crc.fw_addr + 0x20 + 5];
		g_zf_opt_crc.switch_data[0] = data[g_zf_opt_crc.fw_addr + 0x20 + 4];

		I("%s: opt_crc switch: R%08XH <= 0x%02X%02X%02X%02X\n", __func__,
			g_zf_opt_crc.switch_addr,
			g_zf_opt_crc.switch_data[3], g_zf_opt_crc.switch_data[2], g_zf_opt_crc.switch_data[1], g_zf_opt_crc.switch_data[0]);
	}
}

void hx_mcu_opt_hw_crc(int en)
{
	uint8_t data[DATA_LEN_4] = {0};
	uint8_t tmp_addr[DATA_LEN_4] = {0};
	uint8_t read_data[DATA_LEN_4] = {0};
	int retry_cnt = 0;
	...
	if (g_zf_opt_crc.en_start_addr) {
		I("%s: Start opt crc\n", __func__);
		himax_in_parse_assign_cmd(g_zf_opt_crc.start_addr, tmp_addr, sizeof(tmp_addr));
		do {
			if (en) {
				...
				I("%s: clear start_addr \n", __func__);
				memset(data, 0, 4);
			}
			
			g_core_fp.fp_register_write(tmp_addr, DATA_LEN_4, g_zf_opt_crc.start_data, 0);
			usleep_range(1000, 1100);
			g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, read_data, 0);
			I("%s: start: 0x%02X%02X%02X%02X, retry_cnt=%d\n",\
					__func__, read_data[3], read_data[2], read_data[1], read_data[0], retry_cnt);
			retry_cnt++;
		} while ((data[1] != read_data[1] || data[0] != read_data[0])
	...
	if (g_zf_opt_crc.en_end_addr) {
		I("%s: END opt crc\n", __func__);
		retry_cnt = 0;
		himax_in_parse_assign_cmd(g_zf_opt_crc.end_addr, tmp_addr, sizeof(tmp_addr));
		do {
			if (en) {
			...
			g_core_fp.fp_register_write(tmp_addr, DATA_LEN_4, data, 0);
			usleep_range(1000, 1100);
			g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, read_data, 0);
	...
	if (g_zf_opt_crc.en_switch_addr) {
		retry_cnt = 0;
		himax_in_parse_assign_cmd(g_zf_opt_crc.switch_addr, tmp_addr, sizeof(tmp_addr));
		do {
			if (en) {
			...
			I("%s: switch: 0x%02X%02X%02X%02X, retry_cnt=%d\n",
					__func__, read_data[3], read_data[2], read_data[1], read_data[0], retry_cnt);
			retry_cnt++;
		} while ((data[0] != read_data[0]) && retry_cnt < HIMAX_REG_RETRY_TIMES);
	}
}
#endif
...
void himax_mcu_write_sram_0f(u8 *data, uint8_t *addr, int start_index, uint32_t write_len)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			...
			tmp_data = &data[start_index+i * max_bus_size];
			g_core_fp.fp_register_write(tmp_addr,  max_bus_size, tmp_data, 0);
			total_size_temp = total_size_temp - max_bus_size;
		} else {
...
int himax_sram_write_crc_check(u8 *data, uint8_t *addr, int strt_idx, uint32_t len)
{
	...
	do {
		...
		if (crc != 0) {
			...
			tmp_addr[0] = 0x10;
			g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
			I("%s, times%d: isram CRC fail check:[3]=0x%02X,[2]=0x%02X,[1]=0x%02X,[0]=0x%02X\n",
...
int hx_parse_bin_cfg_data(u8 *data, struct zf_info *zf_info_arr, uint8_t *FW_buf, uint8_t *sram_min, uint8_t *ovl_sidx)
{
	...
	part_num = data[cfg_table_pos + 12];
	...
	for (i = 1; i < part_num; i++) {
		/*3. get all partition*/
		memcpy(buf, &data[i * 0x10 + cfg_table_pos], 16);
	...
	for (i = 1; i < part_num; i++) {
		...
		memcpy(&FW_buf[zf_info_arr[i].cfg_addr - dsram_base], &data[zf_info_arr[i].fw_addr], zf_info_arr[i].write_size);
	}
	/* debug info */
	/* for (i = 1; i < part_num; i++) {
	 *	different = 0;
...
int himax_zf_part_info(u8 *data)
{
	...
	uint32_t tmp_p_addr;
#endif
	uint8_t temp_addr[DATA_LEN_4] = {0};
	uint8_t temp_data[DATA_LEN_4] = {0};

	uint8_t ovl_sidx = 0;
#if defined(HX_CODE_OVERLAY) && defined(HX_SMART_WAKEUP)
	uint8_t tmp_addr[4] = {0xFC, 0x7F, 0x00, 0x10};
	uint8_t send_data[DATA_LEN_4] = {0};
	uint8_t recv_data[DATA_LEN_4] = {0};
#endif
#ifdef HX_ZERO_FLASH
#endif
	if (atomic_read(&private_ts->suspend_mode) == HIMAX_STATE_POWER_OFF) {
	...
	if (g_zf_opt_crc.en_opt_hw_crc >= OPT_CRC_RANGE) {
		if (g_core_fp._setup_opt_hw_crc != NULL)
			g_core_fp._setup_opt_hw_crc(data);
	}
	...
	if (g_zf_opt_crc.fw_addr != 0) {
		g_zf_opt_crc.start_addr[3] = data[g_zf_opt_crc.fw_addr + 3];
		g_zf_opt_crc.start_addr[2] = data[g_zf_opt_crc.fw_addr + 2];
		g_zf_opt_crc.start_addr[1] = data[g_zf_opt_crc.fw_addr + 1];
		g_zf_opt_crc.start_addr[0] = data[g_zf_opt_crc.fw_addr];
		...
		g_zf_opt_crc.end_addr[3] = data[g_zf_opt_crc.fw_addr + 11];
		g_zf_opt_crc.end_addr[2] = data[g_zf_opt_crc.fw_addr + 10];
		g_zf_opt_crc.end_addr[1] = data[g_zf_opt_crc.fw_addr + 9];
		g_zf_opt_crc.end_addr[0] = data[g_zf_opt_crc.fw_addr + 8];

		g_zf_opt_crc.end_data[3] = data[g_zf_opt_crc.fw_addr + 15];
		g_zf_opt_crc.end_data[2] = data[g_zf_opt_crc.fw_addr + 14];
		g_zf_opt_crc.end_data[1] = data[g_zf_opt_crc.fw_addr + 13];
		g_zf_opt_crc.end_data[0] = data[g_zf_opt_crc.fw_addr + 12];
	...
	memset(g_update_cfg_buf, 0x00, ic_data->dsram_size * sizeof(uint8_t));

	/*3. Find the data in the bin file */
	/*CFG*/
	cfg_sz = hx_parse_bin_cfg_data(data, zf_info_arr, g_update_cfg_buf, sram_min, &ovl_sidx);
	I("Now cfg_sz=%d\n", cfg_sz);
	if (cfg_sz < 0) {
		ret = LENGTH_FAIL;
		goto END_FUNC;
	...
	/*Fw Entity*/
	memcpy(buf, &data[cfg_table_pos], 16);
	...
	if (G_POWERONOF == 1) {
		if (atomic_read(&private_ts->suspend_mode) == HIMAX_STATE_POWER_OFF) {
			...
			/* FW entity */
		if (himax_sram_write_crc_check(data, zf_info_arr[0].sram_addr, zf_info_arr[0].fw_addr, zf_info_arr[0].write_size) != 0)
			E("%s, HW CRC FAIL\n", __func__);
		else
			I("%s, HW CRC PASS\n", __func__);
		...
		do {
			...
			g_core_fp.fp_register_write(sram_min, cfg_sz, g_update_cfg_buf, 0);
			...
			if (cfg_crc_hw != cfg_crc_sw) {
				...
				temp_addr[1] = 0x00;
				temp_addr[0] = 0x10;
				g_core_fp.fp_register_read(temp_addr, DATA_LEN_4, temp_data, 0);
				I("%s, times%d: dsram CRC fail check:[3]=0x%02X,[2]=0x%02X,[1]=0x%02X,[0]=0x%02X\n",
				__func__, retry, temp_data[3], temp_data[2], temp_data[1], temp_data[0]);
			} else
				I("Config CRC Pass\n");
		...
		if (private_ts->in_self_test == 0) {
			...
			/* ovl_sidx+1 - gesture */
			/* ovl_sidx+2 - border  */
			if (private_ts->suspended && private_ts->plat_data->lowpower_mode)
				ovl_sidx = ovl_sidx + 1;
			else
				ovl_sidx = ovl_sidx + 2;
#else
			...
			if (himax_sram_write_crc_check(data, zf_info_arr[ovl_sidx].sram_addr, zf_info_arr[ovl_sidx].fw_addr, zf_info_arr[ovl_sidx].write_size) != 0)
			...
			if (private_ts->plat_data->lowpower_mode) {
				if (private_ts->suspended)
				...
				do {
					...
					if (memcmp(send_data, recv_data, DATA_LEN_4) == 0)
						break;
					retry++;
				} while (retry < HIMAX_REG_RETRY_TIMES);
			}
#endif
		} else {
			if (himax_sram_write_crc_check(data, zf_info_arr[ovl_sidx].sram_addr, zf_info_arr[ovl_sidx].fw_addr, zf_info_arr[ovl_sidx].write_size) != 0)
...
void himax_mcu_firmware_update_0f(u8 *data, int size)
{
	...
	if (size == 0) {
		E("%s, firmware is not requested, return\n", __func__);
		return;
	}
	...
	/* set n frame=0*/
	/*if (G_POWERONOF == 1)
	 *	g_core_fp.fp_write_sram_0f(data, pfw_op->addr_set_frame_addr, 0xC30C, 4);
	 *else
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	g_core_fp.fp_firmware_update_0f((unsigned char *) fw_entry->data, (int)fw_entry->size);
	if (ret >= 0)
		release_firmware(fw_entry);
...
void himax_parse_fw_ver(void)
{
	u8 *fw_data = private_ts->fw_data_kernel;
	int ic_ver = 0;
	...
	if (strcmp(HX_83112A_SERIES_PWON, private_ts->chip_name) == 0)
		...
		ic_ver = fw_data[ADDR_VER_IC_NAME];

	private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_IC_VER] = ic_ver;
	private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_VER_PROJECT_ID] = fw_data[CID_VER_MAJ_FLASH_ADDR];
	private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_MODULE_VER] = fw_data[PANEL_VERSION_ADDR];
	private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_VER] = fw_data[CID_VER_MIN_FLASH_ADDR];

	I("%s: project name:%s, binary fw ver: HX%02X%02X%02X%02X\n", __func__,
		private_ts->pdata->proj_name,
		private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_IC_VER],
		private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_VER_PROJECT_ID],
		private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_MODULE_VER],
		private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_VER]);
}

void himax_request_firmware_from_bin(void)
{
	int err = NO_ERR;
	const struct firmware *fw_entry = NULL;

	I("%s, Entering\n", __func__);
	I("file name = %s\n", private_ts->plat_data->firmware_name);
	err = request_firmware(&fw_entry, private_ts->plat_data->firmware_name, private_ts->dev);
	if (err < 0) {
		E("%s, fail in line%d error code=%d,file maybe fail\n", __func__, __LINE__, err);
		goto END;
	}

	private_ts->fw_data_kernel = kzalloc(fw_entry->size, GFP_KERNEL);
	if (!private_ts->fw_data_kernel) {
		input_err(true, private_ts->dev, "%s: failed to kzalloc private_ts->fw_data_kernel mem\n", __func__);
		goto err_out;
	}

	memcpy(private_ts->fw_data_kernel, fw_entry->data, fw_entry->size);
	private_ts->fw_size_kernel = (int)fw_entry->size;
	input_info(true, private_ts->dev, "%s: private_ts->fw_size_kernel = %d\n", __func__, private_ts->fw_size_kernel);
...
int himax_mcu_0f_operation_dirly(void)
{
	...
	I("%s, Entering\n", __func__);
	input_info(true, private_ts->dev, "%s: fw_data_ums %s\n",
			__func__, private_ts->fw_data_ums == NULL ? "is NULL" : "use");
	...
	if (private_ts->fw_data_ums == NULL) {
		input_info(true, private_ts->dev, "%s: fw_size_kernel = %d\n", __func__, private_ts->fw_size_kernel);
		if (private_ts->fw_size_kernel == 0)
			himax_request_firmware_from_bin();
		g_core_fp.fp_firmware_update_0f(private_ts->fw_data_kernel, private_ts->fw_size_kernel);

	} else {
		input_info(true, private_ts->dev, "%s: fw_size_ums = %d\n", __func__, private_ts->fw_size_ums);
		g_core_fp.fp_firmware_update_0f(private_ts->fw_data_ums, private_ts->fw_size_ums);
	}
...
void himax_mcu_in_cmd_init(void)
{
	...
	himax_in_parse_assign_cmd(fw_addr_gesture_en, pfw_op->addr_gesture_en, sizeof(pfw_op->addr_gesture_en));
	...
	himax_in_parse_assign_cmd(fw_addr_ap_notify_fw_sus, pfw_op->addr_ap_notify_fw_sus, sizeof(pfw_op->addr_ap_notify_fw_sus));
	...
	himax_in_parse_assign_cmd(fw_addr_gesture_history, pfw_op->addr_gesture_history, sizeof(pfw_op->addr_gesture_history));
#ifdef HX_P_SENSOR
...
//	himax_in_parse_assign_cmd(FW_CAL_ADDR, pfw_op->addr_cal, sizeof(pfw_op->addr_cal));
	...
	himax_in_parse_assign_cmd(fw_addr_ctrl_mpap_ovl,
		pfw_op->addr_ctrl_mpap_ovl, sizeof(pfw_op->addr_ctrl_mpap_ovl));
	himax_in_parse_assign_cmd(fw_data_ctrl_mpap_ovl_on,
		pfw_op->data_ctrl_mpap_ovl_on,
		sizeof(pfw_op->data_ctrl_mpap_ovl_on));

	himax_in_parse_assign_cmd(fw_addr_normal_noise_thx,
		pfw_op->addr_normal_noise_thx,
		sizeof(pfw_op->addr_normal_noise_thx));
	himax_in_parse_assign_cmd(fw_addr_lpwug_noise_thx,
		pfw_op->addr_lpwug_noise_thx,
		sizeof(pfw_op->addr_lpwug_noise_thx));
	himax_in_parse_assign_cmd(fw_addr_noise_scale, pfw_op->addr_noise_scale,
		sizeof(pfw_op->addr_noise_scale));
	himax_in_parse_assign_cmd(fw_addr_recal_thx, pfw_op->addr_recal_thx,
		sizeof(pfw_op->addr_recal_thx));
	himax_in_parse_assign_cmd(fw_addr_palm_num, pfw_op->addr_palm_num,
		sizeof(pfw_op->addr_palm_num));
	himax_in_parse_assign_cmd(fw_addr_weight_sup, pfw_op->addr_weight_sup,
		sizeof(pfw_op->addr_weight_sup));
	himax_in_parse_assign_cmd(fw_addr_normal_weight_a,
		pfw_op->addr_normal_weight_a,
		sizeof(pfw_op->addr_normal_weight_a));
	himax_in_parse_assign_cmd(fw_addr_lpwug_weight_a,
		pfw_op->addr_lpwug_weight_a,
		sizeof(pfw_op->addr_lpwug_weight_a));
	himax_in_parse_assign_cmd(fw_addr_weight_b, pfw_op->addr_weight_b,
		sizeof(pfw_op->addr_weight_b));
	himax_in_parse_assign_cmd(fw_addr_max_dc, pfw_op->addr_max_dc,
		sizeof(pfw_op->addr_max_dc));
	himax_in_parse_assign_cmd(fw_addr_skip_frame, pfw_op->addr_skip_frame,
		sizeof(pfw_op->addr_skip_frame));
	himax_in_parse_assign_cmd(fw_addr_neg_noise_sup,
		pfw_op->addr_neg_noise_sup,
		sizeof(pfw_op->addr_neg_noise_sup));
	himax_in_parse_assign_cmd(fw_data_neg_noise, pfw_op->data_neg_noise,
		sizeof(pfw_op->data_neg_noise));
	himax_in_parse_assign_cmd(fw_addr_ctrl_mpap_ovl,
		pfw_op->addr_ctrl_mpap_ovl, sizeof(pfw_op->addr_ctrl_mpap_ovl));
	...
	pfw_op->addr_except_top_bot_zone = ADDR_EXCEPT_TOP_BOT_ZONE;

	himax_in_parse_assign_cmd(ADDR_GESTURE_HISTORY, pfw_op->addr_gesture_history,
	...
	himax_in_parse_assign_cmd(ADDR_REJECT_IDLE, pfw_op->addr_reject_idle,
	...
	himax_in_parse_assign_cmd(flash_addr_spi200_fifo_rst,
	...
	himax_in_parse_assign_cmd(flash_data_spi200_rxfifo_rst,
