...
/*  Himax Android Driver Sample Code for HX83123 chipset
 *
...
#include "himax_ic_HX83123.h"
#include "himax_modular.h"

static void hx83123a_chip_init(void)
{
	...
	CID_VER_MAJ_FLASH_ADDR	= 0x21502;  /*0x00C002*/
	CID_VER_MAJ_FLASH_LENG	= 1;
	CID_VER_MIN_FLASH_ADDR	= 0x21503;  /*0x00C003*/
	CID_VER_MIN_FLASH_LENG	= 1;
	PANEL_VERSION_ADDR		= 0x21504;  /*0x00E804*/
	PANEL_VERSION_LENG		= 1;
	FW_VER_MAJ_FLASH_ADDR		= 0x21505;  /*0x00C005*/
	FW_VER_MAJ_FLASH_LENG		= 1;
	FW_VER_MIN_FLASH_ADDR		= 0x21506;  /*0x00C006*/
	FW_VER_MIN_FLASH_LENG		= 1;
	CFG_VER_MAJ_FLASH_ADDR	= 0x21600;  /*0x00C100*/
	CFG_VER_MAJ_FLASH_LENG	= 1;
	CFG_VER_MIN_FLASH_ADDR	= 0x21601;  /*0x00C101*/
	CFG_VER_MIN_FLASH_LENG	= 1;
	ADDR_VER_IC_NAME	= 0x21550;
}

void hx83123_burst_enable(uint8_t auto_add_4_byte)
{
	uint8_t tmp_data[4];
...
int hx83123_flash_write_burst(uint8_t *reg_byte, uint8_t *write_data)
{
	uint8_t data_byte[8];
	int i = 0, j = 0;
...
static int hx83123_register_read(uint8_t *read_addr, int read_length, uint8_t *read_data)
{
	uint8_t tmp_data[4];
	int i = 0;
	int address = 0;
	...
	if (read_length > 4)
		hx83123_burst_enable(1);
	else
		hx83123_burst_enable(0);
...
#endif
static void hx83123_pin_reset(void)
{
...
static bool hx83123_sense_off(bool check_en)
{
	uint8_t cnt = 0;
	...
	do {
		if (cnt == 0 || (tmp_data[0] != 0xA5 && tmp_data[0] != 0x00 && tmp_data[0] != 0x87)) {
			...
			hx83123_flash_write_burst(tmp_addr, tmp_writ);
		}
		msleep(20);

		/* check fw status */
		tmp_addr[3] = 0x90; tmp_addr[2] = 0x00; tmp_addr[1] = 0x00; tmp_addr[0] = 0xA8;
		hx83123_register_read(tmp_addr, DATA_LEN_4, tmp_data);
	...
	do {
		 ...
		 */
		tmp_addr[3] = 0x90; tmp_addr[2] = 0x00; tmp_addr[1] = 0x00; tmp_addr[0] = 0xA8;
		hx83123_register_read(tmp_addr, ADDR_LEN_4, tmp_data);
...
#if defined(HX_ZERO_FLASH)
static void hx83123a_reload_to_active(void)
{
	uint8_t addr[DATA_LEN_4] = {0};
	uint8_t data[DATA_LEN_4] = {0};
	uint8_t retry_cnt = 0;

	himax_in_parse_assign_cmd(hx123a_fw_addr_reload_to_active, addr, sizeof(addr));
...
#endif
static void hx83123a_sense_on(uint8_t FlashMode)
{
	...
	hx83123a_reload_to_active();
#endif
}

static bool hx83123a_sense_off(bool check_en)
{
	uint8_t cnt = 0;
	uint8_t tmp_data[DATA_LEN_4];
...
static bool hx83123a_read_event_stack(uint8_t *buf, uint8_t length)
{
...
void himax_hx83123a_cmd_init(void)
{
	I("%s: Entering!\n", __func__);
#ifdef CORE_IC
	...
	himax_in_parse_assign_cmd(hx123a_ic_cmd_incr4, pic_op->data_incr4, sizeof(pic_op->data_incr4));
...
//	himax_in_parse_assign_cmd(fw_addr_safe_mode_release_pw, pfw_op->addr_safe_mode_release_pw, sizeof(pfw_op->addr_safe_mode_release_pw));
	...
	himax_in_parse_assign_cmd(hx123a_fw_addr_hsen_enable, pfw_op->addr_hsen_enable, sizeof(pfw_op->addr_hsen_enable));
	himax_in_parse_assign_cmd(hx123a_fw_addr_smwp_enable, pfw_op->addr_smwp_enable, sizeof(pfw_op->addr_smwp_enable));
...
//	himax_in_parse_assign_cmd(fw_addr_program_reload_page_write, pfw_op->addr_program_reload_page_write, sizeof(pfw_op->addr_program_reload_page_write));
	himax_in_parse_assign_cmd(hx123a_fw_addr_raw_out_sel, pfw_op->addr_raw_out_sel, sizeof(pfw_op->addr_raw_out_sel));
	...
	himax_in_parse_assign_cmd(hx123a_fw_addr_selftest_addr_en, pfw_op->addr_selftest_addr_en, sizeof(pfw_op->addr_selftest_addr_en));
	himax_in_parse_assign_cmd(hx123a_fw_addr_criteria_addr, pfw_op->addr_criteria_addr, sizeof(pfw_op->addr_criteria_addr));
	himax_in_parse_assign_cmd(hx123a_fw_addr_set_frame_addr, pfw_op->addr_set_frame_addr, sizeof(pfw_op->addr_set_frame_addr));
	himax_in_parse_assign_cmd(hx123a_fw_addr_selftest_result_addr, pfw_op->addr_selftest_result_addr, sizeof(pfw_op->addr_selftest_result_addr));
	himax_in_parse_assign_cmd(hx123a_fw_addr_gesture_en, pfw_op->addr_gesture_en, sizeof(pfw_op->addr_gesture_en));
	himax_in_parse_assign_cmd(hx123a_fw_addr_sorting_mode_en, pfw_op->addr_sorting_mode_en, sizeof(pfw_op->addr_sorting_mode_en));
	himax_in_parse_assign_cmd(hx123a_fw_addr_fw_mode_status, pfw_op->addr_fw_mode_status, sizeof(pfw_op->addr_fw_mode_status));
...
//	himax_in_parse_assign_cmd(fw_addr_trigger_addr, pfw_op->addr_trigger_addr, sizeof(pfw_op->addr_trigger_addr));
	himax_in_parse_assign_cmd(hx123a_fw_addr_fw_ver_addr, pfw_op->addr_fw_ver_addr, sizeof(pfw_op->addr_fw_ver_addr));
	himax_in_parse_assign_cmd(hx123a_fw_addr_fw_cfg_addr, pfw_op->addr_fw_cfg_addr, sizeof(pfw_op->addr_fw_cfg_addr));
	himax_in_parse_assign_cmd(hx123a_fw_addr_fw_vendor_addr, pfw_op->addr_fw_vendor_addr, sizeof(pfw_op->addr_fw_vendor_addr));
	himax_in_parse_assign_cmd(hx123a_fw_addr_cus_info, pfw_op->addr_cus_info, sizeof(pfw_op->addr_cus_info));
	himax_in_parse_assign_cmd(hx123a_fw_addr_proj_info, pfw_op->addr_proj_info, sizeof(pfw_op->addr_proj_info));
//	himax_in_parse_assign_cmd(fw_addr_fw_state_addr, pfw_op->addr_fw_state_addr, sizeof(pfw_op->addr_fw_state_addr));
	...
	himax_in_parse_assign_cmd(hx123a_fw_addr_fw_dbg_msg_addr, pfw_op->addr_fw_dbg_msg_addr, sizeof(pfw_op->addr_fw_dbg_msg_addr));
	...
	himax_in_parse_assign_cmd(hx123a_fw_addr_dd_data_addr, pfw_op->addr_dd_data_addr, sizeof(pfw_op->addr_dd_data_addr));
//	himax_in_parse_assign_cmd(hx123a_fw_addr_clr_fw_record_dd_sts, pfw_op->addr_clr_fw_record_dd_sts, sizeof(pfw_op->addr_clr_fw_record_dd_sts));
	himax_in_parse_assign_cmd(hx123a_fw_addr_ap_notify_fw_sus, pfw_op->addr_ap_notify_fw_sus, sizeof(pfw_op->addr_ap_notify_fw_sus));
...
//	himax_in_parse_assign_cmd(fw_data_sorting_status, pfw_op->data_sorting_status, sizeof(pfw_op->data_sorting_status));
	...
	himax_in_parse_assign_cmd(hx123a_fw_usb_detect_addr, pfw_op->addr_usb_detect, sizeof(pfw_op->addr_usb_detect));
#ifdef HX_P_SENSOR
...
#endif
	himax_in_parse_assign_cmd(hx123a_fw_addr_ctrl_mpap_ovl, pfw_op->addr_ctrl_mpap_ovl, sizeof(pfw_op->addr_ctrl_mpap_ovl));
	himax_in_parse_assign_cmd(fw_data_ctrl_mpap_ovl_on, pfw_op->data_ctrl_mpap_ovl_on, sizeof(pfw_op->data_ctrl_mpap_ovl_on));

	himax_in_parse_assign_cmd(hx123a_fw_addr_normal_noise_thx, pfw_op->addr_normal_noise_thx, sizeof(pfw_op->addr_normal_noise_thx));
	himax_in_parse_assign_cmd(hx123a_fw_addr_lpwug_noise_thx, pfw_op->addr_lpwug_noise_thx, sizeof(pfw_op->addr_lpwug_noise_thx));
	himax_in_parse_assign_cmd(hx123a_fw_addr_noise_scale, pfw_op->addr_noise_scale, sizeof(pfw_op->addr_noise_scale));
	himax_in_parse_assign_cmd(hx123a_fw_addr_recal_thx, pfw_op->addr_recal_thx, sizeof(pfw_op->addr_recal_thx));
	himax_in_parse_assign_cmd(hx123a_fw_addr_palm_num, pfw_op->addr_palm_num, sizeof(pfw_op->addr_palm_num));
	himax_in_parse_assign_cmd(hx123a_fw_addr_weight_sup, pfw_op->addr_weight_sup, sizeof(pfw_op->addr_weight_sup));
	himax_in_parse_assign_cmd(hx123a_fw_addr_normal_weight_a, pfw_op->addr_normal_weight_a, sizeof(pfw_op->addr_normal_weight_a));
	himax_in_parse_assign_cmd(hx123a_fw_addr_lpwug_weight_a, pfw_op->addr_lpwug_weight_a, sizeof(pfw_op->addr_lpwug_weight_a));
	himax_in_parse_assign_cmd(hx123a_fw_addr_weight_b, pfw_op->addr_weight_b, sizeof(pfw_op->addr_weight_b));
	himax_in_parse_assign_cmd(hx123a_fw_addr_max_dc, pfw_op->addr_max_dc, sizeof(pfw_op->addr_max_dc));
	himax_in_parse_assign_cmd(hx123a_fw_addr_skip_frame, pfw_op->addr_skip_frame, sizeof(pfw_op->addr_skip_frame));
	himax_in_parse_assign_cmd(hx123a_fw_addr_neg_noise_sup, pfw_op->addr_neg_noise_sup, sizeof(pfw_op->addr_neg_noise_sup));
	himax_in_parse_assign_cmd(fw_data_neg_noise, pfw_op->data_neg_noise, sizeof(pfw_op->data_neg_noise));
	// himax_in_parse_assign_cmd(hx123a_fw_addr_mpfw_option, pfw_op->addr_mpfw_option, sizeof(pfw_op->addr_mpfw_option));
	// himax_in_parse_assign_cmd(hx123a_fw_addr_mpfw_mode_status, pfw_op->addr_mpfw_mode_status, sizeof(pfw_op->addr_mpfw_mode_status));
	// himax_in_parse_assign_cmd(hx123a_fw_addr_leave_idle, pfw_op->addr_leave_idle, sizeof(pfw_op->addr_leave_idle));
#ifdef SEC_FACTORY_MODE
	himax_in_parse_assign_cmd(HX123A_FW_EDGE_BORDER_ADDR, pfw_op->addr_edge_border, sizeof(pfw_op->addr_edge_border));
//	himax_in_parse_assign_cmd(FW_CAL_ADDR, pfw_op->addr_cal, sizeof(pfw_op->addr_cal));
	himax_in_parse_assign_cmd(HX123A_FW_RPORT_THRSH_ADDR, pfw_op->addr_rport_thrsh, sizeof(pfw_op->addr_rport_thrsh));

	himax_in_parse_assign_cmd(hx123a_fw_addr_rotate, pfw_op->addr_rotative_mode,
	...
	pfw_op->addr_grip_zone = hx123a_fw_addr_grip_zone;
	pfw_op->addr_reject_zone = hx123a_fw_addr_reject_zone;
	pfw_op->addr_reject_zone_boud = hx123a_fw_addr_reject_zone_bound;
	pfw_op->addr_except_zone = hx123a_fw_addr_except_zone;
	pfw_op->addr_except_top_bot_zone = hx123a_fw_addr_except_top_bot_zone;

	himax_in_parse_assign_cmd(hx83123a_addr_snr_measurement, pfw_op->addr_snr_measurement,
		sizeof(pfw_op->addr_snr_measurement));

	himax_in_parse_assign_cmd(hx123a_fw_addr_proximity, pfw_op->addr_proximity_mode,
		sizeof(pfw_op->addr_proximity_mode));

	himax_in_parse_assign_cmd(hx123a_func_base, pfw_op->addr_mode_base,
		sizeof(pfw_op->addr_mode_base));

	himax_in_parse_assign_cmd(hx83123a_addr_osr_ctrl, pfw_op->addr_osr_ctrl,
		sizeof(pfw_op->addr_osr_ctrl));

	himax_in_parse_assign_cmd(hx83123a_addr_reject_idle, pfw_op->addr_reject_idle,
...
#ifdef CORE_SRAM
	/* sram start*/
	himax_in_parse_assign_cmd(hx123a_sram_adr_mkey, psram_op->addr_mkey, sizeof(psram_op->addr_mkey));
	himax_in_parse_assign_cmd(hx123a_sram_adr_rawdata_addr, psram_op->addr_rawdata_addr, sizeof(psram_op->addr_rawdata_addr));
...
#endif
#ifdef CORE_DRIVER
	himax_in_parse_assign_cmd(hx123a_driver_addr_fw_define_flash_reload, pdriver_op->addr_fw_define_flash_reload, sizeof(pdriver_op->addr_fw_define_flash_reload));
	himax_in_parse_assign_cmd(hx123a_driver_addr_fw_define_2nd_flash_reload, pdriver_op->addr_fw_define_2nd_flash_reload, sizeof(pdriver_op->addr_fw_define_2nd_flash_reload));
	himax_in_parse_assign_cmd(hx123a_driver_addr_fw_define_int_is_edge, pdriver_op->addr_fw_define_int_is_edge, sizeof(pdriver_op->addr_fw_define_int_is_edge));
	himax_in_parse_assign_cmd(hx123a_driver_addr_fw_define_rxnum_txnum, pdriver_op->addr_fw_define_rxnum_txnum_maxpt, sizeof(pdriver_op->addr_fw_define_rxnum_txnum_maxpt));
	himax_in_parse_assign_cmd(hx123a_driver_addr_fw_define_maxpt_xyrvs, pdriver_op->addr_fw_define_xy_res_enable, sizeof(pdriver_op->addr_fw_define_xy_res_enable));
	himax_in_parse_assign_cmd(hx123a_driver_addr_fw_define_x_y_res, pdriver_op->addr_fw_define_x_y_res, sizeof(pdriver_op->addr_fw_define_x_y_res));
	himax_in_parse_assign_cmd(hx123a_driver_data_df_rx, pdriver_op->data_df_rx, sizeof(pdriver_op->data_df_rx));
	himax_in_parse_assign_cmd(hx123a_driver_data_df_tx, pdriver_op->data_df_tx, sizeof(pdriver_op->data_df_tx));
	himax_in_parse_assign_cmd(hx123a_driver_data_df_pt, pdriver_op->data_df_pt, sizeof(pdriver_op->data_df_pt));
	...
	himax_in_parse_assign_cmd(hx123a_zf_addr_ovl_handshake, pzf_op->addr_ovl_handshake, sizeof(pzf_op->addr_ovl_handshake));
#endif

#endif
	himax_in_parse_assign_cmd(hx83123a_addr_ic_ver_name,
		pfw_op->addr_ver_ic_name,
		sizeof(pfw_op->addr_ver_ic_name));
	himax_in_parse_assign_cmd(hx83123a_fw_addr_gesture_history,
		pfw_op->addr_gesture_history,
		sizeof(pfw_op->addr_gesture_history));
}
#if defined(HX_ZERO_FLASH)	
static void hx83123a_en_hw_crc(int en)
{
	hx_mcu_opt_hw_crc(en);
}
#endif
static void himax_hx83123a_func_re_init(void)
{
	I("%s:Entering!\n", __func__);

	g_core_fp.fp_chip_init = hx83123a_chip_init;
	g_core_fp.fp_sense_on = hx83123a_sense_on;
	g_core_fp.fp_sense_off = hx83123a_sense_off;
	g_core_fp.fp_read_event_stack = hx83123a_read_event_stack;
	g_core_fp.fp_burst_enable = hx83123_burst_enable;
#if defined(HX_ZERO_FLASH)	
	g_core_fp._en_hw_crc = hx83123a_en_hw_crc;
	g_core_fp._setup_opt_hw_crc = hx_mcu_setup_opt_hw_crc;
#endif
}

static bool hx83123_chip_detect(void)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t tmp_addr[DATA_LEN_4];
	bool ret_data = false;
...
#if defined(HX_ZERO_FLASH)
	if (hx83123_sense_off(false) == false) {
#else
	if (hx83123_sense_off(true) == false) {
#endif
		ret_data = false;
		E("%s:hx83123_sense_off Fail:\n", __func__);
		return ret_data;
	}
	...
	for (i = 0; i < 5; i++) {
		...
		tmp_addr[1] = 0x00;
		tmp_addr[0] = 0xD0;
		if (hx83123_register_read(tmp_addr, DATA_LEN_4, tmp_data) != 0) {
			ret_data = false;
			E("%s:hx83123_register_read Fail:\n", __func__);
			return ret_data;
		}
		...
		&& (tmp_data[1] == 0x3a)) {
			strlcpy(private_ts->chip_name,
				HX_83123A_SERIES_PWON,
				30);

			ic_data->flash_size = HX83123A_FLASH_SIZE;

			ic_data->dsram_size = hx83123a_dsram_size;
			ic_data->isram_size = hx83123a_isram_size;
			ic_data->dsram_addr = hx83123a_dsram_addr;
			ic_data->isram_addr = hx83123a_isram_addr;
#if defined(HX_ZERO_FLASH)
			g_zf_opt_crc.en_crc_clear = false;
			g_zf_opt_crc.fw_addr = 0;
			g_zf_opt_crc.en_opt_hw_crc = OPT_CRC_BOTH;
#endif
			I("%s:detect IC HX83123A successfully\n", __func__);
			...
			himax_hx83123a_cmd_init();
			himax_hx83123a_func_re_init();
...
bool _hx83123_init(void)
{
	bool ret = false;

	I("%s\n", __func__);
	ret = hx83123_chip_detect();
	return ret;
}
