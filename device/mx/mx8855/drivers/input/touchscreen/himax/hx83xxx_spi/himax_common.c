 ...
 *	Left(DF), Right(DF)}
 */
	uint8_t gest_event[GEST_SUP_NUM] = {0x80, 0x01};

/*gest_event mapping to gest_key_def*/
	uint16_t gest_key_def[GEST_SUP_NUM] = {114, 114};
...
unsigned long ADDR_VER_IC_NAME;
EXPORT_SYMBOL(ADDR_VER_IC_NAME);
...
int himax_set_gesture(int gesture_state)
{
	...
	do {
		...
		if (memcmp(rec_data, data, DATA_LEN_4) == 0)
			break;
	} while (retry++ < 10);
								...
								size_t len, loff_t *pos)
{
	...
	if (buf[0] == '0')
		ts->plat_data->lowpower_mode = 0;
	else if (buf[0] == '1')
		ts->plat_data->lowpower_mode = 1;
	else
		return -EINVAL;

	g_core_fp.fp_set_SMWP_enable(ts->plat_data->lowpower_mode, ts->suspended);
	HX_SMWP_EN = ts->plat_data->lowpower_mode;
								   ...
								   size_t len, loff_t *pos)
{
	...
	if (buf[0] == '0' && ts->plat_data->lowpower_mode > 0) {
		ts->psensor_flag = false;
		g_core_fp.fp_black_gest_ctrl(false);
	}	else if (buf[0] == '1' && ts->plat_data->lowpower_mode > 0) {
		ts->psensor_flag = true;
		g_core_fp.fp_black_gest_ctrl(true);
	} else if (ts->plat_data->lowpower_mode == 0) {
...
static int i_get_FW(void)
{
	...
	while (retry_count--) {
		if (request_firmware(&image, private_ts->plat_data->firmware_name, private_ts->dev) == 0) {
	...
	if (image != NULL) {
		...
		private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_IC_VER] = image->data[ADDR_VER_IC_NAME];
		private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_VER_PROJECT_ID] = image->data[CID_VER_MAJ_FLASH_ADDR];
		private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_MODULE_VER] = image->data[PANEL_VERSION_ADDR];
		private_ts->plat_data->img_version_of_bin[SEC_INPUT_FW_VER] = image->data[CID_VER_MIN_FLASH_ADDR];
...
#if defined(HX_SMART_WAKEUP) || defined(HX_TOUCH_PROXIMITY)
#ifdef HX_GESTURE_TRACK
...
#ifdef HX_TOUCH_PROXIMITY
	if (ts->plat_data->ed_enable >= 1) {
		if (buf[GEST_PTLG_ID_LEN] == 0xDD &&
			buf[GEST_PTLG_ID_LEN + 1] == 0x55) {
			input_info(true, ts->dev,
				"%s %s: Proximity mode for suspend = 0x%02X,0x%02X,0x%02X,0x%02X \n",
				HIMAX_LOG_TAG, __func__, buf[0], buf[1], buf[2], buf[3]);
			ts->ear_detect_val = buf[0];
			input_info(true, ts->dev, "%s %s proximity state=%d\n",
					HIMAX_LOG_TAG, __func__, ts->ear_detect_val);
			input_report_abs(ts->input_dev_proximity, ABS_MT_CUSTOM, ts->ear_detect_val);
...
static void himax_wake_event_report(void)
{
	...
	if (KEY_EVENT) {
		I(" %s SMART WAKEUP KEY event %d press & release(%d)\n", __func__, KEY_EVENT, KEY_WAKEUP);
...
int himax_cable_detect_func(struct device *dev, bool on)
{
...
static void hx_location_detect(struct himax_ts_data *ts, char *loc, int x, int y)
{
	memset(loc, 0, 4);

	if (x < 60)
		strncat(loc, "E.", 2);
	else if (x < (ts->pdata->screenWidth - 60))
		strncat(loc, "C.", 2);
	else
		strncat(loc, "e.", 2);
	...
	if (y < 48)
		strncat(loc, "S", 1);
	else if (y < (ts->pdata->screenHeight - 96))
		strncat(loc, "C", 1);
	else
		strncat(loc, "N", 1);
}
...
void hx_log_touch_event(struct himax_ts_data *ts)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		if ((((ts->old_finger >> loop_i & 1) == 0) && (g_target_report_data->finger_id[loop_i] == 1))) {
			...
			I("[P] tID:%d.%d x:%d y:%d p:%d major:%d minor:%d loc:%s tc:%d\n",
				...
				ts->plat_data->touch_count);
#else
			I("[P] tID:%d.%d x:%d y:%d loc:%s tc:%d\n",
				loop_i, (ts->input_dev->mt->trkid - 1) & TRKID_MAX, x, y,
				loc, ts->plat_data->touch_count);
#endif
#else
#ifdef SEC_PALM_FUNC
			I("[P] tID:%d.%d p:%d major:%d minor:%d loc:%s tc:%d\n",
				...
				ts->plat_data->touch_count);
#else
			I("[P] tID:%d.%d loc:%s tc:%d\n",
				loop_i, (ts->input_dev->mt->trkid - 1) & TRKID_MAX,
				loc, ts->plat_data->touch_count);
#endif
#endif
			ts->plat_data->hw_param.all_finger_count++;
			if ((ts->plat_data->touch_count > 4) && (ts->plat_data->hw_param.check_multi == 0)) {
				ts->plat_data->hw_param.check_multi = 1;
				ts->plat_data->hw_param.multi_count++;
			}
		} else if ((((ts->old_finger >> loop_i & 1) == 1) && (g_target_report_data->finger_id[loop_i] == 0))) {
			if (ts->plat_data->touch_count > 0)
				ts->plat_data->touch_count--;
			if (ts->plat_data->touch_count == 0) {
				ts->plat_data->print_info_cnt_release = 0;
				ts->plat_data->hw_param.check_multi = 0;
			}
			...
			I("[R] tID:%d loc:%s dd:%d,%d pc:%d mc:%d tc:%d lx:%d ly:%d\n",
				...
				mc, ts->plat_data->touch_count, x, y);
#else
			I("[R] tID:%d loc:%s dd:%d,%d pc:%d mc:%d tc:%d\n",
				loop_i, loc, x - ts->p_x[loop_i], y - ts->p_y[loop_i],
				bitmap_weight(&ts->palm_flag, ts->nFinger_support),
				mc, ts->plat_data->touch_count);
#endif
		}
	}
}
...
static int himax_ts_work_status(struct himax_ts_data *ts)
{
	...
	if (atomic_read(&ts->suspend_mode) && (!FAKE_POWER_KEY_SEND)
		&& (ts->plat_data->lowpower_mode) && (!hx_touch_data->diag_cmd)) {
		if (g_ts_dbg)
			input_info(true, ts->dev,
				"%s %s:Now is SMWP! \n", HIMAX_LOG_TAG,
				__func__);
...
#endif
#ifdef HX_TOUCH_PROXIMITY
	if (ts->plat_data->ed_enable && ts->suspended) {
		if (g_ts_dbg)
			input_info(true, ts->dev,
				"%s %s:Now is PROXIMITY! \n", HIMAX_LOG_TAG,
				__func__);
		result = HX_REPORT_PROXIMITY;
	}
#endif
	/* I("Now Status is %d\n", result); */
	return result;
...
static int himax_touch_get(struct himax_ts_data *ts, uint8_t *buf, int ts_path, int ts_status)
{
	if (g_ts_dbg != 0)
		input_info(true, ts->dev,
				"%s %s: Entering, ts_status=%d, ts_path=%d! \n", HIMAX_LOG_TAG,
				__func__, ts_status, ts_path);
	...
	case HX_REPORT_SMWP_EVENT:
		__pm_wakeup_event(ts->plat_data->sec_ws, TS_WAKE_LOCK_TIMEOUT);
		msleep(20);
	 ...
	 /*SMWP*/ case HX_REPORT_PROXIMITY:
		g_core_fp.fp_burst_enable(0);
		input_err(true, ts->dev,
					"%s %s: read prxoimity, size=%d!\n",
					HIMAX_LOG_TAG, __func__, hx_touch_data->event_size);
		if (!g_core_fp.fp_read_event_stack(buf, hx_touch_data->event_size)) {
			input_err(true, ts->dev,
...
static void himax_touch_info(void)
{
	if (strcmp(HX_83108A_SERIES_PWON, private_ts->chip_name) == 0
		...
		|| strcmp(HX_83123A_SERIES_PWON, private_ts->chip_name) == 0) {
		...
		if (g_last_fw_irq_flag2 != hx_touch_data->hx_state_info[1]) {
			if ((g_last_fw_irq_flag2 & 0x01) != (hx_touch_data->hx_state_info[1] & 0x01))
				I("%s Glove mode change to %d\n",
						HIMAX_LOG_TAG, hx_touch_data->hx_state_info[1] & 0x01);
			if ((g_last_fw_irq_flag2 >> 1 & 0x01) != (hx_touch_data->hx_state_info[1] >> 1 & 0x01))
				I("%s Border change to %d\n",
						HIMAX_LOG_TAG, hx_touch_data->hx_state_info[1] >> 1 & 0x01);
			if ((g_last_fw_irq_flag2 >> 2 & 0x01) != (hx_touch_data->hx_state_info[1] >> 2 & 0x01))
				I("%s VR flag change to %d\n",
						HIMAX_LOG_TAG, hx_touch_data->hx_state_info[1] >> 2 & 0x01);
			if ((g_last_fw_irq_flag2 >> 3 & 0x01) != (hx_touch_data->hx_state_info[1] >> 3 & 0x01))
				I("%s AC mode change to %d\n",
						HIMAX_LOG_TAG, hx_touch_data->hx_state_info[1] >> 3 & 0x01);
			if ((g_last_fw_irq_flag2 >> 4 & 0x01) != (hx_touch_data->hx_state_info[1] >> 4 & 0x01))
				I("%s Lamp noise mode change to %d\n",
						HIMAX_LOG_TAG, hx_touch_data->hx_state_info[1] >> 4 & 0x01);
		}
		...
		if (g_last_fw_irq_flag2 != hx_touch_data->hx_state_info[1]) {
			...
			atomic_set(&private_ts->plat_data->touch_noise_status, hx_touch_data->hx_state_info[1] >> 3 & 0x01);
			if ((g_last_fw_irq_flag2 >> 3 & 0x01) != atomic_read(&private_ts->plat_data->touch_noise_status)) {
				I("%s noise mode change to %d\n",
						HIMAX_LOG_TAG, atomic_read(&private_ts->plat_data->touch_noise_status));
				if (atomic_read(&private_ts->plat_data->touch_noise_status))
					private_ts->plat_data->hw_param.noise_count++;
			}

			atomic_set(&private_ts->plat_data->touch_pre_noise_status, hx_touch_data->hx_state_info[1] >> 4 & 0x01);
			if ((g_last_fw_irq_flag2 >> 4 & 0x01) != atomic_read(&private_ts->plat_data->touch_pre_noise_status)) {
				I("%s lamp noise mode change to %d\n",
						HIMAX_LOG_TAG, atomic_read(&private_ts->plat_data->touch_pre_noise_status));
			}
		}
		g_last_fw_irq_flag = hx_touch_data->hx_state_info[0];
		g_last_fw_irq_flag2 = hx_touch_data->hx_state_info[1];
...
#endif
#if defined(HX_TOUCH_PROXIMITY)
	} else if (ts_path == HX_REPORT_PROXIMITY) {
		memcpy(hx_touch_data->hx_event_buf, buf, hx_touch_data->event_size);
#endif
	} else {
		E("%s, Fail Path!\n", __func__);
...
int himax_parse_report_points(struct himax_ts_data *ts, int ts_path, int ts_status)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		...
		if (x >= 0 && x <= ts->plat_data->max_x && y >= 0 && y <= ts->plat_data->max_y) {
...
static void himax_report_all_leave_event(struct himax_ts_data *ts)
{
	...
	if (ts->plat_data->prox_power_off == 1) {
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		...
		if (g_target_report_data->finger_id[loop_i] != 0) {
			...
			I("[RA] tID:%d loc:%s dd:%d,%d mc:%d tc:%d lx:%d ly:%d\n", loop_i,
				loc, x - ts->p_x[loop_i], y - ts->p_y[loop_i], mc, ts->plat_data->touch_count, x, y);
#else
			I("[RA] tID:%d loc:%s dd:%d,%d mc:%d tc:%d\n", loop_i,
				loc, x - ts->p_x[loop_i], y - ts->p_y[loop_i], mc, ts->plat_data->touch_count);
#endif
		}
	}
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		if (g_target_report_data->x[i] >= 0 && g_target_report_data->x[i] <= ts->plat_data->max_x
				&& g_target_report_data->y[i] >= 0
				&& g_target_report_data->y[i] <= ts->plat_data->max_y)
			valid = true;
		else
			valid = false;
		if (g_ts_dbg != 0)
...
#if defined(HX_TOUCH_PROXIMITY)
//		input_info(true, ts->dev, "%s ear_detect_mode:%d,ear_detect_val:%d\n",
//				HIMAX_LOG_TAG, ts->plat_data->ed_enable, ts->ear_detect_val);
		if (ts->plat_data->ed_enable >= 1) {
			if (ts->ear_detect_val !=  ((hx_touch_data->hx_state_info[1] >> 4) & 0x0F)) {
				ts->ear_detect_val = ((hx_touch_data->hx_state_info[1] >> 4) & 0x0F);
				input_info(true, ts->dev, "%s %s Proximity state =%d\n",
					HIMAX_LOG_TAG, __func__, ts->ear_detect_val);
				input_report_abs(ts->input_dev_proximity, ABS_MT_CUSTOM, ts->ear_detect_val);
				input_sync(ts->input_dev_proximity);
...
void himax_ts_work(struct himax_ts_data *ts)
{
	...
	if ((atomic_read(&ts->suspend_mode) == HIMAX_STATE_LPM) || (atomic_read(&ts->suspend_mode) == HIMAX_STATE_PROX_LPM)) {
		__pm_wakeup_event(ts->plat_data->sec_ws, TS_WAKE_LOCK_TIMEOUT);
		if (!ts->plat_data->resume_done.done) {
			if (!IS_ERR_OR_NULL(ts->plat_data->irq_workqueue)) {
				input_info(true, ts->dev, "%s: disable irq and queue waiting work\n", __func__);
				himax_int_enable(INT_DISABLE_NOSYNC);
				queue_work(ts->plat_data->irq_workqueue, &ts->plat_data->irq_work);
			} else {
				input_err(true, ts->dev, "%s: irq_workqueue not exist\n", __func__);
			}
			return;
		}
	}

	if (debug_data != NULL && debug_data->fp_ts_dbg_func)
...
#ifdef HX_RST_PIN_FUNC
	if (g_core_fp.fp_ic_reset)
		g_core_fp.fp_ic_reset(false, true);
#else
	if (g_core_fp.fp_system_reset)
...
static void himax_print_info_work(struct work_struct *work)
{
	...
	sec_input_print_info(ts->dev, NULL);
...
int himax_chip_common_init(void)
{
	...
	ret = himax_regulator_init(ts);
	if (ret < 0) {
		E("%s: regulator init failed %d\n", __func__, ret);
	...
	atomic_set(&ts->plat_data->shutdown_called, 0);
#if defined(HX_USB_DETECT_GLOBAL)
	...
	ts->input_dev_pad = ts->plat_data->input_dev_pad;
#ifdef HX_TOUCH_PROXIMITY
	ts->input_dev_proximity = ts->plat_data->input_dev_proximity;
#endif
	...
	ts->plat_data->sec_ws = wakeup_source_register(NULL, HIMAX_common_NAME);
#else
	wakeup_source_init(ts->plat_data->sec_ws, HIMAX_common_NAME);
#endif
#endif
#ifdef HX_HIGH_SENSE
	ts->HSEN_enable = 0;
...
#ifdef HX_ZERO_FLASH
	g_core_fp.fp_0f_operation(&ts->work_0f_update.work);
#endif
...
#else
	wakeup_source_trash(ts->plat_data->sec_ws);
#endif
#endif
...
int himax_pinctrl_configure(struct himax_ts_data *ts, bool active)
{
	...
	I("%s : %s%s\n", __func__, active ? "ACTIVE" : "SUSPEND",
			ts->plat_data->gpio_spi_cs > 0 ? " control cs pin" : "");
...
int himax_regulator_init(struct himax_ts_data *ts)
{
	...
	if (ts->pdata->name_lcd_rst) {
		ts->regulator_lcd_rst = devm_regulator_get(ts->dev, ts->pdata->name_lcd_rst);
		if (IS_ERR(ts->regulator_lcd_rst)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_lcd_rst regulator.", __func__);
			return -ENODEV;
		}
		input_info(true, ts->dev, "%s: init %s regulator\n", __func__, ts->pdata->name_lcd_rst);
	}

	for (i = 0; i < ts->regulator_count; i++) {
		ts->regulator[i] = devm_regulator_get(ts->dev, ts->pdata->regulator_name[i]);
		if (IS_ERR(ts->regulator[i])) {
			input_err(true, ts->dev, "%s: Failed to get %s regulator.", __func__, ts->pdata->regulator_name[i]);
			return -ENODEV;
		}
		input_info(true, ts->dev, "%s: init %s regulator\n", __func__, ts->pdata->regulator_name[i]);
	}
...
int himax_ctrl_lcd_reset_regulator(struct himax_ts_data *ts, bool on)
{
	...
	if (!ts->pdata->name_lcd_rst) {
		input_err(true, ts->dev, "%s: name_lcd_rst is null\n", __func__);
		return 0;
	}
	...
	if (on) {
		ret = regulator_enable(ts->regulator_lcd_rst);
		if (ret) {
...
out:
	input_info(true, ts->dev, "%s: %s: done. reset:%d\n",
			__func__, on ? "on" : "off", regulator_is_enabled(ts->regulator_lcd_rst));
...
int himax_ctrl_lcd_regulators(struct himax_ts_data *ts, bool on)
{
	...
	if (on) {
		for (i = 0; i < ts->regulator_count; i++) {
			ret = regulator_enable(ts->regulator[i]);
			if (ret) {
				input_err(true, ts->dev, "%s: Failed to enable %s: %d\n",
						__func__, ts->pdata->regulator_name[i], ret);
				return ret;
			}
			snprintf(temp, sizeof(temp), "%s:%d ",
					ts->pdata->regulator_name[i],
					regulator_is_enabled(ts->regulator[i]));
			strlcat(regulator_log, temp, sizeof(regulator_log));
		}
	} else {
	...
	input_info(true, ts->dev, "%s: %s: done. %s\n", __func__, on ? "on" : "off", regulator_log);
...
int himax_chip_common_early_suspend(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev, "%s %s: START lp:0x%02x cover:%d, ear_mode:%d, ear_val:%d, suspend_mode:%d\n",
			HIMAX_LOG_TAG, __func__, ts->plat_data->lowpower_mode, ts->cover_closed,
			ts->plat_data->ed_enable, ts->ear_detect_val, atomic_read(&ts->suspend_mode));
	...
	if (ts->proxy_1b_en == 0) {
		if (ts->plat_data->ed_enable && (ts->aot_enabled || ts->spay_enabled))
			ts->plat_data->lowpower_mode = 0;
	}
	...
	if (ts->plat_data->lowpower_mode || ts->plat_data->ed_enable) {
		...
		if (ts->plat_data->ed_enable) {
			atomic_set(&ts->suspend_mode, HIMAX_STATE_PROX_LPM);
			I("[himax] %s: set HIMAX_STATE_PROX_LPM\n", __func__);
		} else {
			atomic_set(&ts->suspend_mode, HIMAX_STATE_LPM);
			I("[himax] %s: set SMART_WAKEUP\n", __func__);
		}
...
#endif
	{
		...
		I("[himax] %s: set HIMAX_STATE_POWER_OFF\n", __func__);
		ts->pre_finger_mask = 0;
...
int himax_chip_common_late_suspend(struct himax_ts_data *ts)
{
	...
	if (ts->late_suspended) {
		I("%s: Already late_suspended. Skipped.\n", __func__);
		goto END;
	} else {
	...
	if (atomic_read(&ts->suspend_mode) == HIMAX_STATE_PROX_LPM) {
		if (ts->prox_lp_scan_mode_enabled) {
...
//	int gesture_id = 0; /* 0: double tap, 1: swipe up*/
	...
	if (pack_num > 0) {
		...
		each_packet = kcalloc(pack_num, sizeof(uint8_t *), GFP_KERNEL);
		...
		for (i = 0; i < pack_num; i++) {
			...
			each_packet[i] = kcalloc(5, sizeof(uint8_t), GFP_KERNEL);
			for (j = 0 ; j < 5; j++) {
				each_packet[i][j] = total_buffer[i * 5 + j + 1];
			}
		}
		/* data in format*/
		for (i = 0; i < pack_num; i++) {
...
int himax_chip_common_early_resume(struct himax_ts_data *ts)
{
	...
	if ((atomic_read(&ts->suspend_mode) != HIMAX_STATE_POWER_ON) && (atomic_read(&ts->suspend_mode) != HIMAX_STATE_POWER_OFF)) {
...
int himax_chip_common_late_resume(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev, "%s %s: START lp:%d(%d/%d) cover:%d, ear_mode:%d, ear_val:%d, suspend_mode:%d\n",
			HIMAX_LOG_TAG, __func__, ts->plat_data->lowpower_mode, ts->aot_enabled, ts->spay_enabled,
			ts->cover_closed, ts->plat_data->ed_enable, ts->ear_detect_val, atomic_read(&ts->suspend_mode));
...
#if defined(HX_ZERO_FLASH) && defined(HX_RESUME_SET_FW)
	input_info(true, ts->dev, "%s %s: system reset!\n", HIMAX_LOG_TAG, __func__);
	g_core_fp.fp_system_reset();
	...
	ts->prox_lp_scan_mode_enabled = false;
#ifdef HX_TOUCH_PROXIMITY
	psensor_enable(ts, ts->plat_data->ed_enable);
#endif
...
#ifdef HX_CODE_OVERLAY
	if (ts->plat_data->lowpower_mode && ts->in_self_test == 0)
