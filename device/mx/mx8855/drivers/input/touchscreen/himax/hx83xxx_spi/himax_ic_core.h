...
#define HIMAX_REG_WRITE_READ_RETRY_TIMES 100
...
#if defined(__HIMAX_HX83123_MOD__)
#define HX_MOD_KSYM_HX83123            HX_MOD_KSYM_HX83123
#endif
...
#if IS_ENABLED(CONFIG_TOUCHSCREEN_HIMAX_INCELL_CORE)
...
#if IS_ENABLED(CONFIG_TOUCHSCREEN_HIMAX_ONCELL)
...
#endif
extern void hx_mcu_setup_opt_hw_crc(u8 *data);
extern void hx_mcu_opt_hw_crc(int en);
...
struct hx_guest_info {
	...
	#define fw_addr_ver_ic_name								0x78787878
...
//	#define FW_CAL_ADDR								0x10007088	/*bit[2] */
	...
	#define ADDR_EXCEPT_TOP_BOT_ZONE 0X10007278
...
//	uint8_t addr_cal[4];
	...
	uint32_t addr_except_top_bot_zone;
...
struct himax_core_fp {
	...
	void (*_dd_en_read)(int en);
	void (*_proximity_display_off)(void);
...
//	int (*set_cal_switch) (int set_val);
	...
	void (*fp_write_sram_0f)(u8 *data, uint8_t *addr, int start_index, uint32_t write_len);
	int (*fp_write_sram_0f_crc)(u8 *data, uint8_t *addr, int start_index, uint32_t write_len);
	void (*fp_firmware_update_0f)(u8 *data, int size);
	...
	void (*_setup_opt_hw_crc)(u8 *data);
#endif
