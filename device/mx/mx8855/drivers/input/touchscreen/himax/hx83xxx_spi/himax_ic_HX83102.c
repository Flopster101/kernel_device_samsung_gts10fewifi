...
static void hx83102ab_firmware_update_0f(u8 *data, int size)
{
	uint8_t tmp_addr[4];
	...
	/*bin file start*/
	/*isram*/
	g_core_fp.fp_write_sram_0f(data, pzf_op->data_sram_start_addr, 0, HX_32K_SZ);

	tmp_addr[3] = 0x10;
	tmp_addr[2] = 0x00;
	tmp_addr[1] = 0x73;
	tmp_addr[0] = 0x00;
	g_core_fp.fp_write_sram_0f(data, tmp_addr, 0x8000, (0x200*4));

	tmp_addr[3] = 0x10;
	tmp_addr[2] = 0x00;
	tmp_addr[1] = 0x68;
	tmp_addr[0] = 0x00;
	g_core_fp.fp_write_sram_0f(data, tmp_addr, 0x8800, (0x200*4));
	...
	g_core_fp.fp_write_sram_0f(data, tmp_addr, 0xC178, (84));

	/*FW config*/
	if (G_POWERONOF == 1)
		g_core_fp.fp_write_sram_0f(data, pzf_op->data_fw_cfg_1, 0xBFFE, 388);
	else
		g_core_fp.fp_clean_sram_0f(pzf_op->data_fw_cfg_1, 388, 1);
	/*ADC config*/
	if (G_POWERONOF == 1)
		g_core_fp.fp_write_sram_0f(data, pzf_op->data_adc_cfg_1, 0xe000, (128 * 4));
	else
 ...
 */
	...
	if (count < 10) {
		/*g_core_fp.fp_write_sram_0f(fwp, sram_addr, offset, size);*/
		if (g_core_fp.fp_write_sram_0f_crc((u8 *)fwp->data, sram_addr, offset, size) != 0)
			E("%s, Overlay HW CRC FAIL\n", __func__);
...
static void himax_hx83102j_reg_re_init(void)
{
	...
	himax_in_parse_assign_cmd(hx83102j_FW_RPORT_THRSH_ADDR, pfw_op->addr_rport_thrsh, sizeof(pfw_op->addr_rport_thrsh));
}
