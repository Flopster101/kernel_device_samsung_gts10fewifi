...
static int g_gap_vertical_part[4];
...
static int himax_switch_mode_inspection(int mode)
{
	...
	/*Stop Handshaking*/
	g_core_fp.fp_register_write(psram_op->addr_rawdata_addr, 4, tmp_data, 0);
...
static uint32_t himax_get_rawdata(int32_t RAW[], uint32_t datalen, uint8_t checktype)
{
	...
	for (i = 0; i < ic_data->HX_TX_NUM*ic_data->HX_RX_NUM; i++) {
		if (checktype == HIMAX_WEIGHT_NOISE ||
			...
			checktype == HIMAX_LPWUG_IDLE_NOISE)
			tmp_data =  (((int8_t)tmp_rawdata[(i * 2) + 1] << 8) | tmp_rawdata[(i * 2)]);
		else
			tmp_data =  ((tmp_rawdata[(i * 2) + 1] << 8) | tmp_rawdata[(i * 2)]);
		RAW[i] = tmp_data;
		if (notch_idx < ic_data->notch_sz) {
...
static void himax_bank_search_set(uint16_t Nframe, uint8_t checktype)
{
	...
	g_core_fp.fp_register_read(pfw_op->addr_skip_frame, 4, tmp_data, 0);
	...
	g_core_fp.fp_register_write(pfw_op->addr_skip_frame, 4, tmp_data, 0);
}
...
static void himax_neg_noise_sup(uint8_t *data)
{
	...
	g_core_fp.fp_register_read(pfw_op->addr_neg_noise_sup, 4, tmp_data, 0);

	if ((tmp_data[3] & 0x04) == 0x04) {
		data[2] = pfw_op->data_neg_noise[2];
		data[3] = pfw_op->data_neg_noise[3];

	} else
		I("%s Not support negative noise\n", __func__);

	I("%s: value=0x%02X%02X%02X%02X\n", __func__, data[3], data[2], data[1], data[0]);
}
	...
	if (checktype == HIMAX_WEIGHT_NOISE ||
		...
		if (strcmp(HX_83108A_SERIES_PWON, private_ts->chip_name) == 0
			...
			I("%s: set neg: R%02X%02X%02X%02XH <= 0x%02X%02X%02X%02X\n",
				__func__,
				pfw_op->addr_set_frame_addr[3], pfw_op->addr_set_frame_addr[2], pfw_op->addr_set_frame_addr[1], pfw_op->addr_set_frame_addr[0],
...
static void himax_get_noise_base(uint8_t checktype)/*Normal Threshold*/
{
	...
	switch (checktype) {
	case HIMAX_WEIGHT_NOISE:
		g_core_fp.fp_register_read(pfw_op->addr_normal_noise_thx, 4, tmp_data, 0);
		break;
	case HIMAX_LPWUG_WEIGHT_NOISE:
		g_core_fp.fp_register_read(pfw_op->addr_lpwug_noise_thx, 4, tmp_data, 0);
		break;
	default:
		I("%s Not support type\n", __func__);
	}
	g_core_fp.fp_register_read(pfw_op->addr_noise_scale, 4, tmp_data2, 0);
	tmp_data2[1] = tmp_data2[1] >> 4;
	if (tmp_data2[1] == 0)
		tmp_data2[1] = 1;

	/*normal : 0x1000708F, LPWUG:0x10007093
	* MAX_VALUE = value * scale
	* value is reading from addr_noise_thx
	* scale from addr_noise_scale
	*/
	NOISEMAX = tmp_data[3] * tmp_data2[1];

	g_core_fp.fp_register_read(pfw_op->addr_recal_thx, 4, tmp_data, 0);
...
static uint16_t himax_get_palm_num(void)/*Palm Number*/
{
	...
	g_core_fp.fp_register_read(pfw_op->addr_palm_num, 4, tmp_data, 0);
...
static int himax_get_noise_weight_test(uint8_t checktype)
{
	...
	g_core_fp.fp_register_read(pfw_op->addr_weight_sup, 4, tmp_data, 0);
	if (tmp_data[3] != pfw_op->addr_weight_sup[1] ||
		tmp_data[2] != pfw_op->addr_weight_sup[0])
		return FW_NOT_READY;
	...
	switch (checktype) {
	case HIMAX_WEIGHT_NOISE:
		g_core_fp.fp_register_read(pfw_op->addr_normal_weight_a, 4, tmp_data, 0);
		break;
	case HIMAX_LPWUG_WEIGHT_NOISE:
		g_core_fp.fp_register_read(pfw_op->addr_lpwug_weight_a, 4, tmp_data, 0);
		break;
	default:
		I("%s Not support type\n", __func__);
	}
	...
	g_core_fp.fp_register_read(pfw_op->addr_weight_b, 4, tmp_data, 0);
...
static int hx_turn_on_mp_func(int on)
{
	...
	if (strcmp(HX_83102D_SERIES_PWON, private_ts->chip_name) == 0) { /* Only HX83102D needs this flow */
		...
		if (on) {
			...
			do {
				g_core_fp.fp_register_write(pfw_op->addr_ctrl_mpap_ovl, 4, tmp_data, 0);
				usleep_range(10000, 10001);
				g_core_fp.fp_register_read(pfw_op->addr_ctrl_mpap_ovl, 4, tmp_read, false);
...
{
	I("%s %s: %d,%ld\n", HIMAX_LOG_TAG, __func__, g_gap_vertical_partial, g_gap_vertical_partial * sizeof(int));

	if (g_gap_vertical_partial == 4) {
		...
		g_gap_vertical_part[0] = 0;
		g_gap_vertical_part[1] = 0;
		g_gap_vertical_part[2] = 0;
		g_gap_vertical_part[3] = 0;
	}
...
//	g_dc_max = g_core_fp.fp_get_max_dc();
	...
	if (checktype == HIMAX_ABS_NOISE || checktype == HIMAX_WEIGHT_NOISE) {
		/*  noise weight */
		tmp_addr[3] = pfw_op->addr_weight_sup[3];
		tmp_addr[2] = pfw_op->addr_weight_sup[2];
		tmp_addr[1] = pfw_op->addr_weight_sup[1];
		tmp_addr[0] = pfw_op->addr_weight_sup[0];
		g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);
...
static void get_chip_id(void *dev_data)
{
	...
	snprintf(buf, sizeof(buf), "0x%02x", ic_data->ver_ic_name);
...
static int himax_do_fw_update(const u8 *data, size_t size)
{
	int ret;

	if (!data || size == 0) {
		input_err(true, private_ts->dev, "%s: invalid firmware data\n", __func__);
		return FW_ERR_FILE_READ;
	}
	...
	hx_mcu_bin_desc_get((unsigned char *) data, HX1K);
	ret = himax_zf_part_info((unsigned char *) data);
	if (ret != 0)
		ret = FW_ERR_UPTODATE;
	else
		ret = FW_ERR_NONE;
	...
	default:
		E("%s %s: does not support fw size %d\n",
				HIMAX_LOG_TAG, __func__, (int)size);
	}
	...
	input_info(true, private_ts->dev, "%s: done, ret=%d\n", __func__, ret);
	return ret;
}

static int himax_fw_update_kernel(void)
{
	int ret = 0;
	...
	snprintf(fw_path, MAX_FW_PATH, "%s", private_ts->plat_data->firmware_name);
	...
	ret = himax_do_fw_update(firmware->data, firmware->size);
	if (ret)
		input_err(true, private_ts->dev, "%s: failed to do fw update, ret=%d\n", __func__, ret);
...
static int himax_fw_update_from_storage(void)
{
	int ret = 0;
	const struct firmware *firmware = NULL;
	const u8 *fw_data;
	...
	if (private_ts->plat_data->external_firmware_data && private_ts->plat_data->external_firmware_size > 0) {
		input_info(true, private_ts->dev, "%s: use external_firmware_data\n", __func__);
		if (signing) {
			long spu_ret = 0;
			long ori_size = private_ts->plat_data->external_firmware_size - SPU_METADATA_SIZE(TSP);

			spu_ret = spu_firmware_signature_verify("TSP", private_ts->plat_data->external_firmware_data,
					private_ts->plat_data->external_firmware_size);
			if (spu_ret != ori_size) {
				input_err(true, private_ts->dev, "%s: signature verify failed, spu_ret:%ld, ori_size:%ld\n",
					__func__, spu_ret, ori_size);
				ret = FW_ERR_FILE_SIGN;
				goto FAIL_END;
			}
			private_ts->plat_data->external_firmware_size -= SPU_METADATA_SIZE(TSP);
		}
		fw_data = private_ts->plat_data->external_firmware_data;
		fw_size = private_ts->plat_data->external_firmware_size;
	} else {
	...
	if (fw_size > 0) {
		...
		private_ts->fw_data_ums = vzalloc(fw_size);
		if (!private_ts->fw_data_ums) {
		...
		memcpy(private_ts->fw_data_ums, fw_data, fw_size);
		private_ts->fw_size_ums = fw_size;
	} else {
		I("%s [ERROR] fsize [%zu]\n", __func__, fw_size);
	...
	ret = himax_do_fw_update(fw_data, fw_size);
	if (ret)
		input_err(true, private_ts->dev, "%s: failed to do fw update, ret=%d\n", __func__, ret);
...
FAIL_END:
	if (firmware)
		release_firmware(firmware);
	sec_cmd_io_clear_firmware_data(&private_ts->sec);
	himax_int_enable(INT_ENABLE);
	I("%s: DONE\n", __func__);
	return ret;
}
...
static void fw_update(void *dev_data)
{
	...
	case BUILT_IN:
		ret = himax_fw_update_kernel();
		break;
	case UMS:
		ret = himax_fw_update_from_storage();
		break;
	default:
		ret = -EINVAL;
...
static void get_fw_ver_bin(void *dev_data)
{
	...
	snprintf(buf, sizeof(buf), "%s%02X%02X%02X%02X",
			private_ts->plat_data->ic_vendor_name,
...
static void get_fw_ver_ic(void *dev_data)
{
	...
	if (strcmp(private_ts->pdata->proj_name, "M20") == 0) {
		...
		if (strcmp(HX_83112A_SERIES_PWON, private_ts->chip_name) == 0)
			...
			ic_ver_ic_name = ic_data->ver_ic_name;
...
static int hx_rotate_mode_set(bool is_portrait)
{
	...
	if (is_portrait) {
		data_ary[3] = pfw_op->data_portrait[3];
		data_ary[2] = pfw_op->data_portrait[2];
		data_ary[1] = pfw_op->data_portrait[1];
		data_ary[0] = pfw_op->data_portrait[0];
	} else {
		data_ary[3] = pfw_op->data_landscape[3];
		data_ary[2] = pfw_op->data_landscape[2];
		data_ary[1] = pfw_op->data_landscape[1];
		data_ary[0] = pfw_op->data_landscape[0];
	}

	addr32 = (pfw_op->addr_rotative_mode[3] << 24 |
				pfw_op->addr_rotative_mode[2] << 16 |
				pfw_op->addr_rotative_mode[1] << 8 |
				pfw_op->addr_rotative_mode[0]);
...
int hx_set_grip_exception_zone(int *cmd_param)
{
	...
	} else if (cmd_param[1] >= 3 && cmd_param[1] <= 4){
		pos_ary[0] = 0; pos_ary[1] = 1; pos_ary[2] = 2; pos_ary[3] = 3;
		/*directoin -  Top(3), Bot(4) adjust cmd to Top(1), Bot(2) for settings*/
		cmd_param[1] -= 2;
		data_ary[0] = (cmd_param[1] << 6) | cmd_param[2] >> 8;
		data_ary[1] = cmd_param[2] & 0xFF;
		data_ary[2] = cmd_param[3] >> 8;
		data_ary[3] = cmd_param[3] & 0xFF;
		ret &= hx_cmd_rw_chk(pfw_op->addr_except_top_bot_zone, 4, pos_ary, data_ary);
	} else {
...
static void run_snr_non_touched(void *dev_data)
{
	...
	retry_cnt = 0;
	while (retry_cnt++ < 100 && !atomic_read(&private_ts->plat_data->shutdown_called)) {
...
static int hx_gap_ver_raw(int test_type, uint32_t *org_raw, int *result_raw)
{
	...
	tmp_result_raw = vzalloc(sizeof(int) * rx_num * tx_num);
	memset(tmp_result_raw, 0x0, sizeof(int) * rx_num * tx_num);
	...
	for (i_partial = 0; i_partial < g_gap_vertical_partial; i_partial++) {
		...
		I("%s %s: tmp_start : %d, i_partial : %d g_gap_vertical_part[i_partial]:%d\n",
					HIMAX_LOG_TAG, __func__, tmp_start, i_partial, g_gap_vertical_part[i_partial]);
	...
	for (i = 0; i < tx_num * rx_num && idx_assign < rslt_buf_size; i++) {
		...
		*/
		...
		I("%s %s: now_part : %d,g_gap_vertical_part[now_part]:%d\n",
					HIMAX_LOG_TAG, __func__, now_part, g_gap_vertical_part[now_part]);
		if (now_part % 2 == 1) {
		/*odd part
			using index i to know is in which TX
...
static void aot_enable(void *dev_data)
{
	...
	case 0:
		...
		ts->aot_enabled = 0;
		ts->gesture_state = ts->gesture_state & ~BIT(HX_GESTURE_AOT);
...
static void spay_enable(void *dev_data)
{
	...
	case 0:
		...
		ts->spay_enabled = 0;
		ts->gesture_state = ts->gesture_state & ~BIT(HX_GESTURE_SPAY);
		ts->plat_data->lowpower_mode &= ~SEC_TS_MODE_SPONGE_SWIPE;
		break;
	case 1:
		sec->cmd_state = SEC_CMD_STATUS_OK;
		I("%s: Set spay Mode\n", __func__);
		ts->gesture_cust_en[1] = 1;
		ts->spay_enabled = 1;
		ts->gesture_state = ts->gesture_state | BIT(HX_GESTURE_SPAY);
		ts->plat_data->lowpower_mode |= SEC_TS_MODE_SPONGE_SWIPE;
		break;
...
void psensor_enable(struct himax_ts_data *data, unsigned int mode)
{
	...
	switch (mode) {
	case 0:
		data->plat_data->ed_enable = 0;
		data->ear_detect_val = -1;
		tmp_data[3] = 0x00;
		tmp_data[2] = 0x00;
		tmp_data[1] = 0x00;
		tmp_data[0] = 0x00;
		g_core_fp.fp_register_write(pfw_op->addr_proximity_mode, 4, tmp_data, 0);
		data->ear_detect_val = -1;
		break;
	case 1:
		input_info(true, data->dev, "%s,call mode %d\n", __func__, mode);
		data->plat_data->ed_enable = 1;
		data->ear_detect_val = -1;
		tmp_data[3] = 0xA1;
		tmp_data[2] = 0x1A;
		tmp_data[1] = 0xA1;
...
static void ear_detect_enable(void *device_data)
{
	...
	if (!data->plat_data->support_ear_detect) {
	...
	if (!(sec->cmd_param[0] == 0 || sec->cmd_param[0] == 1 || sec->cmd_param[0] == 3)) {
		...
		data->plat_data->ed_enable = sec->cmd_param[0];
		if (sec->cmd_param[0] != 0) {
	...
	if (data->suspended) {
		input_err(true, data->dev, "%s: now screen off stae, it'll setting after screen on(%d)!\n",
					__func__, data->plat_data->ed_enable);
		goto out;
	}

	mutex_lock(&data->device_lock);

	psensor_enable(data, data->plat_data->ed_enable);
...
static void hx_proxy_dbg(struct himax_ts_data *ts)
{
	...
	g_core_fp.fp_register_read(pfw_op->addr_proximity_debug, 8, tmp_data, false);
...
static void prox_lp_scan_mode(void *device_data)
{
	...
	if (!data->plat_data->prox_lp_scan_enabled) {
	...
	if (data->plat_data->ed_enable == 1 && data->plat_data->lowpower_mode && data->proxy_1b_en) {
...
void himax_set_cover_mode(struct himax_ts_data *ts, bool closed)
{
	...
	if (atomic_read(&ts->plat_data->shutdown_called)) {
		E("%s: %s, but IC is plat_data->shutdown_called\n", __func__, closed ? "on" : "off");
		return;
	}
					...
					const char *buf, size_t count)
{
	...
	if (atomic_read(&ts->plat_data->shutdown_called)) {
		input_err(true, ts->dev, "%s: already called power plat_data->shutdown_called.\n", __func__);
		return count;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ts->plat_data->firmware_name) {
		...
		if (strncmp(ts->plat_data->firmware_name, "tsp_himax/", 10) == 0)
			offset = 10;

		snprintf(vendor_name, sizeof(vendor_name), "%s", ts->plat_data->firmware_name + offset);
		if (vendor_name[7] == '_')
			vendor_name[7] = '\0';
		snprintf(tbuff, sizeof(tbuff), ",\"TVEN\":\"HIMAX_%s\"", vendor_name);
	} else {
		snprintf(tbuff, sizeof(tbuff), ",\"TVEN\":\"HIMAX\"");
	}
	strlcat(buff, tbuff, sizeof(buff));
...
int sec_touch_sysfs(struct himax_ts_data *data)
{
	...
	/* /sys/class/sec/tsp */
	ret = sec_cmd_init(&data->sec, data->dev, sec_cmds,
