		...
		struct sm5714_policy_data *policy)
{
	...
	if (policy->last_state != policy->state) {
...
#else
		policy->tx_data_obj[1].power_data_obj_sink.op_current = 1650 / 10;
#endif
		...
		if (manager->support_vpdo) {
			policy->tx_msg_header.num_data_objs++;
			policy->tx_data_obj[2].power_data_obj_variable.max_current = 2000 / 10;
			policy->tx_data_obj[2].power_data_obj_variable.min_voltage = 5000 / 50;
			policy->tx_data_obj[2].power_data_obj_variable.max_voltage = 9000 / 50;
			policy->tx_data_obj[2].power_data_obj_variable.supply_type =
					POWER_TYPE_VARIABLE;
		}
#if IS_ENABLED(CONFIG_DIRECT_CHARGING)
		if(pd_data->specification_revision == USBPD_REV_30) {
			policy->tx_msg_header.num_data_objs++;
			policy->tx_data_obj[3].power_data_obj_programmable.max_current =
					manager->pps_current ? (manager->pps_current / 50) : (2200 / 50);
			policy->tx_data_obj[3].power_data_obj_programmable.reserved1 = 0;
			policy->tx_data_obj[3].power_data_obj_programmable.min_voltage = 3300 / 100;
			policy->tx_data_obj[3].power_data_obj_programmable.reserved2 = 0;
			policy->tx_data_obj[3].power_data_obj_programmable.max_voltage = 11000 / 100;
			policy->tx_data_obj[3].power_data_obj_programmable.reserved3 = 0;
			policy->tx_data_obj[3].power_data_obj_programmable.pps_power_limited = 0;
			policy->tx_data_obj[3].power_data_obj_programmable.pps_supply = 0;
			policy->tx_data_obj[3].power_data_obj_programmable.supply_type =
					POWER_TYPE_APDO;
		}
#endif
		sm5714_usbpd_send_msg(pd_data, &policy->tx_msg_header,
