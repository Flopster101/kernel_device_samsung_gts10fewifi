...
int sm5714_usbpd_evaluate_capability(struct sm5714_usbpd_data *pd_data)
{
	...
	for (i = 0; i < policy->rx_msg_header.num_data_objs; i++) {
		...
		case POWER_TYPE_APDO:
			if (pd_obj->power_data_obj_programmable.pps_supply == 2) { /* SPR AVS */
				min_volt = pd_obj->power_data_obj_spr_avs.max_current_15v * USBPD_CURRENT_UNIT;
				max_volt = pd_obj->power_data_obj_spr_avs.max_current_20v * USBPD_CURRENT_UNIT;
				dev_info(pd_data->dev, "[%d] SPR AVS max_current_15v(%d)mV, max_current_20v(%d)mV\n",
						i+1, min_volt, max_volt);				
				continue;
			}
...
static int of_sm5714_usbpd_dt(struct sm5714_usbpd_manager_data *_data)
{
	...
	ret = of_property_read_u32(np, "pdic,pps_current",
			&_data->pps_current);
	if (ret < 0)
		sm5714_err("%s error reading pps_current %d\n",
				__func__, _data->pps_current);
