...
uint32_t ChargeUpCyclePulseLow_Time;
uint32_t ChargeUpCyclePulseHigh_Time;
int cpu_start;
int cpu_end;
...
#define SDK_INIT_RETRY_COUNT 10
...
static void configure_pinctrl(struct authon_driver_platform_data *pdata, char *state_name)
{
	int ret = 0;
	struct pinctrl_state *state;

	if (IS_ERR_OR_NULL(pdata->p)) {
		pr_err("%s: failed to get pinctrl\n", __func__);
		return;
	}
	state = pinctrl_lookup_state(pdata->p, state_name);
	if (IS_ERR_OR_NULL(state)) {
		pr_err("%s: could not find pull up state (%li)\n", __func__, PTR_ERR(state));
		return;
	}
	ret = pinctrl_select_state(pdata->p, state);
	if (ret) {
		pr_info("%s: pinctrl_select_state failed (%d)\n", __func__, ret);
		return;
	}
	pr_info("%s: success\n", __func__);
}

static int check_authic_supported(struct device_node *np, struct authon_driver_platform_data *pdata)
{
	int ret = 0;
	int eur_det_gpio = 0;
	...
	if (eur_det_gpio < 0) {
		...
		/* EUR device found -> Change gpio internal configuration to PD
		 * because External PD is present
		 */
		configure_pinctrl(pdata, "pull_down");
	...
	if (ret) {
		...
		if (slavepcb != eur_slavepcb) {
			pr_info("%s Neither EUR device return\n", __func__);
			return -1;
		}
		goto AUTH_IC_DEVICE;
	}
...
long authon_driver_ioctl(struct file *s_file, unsigned int ui_command, unsigned long ul_arg)
{
	...
	if (cpu_start <= cpu_end) {
		sec_auth_setCpuMask(cpu_start, cpu_end);
	}
	switch (ui_command) {
	...
	case SWI_DEV_RESET:
		...
		while (retry) {
			...
			if (ret != EXE_SUCCESS) {
				...
				//Charge up cycle routine (for indirect power mode only)
				ret = authon_exe_charge_up_cycle();
				if (ret != EXE_SUCCESS) {
					retry--;
					continue;
				} else {
					//Set default device address
					Swi_SelectByAddress(ON_DEFAULT_ADDRESS);

					//Check device reset is successful - read device address
					ret = authon_get_swi_address(&device_address);
					if (ret == SDK_SUCCESS) {
						if (device_address == ON_DEFAULT_ADDRESS) {
							pr_info("Device address = 0x%X\r\n", device_address);
							break; //Exit
						} else {
							pr_err("Error: Incorrect device address 0x%X, retry:%d\r\n", device_address, retry);
							retry--;
							continue;
						}
					} else {
						pr_err("Error: Unable to get default address 0x%X, retry:%d\r\n", ON_DEFAULT_ADDRESS, retry);
						retry--;
						continue;
					}
				}
		...
		//Fail to reset IC after maximum retry
		if (retry == 0) {
			pr_err("Error: Device reset failed\r\n");
			lResult = -EFAULT;
			goto cleanup;
		} else { //Reset successful
			pr_info("Device reset passed\r\n");
		}
		break;
...
Send_NVM_Status:
	...
	case SWI_NVM_SYNC_STATUS:
		...
		break;
	case SWI_NVM_WRITE_SINGLE:
		pr_info("SWI_NVM_WRITE_SINGLE:\n");
		if (copy_from_user(wmsg, (uint32_t __user *)ul_arg, sizeof(struct MESSAGE)) != 0) {
			lResult = -EFAULT;
			goto cleanup;
			}

		pr_info("SWI_NVM_WRITE_SINGLE: %d %d\n", wmsg->offset, wmsg->page_count);

		ret = nvm_write_single_userpage(wmsg->offset, wmsg->nvm_data);
		if (ret == APP_NVM_PAGE_LOCKED) {
			pr_err("NVM page is locked\n");
			wmsg->status = APP_NVM_PAGE_LOCKED;
			goto Send_Write_Status;
		}

		if (ret != APP_NVM_SUCCESS) {
			pr_err("Error: Write single NVM page failed, ret=0x%x remain retry: %d\r\n", ret, retry);
			lResult = -EFAULT;
			wmsg->status = ret;
			goto cleanup;
		} else {
...
Send_Write_Status:
		if (copy_to_user((uint32_t __user *) ul_arg, wmsg, sizeof(struct MESSAGE)) != 0) {
			pr_err("Error: Unable to write NVM single page. ret=%x", ret);
			wmsg->status = ret;
			lResult = -EFAULT;
			goto cleanup;
		}
	...
	case SWI_ECC_SEND_MAC_BYTE:
		...
		ret = authon_unlock_nvm_locked(ecc_msg1->mac_byte);
		if (ret == APP_NVM_E_PAGE_UNLOCKED_FAILED)
			pr_info("authon_unlock_nvm_locked failed\n");
		if (ret == APP_NVM_SUCCESS)
			pr_info("authon_unlock_nvm_locked passed\n");
	...
	case INIT_PAGE_BUFFER:
		...
		for (i = 0; i < rmsg->page_count; i += NVM_READ_BLOCK_SIZE) {
			ret = authon_read_nvm(i, buffer_data, NVM_READ_BLOCK_SIZE);
			if (ret != APP_NVM_SUCCESS) {
				pr_err("Error: Read NVM failed, ret=0x%x\r\n", ret);
				rmsg->status = ret;
				lResult = -EFAULT;
				...
				rmsg->status = 0x0;
				start_index = i*4;
				for (j = start_index; j < (start_index + sizeof(buffer_data)); j++)
					rmsg->nvm_data[j] = buffer_data[j - start_index];
			}
...
static int authon_driver_parse_dt(struct device *dev, struct authon_driver_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if (IS_ERR_OR_NULL(pdata->p)) {
			pr_err("%s: failed to get pinctrl from the device node\n", __func__);
		}

		// check if authic supported or not
		ret = check_authic_supported(np, pdata);
		if (ret) {
			pr_info("%s: Auth IC not supported\n", __func__);
			/* For Global model if external pull up is missing
			 * change BAT_SWI internal configuration to PULL UP
			 */
			configure_pinctrl(pdata, "swi_pullup");
			return -EINVAL;
		}
		...
		//For indirect power mode, tPDL is 100ms and direct power tPDL is 2ms.
		...
		if (ret) {
			...
			if (strcmp(power_mode, "indirect") == 0) {
				...
				PowerDownDelay_Time = 100000;
				PowerUpDelay_Time = 10000;
				ChargeUpCyclePulseLow_Time = 5000;
				ChargeUpCyclePulseHigh_Time = 10000;
			} else if (strcmp(power_mode, "direct") == 0) {
				PowerDownDelay_Time = 2000;
				PowerUpDelay_Time = 8000;
				ChargeUpCyclePulseLow_Time = 0;
				ChargeUpCyclePulseHigh_Time = 0;
			} else {
		...
		err = of_property_read_u32(np, "authon,cpu_start", &cpu_start);
		if (err < 0) {
			cpu_start = 0;
			pr_err("%s: error(%d) reading cpu start, setting to default value 0\n", __func__, err);
		}

		err = of_property_read_u32(np, "authon,cpu_end", &cpu_end);
		if (err < 0) {
			cpu_end = -1;
...
int platform_init(void)
{
	if (gpio_is_valid(SWI_GPIO_PIN) == false) {
		...
		if (!gpio_get_value(SWI_GPIO_PIN)) {
			pr_err("%s: ERROR: GPIO %d default LOW\n", __func__, SWI_GPIO_PIN);
			return -1;
		}
	}
	pr_info("%s: done\n", __func__);
...
{
	int retry = SDK_INIT_RETRY_COUNT;
	uint16_t device_address;
	uint16_t ret;
	int i=0;
...
static struct device_attribute sec_auth_attrs[] = {
	...
	SEC_AUTH_ATTR(presence_passrate),
#endif
};
		...
		if (kstrtoint(buf, 0, &num) == 0) {
			...
			for (i = 0; i < num; i++) {
				...
				if (ret == SDK_SUCCESS)
					...
					pr_info("%s: authon_sdk_init fail at itr (%d)\n", __func__, i);
			}
			pr_info("%s: Presence Pass Cnt(%d) out of (%d)\n", __func__, pass_cnt, num);
		}
		ret = count;
	}
		break;
...
static struct device_attribute sec_auth_2nd_attrs[] = {
	...
	SEC_AUTH_2ND_ATTR(presence_passrate),
#endif
};
 ...
 *************************************************************************/
	...
	ret = authon_driver_parse_dt(&pdev->dev, authon_driver->pdata);
	if (ret < 0) {
