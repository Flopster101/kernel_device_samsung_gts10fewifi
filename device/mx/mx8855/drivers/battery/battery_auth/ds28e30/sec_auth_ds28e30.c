...
static int eur_slave_detection(struct device_node *np)
{
	int ret = 0;
	...
	/*  Do not proceed if
	 *	1. slavepcb != EUR_SLAVEPCB
	 */
		if (slavepcb != eur_slavepcb) {
			pr_info("%s Not EUR device, return\n", __func__);
			return -1;
		}
	}
	return 0;
...
#if !IS_ENABLED(CONFIG_BATTERY_AUTH_SLE956681)
static void configure_pinctrl(struct sec_auth_ds28e30_platform_data *pdata, char *state_name)
{
	int ret = 0;
	struct pinctrl_state *state;

	if (IS_ERR_OR_NULL(pdata->p)) {
		auth_err("%s: failed to get pinctrl\n", __func__);
		return;
	}
	state = pinctrl_lookup_state(pdata->p, state_name);
	if (IS_ERR_OR_NULL(state)) {
		auth_err("%s: could not find pull up state (%li)\n", __func__, PTR_ERR(state));
		return;
	}
	ret = pinctrl_select_state(pdata->p, state);
	if (ret) {
		auth_info("%s: pinctrl_select_state failed (%d)\n", __func__, ret);
		return;
	}
	auth_info("%s: success\n", __func__);
}
...
static int check_authic_supported(struct device_node *np, struct sec_auth_ds28e30_platform_data *pdata)
{
	int ret = 0;
	int eur_det_gpio = 0;
	...
	if (eur_det_gpio < 0) {
		...
		if (ret != 0) {
			auth_err("%s: not valid auth IC device, eur_det_gpio(%d)\n", __func__, ret);
			return -1;
		}
		 ...
		 */
#if !IS_ENABLED(CONFIG_BATTERY_AUTH_SLE956681)
		configure_pinctrl(pdata, "pulldown");
#endif
		goto AUTH_IC_DEVICE;
	}
	/* Method 3 : Read project and region specific slavepcb value */
	ret = eur_slave_detection(np);
	if (ret < 0)
		return ret;
	...
	struct sec_auth_ds28e30_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if (IS_ERR_OR_NULL(pdata->p)) {
			auth_err("%s: failed to get pinctrl from the device node\n", __func__);
		}
		// check if authic supported or not
		ret = check_authic_supported(np, pdata);
		if (ret) {
			auth_info("%s: Auth IC not supported\n", __func__);
#if !IS_ENABLED(CONFIG_BATTERY_AUTH_SLE956681)
		...
		ret = of_property_read_u32_array(np, "ds28e30,rw_out_low_loop_cnt", pdata->rw_out_low_loop_cnt, 2);
		if (ret < 0) {
			pdata->rw_out_low_loop_cnt[0] = 3;
			pdata->rw_out_low_loop_cnt[1] = 3;
			pr_err("%s: error(%d) read write out low loop not valid\n", __func__, ret);
			ret = 0;
		}

		ret = of_property_read_u32(np, "ds28e30,read_bit_vamm_cnt", &pdata->read_bit_vamm_cnt);
		if (ret < 0) {
			pdata->read_bit_vamm_cnt = 6;
			pr_err("%s: error(%d) read bit vamm cnt not valid\n", __func__, ret);
			ret = 0;
		}
		...
		ret = of_property_read_u32_array(np, "ds28e30,rw_out_low_loop_cnt2", pdata->rw_out_low_loop_cnt2, 2);
		if (ret < 0) {
			pdata->rw_out_low_loop_cnt2[0] = 3;
			pdata->rw_out_low_loop_cnt2[1] = 3;
			pr_err("%s: error(%d) read write out low loop 2nd not valid\n", __func__, ret);
			ret = 0;
		}

		ret = of_property_read_u32(np, "ds28e30,read_bit_vamm_cnt2", &pdata->read_bit_vamm_cnt2);
		if (ret < 0) {
			pdata->read_bit_vamm_cnt2 = 6;
			pr_err("%s: error(%d) read bit vamm cnt 2nd not valid\n", __func__, ret);
			ret = 0;
		}
