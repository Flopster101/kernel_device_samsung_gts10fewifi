...
static int sm5440_read_reg(struct sm5440_charger *sm5440, u8 reg, u8 *dest)
{
	...
	for (i = 0; i < cnt; ++i) {
		...
		if (ret < 0) {
			dev_err(sm5440->dev, "%s: fail to i2c_read(ret=%d, reg=0x%x)\n", __func__, ret, reg);
			if (i < (cnt - 1))
				msleep(100);
		} else {
			break;
...
int sm5440_bulk_read(struct sm5440_charger *sm5440, u8 reg, int count, u8 *buf)
{
	...
	for (i = 0; i < cnt; ++i) {
		...
		if (ret < 0) {
			dev_err(sm5440->dev, "%s: fail to i2c_bulk_read(ret=%d, reg=0x%x)\n", __func__, ret, reg);
			if (i < (cnt - 1))
				msleep(100);
		} else {
			break;
...
static int sm5440_write_reg(struct sm5440_charger *sm5440, u8 reg, u8 value)
{
	...
	for (i = 0; i < cnt; ++i) {
		...
		if (ret < 0) {
			dev_err(sm5440->dev, "%s: fail to i2c_write(ret=%d, reg=0x%x)\n", __func__, ret, reg);
			if (i < (cnt - 1))
				msleep(100);
		} else {
			break;
...
{
	return sm5440_update_reg(sm5440, SM5440_REG_CNTL5, op_mode, 0x3, 2);
}
...
{
	return sm5440_update_reg(sm5440, SM5440_REG_CNTL3, enable, 0x1, 2);
}

static int sm5440_set_wdt_timer(struct sm5440_charger *sm5440, u8 tmr)
{
	return sm5440_update_reg(sm5440, SM5440_REG_CNTL1, tmr, 0x7, 4);
}

static int sm5440_enable_wdt(struct sm5440_charger *sm5440, bool enable)
{
	return sm5440_update_reg(sm5440, SM5440_REG_CNTL1, enable, 0x1, 7);
}

static int sm5440_set_adc_rate(struct sm5440_charger *sm5440, u8 rate)
{
	return sm5440_update_reg(sm5440, SM5440_REG_ADCCNTL1, rate, 0x1, 1);
}

static int sm5440_enable_adc(struct sm5440_charger *sm5440, bool enable)
{
	return sm5440_update_reg(sm5440, SM5440_REG_ADCCNTL1, enable, 0x1, 0);
}
...
static u32 sm5440_get_dc_error_status(struct i2c_client *i2c)
{
	...
	if (!sm5440->wdt_disable) {
		...
		if (op_mode == 0x0) {
			...
			dev_info(sm5440->dev, "%s: int 0x%x:0x%x:0x%x:0x%x [%s]\n", __func__, st[0], st[1], st[2], st[3], SM5440_DC_VERSION);
