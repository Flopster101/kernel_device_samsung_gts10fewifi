...
void parse_dc_sc_dual_sc_iin_ratio(struct sec_battery_info *battery, struct device_node *np)
{
	int num_age_step = battery->pdata->num_age_step;
	int len = 0;
	long ret = 0;
	...
	p = of_get_property(np, "battery,dc_sc_dual_sc_iin_ratio", &len);
	if (!p) {
		pr_err("%s: dc_sc_dual_sc_iin_ratio is Empty. set default : 50\n", __func__);
		battery->pdata->dc_sc_dual_sc_iin_ratio = alloc_2darr(num_age_step, battery->dc_step_chg_step);
		if (IS_ERR(battery->pdata->dc_sc_dual_sc_iin_ratio)) {
			ret = PTR_ERR(battery->pdata->dc_sc_dual_sc_iin_ratio);
		} else {
			for (i = 1; i < num_age_step; i++) {
				for (j = 0; j < battery->dc_step_chg_step; j++)
					battery->pdata->dc_sc_dual_sc_iin_ratio[i][j] = 50;
			}
		}
	} else {
		len = len / sizeof(u32);
		pr_info("%s: step(%d) * age_step(%d), dc_sc_dual_sc_iin_ratio len(%d)\n",
			__func__, battery->dc_step_chg_step, num_age_step, len);
		...
		if (IS_ERR(battery->pdata->dc_sc_dual_sc_iin_ratio)) {
			...
			ret = init_2darr_with_dt(np, "battery,dc_sc_dual_sc_iin_ratio",
					battery->pdata->dc_sc_dual_sc_iin_ratio, num_age_step, battery->dc_step_chg_step,
					(num_age_step * battery->dc_step_chg_step) != len);

			print_log_for_step_charging_dt_2darr(battery->pdata->dc_sc_dual_sc_iin_ratio,
				num_age_step, battery->dc_step_chg_step, "dc_sc_dual_sc_iin_ratio");
		}

		if (ret) {
			pr_info("%s : dc_sc_dual_sc_iin_ratio read fail\n", __func__);
		}
	}
}
...
int sec_dc_step_charging_dt(struct sec_battery_info *battery, struct device *dev)
{
	...
	if (battery->pdata->dc_sc_dual_charging)
		parse_dc_sc_dual_sc_iin_ratio(battery, np);
