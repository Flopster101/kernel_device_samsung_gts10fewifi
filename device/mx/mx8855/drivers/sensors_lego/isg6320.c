...
struct isg6320_data {
	...
	struct delayed_work cfcal_work;
	...
	int pdic_status;
	int pdic_pre_attach;
	bool is_first_usb_data;
#endif
#if IS_ENABLED(CONFIG_HALL_NOTIFIER)
	struct notifier_block hall_nb;
#endif
#if defined(CONFIG_TABLET_MODEL_CONCEPT)
	struct work_struct pdic_attach_work;
	struct work_struct pdic_detach_work;
...
{
	struct isg6320_data *data = container_of((struct delayed_work *)work,
...
#if defined(CONFIG_TABLET_MODEL_CONCEPT)
static int pdic_write_1ch_thd_gain(struct isg6320_data *data, u8 proxctl4_reg, u8 lsum_type)
{
	int ret = 0;

	ret = isg6320_i2c_write(data, ISG6320_A_PROXCTL4_REG, proxctl4_reg);
	if (ret < 0) {
		pr_err("[GRIP_%d] Tab CTL4_REG failed(%d)\n", data->ic_num, ret);
		return ret;
	}
	ret = isg6320_i2c_write(data, ISG6320_A_LSUM_TYPE_REG, lsum_type);
	if (ret < 0) {
...
#ifdef CONFIG_USE_MULTI_CHANNEL
static int pdic_write_2ch_thd_gain(struct isg6320_data *data, u8 proxctl4_reg, u8 lsum_type)
{
	int ret = 0;

	ret = isg6320_i2c_write(data, ISG6320_B_PROXCTL4_REG, proxctl4_reg);
	if (ret < 0) {
		pr_err("[GRIP_%d] [B] Tab CTL4_REG failed(%d)\n", data->ic_num, ret);
		return ret;
	}
	ret = isg6320_i2c_write(data, ISG6320_B_LSUM_TYPE_REG, lsum_type);
	if (ret < 0) {
...
#endif
static void pdic_thd_write_work(struct isg6320_data *data, u8 proxctl4_reg, u8 lsum_type_a, u8 lsum_type_b)
{
	isg6320_set_debug_work(data, OFF, 0);
	mutex_lock(&data->lock);

	if (data->initialized == OFF) {
		pr_info("[GRIP_%d] %s, not initialized OFF\n", data->ic_num, __func__);
		goto exit_force_pdic_work;
	}

	if (pdic_write_1ch_thd_gain(data, proxctl4_reg, lsum_type_a) < 0)
		goto exit_force_pdic_work;

#ifdef CONFIG_USE_MULTI_CHANNEL
	if (data->multi_use) {
		if (pdic_write_2ch_thd_gain(data, proxctl4_reg, lsum_type_b) < 0)
			goto exit_force_pdic_work;
	}
#endif

exit_force_pdic_work:
	mutex_unlock(&data->lock);
	if (!data->in_suspend)
		isg6320_set_debug_work(data, ON, SHCEDULE_INTERVAL + (data->ic_num << 3));
}

static void pdic_attach_work_func(struct work_struct *work)
{
	struct isg6320_data *data = container_of((struct work_struct *)work,
						struct isg6320_data, pdic_attach_work);
	...
	ret = isg6320_i2c_read(data, ISG6320_LSUM_GAIN_A, &val_a, 1);
	if (ret < 0) {
		pr_err("[GRIP_%d] %s - i2c read fail(%d)\n", data->ic_num, __func__, ret);
		val_a = 0x10;
	}

	ret = isg6320_i2c_read(data, ISG6320_LSUM_GAIN_B, &val_b, 1);
	if (ret < 0) {
		pr_err("[GRIP_%d] %s - i2c read fail(%d)\n", data->ic_num, __func__, ret);
		val_b = 0x10;
	}

	pdic_thd_write_work(data, 0xFF, val_a, val_b);
}

static void pdic_detach_work_func(struct work_struct *work)
{
	struct isg6320_data *data = container_of((struct work_struct *)work,
						struct isg6320_data, pdic_detach_work);

	pr_info("[GRIP_%d] %s, apply usb burst defense code, recover 1ch : %d\n",
			data->ic_num, __func__, data->lsum_a);

#ifdef CONFIG_USE_MULTI_CHANNEL
	if (data->multi_use)
		pr_info("[GRIP_%d] %s, apply usb burst defense code, recover 2ch : %d\n",
				data->ic_num, __func__, data->lsum_b);
#endif

	pdic_thd_write_work(data, 0x58, data->lsum_a, data->lsum_b);
}
#endif
...
static void mcc_work_func(struct work_struct *work)
{
	...
	cancel_delayed_work_sync(&data->cfcal_work);
	isg6320_set_debug_work(data, OFF, 0);
...
#endif
		input_report_rel(grip_noti_input_dev, REL_X, type);
...
static int isg6320_set_normal_mode(struct isg6320_data *data)
{
	...
	schedule_delayed_work(&data->cfcal_work, 0);
...
static ssize_t isg6320_ccic_state_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct isg6320_data *data = dev_get_drvdata(dev);

	if (data->pdic_status)
		return snprintf(buf, PAGE_SIZE, "ccic state on\n");
	else
		return snprintf(buf, PAGE_SIZE, "ccic state off\n");
}

static ssize_t isg6320_ccic_state_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int val;
	int ret;
	...
	if (val == 0) {
		pr_info("[GRIP_%d] %s - ccic state off\n", data->ic_num, __func__);
	} else {
		pr_info("[GRIP_%d] %s - ccic state on\n", data->ic_num, __func__);
		mutex_lock(&data->usb_lock);
		if (data->is_first_usb_data) {
			data->pdic_status = ON;
			data->pre_attach = ON;
			schedule_work(&data->pdic_attach_work);
			data->is_first_usb_data = false;
		}
		mutex_unlock(&data->usb_lock);
	}
...
#if (IS_ENABLED(CONFIG_CCIC_NOTIFIER) || IS_ENABLED(CONFIG_PDIC_NOTIFIER)) && IS_ENABLED(CONFIG_USB_TYPEC_MANAGER_NOTIFIER)
static DEVICE_ATTR(ccic_state, 0664,
	isg6320_ccic_state_show, isg6320_ccic_state_store);
#endif
					     ...
					     unsigned long action, void *data)
{
	...
	if (usb_typec_info.id != PDIC_NOTIFY_ID_ATTACH && usb_typec_info.id != PDIC_NOTIFY_ID_OTG) {
		...
		if (usb_typec_info.id == PDIC_NOTIFY_ID_RID) {
			PD_NOTI_RID_TYPEDEF usb_fac_cable_info = *(PD_NOTI_RID_TYPEDEF *)data;
			...
			case RID_523K:
			case RID_619K:
				if (pdata->pdic_status == ON) {
					schedule_work(&pdata->pdic_detach_work);
					pr_info("[GRIP_%d] come in factory cable reset,  rid : %d\n",
									pdata->ic_num, usb_fac_cable_info.rid);
				}
				break;
			default:
				break;
	...
	if (usb_typec_info.rprd == PDIC_NOTIFY_HOST) {
		...
		pr_info("[GRIP_%d] PDIC_NOTIFY_HOST attach, grip reset skip\n", pdata->ic_num);
	} else if (usb_typec_info.id == PDIC_NOTIFY_ID_OTG){
		pdata->otg_attach_state = usb_typec_info.attach;
		pr_info("[GRIP_%d] PDIC_NOTIFY_ID_OTG attach, grip reset skip\n", pdata->ic_num);
	} else if (pdata->otg_attach_state) {
		pdata->otg_attach_state = usb_typec_info.rprd;
		pr_info("[GRIP_%d] OTG_ATTACH detach, grip reset skip\n", pdata->ic_num);
	} else {
		if (pdata->initialized == ON) {
			mutex_lock(&pdata->usb_lock);
			pdata->is_first_usb_data = false;
			mutex_unlock(&pdata->usb_lock);
#if IS_ENABLED(CONFIG_SENSORS_SKIP_CABLE_RESET)
			pr_info("[GRIP_%d] CCIC event, grip reset skip\n", pdata->ic_num);

			if (pdata->pdic_status == ON)
				schedule_work(&pdata->pdic_attach_work);
			else
				schedule_work(&pdata->pdic_detach_work);
#else
			schedule_delayed_work(&pdata->cfcal_work, 0);
				...
				unsigned long action, void *hall_data)
{
	...
	if (action == HALL_ATTACH) {
		...
		if (strncmp(hall_notifier->name, "certify_hall", sizeof("certify_hall") - 1) == 0 ||
			...
			if (data->ic_num % 2 == 0)
				schedule_delayed_work(&data->cfcal_work, msecs_to_jiffies(400));
			else
				schedule_delayed_work(&data->cfcal_work, 0);
...
static bool create_shared_noti_input_dev(struct i2c_client *client, struct isg6320_data *data)
{
	int ret = -ENODEV;
	...
	if (ret) {
		...
		pr_err("[GRIP_%d] failed to register input dev for noti (%d)\n", data->ic_num, ret);
		mutex_unlock(&noti_lock);
		return false;
	}
...
void remove_shared_noti_input_dev(struct isg6320_data *data)
{
	mutex_lock(&noti_lock);
	if (--noti_input_count == 0) {
		input_unregister_device(grip_noti_input_dev);
		grip_noti_input_dev = NULL;
		pr_info("[GRIP_%d] noti_input_dev remove complete : %d\n", data->ic_num, noti_input_count);
	}
	mutex_unlock(&noti_lock);
}

void isg6320_destroy_noti_lock(void)
{
	if (--module_count == 0)
...
#endif
{
	...
	if (!create_shared_noti_input_dev(client, data))
	...
	INIT_DELAYED_WORK(&data->cfcal_work, cfcal_work_func);
...
#endif
#if IS_ENABLED(CONFIG_TABLET_MODEL_CONCEPT)
	INIT_WORK(&data->pdic_attach_work, pdic_attach_work_func);
	INIT_WORK(&data->pdic_detach_work, pdic_detach_work_func);
#endif
#if (IS_ENABLED(CONFIG_CCIC_NOTIFIER) || IS_ENABLED(CONFIG_PDIC_NOTIFIER)) && IS_ENABLED(CONFIG_USB_TYPEC_MANAGER_NOTIFIER)
	data->is_first_usb_data = true;
	data->pdic_status = OFF;
	data->pre_attach = OFF;

	mutex_init(&data->usb_lock);
...
static void isg6320_module_count(void)
{
	module_count++;
#if defined(CONFIG_SENSORS_ISG6320_SUB)
	module_count++;
