...
unsigned long sgpu_get_nr_swap_pages(void)
{
	return atomic_long_read(&sgpu_swap_pages);
}
...
void sgpu_tt_destroy_notify(struct ttm_tt *ttm)
{
	if (unlikely(!!(ttm->page_flags & SGPU_TTM_TT_FLAG_SWAPPED)))
		atomic_long_sub(ttm->num_pages, &sgpu_swap_pages);
}
...
/*
 * sgpu_tt_unpopulate: a replacement function for ttm_tt_unpopulate
 *
 * Indirectly call ttm_tt_unpopulate via ttm_bo_validate.
 * This is done by using parameters that will cause only the unpopulate stuff to happen.
 */
static void sgpu_tt_unpopulate(struct ttm_device* bdev, struct ttm_tt* ttm)
{
	/* ctx can be all zeroes because it isn't used when only ttm_tt_unpopulate is called */
	struct ttm_operation_ctx fake_ctx = { 0 };
	/* in order for ttm_bo_validate to call ttm_tt_unpopulate, placement must be 0 */
	struct ttm_placement fake_placement = { 0 };
	struct ttm_buffer_object fake_bo = {
		.bdev = bdev,
		.resource = NULL, /* null resource causes subroutines to call ttm_tt_unpopulate */
		.ttm = ttm,
		.type = ttm_bo_type_kernel,
	};

	kref_init(&fake_bo.base.refcount);
	fake_bo.base.size = sizeof(fake_bo);
	dma_resv_init(&fake_bo.base._resv);
	fake_bo.base.resv = &fake_bo.base._resv;
	dma_resv_lock(fake_bo.base.resv, NULL);
	ttm_bo_validate(&fake_bo, &fake_placement, &fake_ctx);
	dma_resv_unlock(fake_bo.base.resv);
}
...
static int sgpu_tt_swapout(struct ttm_device *bdev, struct ttm_tt *ttm, gfp_t gfp_flags)
{
	...
	sgpu_tt_unpopulate(bdev, ttm);

	ttm->swap_storage = swap_storage;
	ttm->page_flags |= SGPU_TTM_TT_FLAG_SWAPPED;

	atomic_long_add(ttm->num_pages, &sgpu_swap_pages);
...
static int sgpu_bo_swapout(struct ttm_buffer_object *tbo)
{
	...
	ret = ttm_bo_wait_ctx(tbo, &ttm_ctx);
	if (ret)
		goto err;
...
#define SGPU_SWAPIN_PENDING   1
#define SGPU_SWAPOUT_PENDING  2
...
struct sgpu_proc_node {
	...
	unsigned long swap_pending;
	bool swap_in_progress;
	wait_queue_head_t wait_swap;
	struct mutex new_swap_req_lock;
	unsigned long req_amount;
	struct work_struct swapout_work_item;
	struct work_struct swapin_work_item;
};
...
static bool sgpu_swapout_continue(struct sgpu_proc_node *proc)
{
	if (!mutex_trylock(&proc->swap_lock))
		return false;

	if (atomic_read(&proc->swap_lock_count) > 0 ||
			test_bit(SGPU_SWAPIN_PENDING, &proc->swap_pending)) {
		mutex_unlock(&proc->swap_lock);
		return false;
	}
...
static void sgpu_swap_stop(struct sgpu_proc_node* proc)
{
	WARN_ON(!mutex_is_locked(&proc->new_swap_req_lock));
	set_bit(SGPU_SWAPIN_PENDING, &proc->swap_pending);
	set_bit(SGPU_SWAPOUT_PENDING , &proc->swap_pending);
	cancel_work_sync(&proc->swapout_work_item);
	cancel_work_sync(&proc->swapin_work_item);
	clear_bit(SGPU_SWAPIN_PENDING, &proc->swap_pending);
	clear_bit(SGPU_SWAPOUT_PENDING, &proc->swap_pending);
}
			      ...
			      unsigned long max_size, struct swap_stat *stat)
{
	...
	list_for_each_entry(bo, &ctx->bo_list, swap_node) {
		...
		if (!sgpu_swapout_continue(ctx->proc))
			return false;
...
static bool sgpu_proc_swapin(struct sgpu_proc_ctx *ctx, unsigned long zero, struct swap_stat *stat)
{
	...
	list_for_each_entry(bo, &ctx->bo_list, swap_node) {
		...
		if (test_bit(SGPU_SWAPOUT_PENDING, &ctx->proc->swap_pending))
			return false;
		...
		if (!ret) {
			for (bo_base = bo->vm_bo; bo_base; bo_base = bo_base->next) {
				...
				struct amdgpu_bo_va *bo_va = container_of(bo_base,
								struct amdgpu_bo_va, base);
				int r;

				if (tbo->base.resv == vm->root.base.bo->tbo.base.resv) {
					r = amdgpu_vm_bo_update(adev, bo_va, false);
					if (r)
						DRM_WARN("Failed to update page table");
				}
			}
		}
		dma_resv_unlock(tbo->base.resv);
...
void sgpu_swapout_work_func(struct work_struct* work)
{
	struct sgpu_proc_node* proc = container_of(work, typeof(*proc), swapout_work_item);
	sgpu_swap_proc(proc, proc->req_amount);
}

void sgpu_swapin_work_func(struct work_struct* work)
{
	struct sgpu_proc_node* proc = container_of(work, typeof(*proc), swapin_work_item);
	sgpu_swap_proc(proc, 0);
}

static void sgpu_swap_setup(struct sgpu_proc_node* proc, unsigned long swap_amount)
{
	mutex_lock(&proc->new_swap_req_lock);
	if (swap_amount) {
		set_bit(SGPU_SWAPOUT_PENDING, &proc->swap_pending);
		cancel_work_sync(&proc->swapin_work_item);
		clear_bit(SGPU_SWAPOUT_PENDING, &proc->swap_pending);
		proc->req_amount = swap_amount;
		queue_work(system_power_efficient_wq, &proc->swapout_work_item);
	} else {
		set_bit(SGPU_SWAPIN_PENDING, &proc->swap_pending);
		cancel_work_sync(&proc->swapout_work_item);
		clear_bit(SGPU_SWAPIN_PENDING, &proc->swap_pending);
		queue_work(system_power_efficient_wq, &proc->swapin_work_item);
	}
	mutex_unlock(&proc->new_swap_req_lock);
}
...
static void sgpu_proc_release(struct kobject *kobj)
{
	 ...
	 */
	lockdep_assert_held(&sgpu_proc_lock);
	WARN_ON(atomic_read(&proc->swap_lock_count) != 0);
...
static void sgpu_kobj_get_ownership(const struct kobject *kobj, kuid_t *uid, kgid_t *gid)
{
	*uid = current_euid();
	*gid = current_egid();
}
...
int sgpu_proc_add_context(struct amdgpu_fpriv *fpriv)
{
	...
	mutex_init(&proc->new_swap_req_lock);
	proc->tgid = current->tgid;
	init_waitqueue_head(&proc->wait_swap);
	INIT_WORK(&proc->swapout_work_item, sgpu_swapout_work_func);
	INIT_WORK(&proc->swapin_work_item, sgpu_swapin_work_func);
	sgpu_proc_init_ctx(&fpriv->proc_ctx);
