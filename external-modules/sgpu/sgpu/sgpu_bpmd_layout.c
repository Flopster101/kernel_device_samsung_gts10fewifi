 ...
 * @grbm_gfx_index: value written to GRBM_GFX_INDEX when register is read
 * @index_register_val: value writen into the index_register
 *
 * Return a packet ID
 */
	...
	uint32_t index_register_val)
{
	...
	struct sgpu_bpmd_layout_reg32_multiple_read_packet packet = {
		...
		.grbm_gfx_index     = grbm_gfx_index,
		.index_register_val = index_register_val,
	};

	DRM_DEBUG("%s: start dumping multiple read packet\n", __func__);

	WARN_ON((read_count == 0) || (values == NULL));
					   ...
					   uint32_t reg32_packet_id)
{
	...
	for (i = 0; i < DRM_SCHED_PRIORITY_COUNT; ++i)
		packet.num_jobs += atomic_read(&ring->num_jobs[i]);
...
/**
 * sgpu_bpmd_layout_dump_ih_ring_packet - dumps the interrupt handler rings
 *
 * @sbo: pointer to output target description
 * @sdev: amdgpu_device pointer
 *
 * Return a packet ID
 */
uint32_t sgpu_bpmd_layout_dump_ih_ring_packet(struct sgpu_bpmd_output *sbo,
					      struct amdgpu_device *sdev)
{
	ssize_t r = 0;
	struct amdgpu_ih_ring *ih_ring = &sdev->irq.ih;
	const size_t total_size = sizeof(struct sgpu_bpmd_layout_ih_ring_packet) + ih_ring->ring_size;
	struct sgpu_bpmd_layout_ih_ring_packet packet = {
		.header = {
		.kind   = SGPU_BPMD_LAYOUT_PACKET_IH_RING,
		.id     = sgpu_bpmd_get_packet_id(),
		.length = total_size - sizeof(struct sgpu_bpmd_layout_packet_header)
		},
		.ring_size	= ih_ring->ring_size,
		.rptr		= ih_ring->rptr,
		.wptr		= amdgpu_ih_get_wptr(sdev, ih_ring),
		.use_doorbell	= ih_ring->use_doorbell,
		.use_bus_addr	= ih_ring->use_bus_addr,
		.use_write_back	= ih_ring->use_write_back,
		.enabled	= ih_ring->enabled
	};
	...
	r = sgpu_bpmd_write(sbo, (const void *) &packet, sizeof(packet),
				SGPU_BPMD_LAYOUT_PACKET_ALIGN_BYTE);
	if (r != sizeof(packet)) {
		DRM_ERROR("failed to dump ih ring packet");
		return SGPU_BPMD_LAYOUT_INVALID_PACKET_ID;
	}

	r = sgpu_bpmd_write(sbo, (const void *) ih_ring->ring, ih_ring->ring_size,
				SGPU_BPMD_LAYOUT_PACKET_ALIGN_BYTE);
	if (r != ih_ring->ring_size) {
		DRM_ERROR("failed to dump ih ring buffer");
		return SGPU_BPMD_LAYOUT_INVALID_PACKET_ID;
	}
