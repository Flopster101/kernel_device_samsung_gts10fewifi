...
#include "pablo-hw-api-mcsc-v5.h"
#include "sfr/pablo-sfr-mcsc-v13_0.h"
#include "is-hw.h"
...
static void is_scaler_set_cinfifo_ctrl(struct pablo_mmio *base, bool strgen)
{
	...
	MCSC_SET_F(base, MCSC_R_YUV_CINFIFO_CONFIG, MCSC_F_YUV_CINFIFO_STALL_BEFORE_FRAME_START_EN,
		0x1);
...
{
	MCSC_SET_F(base, MCSC_R_STAT_CINFIFODUALLAYER_ENABLE,
		MCSC_F_STAT_CINFIFODUALLAYER_ENABLE, en);
	MCSC_SET_F(base, MCSC_R_STAT_CINFIFODUALLAYER_INTERVALS,
		MCSC_F_STAT_CINFIFODUALLAYER_INTERVAL_HBLANK, HBLANK_CYCLE);
	MCSC_SET_R(base, MCSC_R_STAT_CINFIFODUALLAYER_CONFIG, 0x1);

	if (strgen) {
		MCSC_SET_F(base, MCSC_R_STAT_CINFIFODUALLAYER_CONFIG,
			MCSC_F_STAT_CINFIFODUALLAYER_STRGEN_MODE_EN, 0x1);
		/* incremetal */
		MCSC_SET_F(base, MCSC_R_STAT_CINFIFODUALLAYER_CONFIG,
			MCSC_F_STAT_CINFIFODUALLAYER_STRGEN_MODE_DATA_TYPE, 0x1);
		MCSC_SET_F(base, MCSC_R_STAT_CINFIFODUALLAYER_CONFIG,
			MCSC_F_STAT_CINFIFODUALLAYER_STRGEN_MODE_DATA, 255);
	}
}
...
void is_hw_set_crc(struct pablo_mmio *base, u32 seed)
{
	...
	MCSC_SET_F(base, MCSC_R_STAT_CINFIFODUALLAYER_STREAM_CRC,
		MCSC_F_STAT_CINFIFODUALLAYER_CRC_SEED, seed);
}
...
KUNIT_EXPORT_SYMBOL(is_scaler_s_cloader);
...
static void __mcsc_hw_s_horizontal_coeff(struct pablo_mmio *base, struct mcsc_h_coef *h_coef_cfg,
	struct mcsc_v_coef *v_coef_cfg, int plane, u32 sc_coeff_reg, u32 sc_coeff_field,
	u32 reg_offset, u32 field_offset)
{
	int index;
	u32 val;
	...
	if (plane == MCSC_SC_COEF_Y) { /* Y horizontal : 8-tap filter */
		for (index = 0; index <= 8; index++) {
			...
			val = 0;
			val = MCSC_SET_V(base, val,
				sc_coeff_0246_field + (field_offset * index) + SECOND_TAP,
				h_coef_cfg->h_coef_c[index]);
			val = MCSC_SET_V(base, val,
				sc_coeff_1357_field + (field_offset * index) + SECOND_TAP,
				h_coef_cfg->h_coef_d[index]);
			MCSC_SET_R(base, sc_coeff_23_reg + (reg_offset * index), val);

			val = 0;
			val = MCSC_SET_V(base, val,
				sc_coeff_0246_field + (field_offset * index) + FOURTH_TAP,
				h_coef_cfg->h_coef_e[index]);
			val = MCSC_SET_V(base, val,
				sc_coeff_1357_field + (field_offset * index) + FOURTH_TAP,
				h_coef_cfg->h_coef_f[index]);
			MCSC_SET_R(base, sc_coeff_45_reg + (reg_offset * index), val);

			val = 0;
			val = MCSC_SET_V(base, val,
				sc_coeff_0246_field + (field_offset * index) + SIXTH_TAP,
				h_coef_cfg->h_coef_g[index]);
			val = MCSC_SET_V(base, val,
				sc_coeff_1357_field + (field_offset * index) + SIXTH_TAP,
				h_coef_cfg->h_coef_h[index]);
			MCSC_SET_R(base, sc_coeff_67_reg + (reg_offset * index), val);
		}
	} else if (plane == MCSC_SC_COEF_UV) { /* UV horizontal : 4-tap filter */
		for (index = 0; index <= 8; index++) {
			val = 0;
			val = MCSC_SET_V(base, val, sc_coeff_0246_field + (field_offset * index),
...
static void is_scaler_set_poly_scaler_h_coef(struct pablo_mmio *base, u32 output_id,
	struct mcsc_h_coef *h_coef_cfg, struct mcsc_v_coef *v_coef_cfg, int plane)
{
	u32 sc_coeff_reg;
	u32 sc_coeff_field;
	...
	__mcsc_hw_s_horizontal_coeff(base, h_coef_cfg, v_coef_cfg, plane, sc_coeff_reg,
		sc_coeff_field, reg_offset, field_offset);
}
...
static u32 get_scaler_coef_idx(u32 ratio)
{
	u32 idx;
...
static void __is_scaler_set_poly_scaler_coef(struct pablo_mmio *base, u32 output_id, u32 hratio,
	u32 vratio, struct scaler_coef_config *sc_coef, int plane)
{
	struct mcsc_h_coef *h_coef = NULL;
	struct mcsc_v_coef *v_coef = NULL;
	u32 h_coef_idx, v_coef_idx;

	h_coef_idx = get_scaler_coef_idx(hratio);
	v_coef_idx = get_scaler_coef_idx(vratio);

	h_coef = (struct mcsc_h_coef *)&sc_coef->poly_sc_h_coef[h_coef_idx];
	v_coef = (struct mcsc_v_coef *)&sc_coef->poly_sc_v_coef[v_coef_idx];

	is_scaler_set_poly_scaler_h_coef(base, output_id, h_coef, v_coef, plane);
	is_scaler_set_poly_scaler_v_coef(base, output_id, v_coef, plane);
}

void is_scaler_set_poly_scaler_coef(struct pablo_mmio *base, u32 output_id, u32 hratio, u32 vratio,
	struct scaler_coef_config *sc_coefs[], int num_coef)
{
...
static void is_scaler_set_post_scaler_h_coef(struct pablo_mmio *base_addr,
	u32 output_id, struct mcsc_h_coef *h_coef_cfg, struct mcsc_v_coef *v_coef_cfg, int plane)
{
	u32 sc_coeff_reg;
	...
	__mcsc_hw_s_horizontal_coeff(base_addr, h_coef_cfg, v_coef_cfg, plane,
		sc_coeff_reg, sc_coeff_field, reg_offset, field_offset);
}
...
static void __is_scaler_set_post_scaler_coef(struct pablo_mmio *base_addr, u32 output_id,
	u32 hratio, u32 vratio, struct scaler_coef_config *sc_coef, int plane)
{
	struct mcsc_h_coef *h_coef = NULL;
	struct mcsc_v_coef *v_coef = NULL;
	u32 h_coef_idx, v_coef_idx;
	...
	h_coef = (struct mcsc_h_coef *)&sc_coef->post_sc_h_coef[h_coef_idx];
	v_coef = (struct mcsc_v_coef *)&sc_coef->post_sc_v_coef[v_coef_idx];

	is_scaler_set_post_scaler_h_coef(base_addr, output_id, h_coef, v_coef, plane);
...
static void get_wdma_addr_arr(u32 output_id, u32 *addr)
{
	...
	case MCSC_OUTPUT4:
		...
		break;
	default:
		err("invalid output_id(%d)", output_id);
		break;
	}
}
...
void is_scaler_set_djag_hf_enable(struct pablo_mmio *base, u32 hf_enable)
{
	MCSC_SET_F(base, MCSC_R_YUV_DUALLAYERRECOMP_CTRL, MCSC_F_YUV_DUALLAYERRECOMP_BYPASS, !hf_enable);
}

void is_scaler_set_djag_hf_cfg(struct pablo_mmio *base, struct hf_config *hf_cfg)
{
	MCSC_SET_R(base, MCSC_R_YUV_DUALLAYERRECOMP_WEIGHT, hf_cfg->hf_weight);
}
...
static void is_scaler_set_djag_hf_img_size(struct pablo_mmio *base, u32 width, u32 height)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DUALLAYERRECOMP_INPUT_IMG_HSIZE, width);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DUALLAYERRECOMP_INPUT_IMG_VSIZE, height);
	MCSC_SET_R(base, MCSC_R_YUV_DUALLAYERRECOMP_IMG_SIZE, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DUALLAYERRECOMP_IN_CROP_SIZE_H, width);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DUALLAYERRECOMP_IN_CROP_SIZE_V, height);
	MCSC_SET_R(base, MCSC_R_YUV_DUALLAYERRECOMP_IN_CROP_SIZE, val);
}
...
static void is_scaler_set_djag_hf_radial_center(struct pablo_mmio *base, u32 pos_x, u32 pos_y)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_CENTER_X, pos_x);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_CENTER_Y, pos_y);
	MCSC_SET_R(base, MCSC_R_YUV_DUALLAYERRECOMP_RADIAL_CENTER, val);
}
...
static void is_scaler_set_djag_hf_radial_binning(struct pablo_mmio *base, u32 pos_x, u32 pos_y)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_BINNING_X, pos_x);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_BINNING_Y, pos_y);
	MCSC_SET_R(base, MCSC_R_YUV_DUALLAYERRECOMP_RADIAL_BINNING, val);
}

static void is_scaler_set_djag_hf_radial_biquad_factor(struct pablo_mmio *base,
	u32 factor_a, u32 factor_b)
{
	MCSC_SET_F(base, MCSC_R_YUV_DUALLAYERRECOMP_RADIAL_BIQUAD_FACTOR_A,
		MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_BIQUAD_FACTOR_A, factor_a);
	MCSC_SET_F(base, MCSC_R_YUV_DUALLAYERRECOMP_RADIAL_BIQUAD_FACTOR_B,
		MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_BIQUAD_FACTOR_B, factor_b);
}

static void is_scaler_set_djag_hf_radial_gain(struct pablo_mmio *base,
	u32 enable, u32 increment)
{
	MCSC_SET_F(base, MCSC_R_YUV_DUALLAYERRECOMP_RADIAL_GAIN_ENABLE,
		MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_GAIN_ENABLE, enable);
	MCSC_SET_F(base, MCSC_R_YUV_DUALLAYERRECOMP_RADIAL_GAIN_INCREMENT,
		MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_GAIN_INCREMENT, increment);
}

static void is_scaler_set_djag_hf_radial_biquad_scal_shift_adder(struct pablo_mmio *base,
	u32 value)
{
	MCSC_SET_F(base, MCSC_R_YUV_DUALLAYERRECOMP_RADIAL_BIQUAD_SCALE_SHIFT_ADDER,
		MCSC_F_YUV_DUALLAYERRECOMP_RADIAL_BIQUAD_SCALE_SHIFT_ADDER, value);
}
	...
	struct hf_config *hf_cfg, u32 width, u32 height)
{
	...
	if (!hf_cfg->gain_enable) {
		is_scaler_set_djag_hf_radial_gain(base, 0, 0);
		return;
	}

	is_scaler_set_djag_hf_radial_gain(base, hf_cfg->gain_enable, hf_cfg->gain_increment);
	...
	is_scaler_set_djag_hf_radial_biquad_factor(
		base, hf_cfg->biquad_factor_a, hf_cfg->biquad_factor_b);
	is_scaler_set_djag_hf_radial_biquad_scal_shift_adder(base, hf_cfg->gain_shift_adder);
}
...
void is_scaler_set_djag_tuning_param(
	struct pablo_mmio *base, const struct djag_config *djag_cfg, u32 scale_index)
{
	u32 val = 0;

	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_XFILTER_DEJAGGING_WEIGHT0,
		djag_cfg->xfilter_dejagging_coeff_cfg[scale_index].xfilter_dejagging_weight0);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_XFILTER_DEJAGGING_WEIGHT1,
		djag_cfg->xfilter_dejagging_coeff_cfg[scale_index].xfilter_dejagging_weight1);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_XFILTER_HF_BOOST_WEIGHT,
		djag_cfg->xfilter_dejagging_coeff_cfg[scale_index].xfilter_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_CENTER_HF_BOOST_WEIGHT,
		djag_cfg->xfilter_dejagging_coeff_cfg[scale_index].center_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DIAGONAL_HF_BOOST_WEIGHT,
		djag_cfg->xfilter_dejagging_coeff_cfg[scale_index].diagonal_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_CENTER_WEIGHTED_MEAN_WEIGHT,
		djag_cfg->xfilter_dejagging_coeff_cfg[scale_index].center_weighted_mean_weight);
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_THRES_1X5_MATCHING_SAD,
		djag_cfg->thres_1x5_matching_cfg[scale_index].thres_1x5_matching_sad);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_THRES_1X5_ABSHF,
		djag_cfg->thres_1x5_matching_cfg[scale_index].thres_1x5_abshf);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_THRES_1X5_MATCHING, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_THRES_SHOOTING_LLCRR,
		djag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_llcrr);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_THRES_SHOOTING_LCR,
		djag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_lcr);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_THRES_SHOOTING_NEIGHBOR,
		djag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_neighbor);
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_THRES_SHOOTING_UUCDD,
		djag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_uucdd);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_THRES_SHOOTING_UCD,
		djag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_ucd);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_MIN_MAX_WEIGHT,
		djag_cfg->thres_shooting_detect_cfg[scale_index].min_max_weight);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_THRES_SHOOTING_DETECT_1, val);

	MCSC_SET_F(base, MCSC_R_YUV_DJAGFILTER_LFSR_SEED_0, MCSC_F_YUV_DJAGFILTER_LFSR_SEED_0,
		djag_cfg->lfsr_seed_cfg[scale_index].lfsr_seed_0);
	MCSC_SET_F(base, MCSC_R_YUV_DJAGFILTER_LFSR_SEED_1, MCSC_F_YUV_DJAGFILTER_LFSR_SEED_1,
		djag_cfg->lfsr_seed_cfg[scale_index].lfsr_seed_1);
	MCSC_SET_F(base, MCSC_R_YUV_DJAGFILTER_LFSR_SEED_2, MCSC_F_YUV_DJAGFILTER_LFSR_SEED_2,
		djag_cfg->lfsr_seed_cfg[scale_index].lfsr_seed_2);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_0,
		djag_cfg->dither_cfg[scale_index].dither_value[0]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_1,
		djag_cfg->dither_cfg[scale_index].dither_value[1]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_2,
		djag_cfg->dither_cfg[scale_index].dither_value[2]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_3,
		djag_cfg->dither_cfg[scale_index].dither_value[3]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_4,
		djag_cfg->dither_cfg[scale_index].dither_value[4]);
	MCSC_SET_R(base, MCSC_R_YUV_DJAGFILTER_DITHER_VALUE_04, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_5,
		djag_cfg->dither_cfg[scale_index].dither_value[5]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_6,
		djag_cfg->dither_cfg[scale_index].dither_value[6]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_7,
		djag_cfg->dither_cfg[scale_index].dither_value[7]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_VALUE_8,
		djag_cfg->dither_cfg[scale_index].dither_value[8]);
	MCSC_SET_R(base, MCSC_R_YUV_DJAGFILTER_DITHER_VALUE_58, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_SAT_CTRL,
		djag_cfg->dither_cfg[scale_index].sat_ctrl);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_THRES,
		djag_cfg->dither_cfg[scale_index].dither_thres);
	MCSC_SET_R(base, MCSC_R_YUV_DJAGFILTER_DITHER_THRES, val);

	MCSC_SET_F(base, MCSC_R_YUV_DJAGFILTER_CP_HF_THRES, MCSC_F_YUV_DJAGFILTER_CP_HF_THRES,
		djag_cfg->cp_cfg[scale_index].cp_hf_thres);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_CP_ARBI_MAX_COV_OFFSET,
		djag_cfg->cp_cfg[scale_index].cp_arbi_max_cov_offset);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_CP_ARBI_MAX_COV_SHIFT,
		djag_cfg->cp_cfg[scale_index].cp_arbi_max_cov_shift);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_CP_ARBI_DENOM,
		djag_cfg->cp_cfg[scale_index].cp_arbi_denom);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_CP_ARBI_MODE,
		djag_cfg->cp_cfg[scale_index].cp_arbi_mode);
...
void is_scaler_set_predjag_tuning_param(
	struct pablo_mmio *base, const struct predjag_config *predjag_cfg, u32 scale_index)
{
	u32 val = 0;

	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_XFILTER_DEJAGGING_WEIGHT0,
		predjag_cfg->xfilter_dejagging_coeff_cfg[scale_index].xfilter_dejagging_weight0);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_XFILTER_DEJAGGING_WEIGHT1,
		predjag_cfg->xfilter_dejagging_coeff_cfg[scale_index].xfilter_dejagging_weight1);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_XFILTER_HF_BOOST_WEIGHT,
		predjag_cfg->xfilter_dejagging_coeff_cfg[scale_index].xfilter_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_CENTER_HF_BOOST_WEIGHT,
		predjag_cfg->xfilter_dejagging_coeff_cfg[scale_index].center_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DIAGONAL_HF_BOOST_WEIGHT,
		predjag_cfg->xfilter_dejagging_coeff_cfg[scale_index].diagonal_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_CENTER_WEIGHTED_MEAN_WEIGHT,
		predjag_cfg->xfilter_dejagging_coeff_cfg[scale_index].center_weighted_mean_weight);
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_THRES_1X5_MATCHING_SAD,
		predjag_cfg->thres_1x5_matching_cfg[scale_index].thres_1x5_matching_sad);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_THRES_1X5_ABSHF,
		predjag_cfg->thres_1x5_matching_cfg[scale_index].thres_1x5_abshf);
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_THRES_SHOOTING_LLCRR,
		predjag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_llcrr);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_THRES_SHOOTING_LCR,
		predjag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_lcr);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_THRES_SHOOTING_NEIGHBOR,
		predjag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_neighbor);
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_THRES_SHOOTING_UUCDD,
		predjag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_uucdd);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_THRES_SHOOTING_UCD,
		predjag_cfg->thres_shooting_detect_cfg[scale_index].thres_shooting_ucd);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_MIN_MAX_WEIGHT,
		predjag_cfg->thres_shooting_detect_cfg[scale_index].min_max_weight);
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DITHER_VALUE_5,
		predjag_cfg->dither_cfg[scale_index].dither_value[5]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DITHER_VALUE_6,
		predjag_cfg->dither_cfg[scale_index].dither_value[6]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DITHER_VALUE_7,
		predjag_cfg->dither_cfg[scale_index].dither_value[7]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DITHER_VALUE_8,
		predjag_cfg->dither_cfg[scale_index].dither_value[8]);
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_SAT_CTRL,
		predjag_cfg->dither_cfg[scale_index].sat_ctrl);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DITHER_THRES,
		predjag_cfg->dither_cfg[scale_index].dither_thres);
	...
	MCSC_SET_F(base, MCSC_R_YUV_DJAGPREFILTER_FILTER_CP_HF_THRES,
		MCSC_F_YUV_DJAGPREFILTER_CP_HF_THRES, predjag_cfg->cp_cfg[scale_index].cp_hf_thres);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_CP_ARBI_MAX_COV_OFFSET,
		predjag_cfg->cp_cfg[scale_index].cp_arbi_max_cov_offset);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_CP_ARBI_MAX_COV_SHIFT,
		predjag_cfg->cp_cfg[scale_index].cp_arbi_max_cov_shift);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_CP_ARBI_DENOM,
		predjag_cfg->cp_cfg[scale_index].cp_arbi_denom);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_CP_ARBI_MODE,
		predjag_cfg->cp_cfg[scale_index].cp_arbi_mode);
	MCSC_SET_R(base, MCSC_R_YUV_DJAGPREFILTER_CP_ARBI, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_HARRIS_K,
		predjag_cfg->harris_det_cfg[scale_index].harris_k);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_HARRIS_TH,
		predjag_cfg->harris_det_cfg[scale_index].harris_th);
	MCSC_SET_R(base, MCSC_R_YUV_DJAGPREFILTER_HARRIS_DET, val);

	MCSC_SET_F(base, MCSC_R_YUV_DJAGPREFILTER_BILATERAL_C, MCSC_F_YUV_DJAGPREFILTER_BILATERAL_C,
		predjag_cfg->bilateral_c_cfg[scale_index].bilateral_c);
}

void is_scaler_set_djag_dither_wb(struct pablo_mmio *base, struct djag_dither_config *dither_config)
{
	u32 val;
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_BLACK_LEVEL,
		dither_config->dither_black_level);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGFILTER_DITHER_WHITE_LEVEL,
		dither_config->dither_white_level);
	val = MCSC_SET_V(
		base, val, MCSC_F_YUV_DJAGFILTER_DITHER_WB_THRES, dither_config->dither_wb_thres);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_DITHER_WB, val);
}

void is_scaler_set_predjag_dither_wb(
	struct pablo_mmio *base, struct djag_dither_config *dither_config)
{
	u32 val;
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DITHER_BLACK_LEVEL,
		dither_config->dither_black_level);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DITHER_WHITE_LEVEL,
		dither_config->dither_white_level);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGPREFILTER_DITHER_WB_THRES,
		dither_config->dither_wb_thres);
	...
	u32 *out_width, u32 *out_height)
{
	...
	if (*out_width < DJAG_PRE_LINE_BUF_SIZE || exynos_soc_info.main_rev)
#else
	...
	if (min_ratio >= GET_ZOOM_RATIO(10, 16)) {
		...
		*out_height = in_height;
		 ...
		 */
		dbg_hw(2, "leave as much as 1.6 for poly scaler. in_width(%d) out_width(%d)\n",
			in_width, *out_width);
...
static const struct is_reg mcsc_dbg_cr[] = {
	...
	{ 0x0e84, "STAT_CINFIFODUALLAYER_STATUS" },
...
KUNIT_EXPORT_SYMBOL(mcsc_hw_dump);
...
{
	u32 offset = MCSC_R_STAT_DTPDUALLAYER_BYPASS - MCSC_R_YUV_DTPOTFIN_BYPASS;
...
inline int mcsc_hw_g_djag_line_buffer(void)
{
	...
	if (exynos_soc_info.main_rev >= 1)
		DJAG_PRE_LINE_BUF_SIZE = DJAG_PRE_LINE_BUF_SIZE_8192;
	else
		DJAG_PRE_LINE_BUF_SIZE = DJAG_PRE_LINE_BUF_SIZE_4880;
#endif

	return DJAG_PRE_LINE_BUF_SIZE;
}

struct mcsc_int_mask {
	ulong bitmask;
	bool partial;
};

static struct mcsc_int_mask int_mask[INT_TYPE_NUM] = {
	[INT_FRAME_START] = { BIT_MASK(INTR0_MCSC_FRAME_START_INT), false },
	[INT_FRAME_END] = { BIT_MASK(INTR0_MCSC_FRAME_END_INT), false },
	[INT_COREX_END] = { BIT_MASK(INTR0_MCSC_COREX_END_INT_0), false },
	[INT_SETTING_DONE] = { BIT_MASK(INTR0_MCSC_SETTING_DONE_INT), false },
...
void mcsc_hw_g_pmio_cfg(struct pmio_config *cfg)
{
	...
	cfg->num_reg_defaults_raw = (cfg->max_register / PMIO_REG_STRIDE) + 1;
	cfg->dma_addr_shift = LSB_BIT;

	cfg->fields = mcsc_field_descs;
	cfg->num_fields = ARRAY_SIZE(mcsc_field_descs);
}
KUNIT_EXPORT_SYMBOL(mcsc_hw_g_pmio_cfg);
