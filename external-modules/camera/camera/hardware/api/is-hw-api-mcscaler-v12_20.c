...
#include "sfr/is-sfr-mcsc-mmio-v12_20.h"
#else
...
#define MCSC_HWFC_OUTPUT_START MCSC_OUTPUT1
#define MCSC_HWFC_OUTPUT_END MCSC_OUTPUT2
...
static void is_hw_set_crc(struct pablo_mmio *base, u32 seed)
{
	...
	MCSC_SET_F(base, MCSC_R_YUV_DTPOTFIN_STREAM_CRC, MCSC_F_YUV_DTPOTFIN_CRC_SEED, seed);
	MCSC_SET_F(base, MCSC_R_YUV_DTPRDMAIN_STREAM_CRC, MCSC_F_YUV_DTPRDMAIN_CRC_SEED, seed);
	MCSC_SET_F(base, MCSC_R_YUV_MAIN_CTRL_CRC_EN, MCSC_F_YUV_CRC_SEED, seed);
}
	...
	u32 output_id, u32 fmt, u32 plane, u32 dma_idx, u32 width, u32 height, u32 enable)
{
	...
	if (!enable) {
		MCSC_SET_R(base, MCSC_R_YUV_WDMASC_W1_VOTF_EN, 0);
		MCSC_SET_R(base, MCSC_R_YUV_WDMASC_W2_VOTF_EN, 0);
		return;
	}

	if (output_id < MCSC_HWFC_OUTPUT_START || output_id > MCSC_HWFC_OUTPUT_END) {
		err("invalid scaler ID");
		return;
	}
	...
	hwfc_id = output_id - MCSC_HWFC_OUTPUT_START;
...
void is_scaler_set_djag_hf_enable(struct pablo_mmio *base, u32 hf_enable, u32 hf_scaler_enable)
{
	...
	if (hf_enable && !djag_enable)
		...
		val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAGRECOMP_RECOM_ENABLE, hf_enable);
...
{
	return MCSC_GET_R(base, MCSC_R_IP_USE_OTF_PATH) &&
