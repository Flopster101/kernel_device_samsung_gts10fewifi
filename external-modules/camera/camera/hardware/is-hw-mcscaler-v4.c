...
{
	const char *usage_msg = "MCSC debug features\n"
				"\tb[0] : dump sfr\n"
				"\tb[1] : dump sfr once\n"
				"\tb[2] : s2d fs\n"
				"\tb[3] : s2d fe\n"
...
const struct kernel_param *is_hw_mcsc_get_debug_kernel_param_kunit_wrapper(void)
{
	return G_KERNEL_PARAM(debug_mcsc);
}
KUNIT_EXPORT_SYMBOL(is_hw_mcsc_get_debug_kernel_param_kunit_wrapper);
#endif
...
{
	if (unlikely(test_bit(MCSC_DBG_DTP_OTF, &debug_mcsc.value))) {
	...
	if (unlikely(test_bit(MCSC_DBG_DTP_RDMA, &debug_mcsc.value))) {
...
static dma_addr_t *hw_mcsc_get_target_addr(u32 out_id, struct is_frame *frame)
{
	...
	case MCSC_OUTPUT5:
		...
		break;
	default:
		err("[F:%d] invalid output id(%d)", frame->fcount, out_id);
		break;
	}
	...
	struct mcs_param *param, u32 instance)
{
	...
	if (!IS_ENABLED(SKIP_SETFILE_LOAD) && !test_bit(MCSC_DBG_SKIP_SETFILE, &debug_mcsc.value))
...
void is_hw_get_gain_offset_range(int yuv, u32 *y_ofs, u32 *y_gain, u32 *c_gain00, u32 *c_gain01,
...
KUNIT_EXPORT_SYMBOL(is_hw_get_gain_offset_range);
...
void is_hw_bchs_range(void __iomem *base_addr, u32 output_id, int yuv)
{
	...
	is_hw_get_gain_offset_range(
		yuv, &y_ofs, &y_gain, &c_gain00, &c_gain01, &c_gain10, &c_gain11);
...
void is_hw_get_bchs_clamp(int yuv, u32 *y_max, u32 *y_min, u32 *c_max, u32 *c_min)
{
#ifdef ENABLE_10BIT_MCSC
	if (yuv == SCALER_OUTPUT_YUV_RANGE_FULL) {
		*y_max = 1023;
...
KUNIT_EXPORT_SYMBOL(is_hw_get_bchs_clamp);

void is_hw_bchs_clamp(
	void __iomem *base_addr, u32 output_id, int yuv, struct scaler_bchs_clamp_cfg *sc_bchs)
{
	u32 y_max, y_min, c_max, c_min;

	is_hw_get_bchs_clamp(yuv, &y_max, &y_min, &c_max, &c_min);
