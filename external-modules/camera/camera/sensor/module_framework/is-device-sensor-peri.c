...
static struct is_cis *is_sensor_peri_get_cis(struct is_device_sensor_peri *sensor_peri);
static void is_sensor_peri_reset_long_term_mode_exp_gain(struct is_cis *cis);
static void is_sensor_peri_reinit_on_streamoff(struct is_device_sensor_peri *sensor_peri);
static bool is_sensor_peri_check_skip_sub_device(struct is_device_sensor_peri *sensor_peri,
	bool is_stream_on);
static bool is_sensor_peri_check_skip_sub_device_mcu(struct is_device_sensor_peri *sensor_peri);
static int is_sensor_peri_get_ois_power_mode(struct is_device_sensor *device, bool is_stream_on);
static int is_sensor_peri_ois_power_mode_init(struct is_device_sensor *device, int ois_power_mode, bool on);
static void is_sensor_peri_laser_af_control(struct is_device_sensor *device,
	struct is_device_sensor_peri *sensor_peri, bool is_stream_on);
static void is_sensor_peri_af_work(struct is_device_sensor_peri *sensor_peri,
	enum is_peri_af_work_state af_state);
static void is_sensor_peri_ois_work(struct is_device_sensor_peri *sensor_peri,
	enum is_peri_ois_work_state ois_state);
static void is_sensor_peri_set_high_resolution_flash(struct is_device_sensor_peri *sensor_peri);
static void is_sensor_peri_stop_long_term_mode(struct is_cis *cis, struct v4l2_subdev *subdev_cis);
		...
		if (flash->flash_ae.main_fls_strm_on_off_step == 0) {
			if (flash->flash_data.flash_fired == false) {
				...
				if (ret) {
					err("failed to turn off flash at flash expired handler\n");
#ifdef USE_LEDS_FLASH_CHARGING_VOLTAGE_CONTROL
					CALL_VENDOR_INTERFACE_OPS(core->vendor, pdo_ctrl_by_flash, 0);
					CALL_VENDOR_INTERFACE_OPS(core->vendor, muic_afc_request_voltage, FLED, 9);
...
void is_sensor_ois_set_init_work(struct work_struct *data)
{
	...
	ret = CALL_OISOPS(sensor_peri->mcu->ois, ois_set_power_mode, sensor_peri->subdev_mcu, OIS_POWER_MODE_NONE);
	if (ret < 0)
...
int is_sensor_peri_notify_vsync(struct v4l2_subdev *subdev, void *arg)
{
	...
	vsync_count = *(u32 *)arg;
	cis = is_sensor_peri_get_cis(sensor_peri);
	cis->cis_data->sen_vsync_count = vsync_count;

	CALL_CISOPS(cis, cis_update_seamless_history, sensor_peri->subdev_cis);
...
int is_sensor_peri_pre_flash_fire(struct v4l2_subdev *subdev, void *arg)
{
	...
	if ((flash_uctl->flashMode != flash->flash_data.mode) ||
		...
		if (flash->flash_data.mode == CAM2_FLASH_MODE_TORCH) {
			CALL_VENDOR_INTERFACE_OPS(core->vendor, pdo_ctrl_by_flash, 1);
			CALL_VENDOR_INTERFACE_OPS(core->vendor, muic_afc_request_voltage, FLED, 5);
...
void is_sensor_long_term_mode_set_work(struct work_struct *data)
{
	...
	if (step >= 1)
		is_sensor_peri_reset_long_term_mode_exp_gain(cis);
	else
...
void is_sensor_peri_init_work(struct is_device_sensor_peri *sensor_peri)
{
	...
	if (sensor_peri->actuator && sensor_peri->actuator->actuator_ops) {
		if (sensor_peri->actuator->vendor_use_sleep_mode)
...
{
	if ((device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_FHD_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_QHD_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_FHD_HDR10_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_QHD_HDR10_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_FHD_HLG10_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_QHD_HLG10_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_FHD_LOG10_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_QHD_LOG10_VIDEO) {
		return true;
	} else {
		return false;
	}
...
bool is_sensor_peri_check_uhd60fps(struct is_device_sensor *device)
{
#if defined(USE_OIS_POWER_SAVING_UHD60)
	if ((device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_UHD_60FPS_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_UHD_60FPS_HDR10_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_UHD_60FPS_HLG10_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_UHD_60FPS_LOG10_VIDEO)
		return true;
#endif
		return false;
}

bool is_sensor_peri_check_stream_on(u32 position)
{
	...
	if (!device) {
		err("cannot get device");
		return NULL;
	}

	return device->sstream;
}

int is_sensor_peri_check_ois_power_mode(struct is_device_sensor *device, bool is_stream_on)
{
	...
	ret = OIS_POWER_MODE_NONE;

	check_uhd60 = is_sensor_peri_check_uhd60fps(device);
	if (check_uhd60) {
		if (is_stream_on
			&& (module->position == SENSOR_POSITION_REAR || module->position == SENSOR_POSITION_REAR3)) {
			if (!is_sensor_peri_check_stream_on(SENSOR_POSITION_REAR2)
				|| core->mcu->current_power_mode == OIS_POWER_MODE_NONE)
				ret = OIS_POWER_MODE_SINGLE_WIDE;
		} else if (module->position == SENSOR_POSITION_REAR2) {
			if (is_stream_on)
				ret = OIS_POWER_MODE_WIDE_TELE;
			else if (is_sensor_peri_check_stream_on(SENSOR_POSITION_REAR)
				|| is_sensor_peri_check_stream_on(SENSOR_POSITION_REAR3))
				ret = OIS_POWER_MODE_SINGLE_WIDE;
		}
		if (ret != OIS_POWER_MODE_NONE && !core->mcu->need_reset_mcu) {
			core->mcu->need_reset_mcu = true;
			info_mcu("[%s] set ois reset flag.", __func__);
		}
		if (ret == OIS_POWER_MODE_NONE)
			ret = OIS_POWER_MODE_SKIP;

	} else if (is_sensor_peri_check_super_steady(device) || sensor_peri->check_auto_framing) {
	...
	info_mcu("[%s] sensor pos=%d, ois power mode=%s, onoff=%d", __func__,
		module->position, OIS_PW_STR(ret), is_stream_on);
	return ret;
}
#endif


int is_sensor_peri_s_stream(struct is_device_sensor *device, bool on)
{
	int ret = 0;
	struct is_device_sensor_peri *sensor_peri = NULL;
	struct v4l2_subdev *subdev_cis = NULL;
	...
	bool is_ois_normal_mode = false;
	const char *sensor_name;
	int ois_power_mode = 0;
	...
	skip_sub_device = is_sensor_peri_check_skip_sub_device(sensor_peri, on);
	skip_sub_device_mcu = is_sensor_peri_check_skip_sub_device_mcu(sensor_peri);
	ois_power_mode = is_sensor_peri_get_ois_power_mode(device, on);
#if defined(CONFIG_CAMERA_USE_INTERNAL_MCU)
	is_ois_normal_mode = (!skip_sub_device_mcu && (ois_power_mode == OIS_POWER_MODE_NONE));
#endif
	...
	if (on) { /* stream on sequence */
		is_sensor_peri_ois_power_mode_init(device, ois_power_mode, on);

		if (!skip_sub_device) {
			is_sensor_peri_af_work(sensor_peri, AF_ACTIVE_SCHEDULE);
			is_sensor_peri_laser_af_control(device, sensor_peri, on);
		}

		if (is_ois_normal_mode)
			is_sensor_peri_ois_work(sensor_peri, OIS_INIT_SCHEDULE);
		...
		usleep_range(2000, 2010);	//2ms delay
		...
		if (!skip_sub_device) {
			info("[%s] join time E\n", __func__);
			is_sensor_peri_af_work(sensor_peri, AF_ACTIVE_FLUSH);
			info("[%s] join time X\n", __func__);
		}

		if (is_ois_normal_mode)
			is_sensor_peri_ois_work(sensor_peri, OIS_INIT_FLUSH);
		...
		is_sensor_peri_set_high_resolution_flash(sensor_peri);

		ret = CALL_CISOPS(cis, cis_set_flip_mode, subdev_cis);
		if (ret < 0)
			err("err!!! cis set flip mode ret(%d)", ret);

		ret = CALL_CISOPS(cis, cis_update_seamless_history, subdev_cis);
		if (ret < 0)
			err("err!!! cis_update_seamless_history ret(%d)", ret);

		ret = CALL_CISOPS(cis, cis_stream_on, subdev_cis);
		if (ret < 0) {
			err("sensor stream on fail");
		} else {
			set_bit(IS_SENSOR_WAIT_STREAMING, &device->state);
			ret = CALL_CISOPS(cis, cis_wait_streamon, subdev_cis);
			if (ret < 0) {
				err("sensor wait stream on fail");
#ifdef CONFIG_CAMERA_VENDOR_MCD
				is_sensor_gpio_dbg(device);
				ret = CALL_CISOPS(cis, cis_recover_stream_on, subdev_cis);
				if (ret < 0)
					err("cis_recover_stream_on fail");
#endif
			}
		}
	} else { /* stream off sequence */
		is_sensor_peri_ois_power_mode_init(device, ois_power_mode, on);

		if (!skip_sub_device)
			is_sensor_peri_af_work(sensor_peri, AF_SLEEP_SCHEDULE);

		if (is_ois_normal_mode)
			is_sensor_peri_ois_work(sensor_peri, OIS_DEINIT_SCHEDULE);
		...
		if (ret == 0) {
			ret = CALL_CISOPS(cis, cis_wait_streamoff, subdev_cis);
			if (ret < 0) {
				err("sensor wait stream off fail");
#ifdef CONFIG_CAMERA_VENDOR_MCD
				CALL_CISOPS(cis, cis_log_status, subdev_cis);
				ret = CALL_CISOPS(cis, cis_recover_stream_off, subdev_cis);
				if (ret < 0)
					err("cis_recover_stream_off fail");
#endif
			}
		}

		is_sensor_peri_stop_long_term_mode(cis, subdev_cis);
		...
		if (!skip_sub_device) {
			info("[%s] join time E\n", __func__);
			is_sensor_peri_laser_af_control(device, sensor_peri, on);
			is_sensor_peri_af_work(sensor_peri, AF_SLEEP_FLUSH);
			info("[%s] join time X\n", __func__);
		}

		if (is_ois_normal_mode)
			is_sensor_peri_ois_work(sensor_peri, OIS_DEINIT_FLUSH);

		is_sensor_peri_reinit_on_streamoff(sensor_peri);
	}

	if (ret < 0) {
		err("[%s] v4l2_subdev_call(s_stream, on:%d) is fail(%d)", sensor_name, on, ret);
		goto p_err;
	}

#if IS_ENABLED(CONFIG_CAMERA_HW_BIG_DATA)
	is_sec_update_hw_param_af_info(device, 0, on ? HW_PARAM_STREAM_ON : HW_PARAM_STREAM_OFF);
#endif
...
static void is_sensor_peri_reinit_on_streamoff(struct is_device_sensor_peri *sensor_peri)
{
	...
	struct is_core *core = is_get_is_core();
	int sensor_cnt = atomic_read(&core->resourcemgr.global_param.sensor_cnt);
	int ret = 0;
#endif
...
static bool is_sensor_peri_check_skip_sub_device(struct is_device_sensor_peri *sensor_peri,
	bool is_stream_on)
{
	struct is_cis *cis = is_sensor_peri_get_cis(sensor_peri);
	struct is_core *core = is_get_is_core();
	bool skip_sub_device = false;

	dbg_sensor(1, "[%s] scene_mode(%d), opening_hint(%d)\n",
		__func__, cis->cis_data->is_data.scene_mode, core->vendor.opening_hint);
	...
	if (skip_sub_device)
		info("[%s] skip sub device\n", __func__);

	return skip_sub_device;
}
...
static bool is_sensor_peri_check_skip_sub_device_mcu(struct is_device_sensor_peri *sensor_peri)
{
	...
	if (!sensor_peri->mcu)
		return skip_sub_device_mcu;

	dbg_sensor(1, "[%s] scene_mode(%d), opening_hint(%d), support_photo_fastae(%d), skip_video_fastae(%d)\n",
		__func__, cis->cis_data->is_data.scene_mode, core->vendor.opening_hint,
		sensor_peri->mcu->support_photo_fastae, sensor_peri->mcu->skip_video_fastae);

	if (cis->cis_data->is_data.scene_mode == AA_SCENE_MODE_FAST_AE) {
		if (core->vendor.opening_hint == IS_OPENING_HINT_FASTEN_AE
			&& !sensor_peri->mcu->support_photo_fastae)
			skip_sub_device_mcu = true;

		if (core->vendor.opening_hint == IS_OPENING_HINT_FASTEN_AE_VIDEO
			&& sensor_peri->mcu->skip_video_fastae)
			skip_sub_device_mcu = true;
	}

	if (skip_sub_device_mcu)
		info("[%s] skip sub device mcu\n", __func__);
...
static int is_sensor_peri_get_ois_power_mode(struct is_device_sensor *device, bool is_stream_on)
{
	...
	if (!is_stream_on && sensor_peri->check_auto_framing) {
		sensor_peri->check_auto_framing = false;
		info("[%s] off auto framing flag\n", __func__);
	}

#if defined(CONFIG_CAMERA_USE_INTERNAL_MCU) && defined(CAMERA_2ND_OIS)
	ois_power_mode = is_sensor_peri_check_ois_power_mode(device, is_stream_on);
#endif

	return ois_power_mode;
}
...
static int is_sensor_peri_ois_power_mode_init(struct is_device_sensor *device, int ois_power_mode, bool on)
{
	...
	bool check_uhd60 = is_sensor_peri_check_uhd60fps(device);
	int ois_power_mode_val = ois_power_mode;

	if (ois_power_mode_val == OIS_POWER_MODE_NONE)
		return ret;
	...
	if (!(device_mcu && device_mcu->mcu && device_mcu->subdev_mcu)
		&& !(sensor_peri->mcu && sensor_peri->subdev_mcu))
		return ret;

	if (on && ois_power_mode_val < OIS_POWER_MODE_SKIP) {
		if (check_uhd60) {
			if (!sensor_peri->mcu) {
		...
		} else if (!sensor_peri->mcu && module->position == SENSOR_POSITION_REAR3) {
			if (ois_power_mode_val != OIS_POWER_MODE_UW)
				ois_power_mode_val = OIS_POWER_MODE_SINGLE_WIDE;

			current_mcu = device_mcu->mcu;
			subdev_mcu = device_mcu->subdev_mcu;
			info("[%s] Use default mcu.\n", __func__);
		} else if (ois_power_mode_val == OIS_POWER_MODE_SINGLE_WIDE && sensor_peri->mcu) {
			current_mcu = sensor_peri->mcu;
			subdev_mcu = sensor_peri->subdev_mcu;
			info("[%s] Use sensor-peri mcu.\n", __func__);
		}
	} else if (check_uhd60 && ois_power_mode_val == OIS_POWER_MODE_SINGLE_WIDE
				&& module->position == SENSOR_POSITION_REAR2 && sensor_peri->mcu) {
		current_mcu = sensor_peri->mcu;
		subdev_mcu = sensor_peri->subdev_mcu;
		info("[%s] Use sensor-peri mcu.\n", __func__);
	}
	...
	if (current_mcu && subdev_mcu) {
		ret = CALL_OISOPS(current_mcu->ois, ois_set_dev_ctrl, subdev_mcu, 0);
		if (ret < 0)
			err("v4l2_subdev_call(ois_set_dev_ctrl) is fail(%d)", ret);

		ret = CALL_OISOPS(current_mcu->ois, ois_set_power_mode, subdev_mcu, ois_power_mode_val);
		if (ret < 0)
			err("v4l2_subdev_call(ois_set_power_mode) is fail(%d)", ret);
	...
	info("[%s][cam:%d][onoff:%d][mcu:%d] power mode[%s] supersteady[%d] auto framing[%d] UHD60FPS[%d]\n", __func__,
		sensor_peri->module->position, on, (current_mcu && subdev_mcu), OIS_PW_STR(ois_power_mode_val),
		is_sensor_peri_check_super_steady(device), sensor_peri->check_auto_framing, check_uhd60);
...
static void is_sensor_peri_set_high_resolution_flash(struct is_device_sensor_peri *sensor_peri)
{
	...
	if (sensor_peri->flash && sensor_peri->flash->flash_data.high_resolution_flash == true) {
...
static void is_sensor_peri_stop_long_term_mode(struct is_cis *cis, struct v4l2_subdev *subdev_cis)
{
	if (cis->long_term_mode.sen_strm_off_on_enable) {
		cis->long_term_mode.sen_strm_off_on_enable = 0;
		CALL_CISOPS(cis, cis_set_long_term_exposure, subdev_cis);
		info("[%s] stopped long_term_mode\n", __func__);
...
int is_sensor_peri_update_seamless_state(struct is_device_sensor *device)
{
	int ret = 0;
	struct is_device_sensor_peri *sensor_peri = NULL;
	struct is_cis *cis = NULL;
	...
	if (cis_data->pre_remosaic_zoom_ratio != cis_data->cur_remosaic_zoom_ratio) {
		cis_data->remosaic_crop_update_vsync_cnt = cis_data->sen_vsync_count + 1;
		ssinfo("SS_DBG rmc_update_vsync_cnt(%d)\n", cis,
...
int is_sensor_peri_update_seamless_mode(struct is_device_sensor *device)
{
	...
	if (cis_data->seamless_mode_changed) {
		...
		info("%s [%d][%s] SS_DBG GPH[%d] changed[%d] AE delay[%lldus] seamless delay[%lldus]\n",
			__func__, cis_data->sen_vsync_count, cis->sensor_info->name,
			cis_data->group_param_hold, cis_data->seamless_mode_changed,
			(start_ts - cur_vsync_ts) / 1000, (end_ts - start_ts) / 1000);
	}

	return ret;
}

u32 is_sensor_peri_get_max_margin_for_frame_sync(struct is_device_sensor *device)
{
	struct is_core *core = (struct is_core *)device->private_data;
	struct is_device_sensor_peri *sensor_peri;
	struct is_device_sensor *device_sensor;
	struct is_device_csi *csi;
	int vsync_count = 0, i;
	cis_shared_data *cis_data = NULL;
	u32 max_margin_frame_us_time = 0;
	u32 margin_frame_us_time = 0;

	for (i = 0; i < IS_SENSOR_COUNT; i++) {
		device_sensor = &core->sensor[i];
		if (!test_bit(IS_SENSOR_OPEN, &device_sensor->state))
			continue;

		csi = v4l2_get_subdevdata(device_sensor->subdev_csi);
		if (!csi)
			continue;

		vsync_count = atomic_read(&csi->fcount);
		if (vsync_count == 0)
			continue;

		sensor_peri = get_sensor_peri(device_sensor->subdev_module);
		if (!sensor_peri || sensor_peri->cis.dual_sync_type != DUAL_SYNC_TYPE_SW)
			continue;
		...
		margin_frame_us_time
			= sensor_cis_get_margin_frame_us_time(&sensor_peri->cis);

		dbg_sensor(2, "[%s][%s][%d] max_margin_CIT(%d)->(%dms)", __func__,
			sensor_peri->module->sensor_name, cis_data->sen_vsync_count,
			cis_data->max_margin_coarse_integration_time, margin_frame_us_time);

		if (margin_frame_us_time > max_margin_frame_us_time)
			max_margin_frame_us_time = margin_frame_us_time;
	}

	dbg_sensor(2, "[%s]max_margin_frame_us_time(%d)", __func__, max_margin_frame_us_time);

	return max_margin_frame_us_time;
}
...
void is_sensor_peri_compensate_duration_for_frame_sync(struct is_device_sensor *device, u32 vsync_count)
{
...
#ifdef USE_CAMERA_MCD_SW_DUAL_SYNC
	frame_interval = vsync_count - cis_data->dual_sync_count;
	cis_data->dual_sync_count = vsync_count;

	if (group_scount % SW_SYNC_ADJUST_INTERVAL)
		return;
#else
...
#ifdef USE_CAMERA_MCD_SW_DUAL_SYNC
	if (sensor_uctrl->dynamicFrameDuration > cis_data->min_frame_us_time) {
		add_criteria = (sensor_uctrl->dynamicFrameDuration - cis_data->min_frame_us_time)
						* (SW_SYNC_ADJUST_MAX_CRITERIA - SW_SYNC_ADJUST_CRITERIA)
						/ (SW_SYNC_10FPS_DURATION - cis_data->min_frame_us_time);
		...
		add_criteria = max(0ULL, add_criteria);
	...
	dual_sync_diff = cur_ts_s >= cur_ts_m ? cur_ts_s - cur_ts_m : cur_ts_m - cur_ts_s;
	...
	dbg_sensor(2, "[%s][F:%d,%d,%d] T_stmp(%lld,%lld,%lld), Duration(%lld,%lld,%lld),(diff:%lld)", __func__,
		...
		apply_duration_s, cur_duration_m, pre_duration_m,
		dual_sync_diff);
	...
	if ((cur_ts_s >= cur_ts_m) && ((cur_ts_s - cur_ts_m) >= SW_SYNC_ADJUST_CRITERIA + add_criteria)) {
		...
		dbg_sensor(2, "[%s][F:%d] S/W frame sync is working(diff:%lld)",
			__func__, vsync_count, dual_sync_diff);
		return;
	}
#else
...
int is_sensor_peri_actuator_softlanding(struct is_device_sensor_peri *device)
{
	...
	info("nrc_soft_landing not supported, proceed with SW soft landing\n");
