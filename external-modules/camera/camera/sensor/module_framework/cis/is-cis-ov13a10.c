...
#include "is-vendor.h"
#include "is-cis-ov13a10-setA-19p2.h"
#include "is-cis-ov13a10-front-setA-19p2.h"
...
#define SENSOR_NAME "OV13A10"

static bool lte_enabled = false;

int sensor_ov13a10_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	cis->cis_data->sens_config_index_pre = SENSOR_OV13A10_MODE_MAX;
...
static const struct is_cis_log log_ov13a10[] = {
...
int sensor_ov13a10_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);

	sensor_cis_log_status(cis, log_ov13a10, ARRAY_SIZE(log_ov13a10), (char *)__func__);

	return ret;
}

int sensor_ov13a10_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct sensor_ov13a10_private_data *priv = (struct sensor_ov13a10_private_data *)cis->sensor_info->priv;
...
int sensor_ov13a10_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
...
int sensor_ov13a10_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	usleep_range(3000, 3010);	//3ms delay
...
int sensor_ov13a10_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	cit_long = ((cis->cis_data->frame_length_lines / 2) - 12); //max_exposure

	if (lte_enabled) { //Only in case of hyperlapse
		lte_enabled = false;
		ret |= cis->ixc_ops->write16(cis->client, 0x380C, cis->cis_data->line_length_pck);
		ret |= cis->ixc_ops->write16(cis->client, cis->reg_addr->fll, cis->cis_data->frame_length_lines);
...
int sensor_ov13a10_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	int ret = 0;
	int ret_err = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	while (sensor_fcount == 0x0000) {
		usleep_range(CIS_STREAM_ON_WAIT_TIME, CIS_STREAM_ON_WAIT_TIME + 10);
		wait_cnt++;
...
int sensor_ov13a10_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
...
u32 sensor_ov13a10_cis_calc_again_permile(u32 code)
{
	u32 ret = 0;
	int i;

	for (i = 0; i <= MAX_GAIN_INDEX; i++) {
		if (sensor_ov13a10_analog_gain[i][0] == code) {
			ret = sensor_ov13a10_analog_gain[i][1];
			break;
		}
	}
...
u32 sensor_ov13a10_cis_calc_again_closest(u32 permile)
{
	int i;

	if (permile <= sensor_ov13a10_analog_gain[CODE_GAIN_INDEX][PERMILE_GAIN_INDEX])
		return sensor_ov13a10_analog_gain[CODE_GAIN_INDEX][PERMILE_GAIN_INDEX];

	if (permile >= sensor_ov13a10_analog_gain[MAX_GAIN_INDEX][PERMILE_GAIN_INDEX])
		return sensor_ov13a10_analog_gain[MAX_GAIN_INDEX][PERMILE_GAIN_INDEX];

	for (i = 0; i <= MAX_GAIN_INDEX; i++) {
		if (sensor_ov13a10_analog_gain[i][PERMILE_GAIN_INDEX] == permile)
			return sensor_ov13a10_analog_gain[i][PERMILE_GAIN_INDEX];

		if ((int)(permile - sensor_ov13a10_analog_gain[i][PERMILE_GAIN_INDEX]) < 0)
			return sensor_ov13a10_analog_gain[i-1][PERMILE_GAIN_INDEX];
	}

	return sensor_ov13a10_analog_gain[MAX_GAIN_INDEX][PERMILE_GAIN_INDEX];
}

u32 sensor_ov13a10_cis_calc_again_code(u32 permile)
{
	u32 ret = 0, nearest_val = 0;
	int i;

	nearest_val = sensor_ov13a10_cis_calc_again_closest(permile);

	dbg_sensor(1, "[%s] permile %d nearest_val %d\n", __func__, permile, nearest_val);

	for (i = 0; i <= MAX_GAIN_INDEX; i++) {
		if (sensor_ov13a10_analog_gain[i][1] == nearest_val) {
			ret = sensor_ov13a10_analog_gain[i][0];
			break;
		}
	}
...
int sensor_ov13a10_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	u16 digital_gain = 0;
...
int sensor_ov13a10_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	u16 dgain_code = 0;
...
int sensor_ov13a10_cis_set_test_pattern(struct v4l2_subdev *subdev, camera2_sensor_ctl_t *sensor_ctl)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	if (cis->cis_data->cur_pattern_mode != sensor_ctl->testPatternMode) {
		if (sensor_ctl->testPatternMode == SENSOR_TEST_PATTERN_MODE_OFF) {
			info("[%d][%s]: set DEFAULT pattern! (testpatternmode : %d)\n", cis->id, __func__,
...
bool sensor_ov13a10_cis_check_lte(struct v4l2_subdev *subdev, u32 input_duration)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	if (cis_data->is_data.scene_mode != AA_SCENE_MODE_PRO_MODE && cis_data->is_data.scene_mode != AA_SCENE_MODE_HYPERLAPSE)
		return false;

	if (input_duration > 100000) {
		dbg_sensor(1, "[%d][%s][%d] enable LTE : input_duration(%d)\n",
			cis->id, __func__, cis_data->sen_vsync_count, input_duration);
		return true;
	}
...
int sensor_ov13a10_cis_lte_on(struct v4l2_subdev *subdev, u32 frame_duration, u32 input_duration)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
	u64 pclk_mhz;
	u16 cit_long, dgain;
	u16 hts = cis->cis_data->line_length_pck;
	u16 vts = cis->cis_data->frame_length_lines;
	u32 again_code = 0, sensor_again = 0, cal_digital = 0;
	struct ae_param total_dgain, again;
	...
	if (frame_duration == 500000) { //for 15x hyperlapse night
		hts = 0x048C;
		vts = 0xBC80;
	} else if (frame_duration == 1500000) { //for 45x hyperlapse night
		hts = 0x1230;
		vts = 0x8D0A;
	}

	again.long_val = cis_data->auto_exposure[NEXT_FRAME].analog_gain;
	dgain = cis_data->auto_exposure[NEXT_FRAME].digital_gain;

	pclk_mhz = cis_data->pclk / (1000 * 1000);
	cit_long = ((input_duration * pclk_mhz) / (hts * cis->reg_addr->fll_divider)) - (cis_data->min_fine_integration_time / hts);

	if (cit_long > ((vts / 2) - 12))
		cit_long = ((vts / 2) - 12); //max_exposure

	dbg_sensor(1, "[%d][%s][%d] frame_duration(%dms), exposure(%d), cit_long(0x%x)\n",
		cis->id, __func__, cis_data->sen_vsync_count, (frame_duration / 1000), input_duration, cit_long);

	if (!lte_enabled) { //Only first frame of hyperlapse
		/* stream off */
		sensor_ov13a10_cis_stream_off(subdev);
		sensor_ov13a10_cis_wait_streamoff(subdev);
		...
		if (again_code < cis_data->max_analog_gain[0]) {
			...
			total_dgain.long_val = dgain;
			total_dgain.short_val = dgain;
		}
		...
		IXC_MUTEX_LOCK(cis->ixc_lock);
		/* HTS */
		ret |= cis->ixc_ops->write16(cis->client, 0x380C, hts);

		/* VTS */
		ret |= cis->ixc_ops->write16(cis->client, cis->reg_addr->fll, vts);
		...
		/* stream on */
		sensor_ov13a10_cis_stream_on(subdev);
		sensor_ov13a10_cis_wait_streamon(subdev);
	} else {
		IXC_MUTEX_LOCK(cis->ixc_lock);
		/* CIT */
		ret |= cis->ixc_ops->write16(cis->client, cis->reg_addr->cit, cit_long);
...
int sensor_ov13a10_cis_lte_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	u16 cit_long;
...
int sensor_ov13a10_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
	...
	if (sensor_ov13a10_cis_check_lte(subdev, frame_duration) == true) {
		cis_data->cur_frame_us_time = frame_duration;
		cis_data->last_frame_duration = frame_duration;
	} else {
		ret = sensor_ov13a10_cis_lte_off(subdev);
...
int sensor_ov13a10_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
	...
	if (sensor_ov13a10_cis_check_lte(subdev, cis_data->min_frame_us_time) == true) {
		ret = sensor_ov13a10_cis_lte_on(subdev, cis_data->min_frame_us_time, input_duration);
	} else {
		ret = sensor_ov13a10_cis_lte_off(subdev);
...
int sensor_ov13a10_cis_init_state(struct v4l2_subdev *subdev, int mode)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_ov13a10_cis_init,
	.cis_init_state = sensor_ov13a10_cis_init_state,
	.cis_log_status = sensor_ov13a10_cis_log_status,
	.cis_group_param_hold = NULL,
	.cis_set_global_setting = sensor_ov13a10_cis_set_global_setting,
	.cis_mode_change = sensor_ov13a10_cis_mode_change,
	.cis_stream_on = sensor_ov13a10_cis_stream_on,
	.cis_stream_off = sensor_ov13a10_cis_stream_off,
	.cis_wait_streamon = sensor_ov13a10_cis_wait_streamon,
	.cis_wait_streamoff = sensor_ov13a10_cis_wait_streamoff,
	.cis_data_calculation = sensor_cis_data_calculation,
	.cis_set_exposure_time = sensor_ov13a10_cis_set_exposure_time,
	...
	.cis_set_frame_duration = sensor_ov13a10_cis_set_frame_duration,
	...
	.cis_calc_again_code = sensor_ov13a10_cis_calc_again_code,
	.cis_calc_again_permile = sensor_ov13a10_cis_calc_again_permile,
	.cis_set_digital_gain = sensor_ov13a10_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_ov13a10_cis_get_digital_gain,
	...
	//.cis_set_flip_mode = sensor_ov13a10_cis_set_flip_mode,
	.cis_set_test_pattern = sensor_ov13a10_cis_set_test_pattern,
	.cis_set_totalgain = sensor_cis_set_totalgain,
};

DEFINE_I2C_DRIVER_PROBE(cis_ov13a10_probe_i2c)
{
	int ret;
	u32 mclk_freq_khz;
	struct is_cis *cis;
	...
	cis->reg_addr = &sensor_ov13a10_reg_addr;
	...
	if (mclk_freq_khz == 19200) {
		if (strcmp(setfile, "setA") == 0) {
			...
			cis->sensor_info = &sensor_ov13a10_info_A_19p2;
		} else if (strcmp(setfile, "setA_front") == 0) {
			probe_info("%s setfile_front_A mclk: 19.2MHz\n", __func__);
			cis->sensor_info = &sensor_ov13a10_front_info_A_19p2;
		} else {
...
PKV_DEFINE_I2C_DRIVER_REMOVE(cis_ov13a10_remove_i2c)
{
PKV_DEFINE_I2C_DRIVER_REMOVE_RETURN;
}

static const struct of_device_id sensor_cis_ov13a10_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-ov13a10",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_ov13a10_match);

static const struct i2c_device_id sensor_cis_ov13a10_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_ov13a10_driver = {
	.probe	= cis_ov13a10_probe_i2c,
	.remove = cis_ov13a10_remove_i2c,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_ov13a10_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_ov13a10_idt
};

#ifdef MODULE
module_driver(sensor_cis_ov13a10_driver, i2c_add_driver,
	i2c_del_driver);
#else
static int __init sensor_cis_ov13a10_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_ov13a10_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_ov13a10_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_ov13a10_init);
#endif
MODULE_LICENSE("GPL");
