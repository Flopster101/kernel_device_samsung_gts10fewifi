...
int sensor_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (cis->reg_addr->fll_divider > 0)
		cit_long = ((exp.long_val * 1000) / (line_readOut_time_ns * cis->reg_addr->fll_divider)) - (min_fine_int / line_length_pck);
	else
	...
	if (cis->reg_addr->fll_divider > 0)
		cit_short = ((exp.short_val * 1000) / (line_readOut_time_ns * cis->reg_addr->fll_divider)) - (min_fine_int / line_length_pck);
	else
