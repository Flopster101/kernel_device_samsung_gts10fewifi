...
#define SENSOR_NAME "HI1337"

#define POLL_TIME_MS 5
#define STREAM_MAX_POLL_CNT 100

static struct setfile_info *sensor_hi1337_setfile_fsync_info;

/*************************************************
 *  [HI1337 Analog gain formular]
 *
...
u32 sensor_hi1337_cis_calc_again_permile(u32 code)
{
	return ((code * 1000 / 16 + 1000));
}

/*************************************************
 *  [HI1337 Digital gain formular]
 *
...
u32 sensor_hi1337_cis_calc_dgain_code(u32 permile)
{
	u32 buf[2] = {0, 0};
...
u32 sensor_hi1337_cis_calc_dgain_permile(u32 code)
{
	return (((code & 0x1E00) >> 9) * 1000) + ((code & 0x01FF) * 1000 / 512);
}

/* CIS OPS */
int sensor_hi1337_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	cis->cis_data->sens_config_index_pre = SENSOR_HI1337_MODE_MAX;
...
static const struct is_cis_log log_hi1337[] = {
...
int sensor_hi1337_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);

	sensor_cis_log_status(cis, log_hi1337, ARRAY_SIZE(log_hi1337), (char *)__func__);

	return ret;
}

static int sensor_hi1337_cis_group_param_hold_func(struct v4l2_subdev *subdev, bool hold)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_HI1337_GROUP_PARAM_HOLD_ADDR, &hold_set);
	if (ret < 0) {
	...
	ret = cis->ixc_ops->write16(client, SENSOR_HI1337_GROUP_PARAM_HOLD_ADDR, hold_set);
	if (ret < 0) {
...
*/
#if USE_GROUP_PARAM_HOLD
int sensor_hi1337_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	u32 mode = 0;
...
/* TODO :  add the fasten_ae mode
	if (mode == SENSOR_HI1337_1024x768_120FPS) {
		ret = 0;
...
*/
	...
	ret = sensor_hi1337_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err_unlock;
...
int sensor_hi1337_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct sensor_hi1337_private_data *priv = (struct sensor_hi1337_private_data *)cis->sensor_info->priv;
...
int sensor_hi1337_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	...
	//ret = sensor_cis_set_registers(subdev, sensor_hi1337_setfile_fsync_info[HI1337_FSYNC_NORMAL].file,
	//		sensor_hi1337_setfile_fsync_info[HI1337_FSYNC_NORMAL].file_size);
	//if (ret < 0) {
	//	err("sensor_hi1337_setfile_fsync_info(normal mode) fail");
	//	goto p_err_unlock;
	//}
...
#if 0
int sensor_hi1337_cis_enable_frame_cnt(struct i2c_client *client, bool enable)
{
	int ret = -1;
	u8 frame_cnt_ctrl = 0;
	...
	ret = cis->ixc_ops->read8(client, SENSOR_HI1337_MIPI_TX_OP_MODE_ADDR,
			&frame_cnt_ctrl);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
				SENSOR_HI1337_MIPI_TX_OP_MODE_ADDR, ret);
		goto p_err;
	}
	...
	ret = cis->ixc_ops->write8(client, SENSOR_HI1337_MIPI_TX_OP_MODE_ADDR,
			frame_cnt_ctrl);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
				SENSOR_HI1337_MIPI_TX_OP_MODE_ADDR, ret);
...
int sensor_hi1337_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	/* Sensor stream on */
	ret = cis->ixc_ops->write16(cis->client, SENSOR_HI1337_STREAM_ONOFF_ADDR, 0x0100);
	IXC_MUTEX_UNLOCK(cis->ixc_lock);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
				SENSOR_HI1337_STREAM_ONOFF_ADDR, ret);
		goto p_err;
	}
...
int sensor_hi1337_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = NULL;
	...
	ret = sensor_hi1337_cis_group_param_hold_func(subdev, false);
	if (ret < 0)
		err("group_param_hold_func failed at stream off");

	ret = cis->ixc_ops->write16(cis->client, SENSOR_HI1337_STREAM_ONOFF_ADDR, 0x0000);
	IXC_MUTEX_UNLOCK(cis->ixc_lock);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
				SENSOR_HI1337_STREAM_ONOFF_ADDR, ret);
...
int sensor_hi1337_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	u32 wait_cnt = 0;
	...
	do {
		ret = cis->ixc_ops->read16(cis->client, SENSOR_HI1337_ISP_PLL_ENABLE_ADDR, &PLL_en);
		if (ret < 0) {
			err("i2c transfer fail addr(%x) ret = %d\n",
					SENSOR_HI1337_ISP_PLL_ENABLE_ADDR, ret);
			goto p_err;
		}
...
int sensor_hi1337_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	u32 wait_cnt = 0;
...
int sensor_hi1337_cis_set_analog_digital_gain(struct v4l2_subdev *subdev,
	struct ae_param *again, struct ae_param *dgain)
{
	...
	analog_gain = (u8)sensor_hi1337_cis_calc_again_code(again->val);
	...
	/* Set Digital gains */
	if (analog_gain == cis_data->max_analog_gain[0]) {
		dgain_code = (u16)sensor_hi1337_cis_calc_dgain_code(dgain->val);
		...
		cal_analog_val1 = sensor_hi1337_cis_calc_again_code(again->val);
		cal_analog_val2 = sensor_hi1337_cis_calc_again_permile(cal_analog_val1);
		cal_digital = (again->val * 1000) / cal_analog_val2;
		dgain_code = (u16)sensor_hi1337_cis_calc_dgain_code(cal_digital);
		if (cal_digital < 0) {
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_HI1337_ISP_ENABLE_ADDR, &read_val);
	if (ret < 0)
		goto p_err_unlock;

	enable_dgain = read_val | (0x1 << 4); /* [4]: D gain enable */
	ret = cis->ixc_ops->write16(cis->client, SENSOR_HI1337_ISP_ENABLE_ADDR, enable_dgain);
	if (ret < 0)
		goto p_err_unlock;
...
#if 0
int sensor_hi1337_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	ret = cis->ixc_ops->read8(cis->client, cis->reg_addr->again, &analog_gain);
	if (ret < 0)
		goto p_err_unlock;

	*again = sensor_hi1337_cis_calc_again_permile((u32)analog_gain);
...
int sensor_hi1337_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = NULL;
...
int sensor_hi1337_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = NULL;
	...
	if (coarse_int > cis_data->max_coarse_integration_time) {
		dbg_sensor(1, "%s [%d] VCNT[%d] coarse(%d) max(%d)\n",
			__func__, cis->id, cis_data->sen_vsync_count, coarse_int,
	...
	if (coarse_int < cis_data->min_coarse_integration_time) {
		dbg_sensor(1, "%s [%d] VCNT[%d] coarse(%d) min(%d)\n",
			__func__, cis->id, cis_data->sen_vsync_count,
	...
	dbg_sensor(1, "%s [%d] VCNT[%d] target[%d] => FLL[%#x] CIT[%#x]\n",
		__func__, cis->id, cis_data->sen_vsync_count, target_exposure->val,
		cis_data->frame_length_lines, coarse_int);
	...
	struct ae_param *again, struct ae_param *dgain)
{
	...
	/* Set Exposure Time */
	ret = sensor_hi1337_cis_set_exposure_time(subdev, target_exposure);
...
p_err_unlock:
	...
	usleep_range(1000, 1010); /* sleep 1msec */
	return ret;
}

static struct is_cis_ops cis_ops_hi1337 = {
	.cis_init = sensor_hi1337_cis_init,
	.cis_log_status = sensor_hi1337_cis_log_status,
#if USE_GROUP_PARAM_HOLD
	.cis_group_param_hold = sensor_hi1337_cis_group_param_hold,
#endif
	.cis_set_global_setting = sensor_hi1337_cis_set_global_setting,
	.cis_mode_change = sensor_hi1337_cis_mode_change,
	.cis_stream_on = sensor_hi1337_cis_stream_on,
	.cis_stream_off = sensor_hi1337_cis_stream_off,
	.cis_wait_streamoff = sensor_hi1337_cis_wait_streamoff,
	.cis_wait_streamon = sensor_hi1337_cis_wait_streamon,
	...
	.cis_calc_again_code = sensor_hi1337_cis_calc_again_code,
	.cis_calc_again_permile = sensor_hi1337_cis_calc_again_permile,
	.cis_set_digital_gain = sensor_hi1337_cis_set_digital_gain,
...
DEFINE_I2C_DRIVER_PROBE(cis_hi1337_probe_i2c)
{
	int ret;
	struct is_cis *cis;
	...
	if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setA") == 0) {
		...
		sensor_hi1337_setfile_fsync_info = sensor_hi1337_setfile_A_fsync_info;
		cis->sensor_info = &sensor_hi1337_info_A;
	} else {
...
PKV_DEFINE_I2C_DRIVER_REMOVE(cis_hi1337_remove_i2c)
{
PKV_DEFINE_I2C_DRIVER_REMOVE_RETURN;
}

static const struct of_device_id sensor_cis_hi1337_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-hi1337",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_hi1337_match);

static const struct i2c_device_id sensor_cis_hi1337_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_hi1337_driver = {
	.probe	= cis_hi1337_probe_i2c,
	.remove = cis_hi1337_remove_i2c,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_hi1337_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_hi1337_idt
};

#ifdef MODULE
module_driver(sensor_cis_hi1337_driver, i2c_add_driver,
	i2c_del_driver);
#else
static int __init sensor_cis_hi1337_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_hi1337_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_hi1337_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_hi1337_init);
#endif
