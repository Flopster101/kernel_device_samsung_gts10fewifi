...
int is_ixc_pin_control(struct is_module_enum *module, u32 scenario, u32 value)
{
	...
	case SENSOR_SCENARIO_HW_INIT:
		...
		if (sensor_peri->cis.client) {
			...
			if (atomic_read(&core->ixc_rsccount[i2c_channel]) == value) {
				info("%s[%d] cis ixc config(%d), position(%d), scenario(%d), i2c_channel(%d)\n",
				     __func__, __LINE__, i2c_config_state, module->position,
				     scenario, i2c_channel);
				ret |= is_ixc_pin_config(sensor_peri->cis.client,
							i2c_config_state, cis_ixc_type);
			}
		}
		if (device->ois) {
	...
	case SENSOR_SCENARIO_READ_ROM:
		if (module->pdata->rom_id < ROM_ID_MAX) {
			...
			is_vendor_get_position_from_rom_id(module->pdata->rom_id, &position);
			sensor_id = is_vendor_get_sensor_id_from_position(position);
			dualized_sensor_id = is_vender_get_dualized_sensorid(position);

			if (sensor_id == dualized_sensor_id) {
				ret |= is_ixc_pin_config(vendor_priv->dualized_rom[module->pdata->rom_id].client[0], i2c_config_state, I2C_TYPE);
			}else {
				ret |= is_ixc_pin_config(vendor_priv->rom[module->pdata->rom_id].client[0], i2c_config_state, I2C_TYPE);
			}
		}
		break;
...
static struct pablo_kunit_ixc_ops ixc_ops = {
	.pin_control = is_ixc_pin_control,
};

struct pablo_kunit_ixc_ops *pablo_kunit_get_ixc(void)
{
	return &ixc_ops;
}
KUNIT_EXPORT_SYMBOL(pablo_kunit_get_ixc);
#endif
