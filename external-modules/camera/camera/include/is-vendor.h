...
#define CALL_VENDOR_INTERFACE_OPS(s, op, args...) \
	(((s).is_vendor_interface_ops->op) ? ((s).is_vendor_interface_ops->op(args)) : 0);
...
struct is_vendor {
	...
	int isp_max_width;
	int isp_max_height;
	struct is_vendor_interface_ops *is_vendor_interface_ops;
};
...
void is_vendor_get_rom_dualcal_info_from_position(int position, int *rom_dualcal_id,
