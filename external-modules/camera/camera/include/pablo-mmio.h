...
#define PMIO_CFG_RANGE_CHECK_FULL 0
...
static const char * const pmio_formatter_names[] = {
	"bypass",
	"inc",
	"pair",
	"rpt",
...
#define C_LOADER_FMT_INC_ADDR	0x0
#define C_LOADER_FMT_PAIR	0x1
#define C_LOADER_FMT_RPT_ADDR	0x3

#define C_LOADER_HDR_NORMAL	0x0
#define C_LOADER_HDR_EXTEDED	0x1
 ...
 *
 * @dma_addr_shfit:	DMA address shift for header in format synchronization.
 * @phys_base:		Physical base address for header/payload in format-sync.
 * @ignore_phys_base:	If set, physical base address is no longer required when
 *			create each header or specific payload in format-sync.
 *			This is helpful to improve the overall performance of
 *			format-sync.
 * @use_ext_hdr:	Use extended header for repeat address mode, if supports.
 *
 */
struct pmio_config {
	const char *name;
...
int pmio_reset_cache(struct pablo_mmio *pmio);
...
int pmio_cache_fsync_ext(struct pablo_mmio *pmio, void *buf, const void *items,
			 size_t block_count);
...
bool pmio_cache_get_only(struct pablo_mmio *pmio);
void pmio_cache_set_bypass(struct pablo_mmio *pmio, bool enable);
void pmio_cache_mark_dirty(struct pablo_mmio *pmio);

void pmio_clb_hex_dump(const char *title, const void *buf, unsigned int limit);
