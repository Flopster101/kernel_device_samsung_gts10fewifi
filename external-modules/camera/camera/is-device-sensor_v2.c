...
int is_search_sensor_module_with_position(struct is_device_sensor *device, u32 position,
...
int is_sensor_g_module(struct is_device_sensor *device, struct is_module_enum **module)
{
	int ret = 0;
...
static int is_sensor_g_device(struct platform_device *pdev, struct is_device_sensor **device)
{
	int ret = 0;
...
static inline bool is_sensor_cmp_mode(struct is_sensor_cfg *ss_cfg, struct is_sensor_mode *mode)
{
	...
	if (mode->ex_mode_format == EX_FORMAT_12BIT)
		hw_format = HW_FORMAT_RAW12;
	else
		hw_format = HW_FORMAT_RAW10;

	if (ss_cfg->input[CSI_VIRTUAL_CH_0].hwformat != hw_format)
		return false;

	return true;
}

static struct is_sensor_cfg *is_sensor_select_mode(
	struct is_device_sensor *device, struct is_sensor_mode *mode)
{
	...
	minfo("try to find sensor mode(%dx%d@%d) ex_mode(%d:%d) ex_mode_format(0x%x)\n", device,
		mode->width, mode->height, mode->framerate, mode->ex_mode, mode->ex_mode_extra,
		mode->ex_mode_format);
	...
	if (select)
		minfo("sensor mode(%dx%d@%d) ex_mode(%d) = %d (lane:%d)\n", device, select->width,
			select->height, select->framerate, (uint32_t)(select->ex_mode),
			select->mode, (select->lanes + 1));
	else
...
struct is_sensor_cfg *is_sensor_g_mode(struct is_device_sensor *device)
{
	...
	mode.ex_mode_format = device->ex_mode_format;

	select = is_sensor_select_mode(device, &mode);
	if (!select)
		return NULL;
	...
	if ((device->position == SENSOR_POSITION_REAR2) || /* tele1 */
		...
		if (sensor_peri->subdev_cis) {
			CALL_CISOPS(cis, cis_update_pdaf_tail_size, sensor_peri->subdev_cis,
				    select);
			CALL_CISOPS(cis, cis_set_crop_region, sensor_peri->subdev_cis,
				    select->mode);
		}
...
static int __is_sensor_gpio_on(struct is_device_sensor *device)
{
	...
	if (!test_and_set_bit(IS_MODULE_GPIO_ON, &module->state)) {
		...
		ret = is_vendor_sensor_gpio_on_sel(vendor, scenario, &gpio_scenario, module);
		if (ret) {
			clear_bit(IS_MODULE_GPIO_ON, &module->state);
			merr("is_vendor_sensor_gpio_on_sel is fail(%d)", device, ret);
			goto p_err;
		}
		...
		ret = is_vendor_sensor_gpio_on(vendor, scenario, gpio_scenario, module);
		if (ret) {
...
int is_sensor_gpio_off(struct is_device_sensor *device)
{
	...
	if (test_and_clear_bit(IS_MODULE_GPIO_ON, &module->state)) {
		...
		ret = is_vendor_sensor_gpio_off_sel(vendor, scenario, &gpio_scenario, module);
		if (ret) {
			set_bit(IS_MODULE_GPIO_ON, &module->state);
			merr("is_vendor_sensor_gpio_off_sel is fail(%d)", device, ret);
			goto p_err;
		}
...
IS_TIMER_FUNC(is_sensor_snr)
{
	...
	if ((!device->force_stop && !device->snr_check) || sysfs_debug->pattern_en ||
	    !test_bit(IS_SENSOR_START, &device->state))
		return;
...
static int is_sensor_start(struct is_device_sensor *device)
{
	...
	scn = is_set_static_dvfs(&ischain->resourcemgr->dvfs_ctrl, scenario_id, true);
...
p_err_itf_stream_on:
	...
	scenario_id = is_hw_dvfs_get_scenario(ischain, IS_DVFS_STATIC);
	is_set_static_dvfs(&ischain->resourcemgr->dvfs_ctrl, scenario_id, false);
...
static void is_sensor_check_buf(struct is_group *grp, struct is_subdev *sd, struct is_frame *frame)
{
	...
	if (sub_cnt > ldr_cnt + 1)
		mgrwarn("subdev DQ might be blocked: ldr(R%d, P%d, C%d), sub[%s](R%d, P%d, C%d)",
			grp, grp, frame, ldr_req_cnt, ldr_pro_cnt, ldr_com_cnt, sd->name,
			sub_req_cnt, sub_pro_cnt, sub_com_cnt);
}

int is_sensor_buf_tag(struct is_device_sensor *device, struct is_subdev *f_subdev,
		      struct v4l2_subdev *v_subdev, struct is_frame *ldr_frame)
{
	...
	if (frame) {
		...
		if (ret) {
			msrwarn("v4l2_subdev_call(s_rx_buffer) is fail(%d)", device, f_subdev,
				ldr_frame, ret);
			ret = -EINVAL;
		} else {
...
int is_sensor_buf_skip(struct is_device_sensor *device, struct is_subdev *f_subdev,
		       struct v4l2_subdev *v_subdev, struct is_frame *ldr_frame)
{
	int ret = 0;
	unsigned long flags;
...
int is_sensor_sbuf_tag(struct is_device_sensor *device, struct is_subdev *ldr_subdev,
		       struct v4l2_subdev *v_subdev, struct is_frame *ldr_frame, int vc)
{
	int ret = 0;
	unsigned long flags;
	struct is_framemgr *framemgr;
	...
	if (ret) {
		msrwarn("v4l2_subdev_call(s_rx_buffer) is fail(%d)", device, ldr_subdev, ldr_frame,
			ret);
		ret = -EINVAL;
	}
...
int is_sensor_dm_tag(struct is_device_sensor *device, struct is_frame *frame)
{
	int ret = 0;
	u32 hashkey;
	int i;
	...
	if (frame->shot) {
		...
		if (device->timestampboot[hashkey] < device->prev_timestampboot)
			minfo("[SS%d][F%d] Reverse timestampboot(p[%llu], c[%llu])\n", device,
			      device->device_id, frame->fcount, device->prev_timestampboot,
		...
		for (i = 0; i < BITS_PER_LONG / F_ID_SIZE; i++) {
			...
			frame->shot->udm.frame_id[i] = sub_f_id & GENMASK(F_ID_SIZE - 1, 0);
		}
		 ...
		 */
		if (is_debug_support_crta() || is_vendor_check_remosaic_mode_change(frame)) {
				    ...
				    unsigned int notification)
{
	...
	if (frame && (frame->fcount > device->fcount)) {
		mgwarn("frame count is bigger than current (%d > %d)[%d/%d/%d]", device, group,
		       frame->fcount, device->fcount, framemgr->queued_count[FS_REQUEST],
...
static int is_sensor_notify_by_line(struct is_device_sensor *device, unsigned int notification)
{
	int ret = 0;
	struct is_group *group;
	struct is_framemgr *framemgr;
	...
	/* There's no shot */
	if (!frame || !test_bit(group->leader.id, &frame->out_flag)) {
		mgwarn("[F%d] (SENSOR LINE IRQ) There's no Shot", group, group,
		       device->line_fcount);
		goto p_err;
	}
...
static void is_sensor_notify(struct v4l2_subdev *subdev, unsigned int notification, void *arg)
{
	int ret = 0;
	...
	case CSIS_NOTIFY_LINE:
		...
		break;
	case CSIS_NOTIFY_VSYNC:
		ret = v4l2_subdev_call(device->subdev_module, core, ioctl,
				       V4L2_CID_SENSOR_NOTIFY_VSYNC, arg);
		if (ret)
			merr("is sensor notify vsync is fail", device);
		break;
	case CSIS_NOTIFY_VBLANK:
		ret = v4l2_subdev_call(device->subdev_module, core, ioctl,
				       V4L2_CID_SENSOR_NOTIFY_VBLANK, arg);
		if (ret)
...
static void is_sensor_set_nfi(struct work_struct *data)
{
	struct is_device_sensor *ids = container_of(data, struct is_device_sensor, nfi_work);
	bool curr_toggle = ids->nfi_toggle;
	bool next_toggle = !curr_toggle;
	struct is_sensor_cfg *next_sensor_cfg = ids->cfg[next_toggle];
	struct is_device_csi *csi;
	int ret;

	if (!next_sensor_cfg) {
		merr("next_sensor_cfg is NULL", ids);
		return;
	}
	...
	ret = csis_pdp_top_nfi_en(csi, true, &ids->cfg[curr_toggle]->fid_loc);
	if (ret) {
		merr("failed to call csis_pdp_top_nfi_en()", ids);
		return;
	}

	csi->sensor_cfg[next_toggle] = next_sensor_cfg;

	csi_set_nfi(csi, next_sensor_cfg);
...
static int is_sensor_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&device->nfi_work, is_sensor_set_nfi);
...
static void is_sensor_reset_state(struct is_device_sensor *device)
{
	...
	device->cfg[NFI_TOGGLE_0] = NULL;
	device->cfg[NFI_TOGGLE_1] = NULL;
	device->nfi_toggle = false;
}

int is_sensor_open(struct is_device_sensor *device, struct is_video_ctx *vctx)
{
	int ret = 0;
	int ret_err = 0;
...
int is_sensor_close(struct is_device_sensor *device)
{
	...
	cancel_work_sync(&device->nfi_work);

	if (device->subdev_module) {
		ret = v4l2_subdev_call(device->subdev_module, core, ioctl, V4L2_CID_SENSOR_DEINIT,
				       device);
		if (ret)
...
static void set_cis_data(struct is_device_sensor *s, struct is_module_enum *m)
{
	struct is_core *core = is_get_is_core();
	struct is_device_sensor_peri *sp = (struct is_device_sensor_peri *)m->private_data;
...
static void set_actuator_data(struct is_device_sensor *s, struct is_module_enum *m)
{
	struct is_core *core = is_get_is_core();
	struct is_device_sensor_peri *sp = (struct is_device_sensor_peri *)m->private_data;
	...
	if (!IS_ENABLED(CONFIG_CAMERA_USE_INTERNAL_MCU) ||
		...
		if (sp->actuator && (m->pdata->af_product_name == s->actuator[s->pdev->id]->id)) {
...
static void set_flash_data(struct is_device_sensor *s, struct is_module_enum *m)
{
	struct is_device_sensor_peri *sp = (struct is_device_sensor_peri *)m->private_data;
...
static void set_ois_data(struct is_device_sensor *s, struct is_module_enum *m)
{
	struct is_core *core = is_get_is_core();
	struct is_device_sensor_peri *sp = (struct is_device_sensor_peri *)m->private_data;
...
static void set_mcu_data(struct is_device_sensor *s, struct is_module_enum *m)
{
	struct is_core *core = is_get_is_core();
	struct is_device_sensor_peri *sp = (struct is_device_sensor_peri *)m->private_data;
...
static void set_aperture_data(struct is_device_sensor *s, struct is_module_enum *m)
{
	struct is_core *core = is_get_is_core();
	struct is_device_sensor_peri *sp = (struct is_device_sensor_peri *)m->private_data;
...
static void set_eeprom_data(struct is_device_sensor *s, struct is_module_enum *m)
{
	struct is_core *core = is_get_is_core();
	struct is_device_sensor_peri *sp = (struct is_device_sensor_peri *)m->private_data;
...
static void set_laser_af_data(struct is_device_sensor *s, struct is_module_enum *m)
{
	struct is_core *core = is_get_is_core();
	struct is_device_sensor_peri *sp = (struct is_device_sensor_peri *)m->private_data;
...
int is_sensor_s_input(struct is_device_sensor *device, u32 position, u32 scenario,
		      u32 stream_leader)
{
	int ret = 0;
	int ret_smc;
	...
	if (IS_ENABLED(SECURE_CAMERA_IRIS)) {
		ret = is_secure_func(core, device, IS_SECURE_CAMERA_IRIS, device->pdata->scenario,
				     SMC_SECCAM_PREPARE);
		if (ret)
			goto p_err;
	}
	...
	if (!IS_ENABLED(CONFIG_SECURE_CAMERA_USE) || (device->ex_scenario != IS_SCENARIO_SECURE)) {
	...
	ret = is_devicemgr_binding(device, group->device, IS_DEVICE_SENSOR);
	if (ret) {
...
p_err:
	...
	minfo("[SS%d:D] %s(pos:%d, sce:%d)(pos:%d)-%d\n", device, device->device_id, __func__,
	      position, scenario, device->position, ret);
...
static int is_sensor_s_fmt(void *device, struct is_queue *iq)
{
	...
	ids->nfi_toggle = false;
	ids->cfg[ids->nfi_toggle] = is_sensor_g_mode(ids);
	if (!ids->cfg[ids->nfi_toggle]) {
...
int is_sensor_s_framerate(struct is_device_sensor *device, struct v4l2_streamparm *param)
{
	int ret = 0;
	struct v4l2_subdev *subdev_module;
	struct is_module_enum *module;
...
static int sensor_module_set_nfi(struct is_device_sensor *device, struct v4l2_ext_control *ext_ctrl)
{
	int ret;
	struct is_sensor_mode mode;

	ret = pkaw_copy_from_user(&mode, ext_ctrl->ptr, sizeof(struct is_sensor_mode));
	if (ret) {
		err("pkaw_copy_from_user of V4L2_CID_SENSOR_SET_NFI is fail(%d)", ret);
		return ret;
	}

	device->cfg[!device->nfi_toggle] = is_sensor_select_mode(device, &mode);

	return 0;
}

int is_sensor_s_ctrl(struct is_device_sensor *device, struct v4l2_control *ctrl)
{
	int ret = 0;
	struct v4l2_subdev *subdev_module;
	struct v4l2_subdev *subdev_csi;
...
int is_sensor_s_ext_ctrls(struct is_device_sensor *device, struct v4l2_ext_controls *ctrls)
{
	...
	if (!ext_ctrl) {
		merr("ext_ctrl is NULL", device);
		return -EINVAL;
	}

	switch (ext_ctrl->id) {
	case V4L2_CID_SENSOR_SET_NFI:
		ret = sensor_module_set_nfi(device, ext_ctrl);
		if (ret)
			return ret;
		break;
	default:
		ret = v4l2_subdev_call(
			subdev_module, core, ioctl, SENSOR_IOCTL_MOD_S_EXT_CTRL, ctrls);
		if (ret) {
...
int is_sensor_g_ext_ctrls(struct is_device_sensor *device, struct v4l2_ext_controls *ctrls)
{
	int ret = 0;
	struct v4l2_subdev *subdev_module;
	struct v4l2_ext_control *ext_ctrl;
	...
	switch (ext_ctrl->id) {
	default:
		ret = v4l2_subdev_call(subdev_module, core, ioctl, SENSOR_IOCTL_MOD_G_EXT_CTRL,
				       ctrls);
		if (ret) {
			err("s_ext_ctrls is fail(%d)", ret);
			goto p_err;
...
int is_sensor_s_bns(struct is_device_sensor *device, u32 width, u32 height)
{
	FIMC_BUG(!device);
...
int is_sensor_s_frame_duration(struct is_device_sensor *device, u32 framerate)
{
	int ret = 0;
	u64 frame_duration;
	struct v4l2_subdev *subdev_module;
...
int is_sensor_s_exposure_time(struct is_device_sensor *device, u32 exposure_time)
{
	int ret = 0;
	struct v4l2_subdev *subdev_module;
	struct is_module_enum *module;
...
int is_sensor_g_ctrl(struct is_device_sensor *device, struct v4l2_control *ctrl)
{
	int ret = 0;
	struct v4l2_subdev *subdev_module;
...
{
	FIMC_BUG(!device);
	FIMC_BUG(!device->cfg[device->nfi_toggle]);
	return device->cfg[device->nfi_toggle]->ex_mode;
}
...
int is_sensor_g_bns_ratio(struct is_device_sensor *device)
{
	...
	binning = min(BINNING(sensor_width, bns_width), BINNING(sensor_height, bns_height));
...
{
	return device->cfg[device->nfi_toggle]->binning;
}

int is_sensor_g_updated_bratio(struct is_device_sensor *ids, u32 fcount)
{
	int ret = 0;
	struct is_module_enum *module = NULL;
	struct is_device_sensor_peri *sensor_peri = NULL;
	...
	binning_ratio = is_sensor_g_bratio(ids);

	if (IS_ENABLED(CONFIG_ARCH_VELOCE_HYCON))
		return binning_ratio;

	module = (struct is_module_enum *)v4l2_get_subdevdata(ids->subdev_module);
	FIMC_BUG(!module);
	...
	ret = CALL_CISOPS(cis, cis_get_updated_binning_ratio, sensor_peri->subdev_cis, fcount, &binning_ratio);

	if (ret)
		err("CALL_CISOPS is fail(%d)", ret);

	return binning_ratio;
}

u32 is_sensor_g_hdr_mode(struct is_device_sensor *ids, u32 fcount, bool is_short)
{
	struct is_module_enum *module = NULL;
	struct is_device_sensor_peri *sensor_peri = NULL;
	struct is_cis *cis = NULL;
	...
	vsync_count = (fcount > cis->cis_data->sen_vsync_count) ? fcount : cis->cis_data->sen_vsync_count;
	...
	if (cis->cis_data->seamless_history[ref_history_index].mode & SENSOR_MODE_AEB)
		hdr_mode = SENSOR_HDR_MODE_2AEB_2VC;
	else
		hdr_mode = SENSOR_HDR_MODE_SINGLE;
...
void is_sensor_s_otf_out(struct is_device_sensor *device)
{
	...
	if (test_and_clear_bit(IS_DO_SENSOR_OTF_OUT_CFG, &device->aeb_state)) {
		...
		if (otf_out_num)
			v4l2_subdev_call(device->subdev_csi, core, ioctl, SENSOR_IOCTL_S_OTF_OUT,
					 (void *)&otf_out_num);
	}
}
...
static int is_sensor_back_start(void *device, struct is_queue *iq)
{
	...
	ret = v4l2_subdev_call(ids->subdev_module, video, s_routing, 0, 0, 1);
	if (ret) {
		merr("failed at s_routing for module(%d)", ids, ret);
		goto p_err;
	}

	if (IS_ENABLED(CONFIG_SECURE_CAMERA_USE) && (ids->ex_scenario == IS_SCENARIO_SECURE) &&
	...
	if (IS_ENABLED(SECURE_CAMERA_FACE)) {
		ret = is_secure_func(core, ids, IS_SECURE_CAMERA_FACE, ids->ex_scenario,
				     SMC_SECCAM_PREPARE);
		if (ret)
			goto p_err;
	}
	...
	minfo("[SS%d:D] %s(%dx%d, %d)\n", ids, ids->device_id, __func__, ids->image.window.width,
	      ids->image.window.height, ret);

	return 0;

p_err_after_smc:
	if (IS_ENABLED(SECURE_CAMERA_FACE))
		is_secure_func(core, NULL, IS_SECURE_CAMERA_FACE, ids->ex_scenario,
...
static int is_sensor_back_stop(void *device, struct is_queue *iq)
{
	...
	csi = v4l2_get_subdevdata(ids->subdev_csi);
	if (csi)
		pablo_csi_disable(csi);

	ret = v4l2_subdev_call(ids->subdev_module, video, s_routing, 0, 0, 0);
	if (ret)
		merr("failed at s_routing for module(%d)", ids, ret);
...
int is_sensor_standby_flush(struct is_device_sensor *device)
{
	...
	ret = CALL_GROUP_OPS(group, stop);
	if (ret == -EPERM) {
		mgerr("group is already stop(%d), skip start sequence", group, group, ret);
		goto p_skip;
	} else if (ret != 0) {
...
static int is_sensor_wait_asyncshot(struct is_device_sensor *device)
{
	...
	minfo("[ISC:D] stream on %s ready(scnt: %d, qcnt: %d, init: %d, asyn: %d, skip: %d)\n",
	      device, retry ? "OK" : "NOT", scount, qcount, init_shots, group_leader->asyn_shots,
	      group_leader->skip_shots);
p_err:
	return ret;
}

int is_sensor_front_start(struct is_device_sensor *device, u32 instant_cnt, u32 nonblock)
{
	int ret = 0;
	struct v4l2_subdev *subdev_module;
	struct v4l2_subdev *subdev_csi;
	...
	ret = v4l2_subdev_call(device->subdev_module, core, ioctl,
			       V4L2_CID_SENSOR_NOTIFY_ACTUATOR_INIT, 0);
	if (ret)
	...
	mdbgd_sensor("%s(%s, csi ch : %d, size : %d x %d)\n", device, __func__, module->sensor_name,
		     device->pdata->csi_ch, device->image.window.width,
...
p_err:
	if (IS_ENABLED(SECURE_CAMERA_IRIS))
		ret = is_secure_func(NULL, device, IS_SECURE_CAMERA_IRIS, device->pdata->scenario,
				     SMC_SECCAM_UNPREPARE);
...
static int is_sensor_shot(struct is_device_ischain *ischain, struct is_group *group,
			  struct is_frame *check_frame)
{
	int ret = 0;
	unsigned long flags;
	...
	if (!atomic_read(&group->head->scount)) {
		ret = v4l2_subdev_call(sensor->subdev_csi, core, ioctl, SENSOR_IOCTL_CSI_DMA_ATTACH,
				       NULL);
		if (ret) {
			merr("v4l2_csi_call(SENSOR_IOCTL_CSI_DMA_ATTACH) is fail(%d)", sensor, ret);
			goto p_err;
		}
	}
	...
	mgrdbgs(1, " %s(%d) (sensor:%d, line:%d, scount:%d)\n", group, group, frame, __func__,
		frame->index, sensor->fcount, sensor->line_fcount, atomic_read(&group->scount));
...
static struct platform_driver is_sensor_driver = { .probe = is_sensor_probe,
						   .driver = {
...
static struct platform_driver is_sensor_driver = { .id_table = is_sensor_driver_ids,
						   .driver = {
							   .name = IS_SENSOR_DEV_NAME,
							   .owner = THIS_MODULE,
							   .pm = &is_sensor_pm_ops,
...
static int __init is_sensor_init(void)
{
	...
	ret = platform_driver_probe(&is_sensor_driver, is_sensor_probe);
	if (ret)
