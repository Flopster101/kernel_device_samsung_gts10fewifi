...
void is_group_subdev_cancel(struct is_group *group, struct is_frame *ldr_frame,
			    ...
			    bool flush)
{
	...
	for_each_group_child (pos, group) {
		...
		if (pos->device_type != device_type && device_type != IS_DEVICE_MAX)
			break;
				...
				if (sub_frame) {
					if (sub_frame->fcount > ldr_frame->fcount) {
						msrinfo("[WRN] SKIP CANCEL(SBUF:%d > LDRF:%d)(%d, %d)\n",
							pos, subdev, ldr_frame, sub_frame->fcount,
							ldr_frame->fcount, sub_frame->index,
							sub_frame->state);
						break;
					}
					if (sub_frame->stream) {
					...
					CALL_VOPS(sub_vctx, done, sub_frame->index,
						  VB2_BUF_STATE_ERROR);
				} else {
...
static void is_group_cancel(struct is_group *group, struct is_frame *ldr_frame)
{
	unsigned long flags;
	...
	if (!ldr_framemgr->num_frames) {
		...
		mgrwarn("Can't cancel this frame because state(%s) is invalid, skip buffer done",
			group, group, ldr_frame, frame_state_name[ldr_frame->state]);
	...
	ldr_frame->result = SHOT_ERR_CANCEL;
	if (ldr_framemgr->queued_count[FS_PROCESS]) {
		trans_frame(ldr_framemgr, ldr_frame, FS_PROCESS);
	} else {
	...
	mgrinfo("[ERR] CANCEL(i%d)(R%d, P%d, C%d)\n", group, group, ldr_frame, ldr_frame->index,
		ldr_framemgr->queued_count[FS_REQUEST], ldr_framemgr->queued_count[FS_PROCESS],
		ldr_framemgr->queued_count[FS_COMPLETE]);

        framemgr_x_barrier_irqr(ldr_framemgr, FMGR_IDX_20, flags);
}

static void is_group_s_leader(struct is_group *group, struct is_subdev *leader, bool force)
{
	struct is_subdev *subdev;
	...
	list_for_each_entry (subdev, &group->subdev_list, list) {
		...
		if (leader->vctx && subdev->vctx &&
			...
			mgwarn("Invalid subdev instance (%s(%u) < %s(%u))", group, group,
			       leader->name, leader->vctx->refcount, subdev->name,
			       subdev->vctx->refcount);
		}
...
#ifdef DEBUG_AA
static void is_group_debug_aa_shot(struct is_group *group, struct is_frame *ldr_frame)
{
	if (group->prev)
		return;
...
static void is_group_debug_aa_done(struct is_group *group, struct is_frame *ldr_frame)
{
	if (group->prev)
		return;
	...
	if (ldr_frame->shot->dm.flash.vendor_firingStable != group->flash.vendor_firingStable) {
		...
		info("flash stable : %d(%d)\n", group->flash.vendor_firingStable,
		     ldr_frame->fcount);
	}
...
static void is_group_set_torch(struct is_group *group, struct is_frame *ldr_frame)
{
	if (group->prev)
		return;
...
static bool is_group_kthread_queue_work(struct is_group_task *gtask, struct is_group *group,
...
static void is_group_start_trigger(struct is_groupmgr *groupmgr, struct is_group *group,
				   struct is_frame *frame)
{
...
static int is_group_task_probe(struct is_group_task *gtask, u32 id)
{
	gtask->id = id;
	atomic_set(&gtask->refcount, 0);
	clear_bit(IS_GTASK_OPEN, &gtask->state);
...
static int is_group_task_open(struct is_group_task *gtask, int slot)
{
	int ret = 0;

	if (test_bit(IS_GTASK_OPEN, &gtask->state))
		goto p_work;

	CALL_SYNC_REPRO_OPS(gtask->sync_repro, init, gtask);

	/* default gtask's smp_resource value is 1 for guerrentee m2m IP operation */
	sema_init(&gtask->smp_resource, M2M_ASYNC_SHOTS);

	set_bit(IS_GTASK_OPEN, &gtask->state);
p_work:
...
static int is_group_task_start(struct is_group_task *gtask, int slot)
{
	...
	param.sched_priority =
		(slot == GROUP_SLOT_SENSOR) ? TASK_GRP_OTF_INPUT_PRIO : TASK_GRP_DMA_INPUT_PRIO;
...
static int is_group_task_close(struct is_group_task *gtask)
{
	int ret = 0;
	u32 refcount;

	if (!test_bit(IS_GTASK_OPEN, &gtask->state)) {
		err("gtask(%d) is not opened", gtask->id);
		ret = -EINVAL;
		goto p_err;
	}
...
struct is_group *get_ischain_leader_group(u32 instance)
{
	...
	if ((leader_group->device_type == IS_DEVICE_SENSOR) && (leader_group->next) &&
...
static void is_group_set_hw_map(struct is_group *leader_group, struct is_hardware *hw, bool init)
{
	...
	for_each_group_child_double_path (pos, tmp, leader_group) {
		...
		if (head == pos) {
			...
			otf_domain = test_bit(IS_GROUP_OTF_INPUT, &head->state);
			if (otf_domain)
		...
		for (hw_index = 0; hw_index < hw_maxnum; hw_index++) {
			...
			if (otf_domain)
				...
				clear_bit(HW_OTF, &hw_ip->state);
		}

		if (!pos->child) {
			mginfo("%s: hw_map[0x%lX]\n", pos, leader_group, __func__,
			       hw->hw_map[instance]);

			for_each_group_child (pos2, head)
				memcpy(pos2->hw_slot_id, head->hw_slot_id,
				       sizeof(head->hw_slot_id));
		}
	}
}
...
int is_groupmgr_start(struct is_device_ischain *device)
{
	...
	while (group) {
		...
		mginfo(" SRC%02d:%04dx%04d\n", group, group, leader->vid, leader->input.width,
		       leader->input.height);
		list_for_each_entry (subdev, &group->subdev_list, list) {
		...
		if (prev && !test_bit(IS_GROUP_OTF_INPUT, &group->state)) {
			...
			if (!IS_ENABLED(LOGICAL_VIDEO_NODE) &&
				...
				mwarn("%s(%d x %d) != %s(%d x %d)", device, prev->junction->name,
				      prev->junction->output.width, prev->junction->output.height,
				      group->leader.name, group->leader.input.width,
...
static int is_group_open(struct is_group *group, u32 id, struct is_video_ctx *vctx)
{
	...
	/* 2. start kthread */
	ret = is_group_task_open(gtask, group->slot);
	if (ret) {
		mgerr("is_group_task_open is fail(%d)", group, group, ret);
		goto err_group_task_open;
	}
...
err_subdev_open:
	ret_err = is_group_task_close(gtask);
	if (ret_err)
		mgerr("is_group_task_close is fail(%d)", group, group, ret_err);
err_group_task_open:
err_state:
	return ret;
}
...
static int is_group_close(struct is_group *group)
{
	...
	ret = is_group_task_close(gtask);
	if (ret)
		mgerr("is_group_task_close is fail(%d)", group, group, ret);
...
static int is_group_init(struct is_group *group, u32 input_type, u32 stream_leader, u32 stream_type)
{
	...
	if (stream_leader) {
		...
		if (CALL_HW_CHAIN_INFO_OPS(&is_get_is_core()->hardware, check_mlsc_group,
					   group->id))
			set_bit(IS_GROUP_RNR_2ND, &group->state);
	}
...
static int is_group_start(struct is_group *group)
{
	...
	if (test_bit(IS_GROUP_REPROCESSING, &group->state)) {
		group->asyn_shots = M2M_ASYNC_SHOTS;
		group->skip_shots = 0;
		group->init_shots = 0;
		group->sync_shots = 0;
		...
		if (test_bit(IS_GROUP_OTF_INPUT, &group->state)) {
			...
			if (group->sensor) {
				...
				mgerr("no sensor for this group", group, group);
				sensor_fcount = 1;
			}
...
p_err:
	mginfo("bufs: %02d, init : %d, asyn: %d, skip: %d, sync : %d\n", group, group,
	       framemgr ? framemgr->num_frames : 0, group->init_shots, group->asyn_shots,
	       group->skip_shots, group->sync_shots);
	return ret;
}
...
static int is_group_stop(struct is_group *group)
{
	...
	if (!test_bit(IS_GROUP_START, &group->state) && !test_bit(IS_GROUP_START, &head->state)) {
...
static void is_group_override_meta(struct is_group *group, struct is_frame *frame,
				   ...
				   struct is_device_sensor *sensor)
{
	...
	if (!test_bit(IS_GROUP_REPROCESSING, &group->state)) {
		/* PAF */
		if (sensor->cfg[sensor->nfi_toggle]->pd_mode == PD_NONE)
...
static void is_group_override_sensor_req(struct is_framemgr *framemgr, struct is_frame *frame)
{
	int req_cnt = 0;
	...
	list_for_each_entry_reverse (prev, &framemgr->queued_list[FS_REQUEST], list) {
		...
		if (frame->shot->ctl.aa.aeMode == AA_AEMODE_OFF ||
			...
			prev->shot->ctl.sensor.frameDuration =
				frame->shot->ctl.sensor.frameDuration;
		...
		if (frame->shot->ctl.aa.vendor_aeflashMode == AA_FLASHMODE_CAPTURE)
			prev->shot->ctl.aa.vendor_aeflashMode =
				frame->shot->ctl.aa.vendor_aeflashMode;

		prev->shot->ctl.aa.aeExpCompensation = frame->shot->ctl.aa.aeExpCompensation;
		prev->shot->ctl.aa.aeLock = frame->shot->ctl.aa.aeLock;
		prev->shot->ctl.lens.opticalStabilizationMode =
			frame->shot->ctl.lens.opticalStabilizationMode;

		if (frame->shot->ctl.aa.sceneMode == AA_SCENE_MODE_PRO_MODE &&
		    frame->shot->ctl.aa.captureIntent ==
			...
			prev->shot->ctl.aa.vendor_captureExposureTime =
				frame->shot->ctl.aa.vendor_captureExposureTime;
			prev->shot->ctl.aa.vendor_captureCount =
				frame->shot->ctl.aa.vendor_captureCount;
		}
	}
}

static void is_group_update_stripe(struct is_group *group, struct is_frame *frame,
				   struct is_resourcemgr *resourcemgr)
{
...
static void is_group_update_repeat(struct is_group *group, struct is_frame *frame)
{
	...
	repeat_clahe =
		frame->shot_ext->node_group.leader.iterationType == NODE_ITERATION_TYPE_SVHIST;
...
static int is_group_buffer_queue(struct is_group *group, struct is_queue *queue, u32 index)
{
	...
	if (frame->state == FS_FREE) {
		...
		if (!test_bit(IS_GROUP_OTF_INPUT, &group->state) &&
			...
			mgwarn(" request bufs : %d", group, group,
...
static bool is_group_throttling_tick_check(struct is_device_ischain *device, struct is_group *group,
					   struct is_frame *frame)
{
	...
	if (!resourcemgr->limited_fps) {
		...
		mgrinfo("[DVFS] throttling restore scenario(%d)-[%s]\n", group, group, frame,
			static_ctrl->cur_scenario_id, static_ctrl->scenario_nm);
		return true;
	}
...
static void is_group_update_dvfs(struct is_device_ischain *device, struct is_group *group,
				 struct is_frame *frame)
{
	struct is_resourcemgr *resourcemgr = device->resourcemgr;
	struct is_sysfs_debug *sysfs_debug;
	...
	if ((!is_pm_qos_request_active(&dvfs_ctrl->user_qos)) && (sysfs_debug->en_dvfs)) {
		...
		if (unlikely(!atomic_read(&group->scount)))
			is_set_dvfs_fast_launch(dvfs_ctrl);
	}
	mutex_unlock(&dvfs_ctrl->lock);
}
#else
#define is_group_update_dvfs(device, group, frame)                                                 \
	do {                                                                                       \
	} while (0)
#endif

static inline void is_group_shot_recovery(struct is_groupmgr *groupmgr, struct is_group *group,
					  struct is_group_task *gtask, bool try_sdown,
					  bool try_rdown, bool try_gdown[])
{
	struct is_group *pos;
...
static int is_group_shot(struct is_group *group, struct is_frame *frame)
{
	...
	for_each_group_child (pos, group->child) {
		...
		if (!test_bit(IS_GTASK_OPEN, &gtask_child->state))
			continue;
	...
	if (group->sync_shots) {
		...
		if (group->asyn_shots == 0) {
			...
			if ((smp_shot_get(group) < MIN_OF_SYNC_SHOTS))
				try_sync_shot = true;
			else if (atomic_read(&group->backup_fcount) >=
...
static int is_group_done(struct is_group *group, struct is_frame *frame, u32 done_state)
{
	...
	if (is_vendor_check_remosaic_mode_change(frame) && (frame->state == FS_REPEAT_PROCESS))
		is_group_start_trigger(&grpmgr, group, frame);
#endif
...
static int is_group_change_chain(struct is_group *group, u32 next_id)
{
	...
	struct is_group *pos, *start;
	u32 base_id;
	struct is_video_ctx *vctx;
	struct is_framemgr *framemgr;

	if (group->slot != GROUP_SLOT_SENSOR && test_bit(IS_GROUP_OTF_INPUT, &group->state)) {
		mgerr("The group is not a sensor group or DMA input", group, group);
		return -EINVAL;
	}

	if (!test_bit(IS_GROUP_OTF_OUTPUT, &group->state) &&
	    !test_bit(IS_GROUP_RNR_2ND, &group->state)) {
	...
	for_each_group_child (pos, start) {
		if (pos->slot == GROUP_SLOT_SENSOR) {
			mgerr("A child group is invalid.", group, group);
			goto p_err;
		}
		base_id = slot_to_gid[pos->slot];
		...
		ret = is_group_task_close(curr_gtask);
		if (ret) {
			mgerr("is_group_task_close is fail(%d)", pos, pos, ret);
			goto p_err;
		}

		next_gtask = &grpmgr.gtask[base_id + next_id];
		ret = is_group_task_open(next_gtask, pos->slot);
		if (ret) {
			mgerr("is_group_task_open is fail(%d)", pos, pos, ret);
			goto p_err;
		}
...
static struct pablo_kunit_groupmgr_func groupmgr_func = {
	...
	.group_task_open = is_group_task_open,
	.group_task_close = is_group_task_close,
