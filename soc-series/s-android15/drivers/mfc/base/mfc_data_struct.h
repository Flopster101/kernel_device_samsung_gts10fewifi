...
enum mfc_nal_q_mode {
	/* NAL START mode */
	MFC_NAL_Q_DISABLE		= 0,
	/* [SINGLE] NAL QUEUE base mode */
	MFC_NAL_Q_DEFAULT		= 1,
	/* [SINGLE] DPB mode (Not supported yet) */
	MFC_NAL_Q_DPB			= 2,
	/* [SINGLE] Batch mode without HWAPG */
	MFC_NAL_Q_BATCH			= 3,
	/* [SINGLE] Batch mode with HWAPG */
	MFC_NAL_Q_BATCH_HWAPG	= 4,
	/* [MULTI] Linked List */
	MFC_NAL_Q_LL			= 5,
	/* [MULTI] LL with DPB interface (Not supported yet) */
	MFC_NAL_Q_LL_DPB		= 6,
};
...
enum mfc_feature_option {
	...
	MFC_OPTION_STREAM_COPY_DISABLE	= (1 << 15),
};
...
struct mfc_debugfs {
	...
	unsigned int nal_q_mode;
	unsigned int batch_disable;
	unsigned int batch_buf_num;
	unsigned int batch_qos_enable;
...
struct mfc_platdata {
	...
	unsigned int nal_q_dpb_size;
	/* NAL-Q mode & Batch */
	unsigned int nal_q_ll;
	unsigned int nal_q_dpb;
	unsigned int nal_q_batch_dec;
	unsigned int nal_q_batch_enc;
	unsigned int batch_drm;
	unsigned int nal_q_mode1;
...
/* The number of DPB slot for each instance is 16 and it's size is 16 bytes */
#define NAL_Q_DPB_INPUT_NUM		16
#define NAL_Q_DPB_INPUT_SIZE		16
...
typedef struct __DPBInputStr {
	unsigned int DPBIdx;	/* index + 1 */
	unsigned int DPBAddr[3];
} DPBInputStr; /* 4*4 = 16 bytes */
...
typedef enum _nal_queue_state {
	...
	NAL_Q_STATE_BATCH_STARTED, /* when mfc_nal_q_start() is called with batch mode */
...
typedef struct _nal_queue_handle {
	...
	spinlock_t lock;
	int nal_q_exception;
	int nal_q_dpb_idx[MFC_NUM_CONTEXTS];
...
struct mfc_llcaid_sfrs {
	unsigned int	offset;
	unsigned int	data;
};

struct mfc_llcaid_info {
	void __iomem    *base;
	struct mfc_llcaid_sfrs *sfrs;
	unsigned int	num_sfrs;
};
...
struct mfc_core {
	...
	enum mfc_nal_q_mode nal_q_mode;
	...
	unsigned int llcaid_cnt;
	struct mfc_llcaid_info *llcaid_info;
};
...
typedef struct _exynos_hdr_data_ST2094_40 {
...
} exynos_hdr_data_ST2094_40;

typedef struct _exynos_hdr_dynamic_Info {
	unsigned int valid;
	unsigned int reserved[42];

	exynos_hdr_data_ST2094_40 data;
} exynos_hdr_dynamic_Info;

typedef enum _exynos_video_sub_view_info_type {
	VIDEO_SUBVIEW_INFO_TYPE_NONE		= 0,
	VIDEO_SUBVIEW_INFO_TYPE_HDR_DYNAMIC	= 0x1 << 0,
	VIDEO_SUBVIEW_INFO_TYPE_HDR_DYNAMIC_BLOB	= 0x1 << 1,
} exynos_video_sub_view_info_type;

typedef struct _exynos_hdr_dynamic_blob {
	int  nSize;
	char pData[1020];
} exynos_hdr_dynamic_blob;

typedef struct _exynos_video_sub_view_meta {
	exynos_video_sub_view_info_type eType;
	exynos_hdr_dynamic_Info sHdrDynamicInfo;
	exynos_hdr_dynamic_blob sHdrDynamicBlob;
} exynos_video_sub_view_meta;
...
struct mfc_dec {
	...
	void *sub_view_vaddr[MFC_MAX_BUFFERS];
	void *hdr10_plus_full;
	void *hdr10_plus_full_sub;
