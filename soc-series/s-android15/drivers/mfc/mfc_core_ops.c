...
static int __mfc_core_deinit(struct mfc_core *core, struct mfc_ctx *ctx)
{
	...
	if ((ctx->gdc_votf && !(IS_TWO_MODE1(ctx) &&
				...
				struct mfc_core_ctx *core_ctx)
{
	...
	if ((IS_HEVC_DEC(ctx) || IS_AV1_DEC(ctx))) {
		if (MFC_FEATURE_SUPPORT(dev, dev->pdata->hdr10_plus_full)
			...
			if (IS_HEVC_DEC(ctx)) {
				dec->hdr10_plus_full_sub = vmalloc(dec->sh_handle_hdr.data_size);
				if (!dec->hdr10_plus_full_sub)
					mfc_err("failed to allocate hdr10 plus full information data of sub-view");
			}
...
void mfc_core_instance_csd_parsing(struct mfc_core *core, struct mfc_ctx *ctx)
{
	...
	while (1) {
		csd = mfc_check_buf_mb_flag(ctx, core_ctx, MFC_FLAG_CSD);
		if (csd == 1) {
			mfc_clean_core_ctx_int_flags(core_ctx);
