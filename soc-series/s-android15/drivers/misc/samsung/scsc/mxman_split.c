...
static ulong mm_halt_rsp_timeout_ms = 200;
module_param(mm_halt_rsp_timeout_ms, ulong, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(mm_halt_rsp_timeout_ms, "Timeout wait_for_mm_msg_halt_rsp (ms) - default 200");
...
extern bool scsc_pcie_link_state_on(void);
static bool trigger_recovery_for_pcie_link_error = false;
#endif

#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
/* Below module parameters are provided as the options to control independent subsystem recovery. */
static bool indep_subsys_recovery_enabled = true;
module_param(indep_subsys_recovery_enabled, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(indep_subsys_recovery_enabled, "Enable Independent Subsystem Recovery");

static bool moredump_collection_on_error_enable = true;
module_param(moredump_collection_on_error_enable, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(moredump_collection_on_error_enable, "Enable Moredump collection on SUBSYSTEM_ERROR_IND");

static bool fail_after_recovery_enable = false;
module_param(fail_after_recovery_enable, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(fail_after_recovery_enable, "Enable host induced failure of surviving subsystem on SUBSYSTEM_RECOVER_IND");

static bool sable_collection_on_recovery_enable = true;
module_param(sable_collection_on_recovery_enable, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(sable_collection_on_recovery_enable, "Enable Sable collection on SUBSYSTEM_RECOVER_IND");

static bool moredump_collection_on_recovery_enable = true;
module_param(moredump_collection_on_recovery_enable, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(moredump_collection_on_recovery_enable, "Enable Moredump collection on SUBSYSTEM_RECOVER_IND");

static const char *rcvry_state_str[RCVRY_STATE_MAX] = {	"NONE", "CHIP_RESET",
														"DUMP_LOGGING", "SUBSYSTEM_RESET", "WAITING_START",
														"ERROR_IN_PROGRESS"};
...
static const char *rcvry_event_str[RCVRY_EVT_MAX] = {
														"ERROR",
														"RECOVER WLAN", "RECOVER WPAN", "RECOVER HOST", "RECOVER REJECT",
														"DUMP END",
														"RESET DONE", "REOPEN ERROR", "RESET TIMEOUT",
...
/* This variable indicates the current status of whether the independent subsystem recovery is enabled. */
static bool enable_status_to_indep_subsys_recovery = false;

void mxman_set_failure_params(struct mxman *mxman, bool full_recovery);
static void delay_recovery_setting_wq_start(struct mxman *mxman);

/*     This option is used to delay when the host sends the command for recovery mode to PMU
       in order to enable independent subsystem recovery */
static uint pmu_enable_split_recovery_after_ms = 0;
module_param(pmu_enable_split_recovery_after_ms, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(pmu_enable_split_recovery_after_ms, "delay(ms) on re-enabling independent subsystem recovery");

static uint timeout_for_indep_recovery_err_work = 500;
#endif
...
/* 0 : false, 1 : SCAN2MEM DUMP, 2: SCAN2MEM DUMP without BLK_WLBT */
static uint enable_scan2mem_dump = 0;
module_param(enable_scan2mem_dump, uint, S_IRUGO | S_IWUSR);
...
static void call_wlbtd_sable_cb(struct scsc_log_collector_mx_cb *mx_cb, u8 trigger_code, u16 reason_code, bool sable, bool moredump)
{
	call_wlbtd_sable(trigger_code, reason_code, sable, moredump);
}
...
#if defined(CONFIG_SCSC_PCIE_CHIP)
static void mxman_trigger_pcie_recovery_notifier(void)
{
	if (!active_mxman) {
		SCSC_TAG_ERR(MXMAN, "No active MXMAN\n");
		return;
	}
	trigger_recovery_for_pcie_link_error = true;
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	mxman_recover_subsystem(active_mxman, SCSC_PANIC_CODE_HOST << 15, __func__, SCSC_SUBSYSTEM_PMU);
#else
	mxman_fail(active_mxman, SCSC_PANIC_CODE_HOST << 15, __func__, SCSC_SUBSYSTEM_PMU);
#endif
}
#endif
...
static int mxman_reset_work_complete(struct mxman *mxman)
{
	struct rcvry_fsm_thread		*th = &mxman->rcvry_thread;
	struct srvman *srvman = scsc_mx_get_srvman(mxman->mx);

	if (mxman_check_recovery_state_error_in_progress()) {
		SCSC_TAG_WARNING(MXMAN, "Skip reset work due to another error\n");
		srvman_set_error(srvman, ALLOWED_START_STOP);
		return 0;
	}

	reinit_completion(&th->reset_work_completion);

	if(!wait_for_completion_timeout(&th->reset_work_completion, 3*HZ)) {
		SCSC_TAG_WARNING(MXMAN, "reset_work wait for complete timeout\n");
		return -EFAULT;
	}
	SCSC_TAG_INFO(MXMAN, "!\n");
	return 0;
...
static int mxman_rcvry_fsm_complete(struct mxman *mxman, bool full)
{
	struct rcvry_fsm_thread		*th = &mxman->rcvry_thread;
	uint timeout = (full) ? 5*HZ : 3*HZ;

	if (!full && (th->indep_substate != WLAN_INDEP_NORMAL && th->indep_substate != WPAN_INDEP_NORMAL)) {
		SCSC_TAG_WARNING(MXMAN, "Skip waiting reset completion due to chip reset recovery\n");
		return 0;
	}

	reinit_completion(&th->reset_completion);

	if(!wait_for_completion_timeout(&th->reset_completion, timeout)) {
...
int mxman_set_recovery_mode(struct mxman *mxman, bool enable_indep)
{
	...
	if (enable_indep) {
		if (enable_scan2mem_dump) {
			SCSC_TAG_WARNING(MXMAN, "Recovery for scan2mem dump is enabled!\n");
			goto exit;
		}
		if (mxman_recovery_disabled()) {
			SCSC_TAG_WARNING(MXMAN, "Disable recovery handling!(handling:%d)\n", disable_recovery_handling);
			goto exit;
		}
		if (!indep_subsys_recovery_enabled) {
			SCSC_TAG_WARNING(MXMAN, "Disable Independent subsystem recovery!\n");
			goto exit;
		}
		if (enable_status_to_indep_subsys_recovery) {
			SCSC_TAG_WARNING(MXMAN, "Already Enable Independent subsystem recovery!\n");
			goto exit;
		}
	} else {
		if (!enable_status_to_indep_subsys_recovery) {
...
#if defined(CONFIG_SCSC_PCIE_CHIP)
	if (scsc_mx_service_claim(MXMAN_SET_RECOVERY)) {
		SCSC_TAG_INFO(MXMAN, "Error claiming link\n");
		goto exit;
	}
#endif
	ret = mxman_res_set_recovery_mode(mxman, enable_indep);
	if (!ret) {
		enable_status_to_indep_subsys_recovery = enable_indep;
		SCSC_TAG_INFO(MXMAN, "Complete to %s Independent subsystem recovery\n", enable_indep ? "Enable": "Disable");
	} else
		SCSC_TAG_ERR(MXMAN, "Cannot %s Independent subsystem recovery!\n", enable_indep ? "Enable": "Disable");
#if defined(CONFIG_SCSC_PCIE_CHIP)
	scsc_mx_service_release(MXMAN_SET_RECOVERY);
#endif
exit:
	return ret;
}
...
{
#ifndef CONFIG_WLBT_SPLIT_RECOVERY
	return mxman->panic_in_progress;
#else
	return false;
#endif
}
...
static int trigger_mxmgmt_transport_intr(struct mxman *mxman)
{
	...
	mif_stream_get_dump_for_write_gather(&irq_bit, &target);
	SCSC_TAG_INFO(MXMGT_TRANS, "Target subsystem: %s, IRQ Bit number: %d\n", (target == SCSC_MIF_ABS_TARGET_WPAN) ? "WPAN" : "WLAN", irq_bit);

	mif->irq_bit_set(mif, irq_bit, target);

	r = wait_for_completion_timeout(&mxman->mm_msg_halt_rsp_completion, msecs_to_jiffies(mm_halt_rsp_timeout_ms * 4));
	if (r) {
		SCSC_TAG_INFO(MXMAN, "Received MM_HALT_RSP from firmware in 2nd try\n");
		return 0;
	} else {
		SCSC_TAG_INFO(MXMAN, "MM_HALT_RSP (800ms) timeout\n");
...
static int wait_for_mm_msg_halt_rsp(struct mxman *mxman)
{
	...
	if (r) {
		...
		SCSC_TAG_INFO(MXMAN, "MM_HALT_RSP (200ms) timeout\n");
		return trigger_mxmgmt_transport_intr(mxman);
	}
}
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	enable_status_to_indep_subsys_recovery = false;
#endif
...
static void mxman_pmu_message_handler(void *data, u32 cmd)
{
	...
	case MIFPMU_ERROR_WLAN_WPAN:
		...
		break;
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	case MIFPMU_RECOVER_WLAN:
		mxman_recover_subsystem(mxman, SCSC_PANIC_CODE_FW << 15, __func__, SCSC_SUBSYSTEM_WLAN);
		break;
	case MIFPMU_RECOVER_WPAN:
		mxman_recover_subsystem(mxman, SCSC_PANIC_CODE_FW << 15, __func__, SCSC_SUBSYSTEM_WPAN);
		break;
#endif
	default:
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0) && IS_ENABLED(CONFIG_SOC_S5E5515))
...
static int mxman_start(struct mxman *mxman, enum scsc_subsystem sub, void *data, size_t data_sz)
{
	...
	*/
	...
	if (mxman_check_recovery_state_none()) {
		if (pmu_enable_split_recovery_after_ms)
			delay_recovery_setting_wq_start(mxman);
		else {
			ret = mxman_set_recovery_mode(mxman, true);
			if (ret)
				goto error;
		}
	}
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	if (!mxman_check_recovery_state_none()) {
		/* 	It's not displayed during recovery in order to provide the organized log.
			While performing recovery, the driver could gives the logs on panic to you once or twice.*/
		return;
	}
#endif
...
static void process_panic_record(struct mxman *mxman, bool dump)
{
	...
	u32 *ucpu_panic_record = NULL;
	bool ucpu_panic_record_ok = false;
	u32 ucpu_panic_record_length = 0; /* in u32s */
	u32 *ucpu_panic_stack_record = NULL;
	bool ucpu_panic_stack_record_ok = false;
	u32 ucpu_panic_stack_record_offset = 0; /* in bytes */
	u32 ucpu_panic_stack_record_length = 0; /* in u32s */
	bool ucpu_sympathetic_panic_flag = false;
	u32 offset;
#endif
	struct fwhdr_if *fw_if = mxman->fw_wlan;
	   ...
	   (mxman->scsc_panic_sub == SCSC_SUBSYSTEM_WLAN || mxman->scsc_panic_sub == SCSC_SUBSYSTEM_WLAN_WPAN)) {
...
#if defined(CONFIG_SCSC_BB_REDWOOD)
		offset = fw_if->get_panic_record_offset(fw_if, SCSC_MIF_ABS_TARGET_WLAN_5);
		if (offset) {
			ucpu_panic_record = (u32 *)(mxman->fw + offset);
			ucpu_panic_record_ok = fw_parse_ucpu_panic_record(ucpu_panic_record, &ucpu_panic_record_length,
									&ucpu_panic_stack_record_offset, dump, SCSC_MIF_ABS_TARGET_WLAN_5);
		} else {
			SCSC_TAG_INFO(MXMAN, "WLAN UCPU0 panic record doesn't exist in the firmware header\n");
		}

		offset = 0;
		offset = fw_if->get_panic_record_offset(fw_if, SCSC_MIF_ABS_TARGET_WLAN_6);
		if (offset) {
			ucpu_panic_record = (u32 *)(mxman->fw + offset);
			ucpu_panic_record_ok = fw_parse_ucpu_panic_record(ucpu_panic_record, &ucpu_panic_record_length,
									&ucpu_panic_stack_record_offset, dump, SCSC_MIF_ABS_TARGET_WLAN_6);
		} else {
			SCSC_TAG_INFO(MXMAN, "WLAN UCPU1 panic record doesn't exist in the firmware header\n");
		}

		offset = 0;
		offset = fw_if->get_panic_record_offset(fw_if, SCSC_MIF_ABS_TARGET_WLAN_7);
		if (offset) {
			ucpu_panic_record = (u32 *)(mxman->fw + offset);
			ucpu_panic_record_ok = fw_parse_ucpu_panic_record(ucpu_panic_record, &ucpu_panic_record_length,
									&ucpu_panic_stack_record_offset, dump, SCSC_MIF_ABS_TARGET_WLAN_7);
		} else {
			SCSC_TAG_INFO(MXMAN, "WLAN UCPU2 panic record doesn't exist in the firmware header\n");
		}

		offset = 0;
		offset = fw_if->get_panic_record_offset(fw_if, SCSC_MIF_ABS_TARGET_WLAN_8);
		if (offset) {
			ucpu_panic_record = (u32 *)(mxman->fw + offset);
			ucpu_panic_record_ok = fw_parse_ucpu_panic_record(ucpu_panic_record, &ucpu_panic_record_length,
									&ucpu_panic_stack_record_offset, dump, SCSC_MIF_ABS_TARGET_WLAN_8);
		} else {
			SCSC_TAG_INFO(MXMAN, "WLAN UCPU3 panic record doesn't exist in the firmware header\n");
		}
#endif
		...
		case SCSC_R4_V2_MINOR_53:
#if defined(CONFIG_SCSC_BB_REDWOOD)
			if (ucpu_panic_record_ok) {
				/* Save the last UCPU panic record for future display */
				if (sizeof(mxman->last_panic_rec_r) < ucpu_panic_record_length * sizeof(u32)) {
					if (is_bug_on_enabled()) {
						BUG_ON(1);
					} else {
						SCSC_TAG_INFO(MXMAN,
							      "is_bug_on_enabled is disabled. last_panic_record: %d, upuc_panic_record_length: %d\n",
							      mxman->last_panic_rec_r,
							      ucpu_panic_record_length);
						break;
					}
				}
				memcpy((u8 *)mxman->last_panic_rec_r, (u8 *)ucpu_panic_record,
				       ucpu_panic_record_length * sizeof(u32));
				mxman->last_panic_rec_sz = ucpu_panic_record_length;

				ucpu_sympathetic_panic_flag = fw_parse_get_ucpu_sympathetic_panic_flag(ucpu_panic_record);
				if (dump)
					SCSC_TAG_INFO(MXMAN, "ucpu_panic_record_ok=%d ucpu_sympathetic_panic_flag=%d\n",
						      ucpu_panic_record_ok, ucpu_sympathetic_panic_flag);
				/* Check panic stack if present */
				if (ucpu_panic_record_length >= SCSC_R4_V2_MINOR_54) {
					ucpu_panic_stack_record = (u32 *)(mxman->fw + ucpu_panic_stack_record_offset);
					ucpu_panic_stack_record_ok = fw_parse_ucpu_panic_stack_record(
						ucpu_panic_stack_record, &ucpu_panic_stack_record_length, dump, SCSC_MIF_ABS_TARGET_WLAN_5);
				} else {
					ucpu_panic_stack_record_ok = false;
					ucpu_panic_stack_record_length = 0;
				}
				if (ucpu_sympathetic_panic_flag == false) {
					/* process UCPU record */
					full_panic_code = ucpu_panic_record[3];
					mxman->scsc_panic_code |= SCSC_PANIC_CODE_MASK & full_panic_code;
					if (dump)
						print_panic_code(mxman);
					break;
			...
			if (r4_panic_record_ok) {
				...
				if (sizeof(mxman->last_panic_rec_r) < r4_panic_record_length * sizeof(u32)) {
					if (is_bug_on_enabled()) {
						BUG_ON(1);
					} else {
						SCSC_TAG_INFO(MXMAN,
							      "is_bug_on_enabled is disabled. last_panic_record: %d, panic_record_length: %d\n",
							      sizeof(mxman->last_panic_rec_r),
...
static void mxman_create_scandump(struct mxman *mxman, uint32_t enable_scan2mem_dump)
{
	...
	mxman_res_pmu_scan2mem(mxman, enable_scan2mem_dump, true);
	...
	ret = mxman_res_pmu_scan2mem(mxman, enable_scan2mem_dump, false);
	if (!ret) {
		...
		call_wlbtd_ramsd(s2m_size_octets, enable_scan2mem_dump);
 ...
 */
static void mxman_generate_dump(struct mxman *mxman, enum scsc_subsystem sub, bool enable_moredump)
{
	int used = 0;
	u16 reason = (sub == SCSC_SUBSYSTEM_WPAN) ? mxman->scsc_panic_code_wpan : mxman->scsc_panic_code;
	...
	if (disable_recovery_handling == DISABLE_RECOVERY_HANDLING_SCANDUMP &&
		scandump_trigger_fw_panic == reason) {
	...
	if (disable_auto_coredump) {
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	} else if (sable_collection_on_recovery_enable || enable_moredump) {
#else
	} else {
#endif
#ifdef CONFIG_SCSC_WLBTD
...
#if IS_ENABLED(CONFIG_SCSC_LOG_COLLECTION)
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
		scsc_log_collector_schedule_collection_for_recovery(SCSC_LOG_FW_PANIC,
															reason,
															sable_collection_on_recovery_enable,
															enable_moredump);
#else
		...
		* mode */
		if (reason & (SCSC_PANIC_CODE_HOST << 15))
			mxman_res_pmu_monitor(mxman, sub);
		scsc_log_collector_schedule_collection_for_recovery(SCSC_LOG_FW_PANIC,
															reason, true, enable_moredump);
#endif
#else
		...
		*/
		...
		SCSC_TAG_INFO(MXMAN, "Disable All types of dump files\n");
#endif
	}
	...
	if (is_bug_on_enabled()) {
		...
		SCSC_TAG_INFO(MXMAN, "is_bug_on_enabled is disabled.\n");
	}
}

#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
static void mxman_print_recovery_options(void)
{
	SCSC_TAG_INFO(MXMAN, "IR: %s, MC: %s, FR: %s, SR: %s, MR: %s\n", 
						(indep_subsys_recovery_enabled) ? "Y":"N",
						(moredump_collection_on_error_enable) ? "Y":"N",
						(fail_after_recovery_enable) ? "Y":"N",
						(sable_collection_on_recovery_enable) ? "Y":"N",
						(moredump_collection_on_recovery_enable) ? "Y":"N");
}
#endif

static void mxman_module_reset(
	struct mxman *mxman,
	enum scsc_module_client_reason reason)
{
	...
	scsc_mx_module_client_remove(reason);
	srvman_set_error(srvman, ALLOWED_START_STOP);
	scsc_mx_module_client_probe(reason);
...
static void mxman_failure_work_wlan(struct work_struct *work)
{
	...
	struct fwhdr_if *fw_if = mxman->fw_wlan;
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	bool enable_moredump = moredump_collection_on_recovery_enable;
#else
	bool enable_moredump = true;
#endif
	...
	mxman->last_syserr_recovery_time[SYSERR_SUBSYS_RESET_WLAN] = 0;
	...
	if (mxman_subsys_in_failed_state(mxman, SCSC_SUBSYSTEM_WLAN)) {
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
		mxman_print_recovery_options();
#endif
		if (mxman_recovery_disabled()) {
			SCSC_TAG_INFO(MXMAN, "Enable moredump(handling:%d)\n", disable_recovery_handling);
			enable_moredump = true;
		} else if (srvman_check_silent_recovery(srvman, &mxman->last_syserr)) {
			SCSC_TAG_INFO(MXMAN, "Disable moredump due to silent recovery\n");
			enable_moredump = false;
		}
		...
		scsc_wlbtd_get_and_print_build_type();
#endif
		mxlogger_mark_channel_as_crashed(scsc_mx_get_mxlogger(mx), SCSC_MIF_ABS_TARGET_WLAN);
		mxman_generate_dump(mxman, SCSC_SUBSYSTEM_WLAN, enable_moredump);
	}
	SCSC_TAG_INFO(MXMAN, "Auto-recovery: %s\n", mxman_recovery_disabled() ? "off" : "on");
	...
	mxman_send_rcvry_evt_to_fsm(mxman, RCVRY_EVT_DUMP_END);

	if (mxman_reset_work_complete(mxman)) {
		SCSC_TAG_WARNING(MXMAN, "reset_work wait for complete timeout\n");
	}
#else
	if (!mxman_recovery_disabled()) {
		...
		* from services not respecting the .remove .probe callbacks */
		srvman_set_error(srvman, NOT_ALLOWED_START);
		mxman_module_reset(mxman, SCSC_MODULE_CLIENT_REASON_RECOVERY_WLAN);
	}
	mxman->panic_in_progress = false;
#endif
	/**
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	if (!mxman_recovery_disabled()) {
		if (mxman_rcvry_fsm_complete(mxman, false)) {
			mxman_send_rcvry_evt_to_fsm(mxman, RCVRY_EVT_RESET_TIMEOUT);
		}
	}
#endif
}
...
static void mxman_failure_work_wpan(struct work_struct *work)
{
	...
	/* Mark any single service recovery as no longer in progress */
	mxman->syserr_recovery_in_progress = false;
	mxman->last_syserr_recovery_time[SYSERR_SUBSYS_RESET_WPAN] = 0;
	...
	if (mxman_subsys_in_failed_state(mxman, SCSC_SUBSYSTEM_WPAN)) {
		...
		scsc_wlbtd_get_and_print_build_type();
#endif
		mxlogger_mark_channel_as_crashed(scsc_mx_get_mxlogger(mx), SCSC_MIF_ABS_TARGET_WPAN);
		mxman_generate_dump(mxman, SCSC_SUBSYSTEM_WPAN, enable_moredump);
	}
	SCSC_TAG_INFO(MXMAN, "Auto-recovery: %s\n", mxman_recovery_disabled() ? "off" : "on");
	...
	if (!mxman_recovery_disabled()) {
		...
		* from services not respecting the .remove .probe callbacks */
		srvman_set_error(srvman, NOT_ALLOWED_START);
		mxman_module_reset(mxman, SCSC_MODULE_CLIENT_REASON_RECOVERY_WPAN);
	}
	mxman->panic_in_progress = false;
#endif
	/**
...
static void mxman_failure_work(struct work_struct *work)
{
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	struct rcvry_fsm_thread 	*th = &mxman->rcvry_thread;
	bool enable_moredump = moredump_collection_on_error_enable;
#else
	bool enable_moredump = true;
#endif
	u8 subsys = mxman->last_syserr.subsys;
	...
	/* Force the signalling of the subsys */
	mxman->last_syserr.subsys = subsys;
	...
	if ((mxman_subsys_in_failed_state(mxman, SCSC_SUBSYSTEM_WLAN)) ||
		...
		if (disable_recovery_handling) {
			SCSC_TAG_INFO(MXMAN, "Enable moredump(handling:%d)\n", disable_recovery_handling);
			enable_moredump = true;
		} else if (enable_scan2mem_dump) {
			SCSC_TAG_INFO(MXMAN, "Enable moredump due to scan2mem recovery\n");
			enable_moredump = true;
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
		} else if (th->indep_substate == NO_INDEP && srvman_check_silent_recovery(srvman, &mxman->last_syserr)) {
#else
		} else if (srvman_check_silent_recovery(srvman, &mxman->last_syserr)) {
#endif
			SCSC_TAG_INFO(MXMAN, "Disable moredump due to silent recovery\n");
		...
		if (mxman->last_syserr.level != MX_SYSERR_LEVEL_8) {
			...
			if (disable_auto_coredump) {
				SCSC_TAG_INFO(MXMAN,
					      "Driver automatic coredump disabled, not launching coredump helper\n");
#if defined(CONFIG_SCSC_PCIE_CHIP)
			} else if (trigger_recovery_for_pcie_link_error) {
				SCSC_TAG_INFO(MXMAN,"Skip log collection cause this recovery is to recover pcie link error.\n");
#endif
			} else {
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
				scsc_log_collector_schedule_collection_for_recovery(SCSC_LOG_FW_PANIC,
																mxman->scsc_panic_code, true, enable_moredump);
#else
				/* Use wlan panic code as default if exists */
				if (mxman->scsc_panic_code)
					reason = mxman->scsc_panic_code;
	...
	if (!mxman_recovery_disabled()) {
		...
		srvman_set_error(srvman, NOT_ALLOWED_START);
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
		if (th->indep_substate == WLAN_INDEP_ERR_START_FAIL || th->indep_substate == WPAN_INDEP_ERR_START_FAIL) {
			mxman_close(mxman, mxman_check_start_failure_during_indep_recovery(mxman));
			th->indep_substate = NO_INDEP;
		}
#endif
		mxman_module_reset(mxman, SCSC_MODULE_CLIENT_REASON_RECOVERY);
	}
#if !defined(CONFIG_WLBT_SPLIT_RECOVERY)
	mxman->panic_in_progress = false;
#endif
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	if (!mxman_recovery_disabled()) {
		if (mxman_rcvry_fsm_complete(mxman, true)) {
			mxman_send_rcvry_evt_to_fsm(mxman, RCVRY_EVT_RESET_TIMEOUT);
		}
	}
#endif
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	if (!completion_done(&th->delay_setting_completion))
		complete(&th->delay_setting_completion);

	mxman_send_rcvry_evt_to_fsm(mxman, RCVRY_EVT_ERROR);
#else
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
static void mxman_reset_subsystem_work(struct work_struct *work)
{
	struct mxman *mxman = container_of(work, struct mxman, reset_subsystem_work);
	struct rcvry_fsm_thread *th = &mxman->rcvry_thread;
	enum scsc_module_client_reason reason;
	...
	if (th->target_sub == SCSC_SUBSYSTEM_WLAN)
		reason = SCSC_MODULE_CLIENT_REASON_RECOVERY_WLAN;
	else
		reason = SCSC_MODULE_CLIENT_REASON_RECOVERY_WPAN;

	mxman_module_reset(mxman, reason);
exit:
	complete(&th->reset_work_completion);
}

static void reset_subsystem_wq_init(struct mxman *mxman)
{
	mxman->reset_subsystem_wq = create_singlethread_workqueue("reset_subsystem_wq");
	INIT_WORK(&mxman->reset_subsystem_work, mxman_reset_subsystem_work);
}

static void reset_subsystem_wq_stop(struct mxman *mxman)
{
	cancel_work_sync(&mxman->reset_subsystem_work);
	flush_workqueue(mxman->reset_subsystem_wq);
}

static void reset_subsystem_wq_deinit(struct mxman *mxman)
{
	reset_subsystem_wq_stop(mxman);
	destroy_workqueue(mxman->reset_subsystem_wq);
}

static void reset_subsystem_wq_start(struct mxman *mxman)
{
	queue_work(mxman->reset_subsystem_wq, &mxman->reset_subsystem_work);
}

static void mxman_delay_recovery_setting_work(struct work_struct *work)
{
	struct mxman *mxman = container_of(work, struct mxman, delay_recovery_setting_work);
	struct rcvry_fsm_thread *th = &mxman->rcvry_thread;
	int timeout_jiffies = msecs_to_jiffies(pmu_enable_split_recovery_after_ms);

	reinit_completion(&th->delay_setting_completion);
	SCSC_TAG_INFO(MXMAN, "Delay setting independent subsystem recovery for %d msecs\n",
						pmu_enable_split_recovery_after_ms);

	if(!wait_for_completion_timeout(&th->delay_setting_completion, timeout_jiffies)) {
		SCSC_TAG_WARNING(MXMAN, "delay_setting_completion wait for complete timeout\n");
		mxman_set_recovery_mode(mxman, true);
	}
}

static void delay_recovery_setting_wq_init(struct mxman *mxman)
{
	mxman->delay_recovery_setting_wq = create_singlethread_workqueue("delay_recovery_setting_wq");
	INIT_WORK(&mxman->delay_recovery_setting_work, mxman_delay_recovery_setting_work);
}

static void delay_recovery_setting_wq_stop(struct mxman *mxman)
{
	cancel_work_sync(&mxman->delay_recovery_setting_work);
	flush_workqueue(mxman->delay_recovery_setting_wq);
}

static void delay_recovery_setting_wq_deinit(struct mxman *mxman)
{
	delay_recovery_setting_wq_stop(mxman);
	destroy_workqueue(mxman->delay_recovery_setting_wq);
}

static void delay_recovery_setting_wq_start(struct mxman *mxman)
{
	queue_work(mxman->delay_recovery_setting_wq, &mxman->delay_recovery_setting_work);
}

static int mxman_perform_cold_reset(struct mxman *mxman);

static void mxman_indep_recovery_err_work(struct work_struct *work)
{
	struct mxman *mxman = container_of(work, struct mxman, indep_recovery_err_work);
	struct rcvry_fsm_thread *th = &mxman->rcvry_thread;
	int timeout_jiffies = msecs_to_jiffies(timeout_for_indep_recovery_err_work);

	reinit_completion(&th->indep_err_work_comletion);
	SCSC_TAG_INFO(MXMAN, "Wait for %d msecs until service close\n",
						timeout_for_indep_recovery_err_work);

	if(!wait_for_completion_timeout(&th->indep_err_work_comletion, timeout_jiffies)) {
		SCSC_TAG_WARNING(MXMAN, "indep_err_completion wait for complete timeout\n");
	}
	(void)mxman_perform_cold_reset(mxman);
}

void mxman_indep_recovery_err_completion(struct mxman *mxman)
{
	struct rcvry_fsm_thread *th = &mxman->rcvry_thread;
	complete(&th->indep_err_work_comletion);
}

static void indep_recovery_err_wq_init(struct mxman *mxman)
{
	mxman->indep_recovery_err_wq = create_singlethread_workqueue("indep_recovery_err_wq");
	INIT_WORK(&mxman->indep_recovery_err_work, mxman_indep_recovery_err_work);
}

static void indep_recovery_err_wq_stop(struct mxman *mxman)
{
	cancel_work_sync(&mxman->indep_recovery_err_work);
	flush_workqueue(mxman->indep_recovery_err_wq);
}

static void indep_recovery_err_wq_deinit(struct mxman *mxman)
{
	indep_recovery_err_wq_stop(mxman);
	destroy_workqueue(mxman->indep_recovery_err_wq);
}

static void indep_recovery_err_wq_start(struct mxman *mxman)
{
	queue_work(mxman->indep_recovery_err_wq, &mxman->indep_recovery_err_work);
}

int mxman_check_start_failure_during_indep_recovery(struct mxman *mxman)
{
	struct rcvry_fsm_thread 	*th = &mxman->rcvry_thread;
	if (th->indep_substate == WLAN_INDEP_ERR_START_FAIL)
		return (int)SCSC_SUBSYSTEM_WLAN;
	else if (th->indep_substate == WPAN_INDEP_ERR_START_FAIL)
		return (int)SCSC_SUBSYSTEM_WPAN;
	else
		return (int)SCSC_SUBSYSTEM_INVALID;
}

bool mxman_warm_reset_in_progress(void)
{
	return (recovery_state > RCVRY_STATE_CHIP_RESET && recovery_state < RCVRY_STATE_MAX) ? true : false;
}
...
{
	return (recovery_state == RCVRY_STATE_ERROR_IN_PROGRESS) ? true : false;
}
...
static void mxman_init_rcvry_fsm_params(struct mxman *mxman)
{
	...
	th->pmu_monitor_mode = false;
	th->indep_substate = NO_INDEP;
#if defined(CONFIG_SCSC_PCIE_CHIP)
	trigger_recovery_for_pcie_link_error = false;
#endif
}

static int mxman_perform_warm_reset(struct mxman *mxman)
{
	struct rcvry_fsm_thread 	*th = &mxman->rcvry_thread;

	if ((!mxman->fw_wlan) || (!mxman->fw_wpan && th->target_sub == SCSC_SUBSYSTEM_WPAN)) {
	...
	enable_status_to_indep_subsys_recovery = false;
	mxman_set_failure_params(mxman, false);

	th->pmu_monitor_mode = false;
	th->indep_substate = (mxman->scsc_panic_sub == SCSC_SUBSYSTEM_WLAN) ? WLAN_INDEP_NORMAL : WPAN_INDEP_NORMAL;
	...
	return (int)RCVRY_STATE_DUMP_LOGGING;
}
...
static int mxman_perform_cold_reset(struct mxman *mxman)
{
	...
	enable_status_to_indep_subsys_recovery = false;
	mxman_set_failure_params(mxman, true);

	th->pmu_monitor_mode = false;
	queue_work(mxman->failure_wq, &mxman->failure_work);
	return (int)RCVRY_STATE_CHIP_RESET;
}

static int mxman_set_pmu_to_monitor_mode(struct mxman *mxman, enum scsc_subsystem sub)
{
	int ret = 0;
	enum scsc_subsystem target_sub = sub;
#if defined(CONFIG_SCSC_PCIE_CHIP)
	ret = scsc_mx_service_claim(MXMAN_RECOVER);
	if (ret) {
		SCSC_TAG_ERR(MXMAN, "Error claiming link\n");
		return ret;
	}
#endif
	if (enable_scan2mem_dump || fail_after_recovery_enable)
		target_sub = (sub == SCSC_SUBSYSTEM_WLAN) ? SCSC_SUBSYSTEM_WPAN : SCSC_SUBSYSTEM_WLAN;
	ret = mxman_res_pmu_monitor(mxman, target_sub);
#if defined(CONFIG_SCSC_PCIE_CHIP)
	scsc_mx_service_release(MXMAN_RECOVER);
#endif
	return ret;
}

static int mxman_prepare_panic_info(struct mxman *mxman, const char *reason)
{
	struct rcvry_fsm_thread 	*th = &mxman->rcvry_thread;

	SCSC_TAG_INFO(MXMAN, "Current state %d users_wlan=%d users_wpan=%d\n", mxman->mxman_state, mxman->users, mxman->users_wpan);

	switch (mxman->mxman_state) {
	case MXMAN_STATE_STARTED_WLAN:
		th->last_panic_sub = SCSC_SUBSYSTEM_WLAN;
		break;
	case MXMAN_STATE_STARTED_WPAN:
		th->last_panic_sub = SCSC_SUBSYSTEM_WPAN;
		break;
	case MXMAN_STATE_STARTED_WLAN_WPAN:
		th->last_panic_sub = SCSC_SUBSYSTEM_WLAN_WPAN;
		break;
	case MXMAN_STATE_FAILED_WLAN:
	case MXMAN_STATE_FAILED_WLAN_WPAN:
		th->last_panic_sub = (mxman->users) ? SCSC_SUBSYSTEM_WLAN : SCSC_SUBSYSTEM_WPAN;
		break;
	case MXMAN_STATE_FAILED_WPAN:
		th->last_panic_sub = (mxman->users_wpan) ? SCSC_SUBSYSTEM_WPAN : SCSC_SUBSYSTEM_WLAN;
		break;
	default:
		/* The host should shut down in order to return to the normal operation 
		 * even if any worst case occurs. */
		return -EINVAL;
	}
	th->last_panic_code = SCSC_PANIC_CODE_HOST << 15;
...
static int mxman_handle_recovery_error_case(struct mxman *mxman, bool cold_reset)
{
	struct rcvry_fsm_thread         *th = &mxman->rcvry_thread;
	enum recovery_state	target_state;
	...
	if (mxman->users == 0 && mxman->users_wpan == 0) {
		...
		target_state = RCVRY_STATE_NONE;
	} else {
		if (mxman_prepare_panic_info(mxman, __func__)) {
			mxman_close_without_lock(mxman, mxman->users ? SCSC_SUBSYSTEM_WLAN : SCSC_SUBSYSTEM_WPAN);
			target_state = RCVRY_STATE_NONE;
		} else if (cold_reset) {
			switch (mxman->mxman_state) {
				case MXMAN_STATE_FAILED_WLAN:
					mxman->mxman_state = (mxman->users_wpan) ? MXMAN_STATE_STARTED_WLAN_WPAN : MXMAN_STATE_STARTED_WLAN;
					break;
				case MXMAN_STATE_FAILED_WPAN:
					mxman->mxman_state = (mxman->users) ? MXMAN_STATE_STARTED_WLAN_WPAN : MXMAN_STATE_STARTED_WPAN;
					break;
				case MXMAN_STATE_FAILED_WLAN_WPAN:
					mxman->mxman_state = MXMAN_STATE_STARTED_WLAN_WPAN;
					break;
			...
			if (th->indep_substate == WLAN_INDEP_ERR_START_FAIL || th->indep_substate == WPAN_INDEP_ERR_START_FAIL) {
				indep_recovery_err_wq_start(mxman);
				target_state = RCVRY_STATE_CHIP_RESET;
			} else
				target_state = mxman_perform_cold_reset(mxman);
		} else
			target_state = RCVRY_STATE_NONE;
	}
	if (target_state == RCVRY_STATE_NONE) {
		th->indep_substate = NO_INDEP;
#if defined(CONFIG_SCSC_PCIE_CHIP)
		trigger_recovery_for_pcie_link_error = false;
#endif
...
static int mxman_handle_rcvry_fsm(void *data)
{
	...
	while (true) {
		...
		case RCVRY_STATE_NONE:
			if (evt_data->event == RCVRY_EVT_ERROR) {
				recovery_state = mxman_perform_cold_reset(mxman);
			} else if (evt_data->event == RCVRY_EVT_RECOVER_WLAN || evt_data->event == RCVRY_EVT_RECOVER_WPAN) {
				recovery_state = mxman_perform_warm_reset(mxman);
			} else if (evt_data->event == RCVRY_EVT_RECOVER_HOST) {
				if (enable_scan2mem_dump)
					mxman_set_recovery_mode(mxman, false);

				ret = mxman_set_pmu_to_monitor_mode(mxman, th->last_panic_sub);
				if (ret) {
					SCSC_TAG_WARNING(MXMAN, "Failed to set PMU to monitor mode!\n");
					/* Force to proceed Full system recovery because host or fw panic already occured */
					recovery_state = mxman_perform_cold_reset(mxman);
				} else {
					SCSC_TAG_INFO(MXMAN, "Host expects ERROR or RECOVER command from PMU.\n");
					th->pmu_monitor_mode = true;
				}
			}
			break;
		case RCVRY_STATE_CHIP_RESET:
			if (evt_data->event == RCVRY_EVT_RECOVER_REJECT || evt_data->event == RCVRY_EVT_RESET_DONE
				|| evt_data->event == RCVRY_EVT_RESET_ERR || evt_data->event == RCVRY_EVT_RESET_TIMEOUT) {
				mxman_init_rcvry_fsm_params(mxman);
				recovery_state = RCVRY_STATE_NONE;
			}
			break;
		case RCVRY_STATE_DUMP_LOGGING:
			if (evt_data->event == RCVRY_EVT_RECOVER_REJECT) {
				mxman_init_rcvry_fsm_params(mxman);
				recovery_state = RCVRY_STATE_NONE;
			} else if (evt_data->event == RCVRY_EVT_DUMP_END) {
				reset_subsystem_wq_start(mxman);
				recovery_state = RCVRY_STATE_SUBSYSTEM_RESET;
			} else if (evt_data->event == RCVRY_EVT_ERROR) {
				th->indep_substate = (th->indep_substate == WLAN_INDEP_NORMAL) ? WLAN_INDEP_ERR : WPAN_INDEP_ERR;
				recovery_state = RCVRY_STATE_ERROR_IN_PROGRESS;
			}
			break;
		case RCVRY_STATE_SUBSYSTEM_RESET:
			if (evt_data->event == RCVRY_EVT_RESET_DONE) {
				recovery_state = RCVRY_STATE_WAITING_START;
			} else if (evt_data->event == RCVRY_EVT_RESET_ERR || evt_data->event == RCVRY_EVT_RESET_TIMEOUT) {
				recovery_state = mxman_handle_recovery_error_case(mxman, false);
			} else if (evt_data->event == RCVRY_EVT_ERROR) {
				th->indep_substate = (th->indep_substate == WLAN_INDEP_NORMAL) ? WLAN_INDEP_ERR : WPAN_INDEP_ERR;
				recovery_state = RCVRY_STATE_ERROR_IN_PROGRESS;
			}
			break;
		case RCVRY_STATE_WAITING_START:
			if (evt_data->event == RCVRY_EVT_START_SUCCESS) {
				if (pmu_enable_split_recovery_after_ms)
					delay_recovery_setting_wq_start(mxman);
				else
					mxman_set_recovery_mode(mxman, true);
				recovery_state = RCVRY_STATE_NONE;
			} else if (evt_data->event == RCVRY_EVT_START_FAILURE) {
				recovery_state = RCVRY_STATE_NONE;
			} else if (evt_data->event == RCVRY_EVT_ERROR) {
				th->indep_substate = (th->indep_substate == WLAN_INDEP_NORMAL) ? WLAN_INDEP_ERR : WPAN_INDEP_ERR;
				recovery_state = RCVRY_STATE_ERROR_IN_PROGRESS;
			...
			break;
		case RCVRY_STATE_ERROR_IN_PROGRESS:
			if (evt_data->event == RCVRY_EVT_DUMP_END || evt_data->event == RCVRY_EVT_RESET_DONE
				|| evt_data->event == RCVRY_EVT_RESET_ERR || evt_data->event == RCVRY_EVT_RESET_TIMEOUT
				|| evt_data->event == RCVRY_EVT_START_SUCCESS || evt_data->event == RCVRY_EVT_START_FAILURE) {
				if (evt_data->event == RCVRY_EVT_START_FAILURE) {
					th->indep_substate = (th->indep_substate == WLAN_INDEP_ERR)
										? WLAN_INDEP_ERR_START_FAIL : WPAN_INDEP_ERR_START_FAIL;
				}
				recovery_state = mxman_handle_recovery_error_case(mxman, true);
			}
			break;
		default:
			break;
		...
		//if (old_state != recovery_state) {
			SCSC_TAG_INFO(MXMAN, "Recovery state (%s) -> (%s) on event %s. target sub %d complete %d\n",
					rcvry_state_str[old_state], rcvry_state_str[recovery_state],
					rcvry_event_str[evt_data->event], th->target_sub, evt_data->complete);
		//}

		if (evt_data->complete)
			complete(&th->reset_completion);
...
int mxman_recovery_thread_init(struct mxman* mxman)
{
	...
	init_completion(&th->reset_work_completion);
	init_completion(&th->reset_completion);
	init_completion(&th->delay_setting_completion);
	init_completion(&th->indep_err_work_comletion);
...
static void mxman_syserr_recovery_work(struct work_struct *work)
{
	...
	srvman_set_error_subsystem_complete(srvman, mxman->last_syserr.subsys, NOT_ALLOWED_START_STOP);
...
static int __mxman_open(struct mxman *mxman, enum scsc_subsystem sub, void *data, size_t data_sz)
{
...
#endif
	{
		...
		*/
		...
		if (ret) {
			SCSC_TAG_ERR(MXMAN, "mxman_start_boot error %d\n", ret);
			goto error;
		}

#if defined(CONFIG_SCSC_PCIE_CHIP)
		ret = mxman_res_init_common(mxman, mxman_trigger_pcie_recovery_notifier);
#else
	...
	if ((mxman->users == 0 && sub == SCSC_SUBSYSTEM_WLAN) ||
		...
		if (ret) {
			...
			if (!mxman_check_recovery_state_error_in_progress()) {
				mxman_close_on_start_failure(mxman, sub);
				mxman_send_rcvry_evt_to_fsm(mxman, RCVRY_EVT_START_FAILURE);
			}
#else
			mxman_close_on_start_failure(mxman, sub);
#endif
	...
	if (sub == SCSC_SUBSYSTEM_WLAN)
		atomic_inc(&mxman->boot_count_wlan);
	else if (sub == SCSC_SUBSYSTEM_WPAN)
		atomic_inc(&mxman->boot_count_wpan);

	SCSC_TAG_INFO(MXMAN, "boot_count=%d boot_count_wlan=%d boot_count_wpan=%d\n",
				mxman->boot_count, mxman->boot_count_wlan, mxman->boot_count_wpan);
	/*
...
error:
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	mxman_send_rcvry_evt_to_fsm_wait_completion(mxman, (ret) ? RCVRY_EVT_RESET_ERR : RCVRY_EVT_RESET_DONE);
#endif
	SCSC_TAG_INFO(MXMAN, "Exit state %d users_wlan=%d users_wpan=%d with ret= %d \n", mxman->mxman_state, mxman->users, mxman->users_wpan, ret);
	mutex_unlock(&mxman->mxman_mutex);
	return ret;
}
...
int mxman_stop(struct mxman *mxman, enum scsc_subsystem sub)
{
...
#if defined(CONFIG_SCSC_PCIE_CHIP)
	if (scsc_mx_service_check_pcie_link_error()) {
		/* Disable comms channels to subsystem */
	 ...
	 */
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	if (!mxman_subsys_in_failed_state(mxman, sub) && mxman_check_recovery_state_none()) {
#else
	...
	/*
	* If the HALT_RSP timed out, force scandump mode to capture the scandump logs when auto recovery disable
	* if the occurance increases this will cause issue. SO we will revert this in the final round
	*/
	if (ret) {
		if(disable_recovery_handling) {
#if IS_ENABLED(CONFIG_DEBUG_SNAPSHOT)
			SCSC_TAG_INFO(MXMAN, "Force scandump mode on  %s subsystem\n", (sub == SCSC_SUBSYSTEM_WPAN) ? "WPAN" : "WLAN");
			mxman_scan_dump_mode();
#endif
		} else {
			SCSC_TAG_INFO(MXMAN, "Force monitor mode on  %s subsystem\n", (sub == SCSC_SUBSYSTEM_WPAN) ? "WPAN" : "WLAN");
			mxman_res_pmu_monitor(mxman, sub);
			/* Delay to allow FW to enable monitor */
			msleep(500);
		}
	}

	/* Reset subsystem, either from monitor or idle */
	mxman_res_pmu_reset(mxman, sub);
	mxman_res_fw_record_reset(mxman, sub);
	return 0;
}
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	if (!mxman_check_recovery_state_none()) {
		SCSC_TAG_WARNING(MXMAN, "Ignore syserr due to recovery\n");
		return;
	}
#endif
...
void mxman_set_failure_params(struct mxman *mxman, bool full_recovery)
{
	...
	if (th->last_panic_sub == SCSC_SUBSYSTEM_WLAN)
		mxman->last_syserr.subsys = SYSERR_SUBSYS_WLAN;
	else if (th->last_panic_sub == SCSC_SUBSYSTEM_WPAN)
		mxman->last_syserr.subsys = SYSERR_SUBSYS_BT;
	...
	case SCSC_SUBSYSTEM_PMU:
		...
		else if (mxman_subsys_active(mxman, SCSC_SUBSYSTEM_WLAN))
			mxman->mxman_next_state = MXMAN_STATE_FAILED_WLAN;
...
void mxman_recover_subsystem(struct mxman *mxman, u16 failure_source, const char *reason, enum scsc_subsystem sub)
{
	...
	if (failure_source & (SCSC_PANIC_CODE_HOST << 15) || enable_scan2mem_dump || fail_after_recovery_enable) {
#if defined(CONFIG_SCSC_PCIE_CHIP)
		evt = (trigger_recovery_for_pcie_link_error) ? RCVRY_EVT_ERROR : RCVRY_EVT_RECOVER_HOST;
#else
		evt = RCVRY_EVT_RECOVER_HOST;
#endif
	} else {
		switch(sub) {
		case SCSC_SUBSYSTEM_WLAN:
			evt = RCVRY_EVT_RECOVER_WLAN;
			break;
		case SCSC_SUBSYSTEM_WPAN:
			evt = RCVRY_EVT_RECOVER_WPAN;
			break;
		default:
			evt = RCVRY_EVT_ERROR;
			break;
...
void mxman_init(struct mxman *mxman, struct scsc_mx *mx)
{
	...
	reset_subsystem_wq_init(mxman);
	delay_recovery_setting_wq_init(mxman);
	indep_recovery_err_wq_init(mxman);
#else
	mxman->panic_in_progress = false;
#endif
...
#endif
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	indep_recovery_err_wq_deinit(mxman);
	delay_recovery_setting_wq_deinit(mxman);
	reset_subsystem_wq_deinit(mxman);
...
{
	u8 idx = (sub == SCSC_SUBSYSTEM_WLAN) ? SYSERR_SUBSYS_RESET_WLAN : SYSERR_SUBSYS_RESET_WPAN;
	mxman->last_syserr_recovery_time[idx] = value;
}
