...
extern int (*fp_platform_load_pmu_fw_flags)(struct scsc_mif_abs *interface, u32 *ka_patch, size_t ka_patch_len, u32 flags);
...
extern int (*fp_platform_mif_set_chip_id)(struct platform_mif *platform);
extern int (*fp_platform_mif_set_ka_patch)(struct platform_mif *platform, uint32_t *ka_patch_addr, uint32_t *ka_patch_addr_end);
extern int (*fp_platform_mif_set_cfg_done)(struct platform_mif *platform);
extern int (*fp_platform_mif_set_cfg_ack)(struct platform_mif *platform);
...
static void test_all(struct kunit *test)
{
	...
	platform_mif_memory_api_init(scscmif);
	platform_mif_intr_handler_api_init(platform);
	platform_mif_memlog_init(platform);
	platform_mif_irq_interface_init(scscmif);

	fp_platform_mif_set_chip_id(platform);

	u32 ka_patch[4] = {0x11111111, 0x22222222, 0x33333333, 0x44444444};
	uint32_t *ka_patch_addr, *ka_patch_addr_end;
	ka_patch_addr = ka_patch;
	ka_patch_addr_end = ka_patch_addr+4;
	fp_platform_mif_set_ka_patch(platform, ka_patch_addr, ka_patch_addr_end);
	fp_platform_mif_set_cfg_done(platform);
	fp_platform_mif_set_cfg_ack(platform);
	...
	fp_platform_load_pmu_fw_flags(scscmif, "ABCD", 4, 0x0);
	...
	platform_mif_reg_write(SCSC_MIF_ABS_TARGET_WLAN, 0, 0);

	platform_mif_reg_read(SCSC_MIF_ABS_TARGET_WLAN, 0);

	const struct kernel_param *kp = NULL;
	qos_get_param(string, kp);

	void *mem;
	platform_mif_get_ramrp_buff(platform, &mem, 1, 0x00000000);
