...
struct slsi_dev *slsi_dev_attach(struct device *dev, struct scsc_mx *core, struct scsc_service_client *mx_wlan_client)
{
	...
	slsi_wake_lock_init(NULL, &sdev->wlan_wl, "wlan");
	slsi_wake_lock_init(NULL, &sdev->wlan_wl_mlme_evt, "wlan_wl_mlme_evt");
	slsi_wake_lock_init(NULL, &sdev->wlan_wl_mlme, "wlan_mlme");
	slsi_wake_lock_init(NULL, &sdev->wlan_wl_ma, "wlan_ma");
	slsi_wake_lock_init(NULL, &sdev->wlan_wl_roam, "wlan_roam");
	slsi_wake_lock_init(NULL, &sdev->wlan_wl_init, "wlan_init");
	slsi_wake_lock_init(NULL, &sdev->wlan_wl_tx_sched, "wlan_tx_sched");
...
void __exit slsi_dev_unload(void)
{
	...
	slsi_udi_deinit();
	slsi_destroy_sysfs_softap_info();
