...
#define SLSI_MIN_LOG_SIZE (200)
#define SLSI_MAX_LOG_SIZE (1024)
...
static int slsi_monitor_vif_num_set_cb(const char *val, const struct kernel_param *kp)
{
	...
	sdev->monitor_vif_num = monitor_vif_num;
...
static ssize_t sysfs_show_version_info(struct kobject *kobj, struct kobj_attribute *attr,
				       char *buf);
static ssize_t sysfs_show_softap_info(struct kobject *kobj, struct kobj_attribute *attr,
				      char *buf);
...
static ssize_t sysfs_store_ant(struct kobject *kobj, struct kobj_attribute *attr,
			       const char *buf, size_t count);
static ssize_t sysfs_show_max_log_size(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static ssize_t sysfs_store_max_log_size(struct kobject *kobj, struct kobj_attribute *attr,
			       const char *buf, size_t count);
...
static u32 sysfs_max_log_size = SLSI_MIN_LOG_SIZE;
...
static struct kobj_attribute max_log_size_attr = __ATTR(max_chipset_log_size, 0660, sysfs_show_max_log_size,
			       sysfs_store_max_log_size);
static struct kobj_attribute softap_attr = __ATTR(softap, 0660, sysfs_show_softap_info, NULL);
...
/* Retrieve softap information in sysfs global */
static ssize_t sysfs_show_softap_info(struct kobject *kobj,
				       struct kobj_attribute *attr,
				       char *buf)
{
	struct slsi_dev *sdev = slsi_get_sdev();
...
/* Register sysfs softap information */
void slsi_create_sysfs_softap_info(void)
{
#ifndef SLSI_TEST_DEV
	int r;
	...
	if (wifi_kobj_ref) {
		/* Create sysfs file /sys/wifi/softap_info */
		r = sysfs_create_file(wifi_kobj_ref, &softap_attr.attr);
		if (r) {
			/* Failed, so clean up dir */
			pr_err("Can't create /sys/wifi/softap_info\n");
			return;
		}
	} else {
		pr_err("failed to create /sys/wifi/softap_info\n");
	}
#endif
}

/* Unregister sysfs softap information */
void slsi_destroy_sysfs_softap_info(void)
{
	if (!wifi_kobj_ref)
		return;

	/* Destroy /sys/wifi/wifiver file */
	sysfs_remove_file(wifi_kobj_ref, &softap_attr.attr);
...
mac_default:
	...
	SLSI_DBG1(sdev, SLSI_INIT_DEINIT,
		  "MAC addr file NOT found, using default MAC ADDR: " MACSTR "\n", MAC2STR(addr));
#else
	...
	SLSI_DBG1(sdev, SLSI_INIT_DEINIT, "Test Device Address: " MACSTR "\n", MAC2STR(addr));
#endif
}
...
static void write_softap_info_file(struct slsi_dev *sdev)
{
	...
	offset = snprintf(buf + offset, sizeof(buf) - offset, "#softap.info\n");
	offset += snprintf(buf + offset, sizeof(buf) - offset, "DualBandConcurrency=%s\n", sdev->dualband_concurrency ? "yes" : "no");
	offset += snprintf(buf + offset, sizeof(buf) - offset, "DualInterface=%s\n", "yes");
	offset += snprintf(buf + offset, sizeof(buf) - offset, "5G=%s\n", sdev->band_5g_supported ? "yes" : "no");
	offset += snprintf(buf + offset, sizeof(buf) - offset, "maxClient=%d\n", !sdev->softap_max_client ? SLSI_MIB_MAX_CLIENT : sdev->softap_max_client);

	/* following are always supported */
	offset += snprintf(buf + offset, sizeof(buf) - offset, "HalFn_setCountryCodeHal=yes\n");
	offset += snprintf(buf + offset, sizeof(buf) - offset, "HalFn_getValidChannels=yes\n");
...
void slsi_wlan_recovery_init(struct slsi_dev *sdev)
{
	...
	if (level > SLSI_WIFI_CM_IF_SILENT_RECOVERY || level < 5) {
		SLSI_INFO(sdev, "level %d is not for silent recovery\n", level);
		return;
	}
	...
	for (i = 1; i <= CONFIG_SCSC_WLAN_MAX_INTERFACES; i++) {
		if (sdev->netdev[i]) {
			...
			ndev_vif = netdev_priv(dev);
			SLSI_INFO(sdev, "netdev[%d] : iftype %d (viftype %d) available %d activated %d\n",
				  i, ndev_vif->iftype, ndev_vif->vif_type, ndev_vif->is_available, ndev_vif->activated);
			if (ndev_vif->is_available) {
				...
				case NL80211_IFTYPE_P2P_CLIENT:
					if (ndev_vif->vif_type == SLSI_VIFTYPE_UNSPECIFIED)
						ndev_vif->activated = 0;
					else
						cfg80211_disconnected(sdev->netdev[i], HOST_REASONCODE_RECOVERY_REASON,
...
int slsi_start(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	int wifi7enable = sdev->fw_sta_eht_supported;
#endif
	...
	SLSI_INFO(sdev, "cm_if.recovery_state:%d\n", sdev->cm_if.recovery_state);
	...
	SLSI_MUTEX_UNLOCK(sdev->device_config_mutex);
	slsi_set_initial_fw_config(sdev);
	...
	sdev->device_config.host_state &= ~SLSI_HOSTSTATE_LOW_LATENCY_ACTIVE;
...
#ifdef CONFIG_SCSC_WLAN_EHT
	if (sdev->cm_if.recovery_state == SLSI_RECOVERY_SERVICE_CLOSED || sdev->mac_changed) {
		err = slsi_set_boolean_mib(sdev, NULL, SLSI_PSID_UNIFI_EHT_ACTIVATED, wifi7enable, 0);
		if (!err) {
			sdev->fw_sta_eht_supported = wifi7enable;
			if (dev && ndev_vif) {
				ndev_vif->sta.mlo_mode = wifi7enable ? SLSI_CONFIG_MLO_MODE_ANY : SLSI_CONFIG_MLO_MODE_NONE;
			}
		}
	}
#endif
	...
	slsi_mlme_sar_set_index(sdev, NULL, sdev->tas_info.cat2_dsi_id);
		...
		if (hw_available) {
			if (ndev_vif->sta.sta_bss) {
				...
				slsi_mlme_disconnect(sdev, dev, ndev_vif->sta.sta_bss->bssid,
						     FAPI_REASONCODE_UNSPECIFIED_REASON, true, &mlo_vif);
				slsi_handle_disconnect(sdev, dev, ndev_vif->sta.sta_bss->bssid, 0, NULL, 0, mlo_vif);
#else
				slsi_mlme_disconnect(sdev, dev, ndev_vif->sta.sta_bss->bssid,
				      ...
				      int *mib_index)
{
	...
	++*mib_index;
	if (values[*mib_index].type != SLSI_MIB_TYPE_NONE) {
			sdev->fw_mhs_eht_mlo_supported = values[*mib_index].u.boolValue;
			SLSI_INFO(sdev, "SoftAp eht mlo supported: %d\n", sdev->fw_mhs_eht_mlo_supported);
	} else {
		SLSI_INFO(sdev, "SoftAp eht mlo is not supported\n");
	}
	++*mib_index;
	if (values[*mib_index].type == SLSI_MIB_TYPE_OCTET) {
		if (values[*mib_index].u.octetValue.dataLength >= 9 &&
			...
			memcpy(sdev->fw_sta_basic_ml_cap, values[*mib_index].u.octetValue.data,
			       min((unsigned long)values[*mib_index].u.octetValue.dataLength,
				   sizeof(sdev->fw_sta_basic_ml_cap)));
		else
	...
	if (values[*mib_index].type == SLSI_MIB_TYPE_OCTET) {
		if (values[*mib_index].u.octetValue.dataLength >= 9 &&
			...
			memcpy(sdev->fw_mhs_basic_ml_cap, values[*mib_index].u.octetValue.data,
			       min((unsigned long)values[*mib_index].u.octetValue.dataLength,
				   sizeof(sdev->fw_mhs_basic_ml_cap)));
		else
...
static int slsi_get_mib_entry_value(struct slsi_dev *sdev, struct slsi_mib_value *values)
{
	...
	if (values[++mib_index].type != SLSI_MIB_TYPE_NONE) { /* UnifiNanMaxNdpSessions */
		SLSI_CHECK_TYPE(sdev, values[mib_index].type, SLSI_MIB_TYPE_UINT);
		sdev->nan_max_ndp_instances = (values[mib_index].u.uintValue);
	} else {
		SLSI_WARN(sdev, "Error reading mib UnifiNanMaxNdpSessions\n");
	}
#if defined(CONFIG_SCSC_WLAN_TAS)
	...
	SLSI_MUTEX_LOCK(sdev->device_config_mutex);
	if (values[++mib_index].type != SLSI_MIB_TYPE_NONE)
		sdev->device_config.etp_boost = values[mib_index].u.intValue;
	else
		SLSI_DBG2(sdev, SLSI_MLME, "Failed to read roam etput boost\n");
	if (values[++mib_index].type != SLSI_MIB_TYPE_NONE)
		sdev->device_config.etp_mlo_boost = values[mib_index].u.intValue;
	else
		SLSI_DBG2(sdev, SLSI_MLME, "Failed to read roam etp mlo boost\n");
	SLSI_MUTEX_UNLOCK(sdev->device_config_mutex);

	if (values[++mib_index].type == SLSI_MIB_TYPE_UINT)
		sdev->he_gi_value = values[mib_index].u.uintValue;
	else
		SLSI_ERR(sdev, "Invalid type (%d) for SLSI_PSID_UNIFI_RAA_TX_GI_VALUE\n",
			 values[mib_index].type);

	if (values[++mib_index].type == SLSI_MIB_TYPE_UINT)
		sdev->mpdu_ss = values[mib_index].u.uintValue;
	else
		SLSI_ERR(sdev, "Invalid type (%d) for SLSI_PSID_UNIFI_ROAM_MPDUSS\n",
			 values[mib_index].type);

	if (values[++mib_index].type == SLSI_MIB_TYPE_UINT)
		sdev->min_ba_win_size = values[mib_index].u.uintValue;
	else
		SLSI_ERR(sdev, "Invalid type (%d) for SLSI_PSID_UNIFI_ROAM_BA_WIN_SIZE\n",
			 values[mib_index].type);
#endif
	if (values[++mib_index].type == SLSI_MIB_TYPE_OCTET) {
		if (values[mib_index].u.octetValue.dataLength >= 28 &&
		    values[mib_index].u.octetValue.dataLength <= 32)
			memcpy(sdev->fw_sta_he_cap, values[mib_index].u.octetValue.data,
			       values[mib_index].u.octetValue.dataLength);
		else
			SLSI_WARN(sdev, "Invalid sta HE capabilities. length (%d)\n",
				  values[mib_index].u.octetValue.dataLength);
	} else {
		SLSI_DBG2(sdev, SLSI_MLME, "Failed to read sta HE Capabilities\n");
	}

	if (values[++mib_index].type == SLSI_MIB_TYPE_UINT)
		sdev->fw_keepalive_time = values[mib_index].u.uintValue;
...
static int slsi_mib_initial_get(struct slsi_dev *sdev)
{
	...
	static const struct slsi_mib_get_entry get_values[] = {
		...
		{ SLSI_PSID_UNIFI_ROAM_ETPUT_BOOST, { 2, 0 },},
		{ SLSI_PSID_UNIFI_ROAM_ETP_MLO_BOOST, { 0, 0 },},
		{ SLSI_PSID_UNIFI_RAA_TX_GI_VALUE, { 0, 0 } },
		{ SLSI_PSID_UNIFI_ROAM_MPDUSS, { 0, 0 } },
		{ SLSI_PSID_UNIFI_ROAM_BA_WIN_SIZE, { 0, 0 } },
#endif
		{ SLSI_PSID_UNIFI_HE_CAPABILITIES, { 0, 0 } },
		{ SLSI_PSID_UNIFI_HE_CAPABILITIES, { 0, 0 } },
		{ SLSI_PSID_UNIFI_MLMESTA_KEEP_ALIVE_TIMEOUT, { 0, 0 } },
	};
...
int slsi_mib_get_rtt_cap(struct slsi_dev *sdev, struct net_device *dev, struct slsi_rtt_capabilities *cap)
{
	...
	if (values[0].type != SLSI_MIB_TYPE_OCTET) {
		SLSI_ERR(sdev, "Invalid type (%d) for SLSI_PSID_UNIFI_RTT_CAPABILITIES\n", values[0].type);
		kfree(mibrsp.data);
		kfree(values);
		return -EINVAL;
	}
...
int slsi_mib_get_sta_tdls_activated(struct slsi_dev *sdev, struct net_device *dev, bool *tdls_supported)
{
	...
	if (values[0].type != SLSI_MIB_TYPE_UINT) {
		SLSI_ERR(sdev, "Invalid type (%d) for SLSI_PSID_UNIFI_TDLS_ACTIVATED\n", values[0].type);
		kfree(mibrsp.data);
		kfree(values);
		return -EINVAL;
	}
...
int slsi_mib_get_sta_tdls_max_peer(struct slsi_dev *sdev, struct net_device *dev, struct netdev_vif *ndev_vif)
{
	...
	if (values[0].type != SLSI_MIB_TYPE_UINT) {
		SLSI_ERR(sdev, "Invalid type (%d) for SLSI_PSID_UNIFI_MAX_TDLS_CLIENT\n", values[0].type);
		kfree(mibrsp.data);
		kfree(values);
		return -EINVAL;
	}
...
struct slsi_peer *slsi_get_link_peer_from_mac(struct net_device *dev, const u8 *mac)
{
	...
	for (i = 0; i < MAX_SUPP_MLO_LINKS; i++) {
		if (ndev_vif->sta.link_peer_sta_record[i].valid &&
		    compare_ether_addr(ndev_vif->sta.link_peer_sta_record[i].address, mac) == 0)
		return (&(ndev_vif->sta.link_peer_sta_record[i]));
	}
					 ...
					 u8 *peer_address, u16 link_id)
{
	...
	if (WLBT_WARN_ON(link_id >= MAX_NUM_MLD_LINKS)) {
		SLSI_NET_ERR(dev, "Invalid link_id received %d\n", link_id);
		return NULL;
	}
	...
	SLSI_NET_DBG2(dev, SLSI_CFG80211, MACSTR ", link_id:%d\n", MAC2STR(peer_address), link_id);

	while (num_links < MAX_SUPP_MLO_LINKS) {
		if (ndev_vif->sta.link_peer_sta_record[num_links].valid &&
		    SLSI_ETHER_EQUAL(peer_address, ndev_vif->sta.link_peer_sta_record[num_links].address)) {
			SLSI_NET_WARN(dev, "Peer (link_id %d Link addr:" MACSTR ") already exists\n",
				      link_id, MAC2STR(peer_address));
			return NULL;
		}
		if (!ndev_vif->sta.link_peer_sta_record[num_links].valid)
			break;
	...
	if (num_links >= MAX_SUPP_MLO_LINKS) {
		SLSI_NET_WARN(dev, "driver doesn't support more than %d links, num_links %d\n",
			      MAX_SUPP_MLO_LINKS, num_links);
		return NULL;
	}

	peer = &ndev_vif->sta.link_peer_sta_record[num_links];
	...
	SLSI_NET_DBG2(dev, SLSI_CFG80211, "created station link peer " MACSTR " link_id:%d\n",
		      MAC2STR(peer->address), link_id);
...
struct slsi_peer *slsi_peer_add(struct slsi_dev *sdev, struct net_device *dev, u8 *peer_address, u16 aid)
{
	...
	SLSI_NET_DBG2(dev, SLSI_CFG80211, MACSTR ", aid:%d\n", MAC2STR(peer_address), aid);
 ...
 */
	...
	SLSI_NET_DBG2(dev, SLSI_CFG80211, "created station peer " MACSTR " AID:%d\n", MAC2STR(peer->address), aid);
	return peer;
}
...
void slsi_peer_reset_stats(struct slsi_dev *sdev, struct net_device *dev, struct slsi_peer *peer)
{
	...
	SLSI_NET_DBG3(dev, SLSI_CFG80211, "Peer:" MACSTR "\n", MAC2STR(peer->address));
...
int slsi_peer_remove(struct slsi_dev *sdev, struct net_device *dev, struct slsi_peer *peer)
{
	...
	SLSI_NET_DBG2(dev, SLSI_CFG80211, MACSTR "\n", MAC2STR(peer->address));
...
void slsi_vif_deactivated(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	if (level != SLSI_WIFI_CM_IF_SILENT_RECOVERY) {
		SLSI_INFO(sdev, "vif_type is set to UNSPECIFIED, level = %d\n", level);
		ndev_vif->vif_type = SLSI_VIFTYPE_UNSPECIFIED;
	}
	...
	WLBT_WARN(ndev_vif->peer_sta_records, "ifnum:%d, peer_sta_records:%d", ndev_vif->ifnum, ndev_vif->peer_sta_records);
...
#ifdef CONFIG_SCSC_WLAN_EHT
int slsi_populate_bss_record(struct net_device *dev, u16 mlo_vif)
#else
...
#ifdef CONFIG_SCSC_WLAN_EHT
	if (mlo_vif > 0)
		values = slsi_read_mibs_with_vifidx(sdev, dev, get_values, ARRAY_SIZE(get_values), &mibrsp, mlo_vif);
	else
...
#ifdef CONFIG_SCSC_WLAN_EHT
static bool slsi_ml_get_sta_bss(struct slsi_dev *sdev, struct net_device *dev)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct cfg80211_bss *sta_bss;

	SLSI_INFO(sdev, "Get BSS info for(%.*s) BSSID: " MACSTR "\n",
		  (int)ndev_vif->sta.sme.ssid_len, ndev_vif->sta.sme.ssid, MAC2STR(ndev_vif->sta.sme.bssid));
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 17))
	sta_bss = __cfg80211_get_bss(sdev->wiphy, NULL, ndev_vif->sta.sme.bssid, ndev_vif->sta.sme.ssid,
				     ndev_vif->sta.sme.ssid_len, IEEE80211_BSS_TYPE_ANY, IEEE80211_PRIVACY_ANY,
				     NL80211_BSS_USE_FOR_ALL);
#else
	sta_bss =  cfg80211_get_bss(sdev->wiphy, NULL, ndev_vif->sta.sme.bssid, ndev_vif->sta.sme.ssid,
				    ndev_vif->sta.sme.ssid_len, IEEE80211_BSS_TYPE_ANY, IEEE80211_PRIVACY_ANY);
#endif
	if (!sta_bss) {
		...
		ssid.ssid_len = ndev_vif->sta.sme.ssid_len;
		memcpy(ssid.ssid, ndev_vif->sta.sme.ssid, ssid.ssid_len);
		r = slsi_mlme_connect_scan(sdev, dev, 1, &ssid, ndev_vif->sta.sme.channel);
		if (r)
			SLSI_NET_ERR(dev, "slsi_mlme_connect_scan failed\n");
		else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 17))
			sta_bss = __cfg80211_get_bss(sdev->wiphy, NULL, ndev_vif->sta.sme.bssid, ndev_vif->sta.sme.ssid,
						     ndev_vif->sta.sme.ssid_len, IEEE80211_BSS_TYPE_ANY,
						     IEEE80211_PRIVACY_ANY, NL80211_BSS_USE_FOR_ALL);
#else
			sta_bss = cfg80211_get_bss(sdev->wiphy, NULL, ndev_vif->sta.sme.bssid, ndev_vif->sta.sme.ssid,
						   ndev_vif->sta.sme.ssid_len, IEEE80211_BSS_TYPE_ANY,
...
#endif
		if (!sta_bss)
			SLSI_NET_ERR(dev, "cfg80211_get_bss(%.*s) Not found BSSID: " MACSTR " Channel = %d\n",
...
int slsi_retry_connection(struct slsi_dev *sdev, struct net_device *dev)
{
...
#ifdef CONFIG_SCSC_WLAN_EHT
		/* Proceed if non-ml connection or retrying on the same bss with single link */
		if (!ndev_vif->sta.ml_connection || ndev_vif->sta.connecting_links < ndev_vif->sta.max_ml_link)
			break;
		/* Update sta_bss if it is an ml connection. If failed to update sta_bss, go for next valid bss */
		ap_found = slsi_ml_get_sta_bss(sdev, dev);
		if (ap_found) {
			slsi_scan_all_ml_link(dev, sdev, &ndev_vif->sta.sme, IEEE80211_PRIVACY_ANY);
			break;
		}
#else
		break;
	...
	ndev_vif->chan = ieee80211_get_channel(sdev->wiphy, ndev_vif->sta.sme.channel->center_freq);
	SLSI_ETHER_COPY(ndev_vif->sta.bssid, ndev_vif->sta.sme.bssid);
	r = 1;
...
void slsi_free_connection_params(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	ndev_vif->sta.connected_ssid = NULL;
	ndev_vif->sta.connected_ssid_len = 0;
	kfree(ndev_vif->sta.sme.bssid);
	ndev_vif->sta.sme.bssid = NULL;
	kfree(ndev_vif->sta.sme.channel);
	ndev_vif->sta.sme.channel = NULL;
#ifdef CONFIG_SCSC_WLAN_EHT
	if (!ndev_vif->sta.ml_connection)
		return;
	ndev_vif->sta.mlo_band_control = 0;
#endif
}
			  ...
			  )
{
	...
	SLSI_NET_DBG3(dev, SLSI_MLME, "Disconnect(ifnum:%d)\n", ndev_vif->ifnum);
	...
	case FAPI_VIFTYPE_STATION:
	{
		...
		if (!(ndev_vif->sta.vif_status == SLSI_VIF_STATUS_CONNECTING ||
		      ndev_vif->sta.vif_status == SLSI_VIF_STATUS_UNSPECIFIED))
#ifdef CONFIG_SCSC_WLAN_EHT
			slsi_populate_bss_record(dev, mlo_vif);
#else
...
#endif
		ndev_vif->sta.wpa3_external_auth_abort = false;
		...
		ndev_vif->sta.ml_connection = 0;
		ndev_vif->sta.mlo_band_control = 0;
		ndev_vif->sta.max_simul_link = 0;
		memset(&ndev_vif->sta.links, 0, sizeof(ndev_vif->sta.links));
		memset(&ndev_vif->sta.link_peer_sta_record, 0,
		       sizeof(ndev_vif->sta.link_peer_sta_record));
		memset(&ndev_vif->sta.ap_mld_addr, 0, ETH_ALEN);
#endif
#ifdef CONFIG_SCSC_WLAN_STA_ENHANCED_ARP_DETECT
		...
		slsi_mlme_set_keepalive_parameters(sdev, dev, sdev->fw_keepalive_time);
		...
		SLSI_MUTEX_UNLOCK(sdev->device_config_mutex);
		sdev->soft_roaming_scans_allowed = true;
		slsi_mlme_set_low_latency_mode_req(sdev, dev, 0, sdev->soft_roaming_scans_allowed);
		if (slsi_mlme_set_scan_timing_req(sdev, dev, FAPI_LOWLATENCYMODE_LEGACY, 0, 0, 0, 0) == -EOPNOTSUPP)
			slsi_mlme_set_scan_mode_req(sdev, dev, FAPI_LOWLATENCYMODE_LEGACY, 0, 0, 0, 0);
		sdev->latency_param_mask = 0;
		SLSI_MUTEX_LOCK(ndev_vif->scan_mutex);
		slsi_roam_channel_cache_prune(dev, SLSI_ROAMING_CHANNEL_CACHE_TIMEOUT, ndev_vif->sta.last_connected_bss.ssid);
		SLSI_MUTEX_UNLOCK(ndev_vif->scan_mutex);
	...
	case FAPI_VIFTYPE_AP:
	{
		...
		SLSI_NET_DBG3(dev, SLSI_MLME, "MAC:" MACSTR " is_wps:%d Peer State = %d\n",
			      MAC2STR(peer_address),  peer->is_wps, peer->connected_state);
	...
	default:
		SLSI_NET_WARN(dev, "mlme_disconnected_ind(ifnum:%d, unexpected vif type:%d)\n", ndev_vif->ifnum, ndev_vif->vif_type);
		break;
	}
exit:
	return 0;
...
int slsi_set_enhanced_pkt_filter(struct net_device *dev, char *command, int buf_len)
{
	...
	SLSI_INFO(sdev, "Enhanced packet filter is %s\n", (pkt_filter_enable ? "enabled" : "disabled"));
	kfree(ioctl_args);
	return ret;
}
...
int slsi_ip_address_changed(struct slsi_dev *sdev, struct net_device *dev, __be32 ipaddress)
{
	...
	if (ndev_vif->activated && (ndev_vif->vif_type == FAPI_VIFTYPE_AP)) {
		...
		if (ret != 0)
			SLSI_NET_ERR(dev, "slsi_mlme_set_ip_address ERROR. ret=%d\n", ret);
		...
		if (ret != 0)
			SLSI_NET_ERR(dev, "slsi_mlme_set_ipv6_address ERROR. ret=%d\n", ret);
#endif
...
next_scan:
		...
		SLSI_INFO(sdev, "Channel selected = %d\n", sdev->device_config.ap_auto_chan);
	}
	...
	if (chan) {
		...
		if (freq != (u32)ieee80211_channel_to_frequency(chan, band)) {
			if (band == NL80211_BAND_5GHZ && freq < 3000)
				SLSI_NET_DBG2(dev, SLSI_MLME, "Off Band Result : mlme_scan_ind(freq:%d MHz) != DS(freq:%d MHz)\n", freq, ieee80211_channel_to_frequency(chan, band));
...
int slsi_set_wifisharing_permitted_channels(struct net_device *dev, char *buffer, int buf_len)
{
	...
	if (indoor_chan_arg == 0) {
		...
		for (i = 0; i < num_channels; i++) {
			...
			sdev->wifi_sharing_5g_restricted_channels[(sdev->num_5g_restricted_channels)++] =
				indoor_chan_arg;
			      ...
			      struct slsi_dev *sdev, int *wifi_sharing_channel_switched)
{
	...
	SLSI_INFO(sdev, "sta_frequency: %d MHz, sta_band : %d, is_6ghz_ap : %s\n",
	...
	SLSI_INFO(sdev, "ap center freq : %d, ap ch width : %d\n",
...
int slsi_set_mac_randomisation_mask(struct slsi_dev *sdev, u8 *mac_address_mask)
{
	...
	SLSI_DBG1(sdev, SLSI_CFG80211, "Mask is :" MACSTR "\n", MAC2STR(mac_address_mask));
...
#if !(defined(SCSC_SEP_VERSION) && SCSC_SEP_VERSION < 11)
struct slsi_ssid_info *slsi_get_valid_ssid_info_from_list(struct net_device *dev)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
...
static struct slsi_bssid_info *slsi_get_valid_bssid_info_from_list(struct slsi_dev *sdev, struct net_device *dev,
								   struct list_head *bssid_list)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_bssid_info *bssid_info;

	list_for_each_entry(bssid_info, bssid_list, list) {
		if (slsi_is_bssid_in_blacklist(sdev, dev, bssid_info->bssid) || bssid_info->connect_attempted ||
		    SLSI_ETHER_EQUAL(ndev_vif->sta.bssid, bssid_info->bssid))
				continue;
		return bssid_info;
	}
	return NULL;
...
static void slsi_update_sta_sme(struct slsi_dev *sdev, struct net_device *dev, u8 *ssid, u8 ssid_len, u8 *bssid,
			   struct ieee80211_channel *chan)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);

	kfree(ndev_vif->sta.sme.ssid);
	ndev_vif->sta.sme.ssid = slsi_mem_dup(ssid, ssid_len);
	ndev_vif->sta.sme.ssid_len = ssid_len;

	kfree(ndev_vif->sta.sme.bssid);
	ndev_vif->sta.sme.bssid = slsi_mem_dup(bssid, ETH_ALEN);
	kfree(ndev_vif->sta.sme.channel);
	ndev_vif->sta.sme.channel =
		(struct ieee80211_channel *)slsi_mem_dup((u8 *)chan, sizeof(struct ieee80211_channel));
}

#ifdef CONFIG_SCSC_WLAN_EHT
struct slsi_bssid_info *slsi_get_bssid_info_from_bssid(struct slsi_dev *sdev, struct net_device *dev,
						       struct list_head *bssid_list, const u8 *bssid)
{
	struct slsi_bssid_info *bssid_info;
...
int slsi_fill_current_ap_detail(struct slsi_dev *sdev, struct net_device *dev, struct list_head *bssid_list,
				struct slsi_bssid_info *bssid_info)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_vif_sta *sta = &ndev_vif->sta;
	struct slsi_bssid_info *tmp_info;
	struct slsi_mlo_ap_details *mlo_ap_details;
#ifdef SCSC_EHT_CONNECTION_EMLMR
	...
	list_for_each_entry(tmp_info, bssid_list, list) {
		if (!SLSI_ETHER_EQUAL(tmp_info->mld_addr, bssid_info->mld_addr) || !tmp_info->mlo_ap)
			continue;
		bssid_info->mlo_ap = tmp_info->mlo_ap;
		return 0;
	}
	mlo_ap_details = kzalloc(sizeof(*mlo_ap_details), GFP_ATOMIC);
	if (!mlo_ap_details) {
		SLSI_ERR(sdev, "Unable to allocate mem mlo_ap_details\n");
		return -ENOMEM;
	}
	if (sta->mlo_band_control)
		mlo_ap_details->tried_link_comb |= BIT(sta->mlo_band_control - 1);
#ifdef SCSC_EHT_CONNECTION_EMLMR
	if (sta->sta_bss) {
		ml_ie = cfg80211_find_ext_elem(WLAN_EID_EXT_EHT_MULTI_LINK, sta->sta_bss->ies->data,
					       sta->sta_bss->ies->len);
		ml_elem = (void *)ml_ie->data + 1;
		...
		if (control & IEEE80211_MLC_BASIC_PRES_EML_CAPA) {
			...
			eml_cap = ml_elem->variable + common;
			mlo_ap_details->emlsr_supported = eml_cap[0] & IEEE80211_EML_CAP_EMLSR_SUPP;
			mlo_ap_details->emlmr_supported = eml_cap[0] & IEEE80211_EML_CAP_EMLMR_SUPPORT;
		}
	}
#endif
	list_for_each_entry(tmp_info, bssid_list, list) {
		if (!SLSI_ETHER_EQUAL(tmp_info->mld_addr, bssid_info->mld_addr))
			continue;
		tmp_info->mlo_ap = mlo_ap_details;
		mlo_ap_details->num_links++;
		mlo_ap_details->bands |= slsi_freq_to_band(tmp_info->freq / 2);
	}
	return 0;
}

u64 slsi_ml_connection_link_selection(struct net_device *dev, u8 band, struct slsi_bssid_info *bssid_info,
				      struct slsi_mlo_ap_details *mlo_ap_details)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	u8 num_links = min(ndev_vif->sta.max_ml_link, mlo_ap_details->num_links);

	switch(band) {
	case SLSI_FREQ_BAND_6GHZ:
		if (num_links == 3 && ((mlo_ap_details->tried_link_comb & SLSI_LINK_24GHZ_5GHZ_6GHZ) == 0)) {
			mlo_ap_details->tried_link_comb |= SLSI_LINK_24GHZ_5GHZ_6GHZ;
			ndev_vif->sta.max_simul_link = 1;
			return SLSI_LINK_24GHZ_5GHZ_6GHZ;
		} else if (num_links == 1 && ((mlo_ap_details->tried_link_comb & SLSI_LINK_6GHZ) == 0)) {
			mlo_ap_details->tried_link_comb |= SLSI_LINK_6GHZ;
			ndev_vif->sta.max_simul_link = 0;
			bssid_info->connect_attempted = true;
			return SLSI_LINK_6GHZ;
		}
#ifdef SCSC_EHT_CONNECTION_EMLMR
		if ((mlo_ap_details->emlmr_supported || (((mlo_ap_details->bands & SLSI_FREQ_BAND_5GHZ) == 0) ||
							 (mlo_ap_details->tried_link_comb & SLSI_LINK_5GHZ_6GHZ)) &&
#else
		if ((((mlo_ap_details->bands & SLSI_FREQ_BAND_5GHZ) == 0) ||
		     (mlo_ap_details->tried_link_comb & SLSI_LINK_5GHZ_6GHZ)) &&
#endif
		    ((mlo_ap_details->tried_link_comb & SLSI_LINK_24GHZ_6GHZ) == 0) &&
		    (mlo_ap_details->bands & SLSI_FREQ_BAND_2GHZ)) {
			mlo_ap_details->tried_link_comb |= SLSI_LINK_24GHZ_6GHZ;
			ndev_vif->sta.max_simul_link = 1;
			return SLSI_LINK_24GHZ_6GHZ;
		}
		if (((mlo_ap_details->tried_link_comb & SLSI_LINK_5GHZ_6GHZ) == 0) &&
		    (mlo_ap_details->bands & SLSI_FREQ_BAND_5GHZ)) {
			mlo_ap_details->tried_link_comb |= SLSI_LINK_5GHZ_6GHZ;
			ndev_vif->sta.max_simul_link = 1;
			return SLSI_LINK_5GHZ_6GHZ;
		}
		if ((mlo_ap_details->tried_link_comb & SLSI_LINK_6GHZ) == 0) {
			mlo_ap_details->tried_link_comb |= SLSI_LINK_6GHZ;
			ndev_vif->sta.max_simul_link = 0;
			bssid_info->connect_attempted = true;
			return SLSI_LINK_6GHZ;
		...
		break;
	case SLSI_FREQ_BAND_5GHZ:
		if (((mlo_ap_details->tried_link_comb & SLSI_LINK_24GHZ_5GHZ) == 0) &&
		    (mlo_ap_details->bands & SLSI_FREQ_BAND_2GHZ)) {
			mlo_ap_details->tried_link_comb |= SLSI_LINK_24GHZ_5GHZ;
			ndev_vif->sta.max_simul_link = 1;
			return SLSI_LINK_24GHZ_5GHZ;
		}
		if ((mlo_ap_details->tried_link_comb & SLSI_LINK_5GHZ) == 0) {
			mlo_ap_details->tried_link_comb |= SLSI_LINK_5GHZ;
			ndev_vif->sta.max_simul_link = 0;
			bssid_info->connect_attempted = true;
			return SLSI_LINK_5GHZ;
		}
		break;
	case SLSI_FREQ_BAND_2GHZ:
		mlo_ap_details->tried_link_comb |= SLSI_LINK_24GHZ;
		ndev_vif->sta.max_simul_link = 0;
		bssid_info->connect_attempted = true;
		return SLSI_LINK_24GHZ;
	}
	return 0;
}
#endif
...
#ifdef CONFIG_SCSC_WLAN_EHT
#define CHECK_6GHZ_3LINK_COMB(mlo_ap) ((mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_5GHZ_6GHZ) && \
				       (((mlo_ap_details->bands & SLSI_FREQ_BAND_5GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_5GHZ_6GHZ)) && \
				       (((mlo_ap_details->bands & SLSI_FREQ_BAND_2GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_6GHZ)))
#define CHECK_6GHZ_2LINK_COMB(mlo_ap) ((((mlo_ap_details->bands & SLSI_FREQ_BAND_5GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_5GHZ_6GHZ)) && \
				       (((mlo_ap_details->bands & SLSI_FREQ_BAND_2GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_6GHZ)))
#define CHECK_TRIED_COMB(mlo_ap) ((min(ndev_vif->sta.max_ml_link, mlo_ap->num_links) == 3) ? \
				  CHECK_6GHZ_3LINK_COMB(mlo_ap) : \
				   ((min(ndev_vif->sta.max_ml_link, mlo_ap->num_links) == 2) ? \
				    CHECK_6GHZ_2LINK_COMB(mlo_ap) : ((mlo_ap->num_links & SLSI_LINK_6GHZ) == 0)))
#endif

static int slsi_update_retry_params(struct slsi_dev *sdev, struct net_device *dev, struct slsi_bssid_info *bssid_info,
				    struct slsi_ssid_info *ssid_info)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
#ifdef CONFIG_SCSC_WLAN_EHT
	...
	SLSI_ETHER_COPY(ndev_vif->sta.ap_mld_addr, bssid_info->mld_addr);
	if (is_valid_ether_addr(bssid_info->mld_addr)) {
		if (!bssid_info->mlo_ap) {
			r = slsi_fill_current_ap_detail(sdev, dev, &ssid_info->bssid_list, bssid_info);
			if (r < 0 || !bssid_info->mlo_ap)
				return r;
		}
		links = slsi_ml_connection_link_selection(dev, slsi_freq_to_band(bssid_info->freq / 2),
							  bssid_info, bssid_info->mlo_ap);
		ndev_vif->sta.mlo_band_control = find_first_bit((const unsigned long *)&links, 64) + 1;
		ndev_vif->sta.ml_connection = true;
	} else
#endif
	{
#ifdef CONFIG_SCSC_WLAN_EHT
		ndev_vif->sta.ml_connection = false;
#endif
		bssid_info->connect_attempted = true;
	}
				   ...
				   struct ieee80211_channel **channel, bool retry)
{
	...
	ssid_info = slsi_get_valid_ssid_info_from_list(dev);
	if (!ssid_info) {
		SLSI_WARN(sdev, "Failed to find matching ssid_info\n");
		return false;
	}
...
#endif
	{
		bssid_info = slsi_get_valid_bssid_info_from_list(sdev, dev, &ssid_info->bssid_list);
		if (!bssid_info) {
			SLSI_DBG1(sdev, SLSI_MLME, "No bssid to try for retry connection\n");
			return false;
		}
		r = slsi_update_retry_params(sdev, dev, bssid_info, ssid_info);
		if (r < 0) {
			SLSI_ERR(sdev, "Failed to update retry params: %d\n", r);
			return false;
		}
		return true;
	}
...
#ifdef CONFIG_SCSC_WLAN_EHT
	bssid_info = slsi_get_bssid_info_from_bssid(sdev, dev, &ssid_info->bssid_list, ndev_vif->sta.bssid);
	if (!bssid_info) {
		SLSI_DBG1(sdev, SLSI_MLME, "Failed to get bssid_info from list\n");
		return false;
	}
	if (!bssid_info->mlo_ap) {
		r = slsi_fill_current_ap_detail(sdev, dev, &ssid_info->bssid_list, bssid_info);
	...
	mlo_ap_details = bssid_info->mlo_ap;
	/* If all the 6G link combinations are tried and only the single link combination is left, choose between
	 * non-mlo boosted ETP and next valid bss's ETP. */
	if (!bssid_info->connect_attempted && (ndev_vif->chan->band == NL80211_BAND_6GHZ) &&
	    CHECK_TRIED_COMB(mlo_ap_details) && ((mlo_ap_details->tried_link_comb & SLSI_LINK_6GHZ) == 0)) {
		struct slsi_bssid_info *next_bssid_info;

		next_bssid_info = slsi_get_valid_bssid_info_from_list(sdev, dev, &ssid_info->bssid_list);
		if (!next_bssid_info)
			SLSI_DBG1(sdev, SLSI_MLME, "No bssids available next in the list\n");

		if (!next_bssid_info || (bssid_info->nboost_etp > next_bssid_info->etp)) {
			slsi_update_retry_params(sdev, dev, bssid_info, ssid_info);
			bssid_info->connect_attempted = true;
		} else {
			r = slsi_update_retry_params(sdev, dev, next_bssid_info, ssid_info);
			if (r < 0) {
				SLSI_ERR(sdev, "Failed to update retry params: %d\n", r);
				return false;
			}
		...
		if (!bssid_info->connect_attempted) {
			links = slsi_ml_connection_link_selection(dev, slsi_freq_to_band(bssid_info->freq / 2),
								  bssid_info, mlo_ap_details);
			ndev_vif->sta.mlo_band_control = find_first_bit((const unsigned long *)&links, 64) + 1;
			slsi_update_sta_sme(sdev, dev, ssid_info->ssid.ssid, ssid_info->ssid.ssid_len,
					    bssid_info->bssid,
					    ieee80211_get_channel(sdev->wiphy, (bssid_info->freq / 2)));
...
int slsi_set_latency_crt_data(struct net_device *dev, int latency_mode)
{
	...
	if (latency_mode == 0 && (host_state & SLSI_HOSTSTATE_LOW_LATENCY_ACTIVE) &&
		...
		set_host_state = 1;
#ifdef CONFIG_SCSC_WLAN_RX_NAPI_GRO
		latency_mode_stat = SLSI_LATENCY_MODE_DISABLED;
#endif
...
#ifdef CONFIG_SCSC_WLAN_RX_NAPI_GRO
		latency_mode_stat = SLSI_LATENCY_MODE_ENABLED;
#endif
	}
	...
	if (!ret) {
		...
		ndev_vif->gro_stat.latency_mode_stat = latency_mode_stat;
		slsi_dynamic_gro_ctrl(ndev_vif);
		write_unlock_bh(&ndev_vif->gro_lock);
		SLSI_INFO_NODEV("GRO %s (vif_index:%d, qdisc:%s, latency mode:%s)\n",
				ndev_vif->gro_enabled ? "enabled" : "disabled",
				ndev_vif->ifnum,
				ndev_vif->gro_stat.qdisc_stat ? "destroyed" : "created",
				ndev_vif->gro_stat.latency_mode_stat ? "disabled" : "enabled");
	}
#endif
	...
	if (set_scan_mode && ret == -EOPNOTSUPP) {
		ret = slsi_mlme_set_scan_mode_req(sdev, dev, scan_mode, scan_channel_time, home_away_time, home_time,
						  passive_time);
		if (ret != 0)
			SLSI_NET_ERR(dev, "Error in setting the Scan Mode, ret=%d\n", ret);
		else
...
void slsi_failure_reset(struct work_struct *work)
{
	...
	if (sdev->netdev_up_count == 0) {
		...
		if (work_pending(&sdev->recovery_work_on_start)) {
			SLSI_INFO(sdev, "Cancel system_error_user_fail_work for chip recovery!!\n");
			cancel_work_sync(&sdev->system_error_user_fail_work);
		}
	}
...
/* Retrieve max_chipset_log_size in sysfs global */
static ssize_t sysfs_show_max_log_size(struct kobject *kobj,
			      struct kobj_attribute *attr,
			      char *buf)
{
...
/* Update max_chipset_log_size in sysfs global */
static ssize_t sysfs_store_max_log_size(struct kobject *kobj,
			       struct kobj_attribute *attr,
			       const char *buf,
			       size_t count)
{
	...
	r = kstrtoint(buf, 10, &log_size);
	if (r < 0 || log_size < SLSI_MIN_LOG_SIZE || log_size > SLSI_MAX_LOG_SIZE)
		pr_err("Invalid ant value. Min:%d Max:%d\n", SLSI_MIN_LOG_SIZE, SLSI_MAX_LOG_SIZE);
	else
		sysfs_max_log_size = (u32)log_size;

	SLSI_INFO_NODEV("sysfs_max_log_size: %u\n", sysfs_max_log_size);

	return (r == 0) ? count : 0;
}

/* Register sysfs max_chipset_log_size */
void slsi_create_sysfs_max_log_size(void)
{
	...
	if (wifi_kobj_ref) {
		/* Create sysfs file /sys/wifi/max_chipset_log_size */
		r = sysfs_create_file(wifi_kobj_ref, &max_log_size_attr.attr);
		if (r) {
			/* Failed, so clean up dir */
			pr_err("Can't create /sys/wifi/max_chipset_log_size\n");
			return;
		}
	} else {
		pr_err("failed to create /sys/wifi/max_chipset_log_size\n");
	}
}

/* Unregister sysfs max_chipset_log_size */
void slsi_destroy_sysfs_max_log_size(void)
{
	if (!wifi_kobj_ref)
		return;

	/* Destroy /sys/wifi/max_chipset_log_size file */
	sysfs_remove_file(wifi_kobj_ref, &max_log_size_attr.attr);
...
int slsi_set_boolean_mib(struct slsi_dev *sdev, struct net_device *dev, u16 psid, bool val, int idx)
{
	...
	ret = slsi_mib_encode_bool(&mib_data, psid, val, idx);
	if (ret != SLSI_MIB_STATUS_SUCCESS) {
		SLSI_ERR(sdev, "psid:%d no mem for MIB\n", psid);
		ret = -ENOMEM;
		goto exit;
	}

	if (mib_data.dataLength == 0) {
		SLSI_ERR(sdev, "MIB data length NIL \n", psid);
		ret = -EINVAL;
		goto exit;
	}
	...
	ret = slsi_mlme_set(sdev, NULL, mib_data.data, mib_data.dataLength);
	if (ret)
		SLSI_ERR(sdev, "Setting Mib failed psid:%d, err: %d\n", psid, ret);
...
int slsi_get_ap_mld_addr(struct slsi_dev *sdev, const struct element *ml_ie, u8 *mld_addr)
{
	...
	if (ml_ie && ml_ie->datalen > SLSI_MIN_BASIC_ML_IE_COMMON_INFO_LEN) {
		ether_addr_copy(mld_addr, ml_ie->data + 4);
...
int slsi_send_bw_changed_event(struct slsi_dev *sdev, struct net_device *dev, u8 result)
{
	struct sk_buff    *skb = NULL;
	u8                err = 0;
	struct netdev_vif *ndev_vif = netdev_priv(dev);
...
#if (KERNEL_VERSION(4, 1, 0) <= LINUX_VERSION_CODE)
	skb = cfg80211_vendor_event_alloc(sdev->wiphy, &ndev_vif->wdev, NLMSG_DEFAULT_SIZE,
					  SLSI_NL80211_VENDOR_BW_CHANGED_EVENT, GFP_KERNEL);
#else
	skb = cfg80211_vendor_event_alloc(sdev->wiphy, NLMSG_DEFAULT_SIZE,
					  SLSI_NL80211_VENDOR_BW_CHANGED_EVENT, GFP_KERNEL);
#endif
	...
	err = nla_put_u8(skb, SLSI_VENDOR_ATTR_BW_CHANGED, result);
	if (err) {
		SLSI_ERR_NODEV("Failed nla_put err=%d\n", err);
		kfree_skb(skb);
		return -EINVAL;
	}
	SLSI_DBG1(sdev, SLSI_CFG80211, "Event: SLSI_NL80211_VENDOR_BW_CHANGED_EVENT\n");
	cfg80211_vendor_event(skb, GFP_KERNEL);
