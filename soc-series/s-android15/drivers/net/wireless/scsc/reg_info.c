...
void slsi_regd_init_wiphy_not_registered(struct slsi_dev *sdev)
{
	...
	for (band = 0; band < NUM_NL80211_BANDS; band++) {
		...
		for (i = 0; i < sband->n_channels; i++) {
			...
			if (index != -1) {
				start_freq_khz = slsi_world_regdom_custom->reg_rules[index].freq_range.start_freq_khz;
				end_freq_khz = slsi_world_regdom_custom->reg_rules[index].freq_range.end_freq_khz;
				max_bandwidth_khz = slsi_world_regdom_custom->reg_rules[index].freq_range.max_bandwidth_khz;
				if (((index + 1) < slsi_world_regdom_custom->n_reg_rules) &&
				    end_freq_khz == slsi_world_regdom_custom->reg_rules[index + 1].freq_range.start_freq_khz &&
				    slsi_world_regdom_custom->reg_rules[index + 1].flags & NL80211_RRF_AUTO_BW &&
				    slsi_world_regdom_custom->reg_rules[index].flags & NL80211_RRF_AUTO_BW)
					max_bandwidth_khz += slsi_world_regdom_custom->reg_rules[index + 1].freq_range.max_bandwidth_khz;
				if (index > 0 &&
				    slsi_world_regdom_custom->reg_rules[index - 1].freq_range.end_freq_khz  == start_freq_khz &&
				    slsi_world_regdom_custom->reg_rules[index - 1].flags & NL80211_RRF_AUTO_BW &&
				    slsi_world_regdom_custom->reg_rules[index].flags & NL80211_RRF_AUTO_BW)
					max_bandwidth_khz += slsi_world_regdom_custom->reg_rules[index - 1].freq_range.max_bandwidth_khz;
...
/* Get country index from sdev, if not valid return '00' */
int slsi_regd_get_country_index(struct slsi_dev *sdev, char *alpha2) {
	int i;
	...
	for (i = 0; i < sdev->regdb.num_countries; i++) {
		if ((sdev->regdb.country[i].alpha2[0] == '0') &&
		    (sdev->regdb.country[i].alpha2[1] == '0')) {
			return i;
		}
	}
