...
static int bp_log = LOG_FILTER_TX_FAILURE | LOG_FILTER_SHOW_COD | LOG_FILTER_TX_NAPI;
...
static int bp_min_garunteed_mod = 5;
module_param(bp_min_garunteed_mod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(bp_min_garunteed_mod, "Make sure adaptive MOD does fall below this value");

static bool disable_adaptive_mod = false;
module_param(disable_adaptive_mod, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(disable_adaptive_mod, "Disable adaptive MOD");
#endif

#ifdef SCSC_WLAN_PRE_AGGREGATE_AMSDU
static bool pre_amsdu_enabled = true;
module_param(pre_amsdu_enabled, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(pre_amsdu_enabled, "make amsdu samll packet");

static int aggregation_timeout = 2;
module_param(aggregation_timeout, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(aggregation_timeout, "pre-aggregation timeout (ms)");

struct subframe_list subframelist;
static void init_subframe_list(void);
static void deinit_subframe_list(void);
#endif

#if (defined(CONFIG_SCSC_QOS) && !defined(CONFIG_SCSC_WLAN_HIP5))
static bool set_lower_qos_threshold_for_nan = true;
module_param(set_lower_qos_threshold_for_nan, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(set_lower_qos_threshold_for_nan, "set lower qos threshold for nan");
#endif
...
{
	if (WLBT_WARN_ON(!sdev) || WLBT_WARN_ON(!tx_priv) || WLBT_WARN_ON(!gcod) ||
	    WLBT_WARN_ON(!netdev_cod) || WLBT_WARN_ON(!ac_cod))
		return -EINVAL;
...
static __always_inline int slsi_txbp_has_resource(struct tx_netdev_data *tx_priv)
{
	...
	/* Netdev domain */
#ifdef CONFIG_SCSC_WLAN_ADAPTIVE_TX_MOD
	if (disable_adaptive_mod || txbp_priv.vif_cnt == 1) {
		mod /= txbp_priv.vif_cnt;
	} else {
		mod = (mod * (long)atomic_read(&tx_priv->backlog)) / atomic_read(&txbp_priv.backlog);
		if (bp_min_garunteed_mod && mod < bp_min_garunteed_mod)
			mod = bp_min_garunteed_mod;
	}
#else
	mod /= txbp_priv.vif_cnt;
#endif
...
static int slsi_txbp_napi(struct napi_struct *napi, int budget)
{
	...
	for (ac = SLSI_TRAFFIC_Q_VO; ac > -1; ac--) {
		...
		while (ac_budget_limit[ac]) {
			...
			/* NOTE: peer_index 0 is not invalid it is used for M/B cast*/
			...
			if (peer_index >= SLSI_PEER_INDEX_MIN && peer_index <= SLSI_PEER_INDEX_MAX) {
				...
				if (!peer || !peer->valid) {
					SLSI_NET_ERR(tx->ndev,
						     "Drop skb with invalid peer_index:%d peer:%p valid:%d\n",
						     peer_index, peer, (peer)? peer->valid : -1);
					kfree_skb(skb);
					continue;
				}
			}
			...
			if (ret != NETDEV_TX_OK) {
				...
				SLSI_DBG3(sdev, SLSI_TX, "BP: transmit frame fail with ret:%d\n", ret);
				show_cod(tx_priv);
				if (bp_log & LOG_FILTER_TX_FAILURE)
					SLSI_NET_ERR(tx->ndev, "BP: Tx fail remain budget %u\n", ac_budget_limit[ac]);
				...
				tx_priv->ac_cod[ac]++;
#ifdef CONFIG_SCSC_WLAN_ADAPTIVE_TX_MOD
				atomic_dec(&txbp_priv.backlog);
				atomic_dec(&tx_priv->backlog);
#endif
...
tx_done:
	...
	if (work_done < budget) {
		...
		if (bp_log & LOG_FILTER_TX_NAPI)
			SLSI_DBG3(sdev, SLSI_TX, "BP: %s work_done %d\n", tx->ndev->name, work_done);
	}
...
void slsi_dev_attach_post(struct slsi_dev *sdev)
{
	...
	txbp_priv.peers = 0;
#ifdef CONFIG_SCSC_WLAN_ADAPTIVE_TX_MOD
	atomic_set(&txbp_priv.backlog, 0);
#endif
#ifdef CONFIG_SCSC_WLAN_LOAD_BALANCE_MANAGER
	bp_fixed_cpu = -1;
#else
...
{
	/* increase device tx queue to 4000 */
	dev->tx_queue_len = 4000;
}
...
#if (defined(CONFIG_SCSC_QOS) && !defined(CONFIG_SCSC_WLAN_HIP5))
__always_inline void slsi_ndp_vif_activated_post(struct slsi_dev *sdev, struct net_device *dev, struct netdev_vif *ndev_vif)
{
	struct tx_netdev_data *tx_priv = (struct tx_netdev_data *)ndev_vif->tx_netdev_data;

	if (!set_lower_qos_threshold_for_nan)
		return;
	if (!tx_priv) {
		if (atomic_read(&sdev->nan_ndp_in_use) == 0) {
			sdev->saturation_restore_val = slsi_get_saturation_trigger_threshold();
			slsi_set_saturation_trigger_threshold(1);
			sdev->nan_qos_client = slsi_traffic_mon_client_register(sdev, &sdev->hip,
                                                                             TRAFFIC_MON_CLIENT_MODE_EVENTS,
                                                                             SLSI_TRAFFIC_MON_MBPS_TO_BYTES_PER_SEC(30),
                                                                             SLSI_TRAFFIC_MON_MBPS_TO_BYTES_PER_SEC(50),
                                                                             TRAFFIC_MON_DIR_DEFAULT, hip4_traffic_monitor_cb);
		}
		atomic_inc(&sdev->nan_ndp_in_use);
	}
}
#endif
...
__always_inline bool slsi_vif_activated_post(struct slsi_dev *sdev, struct net_device *dev, struct netdev_vif *ndev_vif)
{
	...
	if (ndev_vif->ifnum >= SLSI_NAN_DATA_IFINDEX_START)
		slsi_ndp_vif_activated_post(sdev, dev, ndev_vif);
#endif
	tx_priv = (struct tx_netdev_data *)ndev_vif->tx_netdev_data;
	if (tx_priv)
		return true;
...
#if (defined(CONFIG_SCSC_QOS) && !defined(CONFIG_SCSC_WLAN_HIP5))
__always_inline void slsi_ndp_vif_deactivated_post(struct slsi_dev *sdev)
{
	if (!set_lower_qos_threshold_for_nan)
		return;

	if (atomic_dec_return(&sdev->nan_ndp_in_use) <= 0) {
		atomic_set(&sdev->nan_ndp_in_use, 0);
		slsi_set_saturation_trigger_threshold(sdev->saturation_restore_val);
		if (sdev->nan_qos_client) {
			slsi_traffic_mon_client_unregister(sdev, sdev->nan_qos_client);
			sdev->nan_qos_client = NULL;
			SLSI_ERR_NODEV("nan_qos_client set to: NULL\n");
		} else {
			SLSI_ERR_NODEV("nan_qos_client is NULL\n");
		}
	}
}
#endif
...
#ifdef CONFIG_SCSC_WLAN_ADAPTIVE_TX_MOD
	netdev_backlog = atomic_sub_return(atomic_read(&tx_priv->backlog), &txbp_priv.backlog);
	if (unlikely(netdev_backlog < 0)) {
		SLSI_NET_ERR(dev, "ERR:(%s)global backlog:%d, netdev backlog:%d\n", dev->name, netdev_backlog,
			     atomic_read(&tx_priv->backlog));
	}
#endif
...
#if (defined(CONFIG_SCSC_QOS) && !defined(CONFIG_SCSC_WLAN_HIP5))
	if (ndev_vif->ifnum >= SLSI_NAN_DATA_IFINDEX_START)
		slsi_ndp_vif_deactivated_post(sdev);
#endif
	return true;
}
...
__always_inline void slsi_tx_completed_queue(struct netdev_queue *dev_queue,
					    unsigned int pkts, unsigned int bytes, u16 host_tag)
{
	int num_queued, num_completed;

        num_queued = READ_ONCE(dev_queue->dql.num_queued);
        num_completed = READ_ONCE(dev_queue->dql.num_completed);

	/* Can't complete more than what's in queue */
	if (num_queued - num_completed <= 0) {
		SLSI_WARN_NODEV("it is already completed(host_tag:%d)\n", host_tag);
		return;
	}

	netdev_tx_completed_queue(dev_queue, 1, 1);
	return;
}
...
__always_inline int slsi_tx_mlme_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	if ((ndev_vif->vif_type == FAPI_VIFTYPE_AP) && (host_tag & SLSI_HOST_TAG_VLAN_ID_MASK)) {
	...
	slsi_tx_completed_queue(&dev_to_send->_tx[SLSI_TX_ARP_Q_INDEX], 1, 1, host_tag);
...
static __always_inline void slsi_txbp_schedule_napi(struct net_device *dev, struct tx_netdev_data *tx_priv)
{
	...
	if (tx_info->target_cpu == smp_processor_id()) {
		if (!test_and_set_bit(SLSI_TX_NAPI_POLLING, &tx_info->napi_state)) {
			if (bp_log & LOG_FILTER_TX_NAPI)
				SLSI_DBG3(sdev, SLSI_TX, "BP: schedule tx on current cpu %u\n", tx_info->target_cpu);
		...
		if (cpu_online(tx_info->target_cpu)) {
			if (!test_and_set_bit(SLSI_TX_NAPI_POLLING, &tx_info->napi_state)) {
				if (bp_log & LOG_FILTER_TX_NAPI)
					SLSI_DBG3(sdev, SLSI_TX, "BP: schedule tx on cpu %u\n", tx_info->target_cpu);
...
#ifdef CONFIG_SCSC_WLAN_ADAPTIVE_TX_MOD
	atomic_inc(&txbp_priv.backlog);
	atomic_inc(&tx_priv->backlog);
#endif
							 ...
							 struct sk_buff *skb)
{
	...
	if (ndev_vif->iftype == NL80211_IFTYPE_AP_VLAN) {
		...
		rcu_read_lock();
		vif_index = ((struct netdev_vif *)netdev_priv(ndev_vif->netdev_ap))->vifnum;
		rcu_read_unlock();
	} else {
...
static __always_inline int slsi_tx_transmit_normal(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	if (slsi_skb_cb_get(skb)->is_amsdu)
		fapi_set_u16(skb, u.ma_unitdata_req.configuration_option, FAPI_OPTION_DELAYED | FAPI_OPTION_INLINE);
	else
		fapi_set_u16(skb, u.ma_unitdata_req.configuration_option, FAPI_OPTION_INLINE);
...
#endif
#ifdef SCSC_WLAN_PRE_AGGREGATE_AMSDU
	else if (pre_amsdu_enabled && aggregate_amsdu_packets(sdev, dev, skb) == NETDEV_TX_OK)
		ret = NETDEV_TX_OK;
#endif
	else
...
int slsi_tx_done(struct slsi_dev *sdev, u32 colour, bool more)
{
	...
	ifnum = SLSI_MBULK_COLOUR_GET_IFNUM(colour);
	ac = SLSI_MBULK_COLOUR_GET_AC(colour);
...
#ifdef SCSC_WLAN_PRE_AGGREGATE_AMSDU
static void pre_amsdu_timeout_handler(struct timer_list *timer)
{
	struct subframe_list *subframelist = from_timer(subframelist, timer, timer);

	schedule_work(&subframelist->timeout_work);
}

static struct sk_buff *merge_packets_in_list(struct subframe_list *frame_l)
{
	struct sk_buff *skb, *pos, *next;
	struct msduhdr *mhdr;
	int len, count = 0;

	if (list_empty(&frame_l->list))
		return NULL;

	if (frame_l->count == 1) {
		pos = list_first_entry(&frame_l->list, struct sk_buff, list);
		list_del(&pos->list);
		del_timer(&subframelist.timer);
		INIT_LIST_HEAD(&subframelist.list);
		subframelist.len = 0;
		subframelist.count = 0;
		subframelist.first = NULL;
		subframelist.dev = NULL;

		return pos;
	}

	skb = alloc_skb(frame_l->len + 250, GFP_ATOMIC);
	skb_reserve(skb, 250);

	list_for_each_entry_safe(pos, next, &frame_l->list, list) {
		if (frame_l->first == pos) {
			skb->tstamp = pos->tstamp;
			skb->dev = pos->dev;
			memcpy(skb->cb, pos->cb, sizeof(pos->cb));
			skb_dst_copy(skb, pos);
			__skb_ext_copy(skb, pos);
			__nf_copy(skb, pos, false);
			if (!skb_mac_header_was_set(skb))
				skb_reset_mac_header(skb);
		}

		mhdr = msdu_hdr(pos);
		len = MSDU_HLEN + htons(mhdr->len) - LLC_SNAP_HDR_LEN;
		SLSI_DBG4_NODEV("plen %d clen %d\n", pos->len, len);
		skb_put_data(skb, (void *)mhdr, len);
		if ((++count < frame_l->count) && len % 4)
			skb_put_zero(skb, (4 - (len % 4)) & 0x3);
	...
	slsi_skb_cb_get(skb)->is_amsdu = true;

	return skb;
}

static void pre_amsdu_timeout_work(struct work_struct *work)
{
	struct subframe_list *subframelist = container_of(work, struct frame_list, timeout_work);
	struct sk_buff *skb_merged = NULL;
	struct net_device *dev;
	struct netdev_vif *ndev_vif;
	struct slsi_dev *sdev;

	if (!subframelist || !subframelist->dev) {
		SLSI_INFO_NODEV("subframelist:0x%x dev:0x%x\n", subframelist, subframelist->dev);
		return;
	}

	dev = subframelist->dev;
	ndev_vif = netdev_priv(dev);

	if (!ndev_vif || !ndev_vif->sdev) {
		SLSI_INFO_NODEV("ndev_vif:0x%x sdev:0x%x\n", ndev_vif, ndev_vif->sdev);
		return;
	}

	sdev = ndev_vif->sdev;

	spin_lock_bh(&subframelist->list_lock);
	skb_merged = merge_packets_in_list(subframelist);
	spin_unlock_bh(&subframelist->list_lock);
	if (skb_merged)
		slsi_tx_transmit_normal(sdev, dev, skb_merged);
}
...
static void init_subframe_list(void)
{
	...
	INIT_WORK(&subframelist.timeout_work, pre_amsdu_timeout_work);
	spin_lock_init(&subframelist.list_lock);
	timer_setup(&subframelist.timer, pre_amsdu_timeout_handler, 0);
}
...
static void deinit_subframe_list(void)
{
	...
	del_timer_sync(&subframelist.timer);
	cancel_work_sync(&subframelist.timeout_work);
}

static void aggregate_list_prepare(const struct list_head *head, const struct sk_buff *skb)
{
	struct sk_buff *pos, *next;
	struct msduhdr *mhdr, *mhdr1;
	...
	list_for_each_entry_safe(pos, next, head, list) {
		...
		mhdr1 = msdu_hdr(pos);
		diffs = (unsigned long)pos->dev ^ (unsigned long)skb->dev;
		diffs |= compare_ether_addr(mhdr1->h_dest, mhdr->h_dest) |
			 compare_ether_addr(mhdr1->h_source, mhdr->h_source);

		NAPI_GRO_CB(skb)->same_flow = !diffs;
		NAPI_GRO_CB(skb)->flush = (skb->len > 700);
	}

	spin_unlock_bh(&subframelist.list_lock);
}

static int aggregate_amsdu_packets(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	struct sk_buff *skb_merged = NULL;
	struct msduhdr *mhdr = msdu_hdr(skb);
	struct slsi_peer *peer;
	int len;
	int proto;
	int more;

	proto = htons(mhdr->type);
	if (proto == ETH_P_ARP)
		return -EINVAL;

	peer = slsi_get_peer_from_mac(sdev, dev, mhdr->h_dest);

	if (!peer || !peer->support_amsdu)
		return -EINVAL;

	len = MSDU_HLEN + htons(mhdr->len) - LLC_SNAP_HDR_LEN;

#if (KERNEL_VERSION(5, 2, 0) <= LINUX_VERSION_CODE)
	more = netdev_xmit_more();
#else
	more = skb->xmit_more;
#endif
	SLSI_DBG3(sdev, SLSI_TX, "more =%d\n", more);
	aggregate_list_prepare(&subframelist.list, skb);

	spin_lock_bh(&subframelist.list_lock);
	if (list_empty(&subframelist.list)) {
		list_add_tail(&skb->list, &subframelist.list);
		subframelist.len += len;
		subframelist.count++;
		subframelist.first = skb;
		subframelist.dev = dev;
		mod_timer(&subframelist.timer, jiffies + msecs_to_jiffies(aggregation_timeout));
	} else {
		if (!NAPI_GRO_CB(skb)->same_flow || NAPI_GRO_CB(skb)->flush)
			skb_merged = merge_packets_in_list(&subframelist);
		else if (subframelist.len + len > 1448 || subframelist.count >= 3)
			skb_merged = merge_packets_in_list(&subframelist);
