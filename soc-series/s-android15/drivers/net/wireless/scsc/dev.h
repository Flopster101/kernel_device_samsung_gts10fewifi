...
#define SLSI_FW_API_RATE_PUNCTURED_IDX      (0x1f << 27)
...
/* TODO: FAPI update for reason code 8 in slsi_rx_twt_teardown_indication */
#define FAPI_REASONCODE_SCAN_TRIGGERED 0xa008
...
#define SLSI_MAX_KEEP_ALIVE_INTERVAL          255
#define SLSI_MIN_KEEP_ALIVE_INTERVAL          5
...
static inline struct msduhdr *msdu_hdr(const struct sk_buff *skb)
{
	return (struct msduhdr *)skb_mac_header(skb);
}
...
struct slsi_bssid_info {
	...
	struct slsi_mlo_ap_details *mlo_ap;
#endif
};
#endif
...
enum slsi_link_combinations {
	SLSI_LINK_AUTO = 0,
	SLSI_LINK_24GHZ = BIT(0),
	SLSI_LINK_5GHZ = BIT(1),
	SLSI_LINK_24GHZ_5GHZ = BIT(2),
	SLSI_LINK_6GHZ = BIT(3),
	SLSI_LINK_24GHZ_6GHZ = BIT(4),
	SLSI_LINK_5GHZ_6GHZ = BIT(5),
	SLSI_LINK_24GHZ_5GHZ_6GHZ = BIT(6)
};

struct slsi_mlo_ap_details {
	u8 num_links;
	u8 bands;
...
struct slsi_vif_sta {
	...
	struct {
		...
		u16 cfreq1;
		int width;
		struct slsi_dynamic_bw     dynamic_bw;
...
struct nan_pairing_response_req {
	u16 session_id;
	u16 match_id;
	u8 peer_disc_mac_addr[ETH_ALEN];
};

struct nan_pairing_request_ind {
	u8 peer_nmi_address[ETH_ALEN];
	u16 session_id;
};

struct nan_pairing_response_ind {
	u8 peer_nmi_address[ETH_ALEN];
	u16 session_id;
};

struct nan_pairing_confirm_ind {
...
struct slsi_vif_nan {
	...
	DECLARE_BITMAP(bootstrapping_instance_id_map, SLSI_NAN_MAX_BOOTSTRAPPING_INSTANCES);
	DECLARE_BITMAP(pairing_instance_id_map, SLSI_NAN_MAX_PAIRING_INSTANCES);
	u32 followup_id_map;
	...
	u8 pairing_id_peer_mac[SLSI_NAN_MAX_PAIRING_INSTANCES][ETH_ALEN];
	int nan_enable_pairing_cache;
	int nan_enable_pairing_setup;
	struct nan_pairing_response_req pairing_response_req;
	struct nan_pairing_confirm_ind pairing_confirm_ind;
	struct nan_pairing_request_ind pairing_request_ind;
	struct nan_pairing_response_ind pairing_response_ind;
	int responder;
...
struct netdev_vif {
	...
	void			     *rps_affinity_client;
#else
	void			     *traffic_mon_client
#endif
#ifdef CONFIG_SCSC_WLAN_ANDROID
	struct slsi_wake_lock                   wlan_wl_sae;
#endif
	int grace_period;
	bool sap_ax_mode;
	bool p2p_ax_mode;
...
struct slsi_tas_info {
	...
	struct slsi_wake_lock wlan_wl_tas;
...
struct slsi_dev {
	...
	struct slsi_wake_lock			wlan_wl_mlme_evt;
	struct slsi_wake_lock			wlan_wl_mlme;
	struct slsi_wake_lock			wlan_wl_ma;
	struct slsi_wake_lock			wlan_wl_roam;
	struct slsi_wake_lock			wlan_wl_init;
	struct slsi_wake_lock			wlan_wl_tx_sched;
#endif
	struct slsi_sig_send       sig_wait;
 ...
 */
	...
	u32                        sta_last_connected_chan_freq;
        int                        fw_keepalive_time;
	void			   *traffic_mon_client;
#if (defined(CONFIG_SCSC_QOS) && !defined(CONFIG_SCSC_WLAN_HIP5))
        void                       *nan_qos_client;
	atomic_t                   nan_ndp_in_use;
	u8                         saturation_restore_val;
