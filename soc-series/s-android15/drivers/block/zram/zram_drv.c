...
static const char *default_compressor = CONFIG_VENDOR_ZRAM_DEF_COMP;
...
#ifdef CONFIG_VENDOR_ZRAM_MULTI_COMP
				 ...
				 u32 index)
{
	...
	if (size == PAGE_SIZE) {
		...
		dst = kmap_atomic(page);
		ret = zcomp_decompress(zstrm, src, size, dst, page);
		kunmap_atomic(dst);
	...
	if (unlikely(ret)) {
		pr_err("%s Decompression failed! err=%d, index=%u, len=%u, vaddr=0x%px\n",
			zram->comps[prio]->name, ret, index, size, src);
