...
static void ufs_sec_set_unique_number(struct ufs_hba *hba, u8 *desc_buf)
{
	...
	snprintf(vdi->unique_number, UFS_UN_MAX_DIGITS,  "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
			manid,
...
EXPORT_SYMBOL_GPL(ufs_sec_get_health_desc);
...
static inline int ufs_sec_is_host_available(struct ufs_hba *hba)
{
	...
	if (hba->pm_op_in_progress || hba->is_sys_suspended) {
		pr_err("%s: pm_op_in_progress or suspended.\n", __func__);
		return -EBUSY;
	}
...
static void ufs_sec_print_cmdlog(struct ufs_hba *hba)
{
	...
	dev_err(hba->dev, "%2s: %10s: %2s %3s %4s %9s %6s %16s\n",
...
void ufs_sec_inc_hwrst_cnt(void)
{
	...
	if ((op_cnt->HW_RESET_cnt % 3) == 0)
		ufs_sec_vops_abc_event(vdi, "MODULE=storage@WARN=ufs_hwreset_err");
}
		...
		struct ufs_sec_cmd_info *ufs_cmd)
{
	...
	secdbgMode = ufs_sec_vops_mode_enter_upload(vdi);
	...
	if (sense_key == MEDIUM_ERROR) {
		...
		ufs_sec_vops_abc_event(vdi, "MODULE=storage@WARN=ufs_medium_err");
	} else {
		sense_err->scsi_hw_err++;
		ufs_sec_vops_abc_event(vdi, "MODULE=storage@WARN=ufs_hardware_err");
	}
...
static void ufs_sec_check_hist_on(char *buf)
{
	if (get_vdi_member(hist_on))
		SEC_UFS_ERR_HIST_SUM(buf);
	else
		sprintf(buf, "No write hist\n");
}
...
void ufs_sec_print_err(void)
{
	...
	ufs_sec_check_hist_on(hist_buf);
		...
		unsigned long event, void *panic_msg)
{
	...
	ufs_sec_vops_exin_ufs(vdi, err_buf);
...
void ufs_sec_register_callback(const struct ufs_sec_variant_ops *vops)
{
	...
	if (vdi->hba)
		ufs_sec_add_sysfs_nodes(vdi->hba);

	if (sec_ufs_node_dev)
		sec_ufs_node_dev->type = &ufs_type;
}
EXPORT_SYMBOL_GPL(ufs_sec_register_callback);
