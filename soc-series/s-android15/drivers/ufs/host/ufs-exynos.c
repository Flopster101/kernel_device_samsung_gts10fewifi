...
/* is sense data present in response UPIU valid? */
#define IS_VALID_SENSE_DATA (1 << 7)

/* ASC and ASCQ code for Illegal request,invalid field in CDB */
...
static inline void __pm_qos_ctrl(struct exynos_ufs *ufs, bool op)
{
	...
	s32 int_value = ufs->pm_qos_int_value[pmd->gear];
	s32 val = (op) ? int_value : 0;
...
{
#if !IS_ENABLED(CONFIG_SCSI_UFS_EMUL)
...
#if IS_ENABLED(CONFIG_SCSI_UFS_DBG)
	exynos_ufs_dump_info(hba, handle, ufs->dev);
	ret = exynos_ufs_check_error(handle);
#endif
...
#ifndef CONFIG_SCSI_UFS_EXYNOS_BLOCK_WDT_RST
	if (ret || (hba->saved_err & SYSTEM_BUS_FATAL_ERROR))
...
static void exynos_ufs_set_features(struct ufs_hba *hba)
{
	...
	if (ufs->mcq_not_supported == 1)
		hba->quirks |= UFSHCD_QUIRK_BROKEN_UFS_HCI_VERSION;

}

bool ufs_is_suspend_prepare(struct exynos_ufs *ufs)
{
	return ufs->pm_state == UFS_PM_SUSPEND_PREPARE;
}
...
#if IS_ENABLED(CONFIG_SCSI_UFS_PERF)
	if (!ufs_perf_init(&ufs->perf, ufs->hba))
		dev_err(ufs->dev, "Not enable UFS performance mode\n");
#endif

	ufs->pm_state = UFS_PM_NORMAL;
	hba->ahit = ufs->ah8_ahit;

	return 0;
}

static u32 exynos_ufs_get_hci_version(struct ufs_hba *hba)
{
	u32 ufshci_ver;
	struct exynos_ufs *ufs = to_exynos_ufs(hba);
	struct ufs_vs_handle *handle = &ufs->handle;

	ufshci_ver = hci_readl(handle, REG_UFS_VERSION);
...
static void exynos_mcq_ia_config(struct ufs_hba *hba)
{
	...
	static struct exynos_ia_group ia_table[] = {
		...
		{0, 0x4},
		{0, 0x5},
		{MCQIACR_IAEN | MCQIACR_IAPWEN | MCQIACR_IACTH | MCQIACR_OVAL_T,
			0x106},
		{MCQIACR_IAEN | MCQIACR_IAPWEN | MCQIACR_IACTH | MCQIACR_OVAL_T,
			0x107},
		{MCQIACR_IAEN | MCQIACR_IAPWEN | MCQIACR_IACTH | MCQIACR_OVAL_T,
...
static inline void __copy_dev_info(struct ufs_hba *hba) {
#if !defined(CONFIG_SOC_S5E9945)
	struct exynos_ufs *ufs = to_exynos_ufs(hba);

	ufs->cal_param.vendor = hba->dev_info.wmanufacturerid;
	memcpy(ufs->cal_param.model, hba->dev_info.model,
	       strlen(hba->dev_info.model));
#endif
}
...
static void exynos_ufs_check_sense_data(struct ufs_hba *hba,
				struct ufshcd_lrb *lrbp)
{
	u32 result = 0;
	u32 asc = 0;
	...
	result = be32_to_cpu(lrbp->ucd_rsp_ptr->header.dword_1);
	if (result & SAM_STAT_CHECK_CONDITION) {
		if ( lrbp->ucd_rsp_ptr->sr.sense_data[0] & IS_VALID_SENSE_DATA ) {
			sense_key = lrbp->ucd_rsp_ptr->sr.sense_data[2] & 0x0F;
			asc = lrbp->ucd_rsp_ptr->sr.sense_data[12];
			ascq = lrbp->ucd_rsp_ptr->sr.sense_data[13];
			if (sense_key == ILLEGAL_REQUEST) {
				if ( asc == ASC_INVALID_FIELD_IN_CDB &&
						ascq == ASCQ_INVALID_FIELD_IN_CDB ) {
					pr_info("%s: task %d, Illegal Req,invalid field in cdb.\n",
...
static void exynos_ufs_compl_nexus_t_xfer_req(void *data, struct ufs_hba *hba,
				struct ufshcd_lrb *lrbp)
{
#if IS_ENABLED(CONFIG_SCSI_UFS_DBG) || IS_ENABLED(CONFIG_SCSI_UFS_PERF)
	struct exynos_ufs *ufs = to_exynos_ufs(hba);
	struct ufs_vs_handle *handle = &ufs->handle;
#endif
	int tag;
	...
	/*
	 * After command completion check sense data
	 * for dumping logs and processing errors if required
	 */
	exynos_ufs_check_sense_data(hba, lrbp);
...
static inline void __ufshcd_schedule_eh_work(struct ufs_hba *hba)
{
	bool is_saved_err_fatal = false;
...
#endif
	/* IS.UTPES (bit 12) */
	if (hba->errors & 0x1000)
		is_fatal = true;

	/* imitate ufshcd_check_errors */
	if (is_fatal && !queue_eh_work) {
		hba->force_reset = true;
		hba->saved_err |= hba->errors;
		hba->saved_uic_err |= hba->uic_error;
		__ufshcd_schedule_eh_work(hba);
	}
}
...
resume:
	...
	unregister_pm_notifier(&ufs->pm_nb);
...
#if IS_ENABLED(CONFIG_SCSI_UFS_DBG)
	/* cmd_logging */
	exynos_ufs_cmd_log_start(handle, hba, scmd, hwq_num);
#endif
	/* perf */
	if (scmd) {
		...
		if (ufs->perf) {
			...
			chunk_size = (op == UFS_PERF_OP_R ||
					op == UFS_PERF_OP_W) ?
				rq->__data_len : 0;
#if IS_ENABLED(CONFIG_SCSI_UFS_PERF)
			if (ufs->pm_state != UFS_PM_SUSPEND_PREPARE) {
				ufs_perf_update(ufs->perf, chunk_size, lrbp, op);
			}
#endif
		}
...
static void exynos_ufs_fill_prdt(void *data, struct ufs_hba *hba, struct ufshcd_lrb *lrbp,
		unsigned int segments, int *err)
{
#if IS_ENABLED(CONFIG_ARM64_16K_PAGES)
	struct scsi_cmnd *cmd = lrbp->cmd;
	struct ufshcd_sg_entry *prd;
	struct scatterlist *sg_list = scsi_sglist(cmd);
	struct scatterlist *sg;
	int i, j;
	int multiple, prd_size;
	dma_addr_t dma_addr;

	if (segments) {
		if (hba->quirks & UFSHCD_QUIRK_PRDT_BYTE_GRAN)
			lrbp->utr_descriptor_ptr->prd_table_length =
				cpu_to_le16(segments * ufshcd_sg_entry_size(hba));
		else
			lrbp->utr_descriptor_ptr->prd_table_length = cpu_to_le16(segments);

		prd = lrbp->ucd_prdt_ptr;
		lrbp->utr_descriptor_ptr->prd_table_length = cpu_to_le16(0);

		for_each_sg(sg_list, sg, segments, i) {
			...
			prd_size = cpu_to_le32(((u32) sg_dma_len(sg)) - 1);
			multiple = (prd_size / SZ_4K) + 1;

			for (j = 0; j < multiple; j++) {
				if (prd_size >= SZ_4K) {
					prd->size = cpu_to_le32((u32) SZ_4K - 1);
					prd_size -= SZ_4K;
				} else {
					prd->size = prd_size;
				}

				dma_addr = sg->dma_address + (j * SZ_4K);
				prd->addr = cpu_to_le64(dma_addr);
				prd->reserved = 0;
				prd = (void *)prd + ufshcd_sg_entry_size(hba);
			}

			lrbp->utr_descriptor_ptr->prd_table_length += cpu_to_le16((u16) multiple);
		}
	} else {
...
static void exynos_ufs_register_vendor_hooks(void)
{
	...
	register_trace_android_vh_ufs_fill_prdt(exynos_ufs_fill_prdt, NULL);
#if IS_ENABLED(CONFIG_SEC_UFS_FEATURE)
...
static irqreturn_t exynos_vendor_mcq_cq_events(struct ufs_hba *hba, int irq)
{
	...
	if (exynos_soc_info.main_rev) {
		...
		reg = readl(mcq_opr_base(hba, OPR_CQIS, num) + REG_MCQIACR);
		reg |= MCQIACR_CTR;
		writel(reg, mcq_opr_base(hba, OPR_CQIS, num) + REG_MCQIACR);
...
#if defined(CONFIG_SOC_S5E9945)
	/*
	 * This is related to using IAG. In this case,
	 * HW queue order is meaningful and we don't use the HWQ
	 * dedicated to polling path. So, remove the number of HWQ
	 * for polling. Instead, put it to the head for another use.
	 */
...
static struct ufs_hba_variant_ops exynos_ufs_ops = {
	...
	.get_ufs_hci_version = exynos_ufs_get_hci_version,
					...
					struct exynos_ufs *ufs)
{
	...
	cxt = &ufs->cxt_phy_pwr_off;
	cxt_size = of_property_read_variable_u32_array(dev->of_node,
			"ufs,phy_pwr_off", (u32 *)cxt, read_size, read_size);
	if (cxt_size) {
		dev_info(dev, "%s: enable ufs-phy-pwr-off during sleep\n", __func__);
		exynos_pmu_update(cxt->offset, cxt->mask, cxt->val);
	}
...
static int exynos_ufs_populate_dt(struct device *dev, struct exynos_ufs *ufs)
{
	...
	if (!child_np)
		...
		u32 idx_gear = UFS_HS_G1;
		u32 len_table_dt = of_property_count_u32_elems(child_np, "freq-int");
		u32 len_table = min_t(u32, len_table_dt, LEN_OF_INT_TABLE);
		u32 val[LEN_OF_INT_TABLE];

		of_property_read_u32_array(child_np, "freq-int", val, len_table);
		for (i = 0; i < len_table; i++) {
			ufs->pm_qos_int_value[idx_gear] = (s32)val[i];
			if (val[i])
				pr_info("%s: INT val(HS-G%u) = %u\n",
					__func__, idx_gear, val[i]);
			idx_gear++;
		}
	}
...
#if !defined(CONFIG_SOC_S5E9945)
	ufs->cal_param.need_ovrd_vnd_cal = 0;
	of_property_read_u8(np, "need-override-vendor-cal",
			     &ufs->cal_param.need_ovrd_vnd_cal);
#endif
	...
	if (of_find_property(np, "samsung,ufs-always-on", NULL)) {
		...
		/* override for emulation mode to prevent from accessing pmu */
#if !IS_ENABLED(CONFIG_SCSI_UFS_EMUL)
		ufs->always_on = 1;
#endif
	...
	/* Disable MCQ as not supported for target S5E8855 */
	if (of_find_property(np, "samsung,mcq-not-supported", NULL))
		ufs->mcq_not_supported = 1;
...
#define PA_PEER_SCRAMBLING 0x155B
		...
		exynos_ufs_param_id id)
{
	...
	// Turn scrambling on if device supports
	ufshcd_dme_get(hba, UIC_ARG_MIB(PA_PEER_SCRAMBLING), &data->scrambling);
	ufshcd_dme_set_attr(hba, UIC_ARG_MIB(PA_SCRAMBLING), ATTR_SET_NOR,
			data->scrambling, HOST);

	ret = exynos_change_power_mode(hba, pmd);
	if (ret) {
		...
		exynos_ufs_param_id id)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d %d %d %d %d\n",
			...
			data->t_max_offset, data->scrambling);
}
		...
		exynos_ufs_param_id id)
{
	...
	if (!value) perf->active_cnt++;
	else perf->active_cnt--;
...
static int exynos_ufs_pm_notifier(struct notifier_block *nb,
		unsigned long pm_event, void *v)
{
	struct exynos_ufs *ufs= container_of(nb, struct exynos_ufs, pm_nb);

	switch (pm_event) {
	case PM_SUSPEND_PREPARE:
		pr_info("%s: suspend prepare\n", __func__);
		ufs->pm_state = UFS_PM_SUSPEND_PREPARE;
		break;
...
static int exynos_ufs_probe(struct platform_device *pdev)
{
	...
	ret = exynos_ufs_init_dbg(&ufs->handle, dev->of_node);
	if (ret)
		return ret;
#endif
	...
	if (ufs->always_on) {
		...
		for (i = 0; i < EXYNOS_PMA_OTP_SIZE; i += 4) {
			ret = exynos_smc_readsfr(EXYNOS_PMA_OTP + i, &value);
			if (ret) {
				pr_err("%s: Fail to smc call otp[%d]", __func__, ret);
				return ret;
			}

			p->pma_otp[i] = (u8)FIELD_GET(0xFF, value);
			p->pma_otp[i + 1] = (u8)FIELD_GET(0xFF00, value);
			p->pma_otp[i + 2] = (u8)FIELD_GET(0xFF0000, value);
			p->pma_otp[i + 3] = (u8)FIELD_GET(0xFF000000, value);
		}
	...
	ufs->pm_nb.notifier_call = exynos_ufs_pm_notifier;
	register_pm_notifier(&ufs->pm_nb);
