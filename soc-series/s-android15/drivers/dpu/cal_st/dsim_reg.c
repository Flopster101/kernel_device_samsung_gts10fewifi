...
#define dsim_phy_extra_read(id, offset)		\
	cal_read(dphy_bias_regs_desc(id), offset)
...
/* dsim version */
#define DSIM_VER_REF			0x02090300
...
static void dsim_reg_set_dphy_dither_en(u32 id, u32 en)
{
	...
	dsim_phy_extra_write_mask(id, DSIM_PHY_M0_PLL_CON2, val, DSIM_PHY_PMS_SSCG_EN_MASK);
}
...
static void dsim_reg_set_dphy_wclk_buf_sft(u32 id, u32 cnt)
{
	...
	dsim_phy_extra_write_mask(id, DSIM_PHY_MASTER_CMN_PLL_CTRL2, val,
...
static void dsim_reg_set_pll_freq(u32 id, u32 p, u32 m, u32 s, u32 k)
{
	...
	/* P, M & K value
	 * M_MSB is assumed to be '0'
	 */
	val = DSIM_PHY_PMS_K(k) |
		DSIM_PHY_PMS_M(m) |
		DSIM_PHY_PMS_P(p) |
		DSIM_PHY_PMS_M_MSB(m_msb);
	dsim_phy_extra_write(id, DSIM_PHY_M0_PLL_CON1, val);

	/* S value */
	val = DSIM_PHY_PMS_S(s);
	mask = DSIM_PHY_PMS_S_MASK;
	dsim_phy_extra_write_mask(id, DSIM_PHY_M0_PLL_CON2, val, mask);
}

#if 0 // TBD: Taken from 9955, is it applicable for 8855 ?
static u32 dsim_reg_cal_pll_freq(u32 id, u32 p, u32 m, u32 s, u32 k)
{
	...
	cal_log_debug(id, "[p m s k]: [%u %u %u %u] => %uMhz", p, m, s, k,
			(u32)(pll_freq / 1000));

	return (u32) DIV_ROUND_CLOSEST(pll_freq, 1000); /* Mhz */
}
...
static u32 dsim_reg_get_pll_freq(u32 id)
{
	...
	/* K, M & P value */
	val = dsim_phy_read(id, DSIM_PHY_M0_PLL_CON1);
	k = DSIM_PHY_PMS_K_GET(val);
	m = DSIM_PHY_PMS_M_GET(val) | (DSIM_PHY_PMS_M_MSB_GET(val) << 8);
	p = DSIM_PHY_PMS_P_GET(val) ?:1;

	/* S value */
	val = dsim_phy_read(id, DSIM_PHY_M0_PLL_CON2);
...
static void dsim_reg_set_dphy_timing_values(u32 id,
			struct dphy_timing_value *t, u32 hsmode, u32 use_sdw)
{
	u32 val, mask;
	...
	/* clock lane setting */
        val = DSIM_PHY_MC_T_ULPS_EXIT(t->b_dphyctl);
        dsim_phy_write(id, DSIM_PHY_MC_CLK_CTRL1, val);
         ...
         */
	if (use_sdw) {
		/* TBD: INIT_SKEW_CAL_CHECK_EN_SDW[2] ?? */
		/* skew settings for clock lane*/
		val = DSIM_PHY_MC_SKEW_CAL_INIT_WAIT_TIME_SDW(0xB) |
			DSIM_PHY_MC_SKEW_CAL_INIT_RUN_TIME_SDW(0xC) |
			DSIM_PHY_MC_SKEW_CAL_RUN_TIME_SDW(0x9) |
			DSIM_PHY_MC_INIT_SKEW_CAL_CHECK_EN_SDW(skewcal_en) |
			DSIM_PHY_MC_SKEW_CAL_EN_SDW(skewcal_en);

		dsim_phy_write(id, DSIM_PHY_MC_CLK_SKEW1, val);

		val = DSIM_PHY_MC_T_HS_EXIT_SDW(t->hs_exit) |
			DSIM_PHY_MC_T_CLK_ZERO_SDW(t->clk_zero) |
			DSIM_PHY_MC_T_CLK_PREPARE_SDW(t->clk_prepare) |
			DSIM_PHY_MC_T_LPX_SDW(t->lpx);
		dsim_phy_write(id, DSIM_PHY_MC_CLK_CTRL4, val);

		val = DSIM_PHY_MC_T_CLK_TRAIL_SDW(t->clk_trail) |
			DSIM_PHY_MC_T_CLK_POST_SDW(t->clk_post);
		dsim_phy_write(id, DSIM_PHY_MC_CLK_CTRL5, val);
	} else {
		/* TBD: INIT_SKEW_CAL_CHECK_EN_SDW[2] ?? */

		/* Design team suggestion: When writing SFR before
		 * PLL enable, both normal SFR and shadow SFR need
		 * to be set
		 */
		/* skew settings for clock lane*/
		// Normal SFR
		val = DSIM_PHY_MC_SKEW_CAL_INIT_WAIT_TIME(0xB) |
			DSIM_PHY_MC_SKEW_CAL_INIT_RUN_TIME(0xC) |
			DSIM_PHY_MC_SKEW_CAL_RUN_TIME(0x9) |
			DSIM_PHY_MC_INIT_SKEW_CAL_CHECK_EN(skewcal_en) |
			DSIM_PHY_MC_SKEW_CAL_EN(skewcal_en);
		dsim_phy_write(id, DSIM_PHY_MC_CLK_SKEW0, val);
		// Shadow SFR
                val = DSIM_PHY_MC_SKEW_CAL_INIT_WAIT_TIME_SDW(0xB) |
                        DSIM_PHY_MC_SKEW_CAL_INIT_RUN_TIME_SDW(0xC) |
                        DSIM_PHY_MC_SKEW_CAL_RUN_TIME_SDW(0x9) |
		...
		// Normal SFR
		val = DSIM_PHY_MC_T_HS_EXIT(t->hs_exit) |
			DSIM_PHY_MC_T_CLK_ZERO(t->clk_zero) |
			DSIM_PHY_MC_T_CLK_PREPARE(t->clk_prepare) |
			DSIM_PHY_MC_T_LPX(t->lpx);
		dsim_phy_write(id, DSIM_PHY_MC_CLK_CTRL2, val);
		// Shadow SFR
                val = DSIM_PHY_MC_T_HS_EXIT_SDW(t->hs_exit) |
                        DSIM_PHY_MC_T_CLK_ZERO_SDW(t->clk_zero) |
                        DSIM_PHY_MC_T_CLK_PREPARE_SDW(t->clk_prepare) |
		...
		// Normal SFR
		val = DSIM_PHY_MC_T_CLK_TRAIL(t->clk_trail) |
			DSIM_PHY_MC_T_CLK_POST(t->clk_post);
		dsim_phy_write(id, DSIM_PHY_MC_CLK_CTRL3, val);
		// Shadow SFR
                val = DSIM_PHY_MC_T_CLK_TRAIL_SDW(t->clk_trail) |
                        DSIM_PHY_MC_T_CLK_POST_SDW(t->clk_post);
                dsim_phy_write(id, DSIM_PHY_MC_CLK_CTRL5, val);
        ...
        for (i = 0; i < 4; i++) {
                val = DSIM_PHY_MD_T_LP_EXIT_SKEW(0) |
			DSIM_PHY_MD_T_LP_ENTRY_SKEW(0) |
			DSIM_PHY_MD_T_ULPS_EXIT(t->b_dphyctl);
		mask = DSIM_PHY_MD_T_LP_EXIT_SKEW_MASK |
			DSIM_PHY_MD_T_LP_ENTRY_SKEW_MASK |
			DSIM_PHY_MD_T_ULPS_EXIT_MASK;
                dsim_phy_write_mask(id, DSIM_PHY_MD_DAT_CTRL1(i), val, mask);
		...
		if (use_sdw) {
			/* TBD: INIT_SKEW_CAL_CHECK_EN_SDW[2] ?? */
			val = DSIM_PHY_MC_SKEW_CAL_INIT_RUN_TIME_SDW(0xC) |
				DSIM_PHY_MC_SKEW_CAL_RUN_TIME_SDW(0x9) |
				DSIM_PHY_MC_INIT_SKEW_CAL_CHECK_EN_SDW(skewcal_en) |
				DSIM_PHY_MC_SKEW_CAL_EN_SDW(skewcal_en);
			dsim_phy_write(id, DSIM_PHY_MD_DAT_SKEW1(i), val);

			val = DSIM_PHY_MD_T_HS_TRAIL_SDW(t->hs_trail) |
				DSIM_PHY_MD_T_HS_ZERO_SDW(t->hs_zero) |
				DSIM_PHY_MD_T_HS_PREPARE_SDW(t->hs_prepare) |
				DSIM_PHY_MD_T_LPX_SDW(t->lpx);
			dsim_phy_write(id, DSIM_PHY_MD_DAT_CTRL4(i), val);

			val = DSIM_PHY_MD_T_HS_EXIT_SDW(t->hs_exit);
			dsim_phy_write(id, DSIM_PHY_MD_DAT_CTRL5(i), val);
		} else {
			/* TBD: INIT_SKEW_CAL_CHECK_EN_SDW[2] ?? */
			/* Design team suggestion: When writing SFR before
			 * PLL enable, both normal SFR and shadow SFR need
			 ...
			 */
			// Normal SFR
			val = DSIM_PHY_MC_SKEW_CAL_INIT_RUN_TIME(0xC) |
				DSIM_PHY_MC_SKEW_CAL_RUN_TIME(0x9) |
				DSIM_PHY_MC_INIT_SKEW_CAL_CHECK_EN(skewcal_en) |
				DSIM_PHY_MC_SKEW_CAL_EN(skewcal_en);
			dsim_phy_write(id, DSIM_PHY_MD_DAT_SKEW0(i), val);
			// Shadow SFR
			val = DSIM_PHY_MC_SKEW_CAL_INIT_RUN_TIME_SDW(0xC) |
				DSIM_PHY_MC_SKEW_CAL_RUN_TIME_SDW(0x9) |
                                DSIM_PHY_MC_INIT_SKEW_CAL_CHECK_EN_SDW(skewcal_en) |
			...
			// Normal SFR
			val = DSIM_PHY_MD_T_HS_TRAIL(t->hs_trail) |
				DSIM_PHY_MD_T_HS_ZERO(t->hs_zero) |
				DSIM_PHY_MD_T_HS_PREPARE(t->hs_prepare) |
				DSIM_PHY_MD_T_LPX(t->lpx);
			dsim_phy_write(id, DSIM_PHY_MD_DAT_CTRL2(i), val);
			// Shadow SFR
                        val = DSIM_PHY_MD_T_HS_TRAIL_SDW(t->hs_trail) |
                                DSIM_PHY_MD_T_HS_ZERO_SDW(t->hs_zero) |
                                DSIM_PHY_MD_T_HS_PREPARE_SDW(t->hs_prepare) |
			...
			// Normal SFR
			val = DSIM_PHY_MD_T_HS_EXIT(t->hs_exit);
			dsim_phy_write(id, DSIM_PHY_MD_DAT_CTRL3(i), val);
			// Shadow SFR
                        val = DSIM_PHY_MD_T_HS_EXIT_SDW(t->hs_exit);
                        dsim_phy_write(id, DSIM_PHY_MD_DAT_CTRL5(i), val);
                ...
                val = DSIM_PHY_MD_T_TA_GET(3) | DSIM_PHY_MD_T_TA_GO(0);
		mask = DSIM_PHY_MD_T_TA_GET_MASK | DSIM_PHY_MD_T_TA_GO_MASK;
                dsim_phy_write_mask(id, DSIM_PHY_MD_DAT_CTRL1(i), val, mask);
...
static void dsim_reg_set_dphy_param_dither(u32 id, struct stdphy_pms *dphy_pms)
{
	...
	/* PLL_MFR, PLL_MRR & PLL_SEL_PF */
	val = DSIM_PHY_PLL_MFR(dphy_pms->mfr) |
		DSIM_PHY_PLL_MRR(dphy_pms->mrr) |
		DSIM_PHY_PLL_SEL_PF(dphy_pms->sel_pf);
	mask = DSIM_PHY_PLL_MFR_MASK |
		DSIM_PHY_PLL_MRR_MASK |
		DSIM_PHY_PLL_SEL_PF_MASK;
	dsim_phy_extra_write_mask(id, DSIM_PHY_M0_PLL_CON2, val, mask);

	/* PLL_ICP, PLL_ENB_AFC, PLL_EXTAFC, PLL_FSEL,
	 * PLL_RSEL, PLL_EN_FEED & PLL_FOUT_MASK
	 */
	val = DSIM_PHY_PLL_ICP(dphy_pms->icp) |
		DSIM_PHY_PLL_ENB_AFC(((dphy_pms->afc_enb) ? ~0 : 0)) |
		DSIM_PHY_PLL_EXTAFC(dphy_pms->extafc) |
		DSIM_PHY_PLL_FSEL(((dphy_pms->fsel) ? ~0 : 0)) |
		DSIM_PHY_PLL_RSEL(dphy_pms->rsel) |
		DSIM_PHY_PLL_EN_FEED(((dphy_pms->feed_en) ? ~0 : 0)) |
		DSIM_PHY_PLL_FOUT_MASK(((dphy_pms->fout_mask) ? ~0 : 0));
	mask = DSIM_PHY_PLL_ICP_MASK |
		DSIM_PHY_PLL_ENB_AFC_MASK |
		DSIM_PHY_PLL_EXTAFC_MASK |
		DSIM_PHY_PLL_FSEL_MASK |
		DSIM_PHY_PLL_RSEL_MASK |
		DSIM_PHY_PLL_EN_FEED_MASK |
		DSIM_PHY_PLL_FOUT_MASK_MASK;
	dsim_phy_extra_write_mask(id, DSIM_PHY_M0_PLL_CON0, val, mask);
}
...
{
	dsim_phy_extra_write(id, DSIM_PHY_M0_PLL_CON0, blk_ctl[0]);
	dsim_phy_extra_write(id, DSIM_PHY_M0_PLL_CON1, blk_ctl[1]);
	dsim_phy_extra_write(id, DSIM_PHY_M0_PLL_CON2, blk_ctl[2]);
}

static void dsim_reg_set_m_cmn_pll_ctrl(u32 id, const u32 *blk_cnt)
{
	u32 i;
	for (i = 0; i < ARRAY_SIZE(DSIM_PHY_MASTER_CMN_PLL_CTRL_VAL); i++)
		dsim_phy_extra_write(id, DSIM_PHY_MASTER_CMN_PLL_CTRL(i), blk_cnt[i]);
}
...
static void dsim_reg_set_mc_ana_lptx_con_imp(u32 id, u32 val)
{
	if (val != UINT_MAX) {
		u32 mask = DSIM_PHY_MC_LPTX_CON_IMP_MASK;
		u32 value = DSIM_PHY_MC_LPTX_CON_IMP(val);
		dsim_phy_write_mask(id, DSIM_PHY_MC_ANA_CON(0), value, mask);
	}
}
static void dsim_reg_set_mc_ana_sdw_con(u32 id, const u32 *blk_ctl)
{
	dsim_phy_write(id, DSIM_PHY_MC_ANA_SDW_CON0, blk_ctl[0]);
}

static void dsim_reg_set_mc_serdes_dphy(u32 id, const u32 *blk_ctl)
{
	dsim_phy_write(id, DSIM_PHY_MC_SERDES_CTRL_DPHY0, blk_ctl[0]);
}

/* Master Data Lane General Control Register */
static void dsim_reg_set_md_dat_con(u32 id, const u32 *blk_ctl)
{
	u32 i;

	for (i = 0; i < MAX_DSIM_DATALANE_CNT; i++) {
		dsim_phy_write(id, DSIM_PHY_MD_DAT_CTRL0(i), blk_ctl[0]);
		//TBD: Other MD_DAT_CTRL reg to be configured with default val ??
	}
}
...
static void dsim_reg_set_md_ana_con(u32 id, const u32 *blk_ctl)
{
        ...
        for (i = 0; i < MAX_DSIM_DATALANE_CNT; i++) {
                ...
                /* For D3, ANA_CON1 is absent, so skipped */
                if(i == 3)
                        continue;
...
static void dsim_reg_set_md_ana_lptx_con_imp(u32 id, u32 val)
{
	if (val != UINT_MAX) {
		u32 mask = DSIM_PHY_MD_LPTX_CON_IMP_MASK;
		u32 value = DSIM_PHY_MD_LPTX_CON_IMP(val);
		u32 i;

		for (i = 0; i < MAX_DSIM_DATALANE_CNT; i++)
			dsim_phy_write_mask(id, DSIM_PHY_MD_ANA_CON0(i), value, mask);
	}
}

static void dsim_reg_set_md_ana_sdw_con(u32 id, const u32 *blk_ctl)
{
        u32 i;

        for (i = 0; i < MAX_DSIM_DATALANE_CNT; i++)
                dsim_phy_write(id, DSIM_PHY_MD_ANA_SDW_CON0(i), blk_ctl[0]);
}

static void dsim_reg_set_md_serdes_dphy(u32 id, const u32 *blk_ctl)
{
	u32 i;
	for (i = 0; i < MAX_DSIM_DATALANE_CNT; i++) {
		dsim_phy_write(id, DSIM_PHY_MD_SERDES_CTRL_DPHY0(i), blk_ctl[0]);
	}
}
...
void dsim_reg_set_inv_dpdn(u32 id, u32 inv_clk, u32 inv_data[4])
{
        ...
        val = inv_clk ? (DSIM_PHY_MC_CLK_INV(1)) : (DSIM_PHY_MC_CLK_INV(0));
        mask = DSIM_PHY_MC_CLK_INV_MASK;
        dsim_phy_write_mask(id, DSIM_PHY_MC_CLK_CTRL0, val, mask);

        for (i = 0; i < MAX_DSIM_DATALANE_CNT; i++) {
                val = inv_data[i] ? (DSIM_PHY_MD_DATA_INV(1)) : (DSIM_PHY_MD_DATA_INV(0));
                mask = DSIM_PHY_MD_DATA_INV_MASK;
                dsim_phy_write_mask(id,  DSIM_PHY_MD_DAT_CTRL0(i), val, mask);
        }
}
...
static void dsim_reg_set_dpdn_swap(u32 id, u32 clk_swap)
{
        ...
        val = DSIM_PHY_MC_HSTX_CON_INPUT_SWAP_DPHY(clk_swap);
        mask = DSIM_PHY_MC_HSTX_CON_INPUT_SWAP_DPHY_MASK;
...
static void dsim_reg_sw_reset(u32 id)
{
	...
	/* TBD: As per ML4 reg set - Specifies the DPHY Reset (High active)
...
static int dsim_reg_wait_phy_ready_clane(u32 id, bool en)
{
	...
	ret = readl_poll_timeout_atomic(
			dphy_regs_desc(id)->regs + DSIM_PHY_MC_CLK_CTRL0,
			val, (en == DSIM_PHY_MC_PHY_READY_GET(val)), 10, 2000);
	if (ret) {
...
static int dsim_reg_wait_phy_ready_dlane(u32 id, u32 lane_id, bool en)
{
	...
	reg_id = DSIM_PHY_MD_DAT_CTRL0(lane_id);

	ret = readl_poll_timeout_atomic(dphy_regs_desc(id)->regs + reg_id, val,
			(en == DSIM_PHY_MD_PHY_READY_GET(val)), 10, 2000);
	if (ret) {
		cal_log_err(id, "PHY clock lane is not ready[timeout]\n");
		return ret;
	}
...
static int dsim_reg_enable_lane_phy(u32 id, u32 lane, bool en)
{
	...
	/* [4L Mode D-PHY Master] */

	/* clock lane enable
	 * DCPHY_MASTER_CMN_CTRL0[4]
	 */
	reg_id = DSIM_PHY_MASTER_CMN_CTRL0;
	reg_val = val;
	reg_mask = DSIM_PHY_CLK0_LANE_ENABLE_MASK;
	dsim_phy_extra_write_mask(id, reg_id, val, reg_mask);

	/* [4L Mode D-PHY Master]
	 * Data lane enable
	 * DCPHY_MASTER_CMN_CTRL0[11:8]
	 */
	for (i = 0; i < lane_cnt; i++) {
		reg_mask = DSIM_PHY_DAT_LANE_ENABLE_MASK(i);
		dsim_phy_extra_write_mask(id, reg_id, val, reg_mask);
	}
...
static void dsim_reg_pll_stable_time(u32 id, u32 lock_cnt)
{
	...
	dsim_phy_extra_write_mask(id, DSIM_PHY_MASTER_CMN_PLL_CTRL1,
				val, DSIM_PHY_PLL_LOCK_CNT_MASK);
}
...
static void dsim_reg_set_pll(u32 id, u32 en)
{
	u32 val = en ? ~0 : 0;
	/* PLL_ENABLE
	 * PLL_ENABLE[7:6] = 2'b11 -> Two PLLs
	 * PLL_ENABLE[6]   = 1'b1  -> Only one PLL
	 */
	dsim_phy_extra_write_mask(id, DSIM_PHY_MASTER_CMN_CTRL0,
				val, DSIM_PHY_ONE_PLL_ENABLE_MASK);
}
...
static bool dsim_reg_is_pll_stable(u32 id)
{
	...
	val = dsim_phy_extra_read(id, DSIM_PHY_MASTER_CMN_PLL_STAT0);
	pll_lock = DSIM_PHY_STATE_PLL_LOCK_GET(val);
...
static int dsim_reg_enable_pll(u32 id, u32 en)
{
	...
	ret = readl_poll_timeout_atomic(
		dphy_bias_regs_desc(id)->regs + DSIM_PHY_MASTER_CMN_PLL_STAT0,
		val, en == DSIM_PHY_STATE_PLL_LOCK_GET(val), 10, 2000);
...
static void dsim_reg_set_pll_sleep_self_ctrl(u32 id, u32 en)
{
	...
	dsim_write_mask(id, DSIM_OPTION_SUITE, val, DSIM_OPTION_SUITE_OPT_PLL_SLEEP_SELF_CTRL_MASK);
}
#endif
...
#else
static void dsim_reg_alv_gate_cond_sel(u32 id, u32 on)
{
        u32 val = on ? ~0 : 0;

        dsim_write_mask(id, DSIM_CLK_CTRL, val, DSIM_CLK_CTRL_ALV_GATE_CONDITION);
}
#endif
		...
		struct dsim_reg_config *config, u32 en)
{
	...
	if (en) {
		...
		/* <TBD>: BIAS registers have been removed in 6410
		 * Need to identify the equivalent set of
		 * configurations
		 */
		dsim_reg_set_hs_vod(id, vod);

		/* MASTER common control default value */
		dsim_phy_extra_write(id, DSIM_PHY_MASTER_CMN_CTRL0,
					DSIM_PHY_MASTER_CMN_CTRL_VAL[0]);

		/* set PLL ctrl : default value */
		dsim_reg_set_pll_con(id, DSIM_PHY_M0_PLL_CON_VAL);
		dsim_reg_set_m_cmn_pll_ctrl(id, DSIM_PHY_MASTER_CMN_PLL_CTRL_VAL);
                if ((clks->hs_clk << pll.s) < 3000)
                        dsim_phy_extra_write_mask(id, DSIM_PHY_M0_PLL_CON2,
						DSIM_PHY_PLL_SEL_VCO(1),
                                                DSIM_PHY_PMS_SEL_VCO_MASK);
		...
		dsim_reg_set_dphy_timing_values(id, &t, hsmode, 0);
		...
		dsim_phy_write(id, DSIM_PHY_MC_CLK_CTRL0, DSIM_PHY_MC_CLK_CTRL_VAL[0]);
		...
		/* low power driver impedance control value needs to be
		 * adjusted as per the end product
		 */
		dsim_reg_set_mc_ana_lptx_con_imp(id, config->lp_tx_drive_imp_ctrl);
		dsim_reg_set_mc_ana_sdw_con(id, DSIM_PHY_MC_ANA_SDW_CON_VAL);

		dsim_reg_set_mc_serdes_dphy(id, DSIM_PHY_MC_SERDES_CTRL_DPHY_VAL);
		...
		dsim_reg_set_md_dat_con(id, DSIM_PHY_MD_DAT_CTRL_VAL);
		 ...
		 * adjusted as per the end product
		 */
		dsim_reg_set_md_ana_lptx_con_imp(id, config->lp_tx_drive_imp_ctrl);
		dsim_reg_set_md_ana_sdw_con(id, DSIM_PHY_MD_ANA_SDW_CON_VAL);

		dsim_reg_set_md_serdes_dphy(id, DSIM_PHY_MD_SERDES_CTRL_DPHY_VAL);
		...
		/* TBD: PLL_CON2 - 0x00000024: PLL_SEL_VCO[1] */

		/*set wclk buf sft cnt
		 * As per suppliment guide, it is 3
		 */
		 ...
		 */
		if (dphy_pms) {
			/* Based on osc clock 76.8MHz, a minimum pll lock
			 * count of 100 usec has been configured
			 */
			pll_lock_cnt = 0x1E00;
...
static void dsim_reg_set_dphy_use_shadow(u32 id, u32 en)
{
	...
	dsim_phy_extra_write_mask(id, DSIM_PHY_MASTER_CMN_CTRL0, val,
			DSIM_PHY_USE_SDW_REG_MASK);
}
...
//	dsim_reg_osc_gate_cond_sel(id, 1);
	dsim_reg_alv_gate_cond_sel(id, 1);
...
{
	u64 narrow_hs_clk_min = center_clk * (1000 - NARROW_FRE_HOP_MARGIN);
	u64 narrow_hs_clk_max = center_clk * (1000 + NARROW_FRE_HOP_MARGIN);
...
static void dsim_reg_update_dphy_timing(u32 id, u32 hs_clk, u32 hop_hs_clk)
{
	...
	dsim_reg_set_dphy_timing_values(id, &t, hsmode, 1);
...
{
	u32 val, mask, m_msb;
	/* in phy 6310, the below equation is being used,
	 * (PLL_SLEEP_CNT_MULT + PLL_SLEEP_CNT_MARGIN) * p;
	 * But in ohy 6410, as per S/G directly 0x1E00 can be
	 * used for (100 usec) pll stable count (OSC CLK 76.8MHz)
	 */
	u32 pll_stable_cnt = 0X1E00;
	...
	if (en) {
		...
		mask = DSIM_PHY_PMS_M_MASK |
			DSIM_PHY_PMS_M_MSB_MASK;
		dsim_phy_extra_write_mask(id, DSIM_PHY_M0_PLL_CON1, val, mask);
...
static void __dphy_dump(u32 id, struct dsim_regs *regs)
{
	...
	dpu_print_hex_dump(regs->phy_regs_ex,
			regs->phy_regs_ex + 0x0000, 0x28);
	/* MASTER COMMON */
	cal_log_info(id, "-[MASTER CMN : offset + 0x80]-\n");
	dpu_print_hex_dump(regs->phy_regs_ex,
			regs->phy_regs_ex + 0x0080, 0x6C);
	/* MC */
	cal_log_info(id, "-[MC : offset + 0x200]-\n");
	dpu_print_hex_dump(regs->phy_regs, regs->phy_regs + 0x0000, 0x100);
	/* MD0 */
	cal_log_info(id, "-[CMD 0 : offset + 0x400]-\n");
	dpu_print_hex_dump(regs->phy_regs, regs->phy_regs + 0x0200, 0x100);
	/* MD1 */
	cal_log_info(id, "-[CMD 1 : offset + 0x500]-\n");
	dpu_print_hex_dump(regs->phy_regs, regs->phy_regs + 0x0300, 0x100);
	/* MD2 */
	cal_log_info(id, "-[CMD 2 : offset + 0x600]-\n");
	dpu_print_hex_dump(regs->phy_regs, regs->phy_regs + 0x0400, 0x100);
	/* MD3 */
	cal_log_info(id, "-[MD 3 : offset + 0x700]-\n");
	dpu_print_hex_dump(regs->phy_regs, regs->phy_regs + 0x0500, 0x100);
}
#else
...
void __dsim_dump(u32 id, struct dsim_regs *regs)
{
	...
	dpu_print_hex_dump(regs->regs, regs->regs + 0x0000, 0x140);
	dpu_print_hex_dump(regs->regs, regs->regs + 0x0400, 0x4);
