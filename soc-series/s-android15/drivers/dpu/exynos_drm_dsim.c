...
EXPORT_SYMBOL_IF_KUNIT(dsim_drvdata);
...
EXPORT_SYMBOL_IF_KUNIT(dsim_dump);
...
static void _dsim_init(struct dsim_device *dsim, bool ulps)
{
	...
	dpu_init_freq_hop(dsim, ulps);

	dsim_reg_init(dsim->id, &dsim->config, &dsim->clk_param, !ulps);

	dsim_enable_sync_command(dsim);
...
{
	struct dsim_device *dsim = encoder_to_dsim(exynos_encoder);
	struct drm_crtc *crtc = exynos_encoder->base.crtc;
	...
	dsim->lp_mode_state = new_exynos_crtc_state->exynos_mode.is_lp_mode;
	mutex_unlock(&dsim->cmd_lock);
...
static void _dsim_disable(struct dsim_device *dsim, bool ulps)
{
	...
	dsim_disable_sync_command(dsim);
...
void dsim_enter_ulps_locked(struct dsim_device *dsim)
{
	...
	if (dsim->sync_cmd) {
		dsim_sync_wait_fifo_empty(dsim, 3);
		del_timer(&dsim->sync_cmd->cmd_timer);
	}
...
void dsim_disable_locked(struct exynos_drm_encoder *exynos_encoder)
{
	...
	if (exynos_crtc)
		vmc_atomic_disable(exynos_crtc->vmc);
...
{
	convert_drm_mode_to_timing(&dsim->config.p_timing, mode, exynos_mode);
				  ...
				  const struct exynos_display_mode *exynos_mode)
{
	...
	dsim_adjust_video_timing(dsim, mode, exynos_mode);
		                          ...
		                          struct exynos_drm_crtc_state *exynos_crtc_state)
{
	...
	if (exynos_crtc_state->seamless_modeset & SEAMLESS_MODESET_MRES) {
		if (get_op_mode(&exynos_crtc_state->exynos_mode) == DSIM_COMMAND_MODE)
			dsim_reg_set_mres(dsim->id, config);
		else if (exynos_crtc_state->seamless_modeset & SEAMLESS_MODESET_PSCALER)
			dsim_reg_set_mres(dsim->id, config);
	}
	...
	if (exynos_crtc_state->seamless_modeset & SEAMLESS_MODESET_LP)
		dsim->lp_mode_state = exynos_crtc_state->exynos_mode.is_lp_mode;

	if (exynos_crtc_state->seamless_modeset & SEAMLESS_MODESET_OPMS) {
		if (exynos_crtc_state->exynos_mode.vhm) {
			dsim_reg_opmode_switch(dsim->id, &dsim->config, &dsim->clk_param);
			vmc_atomic_switching(dsim_get_exynos_crtc(dsim)->vmc, true);
		}
	}
out:
	mutex_unlock(&dsim->cmd_lock);
				 ...
				 struct drm_connector_state *conn_state)
{
	...
	vmc_atomic_set_config(exynos_crtc->vmc, &crtc_state->adjusted_mode,
			&exynos_crtc_state->exynos_mode);
	if (exynos_crtc_state->seamless_modeset) {
		dsim_atomic_seamless_mode_set(dsim, exynos_crtc_state);
	}
}
			     ...
			     struct drm_connector_state *conn_state)
{
	...
	if (exynos_crtc_state->seamless_modeset &&
		...
		exynos_crtc_state->seamless_modeset = false;
...
static int dsim_parse_dt(struct dsim_device *dsim)
{
	...
	if (of_property_read_u32(np, "dsi-lp-tx-drive-imp-ctrl",
				&dsim->config.lp_tx_drive_imp_ctrl))
		dsim->config.lp_tx_drive_imp_ctrl = UINT_MAX;
	...
	if (of_property_read_u32(np, "s_cmd_sram", &dsim->s_cmd_sram))
		dsim->s_cmd_sram = 0;

	if (of_property_read_u32(np, "g_cmd_sram", &dsim->g_cmd_sram))
		dsim->g_cmd_sram = DSIM_PL_FIFO_THRESHOLD - dsim->s_cmd_sram;

	dsim->config.ulps_fh_skip =
		of_property_read_bool(np, "ulps-exit-fh-skip");
...
{
	struct dsim_underrun_config *underrun = &dsim->underrun_config;
	const int underrun_max_count = 12;
...
__weak void dsim_reg_set_pll_sleep_block_release(u32 id) { return; }
__weak void dsim_reg_stop_pll_sleep(u32 id, u32 stop) { return; }

extern bool dpu_sfr_dump;
static irqreturn_t dsim_irq_handler(int irq, void *dev_id)
{
	...
	if (int_src & DSIM_INTSRC_VT_STATUS) {
		...
		if (dsim->config.mode == DSIM_VIDEO_MODE) {
			if (dsim->config.vmc_en && decon && !__is_vdo_to_cmd(decon)) {
				complete(&decon->framestart_done);
				dsim_reg_set_pll_sleep_block_release(dsim->id);
				DPU_EVENT_LOG("DSIM_FRAMESTART", exynos_crtc, 0, NULL);
			} else {
	...
	dsim_sync_cmd_irq_handler(dsim, int_src);
...
{
	dsim_reg_stop_pll_sleep(dsim->id, stop);
}
...
int dsim_exit_pll_sleep(struct dsim_device *dsim)
{
	...
	case DPHY_SLEEP_CTRL_LINK:
		dsim_debug(dsim, "pll will be woken up by writing commad\n");
		break;
...
void dsim_allow_pll_sleep(struct dsim_device *dsim)
{
	switch (dsim->pll_sleep) {
	case DPHY_SLEEP_CTRL_LINK:
		dsim_debug(dsim, "pll will fall asleep by commad fifo empty\n");
		break;
	case DPHY_SLEEP_CTRL_DECON:
...
err_exit:
	mutex_unlock(&dsim->cmd_lock);
	dsim_debug(dsim, "CMD_ID(%#x) ADDR(%#x) TX_LEN(%d)",
			id, *(u8 *)dsim->fcmd_buf_vaddr, d1);
	DPU_EVENT_LOG("DSIM_FAST_COMMAND", exynos_crtc, 0,
			"CMD_ID(%#x) TX_LEN(%d) buf_vaddr(%#lx)",
		...
		struct dma_buf *buf)
{
	...
	dma->sg_table = dma_buf_map_attachment_unlocked(dma->attachment,
			DMA_TO_DEVICE);
	if (IS_ERR_OR_NULL(dma->sg_table)) {
		dsim_err(dsim, "dma_buf_map_attachment_unlocked() failed: %ld\n",
...
static int dsim_alloc_fcmd_memory(u32 id)
{
	...
	ret = dma_buf_vmap_unlocked(dsim->fcmd_buf, &map);
	if (ret) {
			    ...
			    const struct mipi_dsi_msg *msg)
{
	...
	if (fcmd->xfer_unit > dsim->g_cmd_sram)
		dsim_err(dsim, "fcmd transfer align error(%d, %d)\n", fcmd->xfer_unit, dsim->g_cmd_sram);
...
{
	if ((dsim->g_cmd_sram - dsim->burst_cmd.pl_count) > word_cnt)
		return true;
	else
		return false;
}
			      ...
			      bool wait_vsync, bool wait_fifo)
{
	...
	if (!dsim_reg_check_pkt_go_rdy(dsim->id)) {
		dsim_dump(dsim);
		goto err_exit;
	}
...
dsim_write_data(struct dsim_device *dsim, const struct mipi_dsi_msg *msg)
{
	...
	dsim_debug(dsim, "type[%#x], cmd[%#x], tx_len[%zu]\n",
	...
	case MIPI_DSI_RX_GENERIC_LONG_READ_RESPONSE:
		...
		while (i < rx_size) {
			...
			dsim_debug(dsim, "payload: 0x%x i=%d max=%d\n", rx_fifo,
					i, rx_max);
			if (rx_max > msg->rx_len) {
				dsim_err(dsim, "rx size is over the capacity of rx buffer.\n");
				break;
			}
...
static int dsim_probe(struct platform_device *pdev)
{
	...
	dsim->sync_cmd = exynos_sync_cmd_register(dsim);
