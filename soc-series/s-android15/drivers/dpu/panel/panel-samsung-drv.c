...
const char exynos_panel_cmd_string[][DRM_PROP_NAME_LEN] = {
	...
	"exynos,freq-step-change-cmds",
	"exynos,panel-update-cmds",
	"cmd:out-of-range",
};

static bool __bq_is_empty(struct brightness_queue *bq)
{
	return ((bq->front == bq->rear) ? true : false);
}

static bool __bq_is_full(struct brightness_queue *bq)
{
	return ((bq->front == ((bq->rear + 1) % bq->fsz)) ? true : false);
}

static u32 __bq_top(struct brightness_queue *bq)
{
	return __bq_is_empty(bq) ? UINT_MAX : bq->queue[bq->front];
}

static void __bq_delq(struct brightness_queue *bq)
{
	if (!__bq_is_empty(bq))
		bq->front = (bq->front + 1) % bq->fsz;
}

static void __bq_addq(struct brightness_queue *bq, u32 data)
{
	if (__bq_is_full(bq))
		__bq_delq(bq);

	bq->queue[bq->rear] = data;
	bq->rear = (bq->rear + 1) % bq->fsz;
}

static u32 __bq_pull(struct brightness_queue *bq)
{
	u32 val = __bq_top(bq);
...
static u32 __bq_flush(struct brightness_queue *bq)
{
	int rear = (bq->rear == 0) ? bq->fsz : bq->rear;
	u32 val = bq->queue[(rear - 1) % bq->fsz];

	bq->front = bq->rear;
	return val;
}

static void __bq_reset(struct brightness_queue *bq, int fsz)
{
	bq->front = 0;
	bq->rear = 0;
	bq->fsz = fsz + 1;
...
static u32 __bq_count(struct brightness_queue *bq)
{
	return (bq->rear >= bq->front) ? bq->rear - bq->front :
		bq->fsz + bq->rear - bq->front;
}
...
exynos_panel_get_exynos_crtc(struct exynos_panel *ctx)
{
	struct drm_encoder *encoder = ctx->exynos_bridge.base.encoder;

	if (!encoder || !encoder->crtc)
		return NULL;

	return to_exynos_crtc(encoder->crtc);
}

static void exynos_panel_set_shutdown(struct exynos_panel *ctx, bool progress)
{
	struct exynos_drm_crtc *exynos_crtc = exynos_panel_get_exynos_crtc(ctx);
	struct exynos_drm_crtc_state *exynos_crtc_state;

	if (!exynos_crtc) {
		panel_info(ctx, "encoder or crtc is not attached\n");
		return;
	}

	exynos_crtc_state = to_exynos_crtc_state(exynos_crtc->base.state);
	exynos_crtc_state->shutdown_inprogress = progress;
}

static int exynos_panel_parse_gpios(struct exynos_panel *ctx)
{
	struct device *dev = ctx->dev;
	bool panel_reconfigure = false;
	int ret = 0;
	u32 val = 0;
	...
	ret = of_property_read_u32(dev->of_node, "panel-reconfigure", &val);
	/* if ret == -EINVAL, property is not existed */
	if (ret == 0 && val == 1) {
		panel_info(ctx, "panel is set to be reconfigured again.\n");
		panel_reconfigure = true;
	}

	if (gpiod_get_raw_value(ctx->reset_gpio) > 0) {
		if (panel_reconfigure)
...
{
	struct brightness_desc *br_desc = &ctx->desc->brightness;
	u32 max, min, br;
...
static void __update_brightness_internal(struct exynos_panel *ctx, uint64_t brightness)
{
	const struct exynos_panel_funcs *exynos_panel_func;
	u32 new_br = (u32)brightness;
	int ret = 0;

	exynos_panel_func = ctx->func_set->exynos_panel_func;
	if (exynos_panel_func && exynos_panel_func->set_brightness)
		ret = exynos_panel_func->set_brightness(ctx, new_br);
	else
		ret = exynos_dcs_set_brightness(ctx, new_br);

	if (!ret)
		ctx->desc->brightness.cur = new_br;

	panel_debug(ctx, "cur_br(%d)\n", new_br);
}

static int __create_dimming_map(struct exynos_panel *ctx, u32 dst)
{
	struct brightness_desc *br_desc = &ctx->desc->brightness;
	struct brightness_queue *bq = &br_desc->dimq;
	int i, dir, sz;
	u32 src, new;
	...
	if (sz <= 0 || !br_desc->enable_dimming)
		goto exit;

	__bq_addq(bq, src);
	for (i = 1; i < sz - 1; ++i) {
		new = (u32)(src + (dir * 1000 / sz * i / 1000));
		__bq_addq(bq, new);
	}
...
static void __notify_brightness_count(struct exynos_panel *ctx, u32 cnt, u32 full_bq_cnt)
{
	struct drm_crtc *crtc = ctx->exynos_bridge.base.encoder->crtc;

	if (crtc) {
		struct exynos_drm_crtc *exynos_crtc = to_exynos_crtc(crtc);

		exynos_self_refresh_set_dimming_cnt(exynos_crtc->self_refresh, cnt, full_bq_cnt);
	}
}
...
static int exynos_update_status(struct backlight_device *bl)
{
	...
	if (__create_dimming_map(ctx, __map_br_to_weight_br(ctx, brightness)) == 0)
		goto exit;

	if (br_desc->enable_dimming)
		__notify_brightness_count(ctx, __bq_count(&br_desc->dimq),
				__bq_count(&br_desc->dimq));
	else
		__update_brightness_internal(ctx, (uint64_t)__bq_pull(&br_desc->dimq));
		...
		const struct exynos_drm_connector_state *exynos_conn_state)
{
	...
	const struct brightness_desc *br_desc = &ctx->desc->brightness;
	...
	drm_printf(p, "\tbrightness: [%u, %u] curr: %u user_req: %u\n",
			br_desc->min, br_desc->max, br_desc->cur,
			exynos_conn_state->panel_brightness_level);
			  ...
			  const u8 *data)
{
	...
	if (ctx->cmdset_msg_total >= MAX_CMDSET_NUM ||
	    ctx->cmdset_payload_total + size >= dsim->g_cmd_sram) {
			...
			struct drm_property *property, uint64_t val)
{
	...
	if (property == p->backlight_level)
		exynos_conn_state->panel_brightness_level = val;
			...
			struct drm_property *property, uint64_t *val)
{
	...
	if (property == p->max_luminance)
		...
		*val = exynos_conn_state->panel_brightness_level;
	else
		return -EINVAL;
...
exynos_panel_connector_query_status(struct exynos_drm_connector *exynos_conn)
{
	...
	if (dsim->emul_mode) {
		panel_info(ctx, "emulation panel\n");
		goto out;
	}
					 ...
					 bool cmap)
{
	...
	if (cmap) {
		if (__create_dimming_map(ctx, exynos_conn_state->panel_brightness_level) == 0)
			goto exit;
		full_bq_cnt = __bq_count(&br_desc->dimq);
	}

	remain_bq_cnt = __bq_count(&br_desc->dimq);
	if (remain_bq_cnt == 0) {
		panel_warn(ctx, "update brightness requested, but remain bq is empty\n");
		goto exit;
	}
	__update_brightness_internal(ctx, (uint64_t)__bq_pull(&br_desc->dimq));
	remain_bq_cnt = __bq_count(&br_desc->dimq);
	panel_debug(ctx, "remain_bq_cnt:%u\n", remain_bq_cnt);

	if (cmap)
		__notify_brightness_count(ctx, remain_bq_cnt, full_bq_cnt);

exit:
	mutex_unlock(&ctx->mode_lock);

	return remain_bq_cnt;
}

static void __print_step_pattern_info(struct exynos_panel *ctx,
                                      const struct exynos_freq_step *freq_step)
{
	...
	n = scnprintf(buf, SZ_128, "[%d] dur x num:", freq_step->idx);
	for (i = 0; i < freq_step->cnt; i++)
		n += scnprintf(buf + n, SZ_128 - n, " %ux%u",
				freq_step->dur[i], freq_step->num_repeat[i]);

	panel_debug(ctx, "%s\n", buf);
}

const struct exynos_freq_step *
exynos_panel_connector_get_freq_step(struct exynos_drm_connector *exynos_conn,
		                     u32 frame_interval_ns)
{
	...
	mutex_lock(&ctx->mode_lock);
	esync_hz = ctx->current_mode->exynos_mode.esync_hz;
	if (esync_hz == 0 || desc->num_freq_steps == 0) {
		goto out;
	}

	idx = DIV_ROUND_CLOSEST(frame_interval_ns, HZ2NS(esync_hz));
	if (idx >= desc->num_freq_steps) {
		panel_err(ctx, "invalid step, idx(%u) >= freq step(%zu)\n",
				idx, desc->num_freq_steps);
		goto out;
	}

	if (desc->freq_steps[idx].cnt == 0) {
		panel_err(ctx, "failed to get freq step for (%u)\n", frame_interval_ns);
		goto out;
	}

	freq_step = &desc->freq_steps[idx];
out:
	mutex_unlock(&ctx->mode_lock);
	return freq_step;
}
...
static void
exynos_panel_connector_set_freq_step(struct exynos_drm_connector *exynos_conn,
                                     u32 frame_interval_ns)
{
	struct exynos_panel *ctx = exynos_connector_to_panel(exynos_conn);
	...
	freq_step = exynos_panel_connector_get_freq_step(exynos_conn, frame_interval_ns);
	if (!freq_step)
		return;

	mutex_lock(&ctx->mode_lock);
	funcs = ctx->func_set->exynos_panel_func;
	if (funcs->change_freq_step)
		funcs->change_freq_step(ctx, freq_step);
        __print_step_pattern_info(ctx, freq_step);
	mutex_unlock(&ctx->mode_lock);
}

static void
exynos_panel_connector_update(struct exynos_drm_connector *exynos_conn)
{
	struct exynos_panel *ctx = exynos_connector_to_panel(exynos_conn);
	const struct exynos_panel_funcs *funcs;
	...
	mutex_lock(&ctx->mode_lock);
	funcs = ctx->func_set->exynos_panel_func;
	if (funcs->panel_update)
		funcs->panel_update(ctx);
	mutex_unlock(&ctx->mode_lock);
}
...
static const struct exynos_drm_connector_funcs exynos_panel_connector_funcs = {
	...
	.get_freq_step = exynos_panel_connector_get_freq_step,
	.set_freq_step = exynos_panel_connector_set_freq_step,
	.update_panel = exynos_panel_connector_update,
};
			...
			const struct drm_display_mode *mode)
{
	...
	for_each_all_panel_mode(ctx->desc, pmode, i) {
		if (drm_mode_equal(&pmode->mode, mode) &&
				!strncmp(pmode->mode.name, mode->name, DRM_DISPLAY_MODE_LEN))
			return pmode;
	}
			...
			const struct exynos_panel_mode *old_pmode)
{
	...
	if (new_pmode->exynos_mode.is_lp_mode != old_pmode->exynos_mode.is_lp_mode)
		exynos_crtc_state->seamless_modeset |= SEAMLESS_MODESET_LP;
	if ((new_pmode->exynos_mode.scaler_type != old_pmode->exynos_mode.scaler_type) ||
			(new_pmode->exynos_mode.scaler_type &&
			 !drm_mode_match(new_mode, old_mode, DRM_MODE_MATCH_TIMINGS)))
		exynos_crtc_state->seamless_modeset |= SEAMLESS_MODESET_PSCALER;
	if ((new_pmode->exynos_mode.mode_flags & MIPI_DSI_OPMODE_FLAGS) &&
		(old_pmode->exynos_mode.mode_flags & MIPI_DSI_OPMODE_FLAGS)) {
		if ((new_pmode->exynos_mode.mode_flags & MIPI_DSI_MODE_VIDEO) !=
			(old_pmode->exynos_mode.mode_flags & MIPI_DSI_MODE_VIDEO))
			exynos_crtc_state->seamless_modeset |= SEAMLESS_MODESET_OPMS;
	}
}
...
static inline void exynos_drm_update_scaler_info(struct drm_crtc_state *old_crtc_state,
						 struct drm_crtc_state *new_crtc_state,
						 const struct exynos_panel_mode *pmode)
{
	struct exynos_drm_crtc_state *old_exynos_crtc_state, *new_exynos_crtc_state;
	u32 from_hor, from_ver, to_hor, to_ver;

	old_exynos_crtc_state = to_exynos_crtc_state(old_crtc_state);
	new_exynos_crtc_state = to_exynos_crtc_state(new_crtc_state);

	new_exynos_crtc_state->pscaler.hdisplay = pmode->exynos_mode.desired_hdisplay;
	new_exynos_crtc_state->pscaler.vdisplay = pmode->exynos_mode.desired_vdisplay;
	new_exynos_crtc_state->pscaler.type = pmode->exynos_mode.scaler_type;

	from_hor = pmode->mode.hdisplay;
	from_ver = pmode->mode.vdisplay;
	to_hor = IS_PSCALER_ENABLED(new_exynos_crtc_state->pscaler.type) ?
		pmode->exynos_mode.desired_hdisplay : from_hor;
	to_ver = IS_PSCALER_ENABLED(new_exynos_crtc_state->pscaler.type) ?
		pmode->exynos_mode.desired_vdisplay : from_ver;

	if (new_exynos_crtc_state->pscaler.type !=
			old_exynos_crtc_state->pscaler.type)
		pr_info("post scaler <type:%u> (%u x %u) --> (%u x %u)\n",
				new_exynos_crtc_state->pscaler.type,
				from_hor, from_ver, to_hor, to_ver);
}
				      ...
				      struct drm_atomic_state *state)
{
	...
	struct exynos_drm_connector_state *old_exynos_conn_state, *new_exynos_conn_state;
	...
	if (!new_connector_state->best_encoder) {
		panel_err(ctx, "encoder is null\n");
		return 0;
	}

	old_crtc_state = drm_atomic_get_old_crtc_state(state, new_connector_state->crtc);
	...
	old_exynos_conn_state = to_exynos_connector_state(old_connector_state);
	new_exynos_conn_state = to_exynos_connector_state(new_connector_state);
	new_exynos_conn_state->bypass_panel = is_active_changed_lp_mode(ctx, pmode,
							new_crtc_state, old_crtc_state);
	new_exynos_crtc_state =	to_exynos_crtc_state(new_crtc_state);
	if (old_crtc_state->active == false && new_exynos_conn_state->bypass_panel)
			new_exynos_crtc_state->skip_hw_update = true;

	new_exynos_conn_state->brightness_level_changed =
		old_exynos_conn_state->panel_brightness_level !=
		new_exynos_conn_state->panel_brightness_level;

	old_pmode = exynos_panel_get_mode(ctx, &old_crtc_state->mode);
	if (old_pmode) {
		if (old_pmode->exynos_mode.is_lp_mode != pmode->exynos_mode.is_lp_mode) {
			panel_info(ctx, "LP mode switched (%d > %d)\n",
				old_pmode->exynos_mode.is_lp_mode, pmode->exynos_mode.is_lp_mode);
	...
	new_exynos_crtc_state->exynos_mode = pmode->exynos_mode;
	if (!new_exynos_crtc_state->exynos_mode.bts_fps)
		new_exynos_crtc_state->exynos_mode.bts_fps =
				drm_mode_vrefresh(&pmode->mode);

	if (is_seamless_mode_change(new_crtc_state, old_crtc_state)) {
		old_pmode = exynos_panel_get_mode(ctx, &old_crtc_state->mode);
		exynos_drm_connector_check_seamless_modeset(
				new_exynos_crtc_state, pmode, old_pmode);
	} else if (IS_PSCALER_ENABLED(new_exynos_crtc_state->exynos_mode.scaler_type))
		new_exynos_crtc_state->seamless_modeset |= SEAMLESS_MODESET_PSCALER;

	exynos_drm_update_scaler_info(old_crtc_state, new_crtc_state, pmode);
...
static int exynos_panel_attach_properties(struct exynos_panel *ctx)
{
	...
	if (ctx->desc->brightness.commit_supported)
		drm_object_attach_property(obj, p->backlight_level, desc->brightness.cur);
				  ...
				  const struct drm_display_mode *adjusted_mode)
{
	...
	if (funcs) {
		...
		if (!ctx->self_refresh_active && funcs->mode_set) {
			new_crtc_state = conn->state->crtc->state;
			new_exynos_crtc_state =	to_exynos_crtc_state(new_crtc_state);
			if (!curr_pmode)
				new_exynos_crtc_state->seamless_modeset |=
					(SEAMLESS_MODESET_VREF | SEAMLESS_MODESET_MRES);

			funcs->mode_set(ctx, pmode, new_exynos_crtc_state->seamless_modeset);
		}
	}
...
static void exynos_panel_bridge_reset(struct exynos_drm_bridge *exynos_bridge)
{
	...
	for (i = 0; i < 3; ++i) {
		if (pdesc->reset_delay[i])
			usleep_range(pdesc->reset_delay[i], pdesc->reset_delay[i] + 1000);
...
{
	struct exynos_drm_crtc *exynos_crtc = exynos_panel_get_exynos_crtc(panel);

	if (exynos_crtc && exynos_crtc->ops->emergency_off)
...
static struct device_node *__get_connect_panel_node(struct exynos_panel *ctx)
{
	const char *panel_str = NULL;

	if (of_property_read_string(ctx->dev->of_node, "connect-panel", &panel_str)) {
		if (of_property_read_string(ctx->dev->of_node, "default-panel", &panel_str)) {
			panel_err(ctx, "failed to find panel string\n");
			return NULL;
		}
	}

	return of_find_node_by_name(ctx->dev->of_node, panel_str);
}
...
static void exynos_panel_parse_vendor_params(struct device *dev, struct exynos_panel *ctx)
{
		...
		np = __get_connect_panel_node(ctx);
		if (!np) {
...
static void exynos_panel_set_dqe_xml(struct device *dev, struct exynos_panel *ctx)
{
	...
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(dev);
	struct dsim_device *dsim = host_to_dsi(dsi->host);
	enum exynos_drm_output_type output_type = dsim ? dsim->output_type : EXYNOS_DISPLAY_TYPE_DSI;
	...
	drm_for_each_crtc(crtc, drm_dev)
		if (to_exynos_crtc(crtc)->possible_type & output_type) {
...
struct dsim_fcmd *create_dsim_fast_cmd(struct dsim_device *dsim, const u8 *payload, u32 size, u32 align)
{
	...
	u32 remaineder = size;
	u32 size_aligned;
	u8 *tx_buf = (u8 *)dsim->fcmd_buf_vaddr;
	int i;
	...
	payload_xfer_unit = rounddown(min((u32)dsim->g_cmd_sram, size_aligned), align);
	...
	for (i = 0; i < xfer_cnt; i++) {
		...
		memcpy(&tx_buf[i * xfer_unit_aligned],
...
int drm_mipi_fcmd_write(void *_ctx, const u8 *payload, int size, u32 align)
{
	...
	mutex_lock(&lock);
	fcmd = create_dsim_fast_cmd(dsim, payload, size, align);
	if (IS_ERR(fcmd)) {
...
static ssize_t panel_step_pattern_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	return 0;
}

static ssize_t panel_step_pattern_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t len)
{
	struct exynos_panel *ctx = dev_to_exynos_panel(dev);
	...
	if (ret) {
		panel_err(ctx, "invalid freq step pattern\n");
		return ret;
	}

	exynos_panel_connector_set_freq_step(&ctx->exynos_connector, HZ2NS(fps));

	return len;
}

static ssize_t brightness_dimming_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct exynos_panel *ctx = dev_to_exynos_panel(dev);
	int ret = 0;
	...
	mutex_lock(&ctx->mode_lock);
	ret = scnprintf(buf, PAGE_SIZE, "%u\n", ctx->desc->brightness.enable_dimming);
	mutex_unlock(&ctx->mode_lock);

	return ret;
}

static ssize_t brightness_dimming_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t len)
{
	struct exynos_panel *ctx = dev_to_exynos_panel(dev);
	...
	panel_info(ctx, "dimming %s\n", enable ? "ENABLE": "DISABLE");

	mutex_lock(&ctx->mode_lock);

	if (br_desc->enable_dimming == enable)
		goto exit;

	if (!enable && !__bq_is_empty(&br_desc->dimq))
		__update_brightness_internal(ctx, (uint64_t)__bq_flush(&br_desc->dimq));

	br_desc->enable_dimming = enable;
exit:
	mutex_unlock(&ctx->mode_lock);
...
static DEVICE_ATTR_RW(panel_step_pattern);
static DEVICE_ATTR_RW(brightness_dimming);
...
static const struct attribute *panel_attrs[] = {
	...
	&dev_attr_panel_step_pattern.attr,
	&dev_attr_brightness_dimming.attr,
	NULL
};

static int get_array_count(u32 *data, int total, int min_cnt)
{
	int i, arr_len = 0, arr_cnt = 0;

	for (i = 0; i < total; i += (arr_len + min_cnt)) {
		arr_len = data[i];
		arr_cnt++;
	}
					...
					struct exynos_display_cmd_set *clist)
{
	...
	for (i = 0; i < NUM_OF_CMDS; ++i) {
		...
		cmd_cnt = get_array_count(buf, cnt, min_cnt);
		if (cmd_cnt < 1)
				     ...
				     struct exynos_videomode *vm)
{
	...
	of_property_read_u32(np, "exynos,desired-hactive", &vm->desired_hactive);
	of_property_read_u32(np, "exynos,desired-vactive", &vm->desired_vactive);
	of_property_read_u32(np, "exynos,post-scaler-type", &vm->scaler_type);
				     ...
				     struct exynos_display_mode *emode)
{
	...
	emode->desired_vdisplay = vm->desired_vactive;
	emode->desired_hdisplay = vm->desired_hactive;
	emode->scaler_type = EXYNOS_SCALER_NONE;
	if (emode->desired_vdisplay && emode->desired_hdisplay) {
		if (vm->scaler_type < EXYNOS_SCALER_MAX)
			emode->scaler_type = vm->scaler_type;
	}
}
					      ...
					      struct drm_dsc_config *cfg)
{
	...
	/* PPS 6 ~ 11 */
	cfg->pic_height = vm->desired_vactive ? : vm->vactive;
	cfg->pic_width = vm->desired_hactive ? : vm->hactive;
...
static int of_parse_exynos_display_mode(struct exynos_panel *ctx,
					const struct device_node *mode_np,
					struct exynos_panel_mode *pmode)
{
	...
	if (of_parse_exynos_videomode(mode_np, &vm)) {
		panel_err(ctx, "%pOF: error in timing\n", mode_np);
		return -ENOENT;
	}
	...
	of_parse_exynos_display_cmds(mode_np, clist);
	...
	if (!of_property_read_u32(mode_np, "exynos,res-plane-cnt", &val))
		pmode->exynos_mode.plane_cnt = val;

	if (of_property_read_bool(mode_np, "exynos,video-hybrid-mode"))
		pmode->exynos_mode.vhm = true;

	if (!of_property_read_u32(mode_np, "exynos,esync_hz", &val))
		pmode->exynos_mode.esync_hz = val;

	return 0;
}

/**
 * exynos_panel_parse_display_modes - parse all display_timing entries and cmds entries
 *                                 from a device_node
 * @np: device_node with the subnodes
 * @ctx: exynos panel context
 **/
static int exynos_panel_parse_display_modes(struct exynos_panel *ctx)
{
	...
	char *resol_name, *fps_name, *op_name;
	int ret = 0;
	...
	for_each_child_of_node(timings_np, mode_np) {
		...
		op_name = mode_name;
		resol_name = strsep(&op_name, "@");
		comm_np = of_get_child_by_name(np, resol_name);
		exynos_duplicate_display_props(ctx->dev, mode_np, comm_np);
		of_node_put(comm_np);

		fps_name = strsep(&op_name, "_");
		comm_np = of_get_child_by_name(np, fps_name);
		exynos_duplicate_display_props(ctx->dev, mode_np, comm_np);
		of_node_put(comm_np);

		if (op_name) {
			comm_np = of_get_child_by_name(np, op_name);
			exynos_duplicate_display_props(ctx->dev, mode_np, comm_np);
			of_node_put(comm_np);
		}

		ret = of_parse_exynos_display_mode(ctx, mode_np, pmode);
		if (ret)
			goto err;
...
static int of_parse_exynos_step_patterns(struct exynos_panel *ctx,
                                          const struct device_node *np)
{
	struct exynos_panel_desc *pdesc;
	struct exynos_freq_step *freq_steps;
	int min_cnt = 1; /* num of cmd */
	int num;
	u32 *arrays;
	u32 i, j, offset, len;

	num = of_property_count_u32_elems(np, "samsung,step-patterns");
	if (num < 0)
		return 0;

	pdesc = ctx->desc;
	arrays = kzalloc(sizeof(u32) * num, GFP_KERNEL);
	of_property_read_u32_array(np, "samsung,step-patterns", arrays, num);

	pdesc->num_freq_steps = get_array_count(arrays, num, min_cnt);
	freq_steps = devm_kzalloc(ctx->dev, pdesc->num_freq_steps * sizeof(*freq_steps),
			GFP_KERNEL);
	offset = 0;
	for (i = 0; i < pdesc->num_freq_steps; i++) {
		len = arrays[offset++];
		freq_steps[i].idx = arrays[offset++];
		freq_steps[i].cnt = (len - 1) / 2;
		for (j = 0; j < freq_steps[i].cnt; j++) {
			freq_steps[i].dur[j] = arrays[offset++];
			freq_steps[i].num_repeat[j] = arrays[offset++];
		}
	}
	kfree(arrays);

	pdesc->freq_steps = freq_steps;

	for (i = 0; i < pdesc->num_freq_steps; i++)
		__print_step_pattern_info(ctx, &pdesc->freq_steps[i]);

	return 0;
}

static int of_parse_exynos_brightness_desc(struct exynos_panel *ctx,
                                           const struct device_node *np)
{
	struct brightness_desc *br_desc = &ctx->desc->brightness;

	if (of_property_read_u32(np, "samsung,max-brightness", &br_desc->max))
		br_desc->max = 1023;
	if (of_property_read_u32(np, "samsung,default-brightness", &br_desc->cur))
		br_desc->cur = 512;
	if (of_property_read_u32(np, "samsung,min-brightness", &br_desc->min))
		br_desc->min = 0;

	br_desc->endian_swap = of_property_read_bool(np, "samsung,bl-endian-swap");
	br_desc->commit_supported = of_property_read_bool(np, "samsung,bl-commit-supported");

	if (of_property_read_u32(np, "samsung,bl-dimming-step",	&br_desc->dimming_step))
		br_desc->dimming_step = 0;
	if (br_desc->dimming_step > BRIGHTNESS_MAX_QUEUE_SZ)
		br_desc->dimming_step = BRIGHTNESS_MAX_QUEUE_SZ;
	if (br_desc->dimming_step) {
		__bq_reset(&br_desc->dimq, br_desc->dimming_step);
		br_desc->enable_dimming = true;
	} else {
		__bq_reset(&br_desc->dimq, 1);
		br_desc->enable_dimming = false;
	}

	panel_info(ctx, "brightness <%u %u %u>\n", br_desc->min, br_desc->cur, br_desc->max);
	panel_info(ctx, "brightness commit_supported:%s endian_swap:%d\n",
			br_desc->commit_supported ? "true" : "false", br_desc->endian_swap);
	panel_info(ctx, "brightness dimming(%d) step(%d)\n",
			br_desc->enable_dimming, br_desc->dimming_step);

	return 0;
}

static int exynos_panel_parse_descriptor(struct exynos_panel *ctx)
{
	struct device *dev = ctx->dev;
	...
	ret = of_property_read_string(np, "samsung,panel-name", &panel_name);
	if (!ret)
	...
	ret = of_property_read_u32(np, "samsung,data-lane-count", &pdesc->data_lane_cnt);
	if (ret)
		pdesc->data_lane_cnt = 4;

	ret = of_property_read_u32(np, "samsung,hdr-formats", &pdesc->hdr_formats);
	if (ret)
		pdesc->hdr_formats = 0;

	ret = of_property_read_u32(np, "samsung,max-luminance", &pdesc->max_luminance);
	if (ret)
		pdesc->max_luminance = 0;

	ret = of_property_read_u32(np, "samsung,max-avg-luminance", &pdesc->max_avg_luminance);
	if (ret)
		pdesc->max_avg_luminance = 0;

	ret = of_property_read_u32(np, "samsung,min-luminance", &pdesc->min_luminance);
	if (ret)
	...
	ret = of_property_read_u32(np, "samsung,reset-position", &pdesc->reset_pos);
	if (ret)
		pdesc->reset_pos = BRIDGE_RESET_LP11_HS;

	ret = of_property_read_u32_array(np, "samsung,reset-sequence", pdesc->reset_seq, 3);
	if (ret) {
		pdesc->reset_seq[0] = 1;
		pdesc->reset_seq[1] = 0;
		pdesc->reset_seq[2] = 1;
	...
	ret = of_property_read_u32_array(np, "samsung,reset-udelay", pdesc->reset_delay, 4);
	if (ret) {
		pdesc->reset_delay[0]= 0;
		pdesc->reset_delay[1] = 5000;
		pdesc->reset_delay[2] = 5000;
		pdesc->reset_delay[3] = 10000;
	}

	ret = of_property_read_u32(np, "samsung,lcd-width-mm", &pdesc->width_mm);
	if (ret)
		pdesc->width_mm = 75;

	ret = of_property_read_u32(np, "samsung,lcd-height-mm", &pdesc->height_mm);
	if (ret)
		pdesc->height_mm = 165;

	if (of_parse_exynos_brightness_desc(ctx, np))
		return -EINVAL;

	if (of_parse_exynos_step_patterns(ctx, np))
		return -EINVAL;
...
static int exynos_panel_parse_dt(struct exynos_panel *ctx)
{
	...
	ret = exynos_panel_parse_descriptor(ctx);
	if (ret)
		goto err;

	ret = exynos_panel_parse_display_modes(ctx);
	if (ret)
		goto err;
...
int exynos_panel_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->bl->props.max_brightness = ctx->desc->brightness.max;
	ctx->bl->props.brightness = ctx->desc->brightness.cur;
	...
	panel_info(ctx, "samsung common panel-%d driver has been probed\n", ctx->id);
...
void exynos_panel_shutdown(struct mipi_dsi_device *dsi)
{
	...
	mutex_lock(&ctx->mode_lock);
	if (is_panel_enabled(ctx))
		exynos_panel_set_shutdown(ctx, true);
	drm_panel_disable(&ctx->panel);

	is_vid_mode = (ctx->current_mode->exynos_mode.mode_flags &
						MIPI_DSI_MODE_VIDEO) != 0;
	if (is_vid_mode)
		mipi_dsi_detach(dsi);
