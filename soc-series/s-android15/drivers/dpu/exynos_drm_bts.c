...
#define FHD_MAX_RATIO	566U
#define ENABLE_BTS_HINT	0

#define REQUEST_BW_LIMIT	(36000000U)
...
static inline u32 __output_width(struct decon_config *cfg)
{
	return cfg->pscaler ? cfg->desired_width : cfg->image_width;
}

static inline u32 __output_height(struct decon_config *cfg)
{
	return cfg->pscaler ? cfg->desired_height : cfg->image_height;
}
...
static inline u32 dpu_bts_get_one_line_time(struct decon_device *decon)
{
	...
	tot_v += __output_height(&decon->config);
...
static u64 dpu_bts_get_resol_clock(struct decon_device *decon)
{
	...
	fps = decon->bts.fps;
	xres = __output_width(&decon->config);
	yres = __output_height(&decon->config);
	...
	margin = 1100 + (12000U * (u64)slice_cnt + 20000) / xres;
		...
		u32 *module_cycle, u32 *basic_cycle)
{
	...
	dsi_cycle = __output_width(&decon->config);
	 ...
	 */
	base_cycle = (__output_width(&decon->config) * 11 / 10 + dsi_cycle) / ppc;
		...
		struct dpu_bts_win_config *config, u64 resol_clk, u32 max_clk)
{
	...
	if (is_scale) {
		...
		if ((ppc != 0 && ppc <= 2 && ppc_scaler == 2) ||
			...
			if (ppc <= 2 && ppc_scaler == 2) {
				/*s5e8855 case*/
				line_a_ppc = ((ratio_v - 2) * cnt_per_clk) + max(cnt_per_clk, config->dst_w);
			...
			if (ppc <= 2 && ppc_scaler == 2) {
				/*s5e8855 case*/
				line_b_ppc = ((ratio_v - 1) * cnt_per_clk) + max(cnt_per_clk, config->dst_w);
	...
	if ( (ppc != 0) && (aclk_disp < (resol_clk / ppc)) )
...
void dpu_bts_set_bus_qos(const struct exynos_drm_crtc *exynos_crtc)
{
	...
	if (is_version_above(&decon->config.version, 7, 0)) {
		...
		if ((decon->config.out_type & DECON_OUT_DSI)
			...
			decon->bts.int_default = 0;
			decon->bts.mif_default = 0;
		}
	}
...
static bool dpu_bts_check_max_perf(struct decon_device *decon)
{
	if (is_version(&decon->config.version, 6, 20)) {
		if ((decon->config.mode.op_mode == DECON_COMMAND_MODE) &&
			...
			DPU_DEBUG_BTS(decon, "[Command Mode] dpu is runnning with max performance\n");
			return true;
		} else {
			exynos_pm_qos_update_request(&decon->bts.int_qos, decon->bts.int_default);
			exynos_pm_qos_update_request(&decon->bts.mif_qos, decon->bts.mif_default);
...
static bool __check_suspicious_layer(u32 half_h, u32 full_w, u32 y, u32 w, u32 h)
{
	u32 min, max;

	if (y > half_h || w != full_w || w > FHD_WIDTH)
		return false;
...
static void dpu_bts_bandwidth_boost(struct decon_device *decon)
{
	...
	for (i = 0; i < decon->win_cnt; ++i) {
		...
		if (__check_suspicious_layer(half, decon->config.image_width,
					config[i].dst_y, config[i].dst_w, config[i].dst_h))
...
static void dpu_bts_find_max_disp_freq(struct decon_device *decon)
{
	...
	for (i = 0; i < decon->win_cnt; ++i) {
		if (!is_version(&decon->config.version, 6, 27))
			continue;
...
void dpu_bts_update_bw(struct exynos_drm_crtc *exynos_crtc, bool shadow_updated)
{
	...
	if (bw.write > REQUEST_BW_LIMIT)
		bw_limit = 0;
	else
		bw_limit = REQUEST_BW_LIMIT - bw.write;

	/* update peak & read bandwidth per DPU port */
	bw.peak = decon->bts.peak > bw_limit ? bw_limit : decon->bts.peak;
	bw.read = decon->bts.read_bw > bw_limit ? bw_limit : decon->bts.read_bw;
