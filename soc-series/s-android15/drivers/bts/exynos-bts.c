...
#define CAMERA_DEFAULT	6

static bool is_camera_scen = false;

static void exynos_bts_update_pm_qos(void)
{
	int disp_freq = 0;

	spin_lock(&btsdev->lock);
	disp_freq = btsdev->int_disp_freq;

	if (disp_freq > 134000 && disp_freq <= 178000)
		disp_freq = 200000; //This is to guarantees DISP freq > INT freq

	pr_info("%s: display -> int request freq = %d\n", __func__, disp_freq);
	spin_unlock(&btsdev->lock);

	exynos_pm_qos_update_request(&btsdev->bts_pm_qos, disp_freq);
}

static int exynos_bts_pm_qos_notifier(struct notifier_block *nb, unsigned long val, void *v)
{
	spin_lock(&btsdev->lock);
	...
	btsdev->int_disp_freq = (int)val;
	spin_unlock(&btsdev->lock);

	exynos_bts_update_pm_qos();
...
static void bts_calc_bw(void)
{
	...
	if (check_add_overflow(total_read, total_write, &btsdev->total_bw))
		btsdev->total_bw = U32_MAX;
	...
	mif_freq = (btsdev->total_bw / bw_params->mif_bus_width / bw_params->mif_util) * 100;
	int_freq = (btsdev->peak_bw / bw_params->bus_width / bw_params->int_util) * 100;

	BTSDBG_LOG(btsdev->dev, "BW: T:%.8u R:%.8llu W:%.8llu P:%.8u MIF:%.8u INT:%.8u\n",
...
#if IS_ENABLED(CONFIG_SOC_S5E8855)
		if (index == CAMERA_DEFAULT) {
			is_camera_scen = true;
			btsdev->int_disp_freq = exynos_pm_qos_request(PM_QOS_DISPLAY_THROUGHPUT);
			flag = 1;
		}
#endif
	}
...
#if IS_ENABLED(CONFIG_SOC_S5E8855)
			if (index == CAMERA_DEFAULT) {
				is_camera_scen = false;
				btsdev->int_disp_freq = 0;
				flag = 1;
			}
#endif
		}
...
static int bts_probe(struct platform_device *pdev)
{
	...
	btsdev = devm_kzalloc(&pdev->dev, sizeof(struct bts_device), GFP_KERNEL);
	if (btsdev == NULL)
		return -ENOMEM;
...
#elif IS_ENABLED(CONFIG_EXYNOS_ESCAV1) || defined(CONFIG_EXYNOS_USE_SCI_LITE)
	...
	device_enable_async_suspend(btsdev->dev);

#if IS_ENABLED(CONFIG_SOC_S5E8855)
	exynos_pm_qos_add_request(&btsdev->bts_pm_qos, PM_QOS_DEVICE_THROUGHPUT, 0);

	btsdev->nb.notifier_call = exynos_bts_pm_qos_notifier;
	btsdev->nb.priority = INT_MAX - 2;

	ret = exynos_pm_qos_add_notifier(PM_QOS_DISPLAY_THROUGHPUT, &btsdev->nb);
	if (ret < 0) {
		exynos_pm_qos_remove_notifier(PM_QOS_DISPLAY_THROUGHPUT, &btsdev->nb);
		pr_info("%s failed to add pm qos notifier\n", __func__);
	}
#endif
