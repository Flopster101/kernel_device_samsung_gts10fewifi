...
int sc_hwset_src_image_format(struct sc_dev *sc, struct sc_frame *frame)
{
	...
	print_sc(SC_DBG_SFR, "src_cfg(0x%x)\n", val);
	return 0;
}
...
int sc_hwset_dst_image_format(struct sc_dev *sc, struct sc_frame *frame)
{
	...
	print_sc(SC_DBG_SFR, "dst_cfg(0x%x)\n", val);
...
void sc_hwset_pre_multi_format(struct sc_dev *sc, bool src, bool dst)
{
	...
	if (src && ((cfg & SCALER_CFG_FMT_MASK) == SCALER_CFG_FMT_ARGB8888)) {
		...
		print_sc(SC_DBG_SFR, "src_cfg(0x%lx)\n", cfg);
	}
	...
	if (dst && ((cfg & SCALER_CFG_FMT_MASK) == SCALER_CFG_FMT_ARGB8888)) {
		...
		print_sc(SC_DBG_SFR, "dst_cfg(0x%lx)\n", cfg);
	}
}

VISIBLE_IF_KUNIT void get_blend_value(unsigned int *cfg, u32 val, bool pre_multi)
{
	unsigned int tmp;
...
EXPORT_SYMBOL_IF_KUNIT(get_blend_value);
		...
		unsigned char g_alpha)
{
	...
	print_sc(SC_DBG_SFR, "src_blend_color is 0x%x, %d\n", cfg, pre_multi);
	...
	print_sc(SC_DBG_SFR, "src_blend_alpha is 0x%x\n", cfg);
	...
	print_sc(SC_DBG_SFR, "dst_blend_color is 0x%x\n", cfg);
	...
	print_sc(SC_DBG_SFR, "dst_blend_alpha is 0x%x\n", cfg);
...
EXPORT_SYMBOL_IF_KUNIT(sc_hwset_blend);
...
void sc_hwset_color_fill(struct sc_dev *sc, unsigned int val)
{
	...
	print_sc(SC_DBG_SFR, "color filled is 0x%08x\n", val);
}
EXPORT_SYMBOL_IF_KUNIT(sc_hwset_color_fill);
			...
			struct sc_csc *csc)
{
	...
	for (i = 0, j = 0; i < 9; i++, j += 4) {
		...
		print_sc(SC_DBG_SFR, "csc value %d - %d\n", i, csc_eq_val[i]);
	}
...
VISIBLE_IF_KUNIT u32 sc_coef_adjust(u32 val)
{
	/*
...
EXPORT_SYMBOL_IF_KUNIT(sc_coef_adjust);
				...
				unsigned int filter)
{
	...
	print_sc(SC_DBG_SFR, "yfilter(%u), cfilter(%u)\n", yfilter, cfilter);
...
{
	u32 yspan = frame->spansize[SC_PLANE_Y];
	u32 cspan = frame->spansize[SC_PLANE_CB];
	...
	print_sc(SC_DBG_SFR, "yspan(%u), cspan(%u)\n", yspan, cspan);
}
...
static void sc_hwset_src_2bit_addr(struct sc_dev *sc, struct sc_frame *frame)
{
	...
	print_sc(SC_DBG_SFR, "yaddr_2bit(0x%x), caddr_2bit(0x%x)\n",
			yaddr_2bit, caddr_2bit);
...
void sc_hwset_src_addr(struct sc_dev *sc, struct sc_frame *frame)
{
	...
	print_sc(SC_DBG_SFR, "SC_PLANE_Y(0x%llx), SC_PLANE_CB(0x%llx), SC_PLANE_CR(0x%llx)\n",
			addr->ioaddr[SC_PLANE_Y], addr->ioaddr[SC_PLANE_CB], addr->ioaddr[SC_PLANE_CR]);
...
void sc_print_hex_dump(struct sc_dev *sc, const void *buf, size_t len)
{
	...
	for (i = 0; i < len; i += ROW_LEN) {
		...
		snprintf(prefix_buf, sizeof(prefix_buf), "[sc_sfr_dump][%08lX] ", p);
...
{
	print_sc(SC_INFO, "Dumping control registers...\n");
	print_sc(SC_INFO, "------------------------------------------------\n");
...
u32 sc_hwget_and_clear_irq_status(struct sc_dev *sc)
{
	...
	print_sc(SC_DBG_SFR, "INT_STATUS(0x%x)\n", val);
	return val;
}
...
void sc_hwset_votf_ring_clk_en(struct sc_dev *sc)
{
	...
	print_sc(SC_DBG_SFR, "votf_ref_count(%d)\n", atomic_read(&sc->votf_ref_count));
}
EXPORT_SYMBOL_IF_KUNIT(sc_hwset_votf_ring_clk_en);
...
EXPORT_SYMBOL_IF_KUNIT(sc_hwset_init_votf);
...
void sc_hwset_votf_en(struct sc_dev *sc, bool enable)
{
	...
	writel(cfg, sc->regs + SCALER_CFG);
	print_sc(SC_DBG_SFR, "votf is %s\n", enable ? "enabled" : "disabled");
}
EXPORT_SYMBOL_IF_KUNIT(sc_hwset_votf_en);
...
EXPORT_SYMBOL_IF_KUNIT(sc_hwset_votf);

VISIBLE_IF_KUNIT void sc_votf_read_reg_and_print(struct sc_dev *sc, u32 offset)
{
	print_sc(SC_ERR, "[%x] : %x\n",
		offset, readl_relaxed(sc->votf_regs + offset));
}
EXPORT_SYMBOL_IF_KUNIT(sc_votf_read_reg_and_print);
...
bool sc_hwset_clear_votf_clock_en(struct sc_tws *tws)
{
	...
	if (timestamp >= 33) {
		...
		print_sc(SC_ERR, "TWS hasn't completed for %lu ms\n", timestamp);
		print_sc(SC_ERR,
			"vOTF Idx : TWS(%d), TRS(%d), DPU(%d), BUF(%d)\n",
			tws_idx, trs_idx, dpu_dma_idx, buf_idx);

		print_sc(SC_ERR, "vOTF dump\n");
...
EXPORT_SYMBOL_IF_KUNIT(sc_hwset_clear_votf_clock_en);
...
EXPORT_SYMBOL_IF_KUNIT(sc_hwset_tws_flush);
