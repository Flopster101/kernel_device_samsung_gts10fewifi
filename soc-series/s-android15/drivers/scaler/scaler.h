...
#define MSCL_NORMAL_S2MPU_NAME "SCALER"
#define MSCL_SECURE_S2MPU_NAME "TZMP2_SCALER"
#endif
...
#define SC_DBG_FUNC	4
#define SC_DBG_SFR	3
#define SC_DBG		2
#define SC_INFO	1
#define SC_ERR		0

#define print_sc(level, fmt, args...)					\
	do {								\
		if (level == SC_ERR) {					\
			pr_err("[%s:%d] "				\
...
#define sc_fmt_is_sbwc_lossy_FR(x)	((x == V4L2_PIX_FMT_NV12M_SBWCL_64_8B_FR) || \
					 (x == V4L2_PIX_FMT_NV12M_SBWCL_64_10B_FR))
...
/* for priority */
#define SC_CTX_PRIORITY_HIGH	(0)
#define SC_CTX_PRIORITY_LOW	(1)
 ...
 * @spansize:   stride value to be written to SFR(sbwc: byte unit, non-sbwc: pixel unit)
 * @sizes:	Size required for pixelformat
 */
struct sc_frame {
	...
	__u32			spansize[SC_MAX_PLANES];

	struct sc_addr			addr;
	__u32			sizes[SC_MAX_PLANES];
	__u8			byte32num;
	bool			pre_multi;
};
...
struct sc_buf_info {
	...
	unsigned int plane_len[SC_MAX_PLANES];
};
...
#if IS_ENABLED(CONFIG_EXYNOS_S2MPU)
	struct s2mpu_notifier_block s2mpu_normal_nb;
	struct s2mpu_notifier_block s2mpu_secure_nb;
#endif
...
struct sc_ctx {
	...
	struct sc_buf_info		src_buf_info;
	struct sc_buf_info		dst_buf_info;
};
