...
#define SCALER_CFG_CSC_PREV_VERSION	(1 << 11) /* hidden reg for disable function about bit-extension */
...
#define SCALER_CFG_SBWC_BYTE_ALIGN_MASK (0x3 << 14)
#define SCALER_CFG_SBWC_BYTE_ALIGN(x)   ((((x) >> 6) & 0x3) << 14)
#define SCALER_CFG_SBWC_LOSSY_FR_MODE	(0x2 << 11)
				...
				unsigned int chshift, unsigned int cvshift)
{
	...
	print_sc(SC_DBG_SFR, "left(%d), top(%d), chshift(%u), cvshift(%u)\n",
			left, top, chshift, cvshift);
}
			...
			unsigned int chshift, unsigned int cvshift)
{
	...
	print_sc(SC_DBG_SFR, "width %d, height %d\n", pre_width, pre_height);
...
static inline void sc_hwset_dst_pos(struct sc_dev *sc, __s32 left, __s32 top)
{
	...
	print_sc(SC_DBG_SFR, "left(%d), top(%d)\n", left, top);
}
...
static inline void sc_hwset_hratio(struct sc_dev *sc, u32 ratio, u32 pre_ratio)
{
	...
	print_sc(SC_DBG_SFR, "ratio(%d), pre_ratio(%u)\n", ratio, pre_ratio);
}
...
static inline void sc_hwset_flip_rotation(struct sc_dev *sc, u32 flip_rot_cfg)
{
	...
	print_sc(SC_DBG_SFR, "flip_rot_cfg(0x%x)\n", flip_rot_cfg);
}
...
static inline void sc_hwset_int_en(struct sc_dev *sc)
{
	...
	print_sc(SC_DBG_SFR, "int_en(0x%x)\n", val);
}
...
static inline void sc_hwset_csc_prev_ver(struct sc_dev *sc, bool en)
{
	unsigned long cfg = readl(sc->regs + SCALER_CFG);

	if (en)
		cfg |= SCALER_CFG_CSC_PREV_VERSION;
	else
		cfg &= ~SCALER_CFG_CSC_PREV_VERSION;
...
static inline void sc_hwset_src_init_phase(struct sc_dev *sc, struct sc_init_phase *ip)
{
	if (ip->yh) {
		...
		print_sc(SC_DBG_SFR, "initial phase value is yh 0x%x, ch 0x%x\n",
	...
	if (ip->yv) {
		...
		print_sc(SC_DBG_SFR, "initial phase value is yv 0x%x, cv 0x%x\n",
