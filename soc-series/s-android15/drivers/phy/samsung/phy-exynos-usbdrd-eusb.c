...
#include <linux/of_gpio.h>
	...
	&& IS_ENABLED(CONFIG_PHY_SUPPORT_SEC_REPEATER_CB)
		...
		struct device_attribute *attr, const char *buf, size_t n)
{
	...
	for (i = 0; i < tune_num; i++) {
		if (!strncmp(phy_drd->usbphy_info.tune_param[i].name, tune_name,
			strlen(phy_drd->usbphy_info.tune_param[i].name) + 1)) {
		...
		struct device_attribute *attr, const char *buf, size_t n)
{
	...
	for (i = 0; i < tune_num; i++) {
		if (!strncmp(phy_drd->usbphy_sub_info.tune_param[i].name, tune_name,
			strlen(phy_drd->usbphy_sub_info.tune_param[i].name) + 1)) {
				phy_drd->ss_tune_param_value[i][0] = tune_val;
				phy_drd->ss_tune_param_value[i][1] = tune_val;
			phy_exynos_snps_usbdp_tune_each_param(&phy_drd->usbphy_sub_info,
							tune_name, tune_val);
		}
	}
...
int exynos_usbdrd_phy_port_external_control(bool used_phy_port, bool test);
		...
		struct device_attribute *attr, const char *buf, size_t n)
{
	...
	exynos_usbdrd_phy_port_external_control(phy_port_value, 0);
 ...
 */
static int exynos_rate_to_clk(struct exynos_usbdrd_phy *phy_drd)
{
	int ret;
							...
							struct device_node *node)
{
	...
	for_each_child_of_node(node, child) {
		...
		if (ret == 0) {
			...
			/* set defalut init value as USBPHY_MODE_DEV */
			ss_tune_param[param_index].value = phy_drd->ss_tune_param_value[param_index][USBPHY_MODE_DEV];
		} else {
...
static int exynos_usbdrd_get_sub_phyinfo(struct exynos_usbdrd_phy *phy_drd)
{
	struct device *dev = phy_drd->dev;
	struct device_node *tune_node;
	struct snps_add_info *add_info;
	int ret;
	int value;

	if (!of_property_read_u32(dev->of_node, "sub_phy_version", &value)) {
		phy_drd->usbphy_sub_info.version = value;
		dev_info(dev, "sub_phy_version: 0x%x\n", value);
	} else {
		dev_err(dev, "can't get sub_phy_version\n");
		return -EINVAL;
	}

	add_info = devm_kzalloc(dev, sizeof(*add_info), GFP_KERNEL);

	if (!of_property_read_u32(dev->of_node, "snps_usbdp_ram_mode", &value)) {
		add_info->usbdp_mode = value;
	} else {
		dev_err(dev, "can't get snps_usbdp_ram_mode\n");
		add_info->usbdp_mode = 0;
	}
	phy_drd->usbphy_sub_info.add_info = (void *)add_info;
							...
							int phy_state)
{
	...
	if (!ss_tune_param) {
		dev_info(phy_drd->dev, "%s: no hs_tune_param. return!\n", __func__);
		return;
	}
	...
	/* HW setting isn't needed because it's done in phy_enable */
...
static int exynos_usbdrd_phy_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(phy_drd->dev, "%s: Not supported ref clock %lu\n",
				__func__, clk_get_rate(phy_drd->ref_clk));
		goto err1;
	}
skip_clock:
...
#ifdef CONFIG_PM
static int exynos_usbdrd_phy_suspend(struct device *dev)
{
	struct exynos_usbdrd_phy *phy_drd = dev_get_drvdata(dev);

	dev_info(dev, "%s, is_conn = %d\n", __func__, phy_drd->is_conn);

	if (usb_user_scenario == AUDIO_MODE_IN_CALL) {
		return 0;
	}

	if (otg_is_connect() == OTG_DEVICE_CONNECT) {
		exynos_usbdrd_utmi_vendor_set(phy_drd, 1, 0);
#ifdef CONFIG_EXYNOS_USBDRD_PHY30
		exynos_usbdrd_pipe3_vendor_set(phy_drd, 1, 0);
#endif
	}
...
static int exynos_usbdrd_phy_resume(struct device *dev)
{
	...
	if (otg_is_connect() == OTG_DEVICE_CONNECT) {
#ifdef CONFIG_EXYNOS_USBDRD_PHY30
		exynos_usbdrd_pipe3_vendor_set(phy_drd, 1, 1);
#endif
		exynos_usbdrd_utmi_vendor_set(phy_drd, 1, 1);
		exynos_usbdrd_utmi_vendor_set(phy_drd, 0, 0);
	}
...
static const struct dev_pm_ops exynos_usbdrd_phy_dev_pm_ops = {
	.suspend	= exynos_usbdrd_phy_suspend,
