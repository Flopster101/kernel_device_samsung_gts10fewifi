...
struct dwc3_exynos {
	...
	struct  notifier_block usb_itmon_nb;
#endif

#if IS_ENABLED(CONFIG_DEBUG_SNAPSHOT)
	struct  notifier_block usb_panic_nb;
#endif
};
...
/**
 * dwc3_reg_group - DWC3 register group
 * @start: register group start address
 * @end: register group end address
 * @off: register offset from the base address
 */
struct dwc3_reg_group {
	u32 start;
	u32 end;
	u32 off;
};
