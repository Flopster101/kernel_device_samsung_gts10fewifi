...
bool is_b_sess_user;
EXPORT_SYMBOL_GPL(is_b_sess_user);
/* -------------------------------------------------------------------------- */
...
static int exynos_usb_panic_handler(struct notifier_block *nb,
		unsigned long act, void *data)
{
	int ret = 0;
	struct dwc3_exynos *exynos = container_of(nb, struct dwc3_exynos, usb_panic_nb);

	pr_info("%s: Start to dump the dwc3 reg\n", __func__);

	if (!exynos) {
		pr_err("%s: exynos is NULL!!\n", __func__);
		return -ENOENT;
	}

	if (!pm_runtime_suspended(exynos->dwc->dev))
		ret = dwc3_exynos_reg_dump(exynos);
	else
		pr_info("%s: USB block power is off, skipping SFR dump\n", __func__);

	if (ret)
		pr_info("%s: error return %d\n", __func__, ret);
	return 0;
}

static int exynos_usb_itmon_handler(struct notifier_block *nb,
		unsigned long act, void *data)
{
	int ret = 0;
	struct dwc3_exynos *exynos = container_of(nb, struct dwc3_exynos, usb_itmon_nb);
...
static struct dwc3_reg_group dwc3_reg[] = {
	{ DWC3_GSBUSCFG0,	DWC3_VER_TYPE,				0x04},
	{ DWC3_GUSB2PHYCFG(0),	DWC3_GUSB3PIPECTL(0),			0x40},
	{ DWC3_GTXFIFOSIZ(0),	DWC3_GEVNTCOUNT(0),			0x04},
	{ DWC3_DCFG,		DWC3_DGCMD,				0x04},
	{ DWC3_DEP_BASE(0),	DWC3_DEP_BASE(31) + DWC3_DEPCMD,	0x04},
	{ DWC3_OCFG,		DWC3_OSTS,				0x04},
};

/**
 * dwc3_exynos_reg_dump - receive ITMON or kernel panic notifier event.
 */
int dwc3_exynos_reg_dump(struct dwc3_exynos *exynos)
{
	u32			buff_off = 0;
	u32			reg_off;
	u32			reg;
	...
	pr_info("## Start dump: DWC3 registers: base_vadd: 0x%08lx ##\n",
			(unsigned long)exynos->dwc->regs);

	while (reg_group_off < ARRAY_SIZE(dwc3_reg)) {
		reg_off = dwc3_reg[reg_group_off].start;
		while (reg_off <= dwc3_reg[reg_group_off].end) {
			if (reg_off % 32 == 0)
				buff_off += snprintf(regdump + buff_off,
						sizeof(regdump) - buff_off,
						"\n0x%04x: ", reg_off);

			reg = dwc3_exynos_readl(exynos->dwc->regs, reg_off);
			buff_off += snprintf(regdump + buff_off,
					sizeof(regdump) - buff_off,
					"  0x%08x", reg);

			reg_off += dwc3_reg[reg_group_off].off;
			if (buff_off >= (sizeof(regdump) - 16)) {
				pr_info("%s", regdump);
				buff_off = 0;
				if (reg_off % 32 != 0)
					buff_off += snprintf(regdump + buff_off,
	...
	pr_info("### End dump: DWC3 registers ###\n\n");
	return 0;
}
EXPORT_SYMBOL_GPL(dwc3_exynos_reg_dump);
...
static int dwc3_exynos_probe(struct platform_device *pdev)
{
	...
	reg = dwc3_exynos_readl(exynos->dwc->regs, DWC3_GCTL);
	reg |= DWC3_GCTL_DSBLCLKGTNG;
	pr_info("%s : 0x%x\n", __func__, reg);
	dwc3_exynos_writel(exynos->dwc->regs, DWC3_GCTL, reg);
	...
	exynos->xhci_pm_ops = kzalloc(sizeof(struct dev_pm_ops), GFP_ATOMIC);
	if (!exynos->xhci_pm_ops) {
		dev_err(dev,"Failed to get memory for xhci_pm_ops\n");
		goto populate_err;
	}
...
#if IS_ENABLED(CONFIG_EXYNOS_ITMON) || IS_ENABLED(CONFIG_EXYNOS_ITMON_V2)
	/* add itmon notifier */
	exynos->usb_itmon_nb.notifier_call = exynos_usb_itmon_handler;
	itmon_notifier_chain_register(&exynos->usb_itmon_nb);
#endif

#if IS_ENABLED(CONFIG_DEBUG_SNAPSHOT)
	/* register panic handler */
	exynos->usb_panic_nb.notifier_call = exynos_usb_panic_handler;
