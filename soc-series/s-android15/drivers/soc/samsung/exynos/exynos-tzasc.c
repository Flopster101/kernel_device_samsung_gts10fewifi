...
#if defined(CONFIG_SOC_S5E9955) || defined(CONFIG_SOC_S5E9945)
...
{
	int ret = UNKNOWN;
	int cpu_id[4] = { 0x10, 0x11, 0x12, 0x13 };
	int i = 0;
...
static int find_master_info(int master_id)
{
	...
	if (!(master_id & CPU_MASTER_MASK)) {
		...
		ret = master_id + 2;	/* Added for master_name print */
	}
	return ret;
}
#endif
...
static irqreturn_t exynos_tzasc_irq_handler_thread(int irq, void *dev_id)
{
...
#elif defined(CONFIG_SOC_S5E8855)
	char master_name[32][9] = { "UNKNOWN", "CPU", "D_ICPU", "D1_CSIS", "D0_CSTAT", "D0_RGBP", "D1_RGBP", "D_YUVP", "D0_CSIS", "D1_CSTAT", "D2_RGBP", "D3_RGBP",
				    "D0_M2M", "D1_M2M", "D_PERIC", "G_CSSYS", "D_AUD", "D_MFC", "D_WLBT", "D0_DNC", "D1_DNC", "D0_MODEM", "D1_MODEM","D2_MODEM",
				    "D0_DPU", "D1_DPU", "D_PSP", "D_ALIVE", "D_HSI", "D_USB", "D_G3D"};
#endif
	...
	for (i = 0; i < ch_num; i++) {
		...
		if (tzc_ver == TZASC_VERSION_TZC380) {
			...
			pr_info("- Fail ID : %#x\n",
					(fail_id & TZASC_SMC_FAIL_ID_MASTER_MASK) >>
					TZASC_SMC_FAIL_ID_MASTER_SHIFT);

			pr_info("- Fail Master : %s\n", master_name[master_id]);

			pr_info("- Fail Region Num : %d\n", data->fail_info[i].tzasc_fail_region_num);

			pr_info("- Fail Region Start : %#lx\n", data->fail_info[i].tzasc_fail_region_start);

			pr_info("- Fail Region End : %#lx\n", data->fail_info[i].tzasc_fail_region_end);

			pr_info("- Fail Region2 Start : %#lx\n", data->fail_info[i].tzasc_fail_region2_start);

			pr_info("- Fail Region2 END : %#lx\n", data->fail_info[i].tzasc_fail_region2_end);

			pr_info("- Fail Region2 Attr : %#x\n", data->fail_info[i].tzasc_fail_region2_attr);

			pr_info("- Fail Region3 Start : %#lx\n", data->fail_info[i].tzasc_fail_region3_start);

			pr_info("- Fail Region3 END : %#lx\n", data->fail_info[i].tzasc_fail_region3_end);

			pr_info("- Fail Region3 Attr : %#x\n", data->fail_info[i].tzasc_fail_region3_attr);
		}
