...
static unsigned int *itmon_invalid_addr;
static int num_invalid_addr;
...
static void itmon_en_ok_resp(struct itmon_dev *itmon,
                                struct itmon_nodegroup *group,
                                struct itmon_nodeinfo *node,
                                bool en)
{
        ...
        val = readl(reg + OVR_TO_OK_RESP);

        if (node) {
                if (en)
                        val |= (1 << node->ok_resp_id);
                else
                        val &= ~(1 << node->ok_resp_id);

		writel(val, reg + OVR_TO_OK_RESP);
                name = node->name;
        } else {
                node = group->nodeinfo;
                for (i = 0; i < group->nodesize; i++) {
                        if (!node[i].ok_resp_en || node[i].ok_resp_id == 0xFF)
                                continue;

                        if (en)
                                val |= (1 << node[i].ok_resp_id);
                        else
                                val &= ~(1 << node[i].ok_resp_id);
                }
		writel(val, reg + OVR_TO_OK_RESP);
        ...
        dev_dbg(itmon->dev, "ITMON - %s OK Resp override %sabled\n",
                        name, en == true ? "en" : "dis");
}
				   ...
				   struct itmon_nodeinfo *node)
{
	...
	if (policy->chk_ok_resp && group->ok_resp_support)
		itmon_en_ok_resp(itmon, group, node, policy->en_ok_resp);
...
int itmon_en_by_name(const char *name, bool en)
{
	...
	if (group->ok_resp_support && node->ok_resp_en &&
			node->ok_resp_id != NOT_SUPPORT)
		itmon_en_ok_resp(g_itmon, group, node, en);
				...
				bool en)
{
	...
	for (i = 0; i < group->nodesize; i++) {
		...
		if (group->ok_resp_support)
			if (node[i].ok_resp_en && node[i].ok_resp_id != NOT_SUPPORT)
				itmon_en_ok_resp(itmon, group, &node[i], en);
	}
}
				       ...
				       char *cpu_name)
{
	...
	for (i = 0; i < (int)ARRAY_SIZE(itmon_cpu_node_string); i++) {
		if (!strncmp(name, itmon_cpu_node_string[i],
			...
			u32 low_bit = pdata->lpid_low_bit;
			u32 high_bit = pdata->lpid_high_bit;

			if (userbit & pdata->el2_mask)
				el2 = 1;

			if (!(userbit & pdata->strong_mask))
				strong = 1;

			if (!(low_bit == 0 && high_bit == 0)) {
				unsigned int lpid_mask = GENMASK(high_bit, low_bit);
				core_num = ((userbit & (lpid_mask)) >> low_bit);
			} else {
				...
				struct itmon_nodegroup *group)
{
	...
	if (group)
		dev_dbg(itmon->dev, "group %s frz_support %s\n",
	...
	for_each_set_bit(bit, &tmout_frz_stat, group->nodesize) {
		node = itmon_get_nodeinfo_group_by_tmout_offset(itmon, group, bit);
		...
		for (trans_type = TRANS_TYPE_WRITE; trans_type < TRANS_TYPE_NUM; trans_type++) {
			...
			for (i = 0; i < num; i++) {
				...
				payload0 = readl(group->regs + OFFSET_TMOUT_REG +
						TMOUT_PAYLOAD_FROM_BUFFER + rw_offset);
				payload1 = readl(group->regs + OFFSET_TMOUT_REG +
						TMOUT_PAYLOAD_FROM_SRAM_1 + rw_offset);
				payload2 = readl(group->regs + OFFSET_TMOUT_REG +
						TMOUT_PAYLOAD_FROM_SRAM_2 + rw_offset);	/* mid/small: payload 1 */
				payload3 = readl(group->regs + OFFSET_TMOUT_REG +
						TMOUT_PAYLOAD_FROM_SRAM_3 + rw_offset);	/* mid/small: payload 2 */
				payload4 = readl(group->regs + OFFSET_TMOUT_REG +
						TMOUT_USER_FROM_BUFFER_0 + rw_offset);	/* mid/small: payload 3 */
				payload5 = readl(group->regs + OFFSET_TMOUT_REG +
						TMOUT_USER_FROM_BUFFER_1 + rw_offset);
				...
				if(valid) {
					...
					if (p_itmon_history) {
						snprintf(p_itmon_history->data[p_itmon_history->idx],
								sizeof(p_itmon_history->data[0]),
								"%lu.%03lu,%s%s,%s,0x%lx,%s",
								(unsigned long)ts_nsec, rmem_nsec / 1000000,
								port ? port->port_name : NO_NAME,
								master_name ? master_name : NO_NAME,
								node->name, addr, itmon_errcode_short[ERR_TMOUT_FREEZE]);
				 ...
				 struct itmon_tracedata *data)
{
	...
	if (m_node && det_node && (data->err_code != ERR_TMOUT_FREEZE)) {
...
static void itmon_report_prterr_traceinfo(struct itmon_dev *itmon,
				   struct itmon_traceinfo *info,
				   unsigned int trans_type)
{
	u64 ts_nsec = local_clock();
	...
	if (p_itmon_history) {
		snprintf(p_itmon_history->data[p_itmon_history->idx], sizeof(p_itmon_history->data[0]),
				"%lu.%03lu,%s, %s,%s",
				(unsigned long)ts_nsec, rmem_nsec / 1000000,
				info->src, info->dest ? info->dest : NO_NAME,
...
static void itmon_report_traceinfo_by_errcode(struct itmon_dev *itmon,
				struct itmon_traceinfo *info,
				unsigned int trans_type)
{
	switch (info->err_code) {
	case ERR_TMOUT_FREEZE:
	/* Timeout Freeze */
		if (info->s_node->type == M_NODE ||
				info->s_node->type == T_M_NODE)
			itmon_report_prterr_traceinfo(itmon, info, trans_type);
		break;
	case ERR_PRTCHK_ARID_RID ... ERR_PRTCHK_AWID_RID:
	case ERR_PRTCHK_AWLEN_WLAST_EL ... ERR_PRTCHK_AWLEN_WLAST_NL:
	/* Protocol Checker */
		itmon_report_prterr_traceinfo(itmon, info, trans_type);
		break;
	default:
				   ...
				   unsigned int trans_type)
{
	...
	/* ERR_TMOUT_FREEZE don't have to display M/S_NODE because
	 *  * it displays only previous transaction just before stuck transaction */
	if (data->err_code == ERR_TMOUT_FREEZE &&
			(data->det_node->type == S_NODE ||
			 data->det_node->type == T_S_NODE))
		return;
...
static void itmon_analyze_errlog(struct itmon_dev *itmon)
{
	...
	for (trans_type = 0; trans_type < TRANS_TYPE_NUM; trans_type++) {
		list_for_each_entry_safe(info, next_info, &pdata->infolist[trans_type], list) {
			...
			itmon_report_traceinfo_by_errcode(itmon, info, trans_type);
			list_del(&info->list);
			kfree(info);
		}
	}
				     ...
				     struct device_node *np)
{
	...
	for (i = 0; i < MAX_CUSTOMIZE; i++) {
		...
		if (!node_np) {
			dev_dbg(itmon->dev, "%s device node is not found\n",
					snode_name);
			break;
		}
...
static int itmon_load_keepdata(struct itmon_dev *itmon)
{
	...
	/* lpid config data load*/
	pdata->lpid_low_bit = kdata->lpid_low_bit;
	pdata->lpid_high_bit = kdata->lpid_high_bit;

	if (kdata->strong_mask)
		pdata->strong_mask = kdata->strong_mask;
	else
		pdata->strong_mask = BIT(1);

	if (kdata->el2_mask)
		pdata->el2_mask = kdata->el2_mask;
	else
		pdata->el2_mask = BIT(0);
...
static void itmon_parse_dt(struct itmon_dev *itmon)
{
	...
	if (pdata->lpid_high_bit < pdata->lpid_low_bit) {
		dev_warn(itmon->dev,
				"lpid-high-bit(%u) should not less than lpid-low-bit(%u)! \n",
				pdata->lpid_high_bit, pdata->lpid_low_bit);
		pdata->lpid_high_bit = pdata->lpid_low_bit = 0;
	}
	...
	num_invalid_addr = of_property_count_u32_elems(np, "invalid-addr");
	if (num_invalid_addr <= 0)
		dev_info(itmon->dev, "There is no invalid address\n");
	else {
		itmon_invalid_addr = devm_kzalloc(itmon->dev,
				num_invalid_addr * sizeof(*itmon_invalid_addr),
				GFP_KERNEL);
		if (!itmon_invalid_addr)
			dev_err(itmon->dev, "Allocation failed, skip setting invalid address\n");
		else
			if (of_property_read_u32_array(np, "invalid-addr",
						itmon_invalid_addr,
						num_invalid_addr))
				dev_err(itmon->dev, "Invalid address array read failed\n");
	}
