...
static struct latency_info *latency_el3;
#endif
...
int esca_get_nfc_log_buf(struct nfc_clk_req_log **buf, u32 *last_ptr, u32 *len)
{
	...
	eclk_logbuf = (struct ext_clk_log_buf *)(sram_base + flxp_eclkbuf->p_log_bufs);

	if (nfc_clkreq_idx >= flxp_eclkbuf->nr_reqs)
		return -EINVAL;

	nfc_logs = (struct ext_clk_log *)(sram_base + eclk_logbuf[nfc_clkreq_idx].p_logs);
...
#if IS_ENABLED(CONFIG_ESCA_SLEEP_PROFILER)
static ssize_t flexpmu_dbg_sleep_latency_profiler_read(int fid, char *buf)
{
	ssize_t ret = 0;
	int latency;
	...
	latency = (latency_el3->sum[0] / latency_el3->cnt);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
		"%15s %20s: %4d[us]\n", "[Suspend seq]", "Kernel end - wfi", latency);

	latency = (latency_soc[4].sum[0] / latency_soc[4].cnt);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%15s %20s: %4d[us]\n", "[Suspend seq]", "APSOC down", latency);

	latency = (latency_mif[4].sum[0] / latency_mif[4].cnt);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%15s %20s: %4d[us]\n", "[Suspend seq]", "MIF down", latency);

	latency = (latency_mif[4].sum[1] / latency_mif[4].cnt);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%15s %20s: %4d[us]\n", "[Resume seq]", "MIF up", latency);

	latency = (latency_soc[4].sum[1] / latency_soc[4].cnt);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%15s %20s: %4d[us]\n", "[Resume seq]", "APSOC up", latency);

	latency = (latency_el3->sum[1] / latency_el3->cnt);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
		"%15s %20s: %4d[us]\n", "[Resume seq]", "EPBL - kernel start", latency);
...
#if IS_ENABLED(CONFIG_ESCA_SLEEP_PROFILER)
	FLEXPMU_DBG_FUNC_READ(sleep_latency_profiler),
#endif
};
...
#if IS_ENABLED(CONFIG_ESCA_SLEEP_PROFILER)
void exynos_flexpmu_dbg_sleep_latency(void)
{
	int latency;
...
EXPORT_SYMBOL_GPL(exynos_flexpmu_dbg_sleep_latency);
#endif

void exynos_flexpmu_dbg_show_lcd_status(void)
{
	if (flxp_cpu_pm->lcd_status == LCD_NONE) {
		pr_info("%s: Last Core Detection Status : %d\n", EXYNOS_FLEXPMU_DBG_PREFIX, flxp_cpu_pm->lcd_status);
		return;
	}

	if (flxp_cpu_pm->lcd_status > LCD_NONE) {
		pr_info("%s: Last Core Detection Success\n", EXYNOS_FLEXPMU_DBG_PREFIX);
	} else {
		pr_info("%s: Last Core Detection Fail\n", EXYNOS_FLEXPMU_DBG_PREFIX);
		pr_info("%s: Failed Core : %d\n", EXYNOS_FLEXPMU_DBG_PREFIX, flxp_cpu_pm->lcd_fail_core);
	}
}
EXPORT_SYMBOL_GPL(exynos_flexpmu_dbg_show_lcd_status);

void exynos_flexpmu_dbg_clr_lcd_status(void)
{
	flxp_cpu_pm->lcd_status = LCD_NONE;
}
EXPORT_SYMBOL_GPL(exynos_flexpmu_dbg_clr_lcd_status);
...
static int flexpmu_dbg_probe(struct platform_device *pdev)
{
	...
	u64 __iomem flexpmu_dbg_addr = 0;
...
#if IS_ENABLED(CONFIG_ESCA_SLEEP_PROFILER)
	latency_el3 = (struct latency_info *)(sram_base + flxp_dbg->latency_el3);
#endif
