...
#if IS_ENABLED(CONFIG_USB_CONFIGFS_F_DM)
	...
	struct direct_dm_desc *curr_desc, void *addr)
{
	...
	dc->desc_rgn[dc->curr_desc_pos].status &= ~BIT(DDM_DESC_S_DONE_CP);
...
#else
	mif_err_limited("f_dm is not enabled\n");
#endif
...
static void direct_dm_rx_func(unsigned long arg)
{
	...
	for (i = 0; i < dc->num_desc; i++) {
		...
		if (!(curr_desc.status & BIT(DDM_DESC_S_DONE_CP))) {
			if (unlikely(dc->enable_debug))
				mif_info("DDM_DESC_S_DONE_CP is not set %d 0x%llx\n",
					 dc->curr_desc_pos, curr_desc.cp_buff_paddr);

			break;
		}

		if (!dc->usb_done_status[dc->curr_desc_pos] &&
			(dc->curr_desc_pos == dc->curr_done_pos)) {
			mif_info_limited("usb is not completion. prev is enqueued:%d\n",
				dc->curr_done_pos);
			break;
		}
		...
		if (unlikely(dc->enable_debug))
			mif_info("pos:%d len:%d a:%pK/0x%lx/0x%llx upper:%d done:%d\n",
				 ...
				 vaddr, paddr, curr_desc.cp_buff_paddr,
				 upper_layer_req, dc->desc_rgn[dc->curr_desc_pos].status);

		if (unlikely(upper_layer_req)) {
			ret = direct_dm_send_to_upper_layer(dc, &curr_desc, vaddr);
			if (ret)
				break;

			rcvd++;
		} else {
			dc->usb_done_status[dc->curr_desc_pos] = false;
			ret = direct_dm_send_to_usb(dc, &curr_desc, phys_to_virt(paddr));
			if (ret)
				break;
...
static void direct_dm_run_rx_func(struct direct_dm_ctrl *dc)
{
	...
	if (dc->use_rx_task)
		tasklet_schedule(&dc->rx_task);
	else
		direct_dm_rx_func((unsigned long)dc);
}

/* IRQ handler */
irqreturn_t direct_dm_irq_handler(int irq, void *arg)
{
	struct direct_dm_ctrl *dc = (struct direct_dm_ctrl *)arg;
...
EXPORT_SYMBOL(direct_dm_irq_handler);
...
static void direct_dm_usb_active_noti(void *arg)
{
	...
	dc->usb_active = true;
	dc->info_rgn->usb_connection = dc->usb_active;
}
...
static void direct_dm_usb_completion_noti(void *addr, int length, void *arg)
{
	...
	dc->desc_rgn[pos].status &= ~BIT(DDM_DESC_S_DONE_CP);
	dc->usb_done_status[pos] = true;
...
#endif /* CONFIG_USB_CONFIGFS_F_DM */
		...
		struct device_attribute *attr, char *buf)
{
	...
	count += scnprintf(&buf[count], PAGE_SIZE - count,
		"silent_log:%d usb_connection:%d\n",
		dc->info_rgn->silent_log,
		dc->info_rgn->usb_connection);
	count += scnprintf(&buf[count], PAGE_SIZE - count,
		...
		const char *buf, size_t count)
{
	...
	for (i = 0; i < val; i++) {
		...
		if (dc->desc_rgn[_test_dm_desc_pos].status & BIT(DDM_DESC_S_DONE_CP)) {
			mif_err("DDM_DESC_S_DONE_CP is already set. pos:%d\n",
				_test_dm_desc_pos);
			break;
		}
		...
		dc->desc_rgn[_test_dm_desc_pos].status |= BIT(DDM_DESC_S_DONE_CP);
...
struct direct_dm_ctrl *direct_dm_get_dc(void)
{
	return _dc;
}
EXPORT_SYMBOL(direct_dm_get_dc);
...
static int direct_dm_setup_region(struct direct_dm_ctrl *dc)
{
	...
	ddm_pbase = cp_shmem_get_base(dc->shm_rgn_index);
	if (!ddm_pbase) {
		mif_err("ddm_pbase is null\n");
		return -ENOMEM;
	}
	ddm_rgn_size = cp_shmem_get_size(dc->shm_rgn_index);
	if (!ddm_rgn_size) {
		mif_err("ddm_rgn_size is null\n");
		return -ENOMEM;
	}
	mif_info("ddm_pbase:0x%lx ddm_rgn_size:0x%08x\n",
		ddm_pbase, ddm_rgn_size);

	if (!dc->cp_ddm_pbase) {
		dc->cp_ddm_pbase = ddm_pbase - cp_shmem_get_base(SHMEM_CP) +
			CP_CPU_BASE_ADDRESS;
		mif_info("cp_ddm_pbase:0x%llx\n", dc->cp_ddm_pbase);
	}
	...
	dc->usb_done_status = kzalloc(sizeof(bool *) * dc->num_desc, GFP_KERNEL);
	if (dc->usb_done_status == NULL) {
		mif_err("kzalloc() error for usb_done_status\n");
		return -ENOMEM;
	}
...
#else
static inline int direct_dm_register_irq(struct direct_dm_ctrl *dc)
{
	return 0;
}
#endif
	...
	struct direct_dm_ctrl *dc)
{
	...
	mif_dt_read_u64_noerr(np, "cp_base", dc->cp_ddm_pbase);
...
err:
	mif_err("Direct DM driver probe failed\n");
